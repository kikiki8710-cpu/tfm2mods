...
- ⬜**banpick_order 스텁 본문 재작성**(~~disp 재핀~~ → **disp 치환으로는 불가**·레지스터/구조 변경 실증 = §4d.3. 컨테이너·사이트 15/15는 확보됨) · ~~comptest 바이트패치 19~~ → **✅17/17 재핀 완료(§4c.1)** · **ai_adjust 바이트패치 908·JT 베이스**(665건 매핑은 확보·§4c.2).
