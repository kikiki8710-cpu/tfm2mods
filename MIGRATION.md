...
> ★**편입 실익(실제)**: cfg의 `dd_frontier_mult=350`·`dd_lane_margin=600`·`dd_cover_count=0`·`dd_ratio_thr=31` **4종이 전부 COVER 블록 안**이라 이제야 실제 AI 판단에 반영됨. `dd_near_dist=110250000`은 COVER가 아니라 `my_dd7700_code` STAGE4(engage) 소관 ⟹ engage가 code 2를 낼 때만 반영 = **부분 적용**.
...
- **[C] `0x22efed0`** = 위협 유의성 필터 leaf(disc0/1/3·disc4 경로) — 사다리 7(`0x22effff`~`0x22f0023`). branch A(소극 경로 별도 4임계 @`0x22f0067~79`) = ⬜미배선 후보.
...
- ⬜sv 미배선 후보 2건: [D] disc5/6 leaf(발화 확인 선행) · [C] branch A 4임계.
- ⬜disc 번호 정본 재확인(JT2 +1 미스터리, §6) / ⬜disc4 재검증→`d4_repl=1` 복귀 검토(레버 7종 부활 효과) / ⬜개별 120틱 단기 시야창 8곳(미착수 아이디어).
