...
#### 4i.4 ⬜로그 근거가 없는 항목 — ★2026-08-27 유저 재확인으로 **5종 정상 확정**
★**유저 확인(2026-08-27)**: `level_cap` · `community_reaction_mod` · `Spectator_Chat` · **다람이 8종** · `banpick_illust` = **"이거는 잘돼"** ⟹ 이 5항목(다람이 8종 포함 12모드)은 **정상 확정**. 남은 미확인 = serpen 경기중 기능 · comptest(아래 §4i.5 = **문제 발생**).

(이하 원문)

#### 4i.4b 원 기록(육안 확인만)
serpen **경기 중 기능**(런처 발화 0 = 이번엔 경기 미관전) · comptest **조합테스트 기능**(릴리스 빌드라 로깅 OFF — 단 병렬 실행은 유저 실측으로 **작동 확인**, §4i.5) · banpick_illust(진단 로깅 OFF) · Spectator_Chat · mod_order · 다람이 8종.
**정상이라는 뜻이 아니라 "로그로는 확인 못 했다"는 뜻**이므로, 다음 플레이에서 해당 화면을 거치면 자동으로 근거가 쌓인다.

#### 4i.5 ⬜조합테스트 **병렬 실행 = 작동하나 매우 느림**(2026-08-27 유저 실측 · 원인 미조사)

- **기능은 산다**: 유저 확인 — "디폴트가 5개 실행이라 그냥 하니까 **5개 켜지긴 했는데 엄청 느려졌어**".
  ⟹ `CONC_ON=true` · `PARALLEL_ON=true` · `CONC_RUNS_MAX=10` 경로가 0.5.7에서도 발화한다는 뜻. 재핀 실패로 죽은 것이 아니다.
- ⚠**"UI가 없다"는 기능 부재가 아니다** — 병렬 발사 개수를 고르는 화면이 없을 뿐이고, 기본값(5)으로 그냥 돈다. 첫 관측에서 "병렬실행 없다"로 읽혔으나 유저가 곧 정정.
- ⬜**느려진 원인 = 미조사**(유저 지시로 보류). 다음에 볼 단서 후보:
  1. ★**0.5.7 패치노트 "진단 로그 보강"** — "서버·클라이언트 결과 대조를 팀 합계에서 **선수 단위로 확대**"가 명시돼 있다. sim 1회당 대조 비용이 선수 수만큼 늘었다면 **게임 자체가 느려진 것**이고 모드 탓이 아니다. **모드 없이 0.5.7 조합테스트 1회 소요시간을 재보면 즉시 갈린다**(가장 먼저 할 것).
  2. 병렬 sim 스레드 경합 — `CONC_PROBE_ON=false`라 스레드 계측이 꺼져 있어 현재 로그로는 판별 불가. 진단 빌드에서 `tickmap`/`SIMBODY_HITS` 로 관측.
  3. 빌드 플래그 — `opt-level=1` 고정(2026-07-18 확정, opt2/3는 STATUS_STACK_OVERFLOW). 0.5.6과 동일 조건이므로 **회귀 원인은 아님**(대조군으로만).
  4. `parallel = 0|1` cfg — 순차로 바꿔 1회 소요시간을 비교하면 "병렬 오버헤드"인지 "게임이 느려진 것"인지 분리된다.
- ⚠**판정 주의**: 0.5.6 대비 느려진 것인지, 원래 5발 병렬이 그만큼 걸리는 것인지 **기준선이 없다**. 0.5.6 exe로 같은 조건을 재보거나(구버전 브랜치), 모드 OFF 대조가 필요하다.
- comptest 는 릴리스 빌드라 `LOG_ENABLED=false` ⟹ **이번 판 진단 로그가 없다**(파일 최신 = 08-09). 원인 조사 시 진단 빌드가 필요하다.

### 4j. ★DropdownRunner 선택 인덱스 오프셋 재핀 (2026-08-28, 0.5.7) — item_tactics 개인전술 "지정이 저장 안 됨" 종결

**증상**: 0.5.7 복구 후 `tfm2_item_tactics` 개인전술 화면에서 **드롭다운 옵션 목록은 정상**인데 **선택이 전혀 저장되지 않음**(크래시 없음 = 조용한 실패).

#### 4j.1 확정 값 (0.5.6 → 0.5.7)

| 필드 | 0.5.6 | 0.5.7 | 비고 |
|---|---|---|---|
| **선택 인덱스(u64)** | `+0x1788` | **`+0x1790`** | **+8 이동**. `u64::MAX` = 미선택 |
| 팝업 상태 dword | `0x1790` | **`0x1798`** | ⚠구 `0x1788`을 그대로 읽으면 이 필드의 인접값을 읽는다 |
| 구조체 크기 | `0x17C0` | **`0x17C8`** | 삽입 지점 = `0x1578~0x1788` 구간의 8B 필드 |
| 옵션 Vec | `+0x1528` | `+0x1528` | **불변**(ptr/cap/len·원소 0xf8) |
| 팝업 핸들 | `+0x1570` | `+0x1570` | **불변** |
| `max_items_height` present/값 | `+0x1150`/`+0x1154` | 동일 | **불변** |

- 확정 근거(ui_kit RE, 2026-08-27 · 3중): ①set_item 첫 store — 0.5.6 `0x1c1710` `mov [rcx+0x1788],rdx` ↔ 0.5.7 `0x1c2e00` `mov [rcx+0x1790],rdx` ②클릭 히트테스트 store `0x1cd952` `mov [rdi+0x1790],r14` ③SDK rlib 컴파일 asm `selected_item()` = `movq 6032(%rcx)`.
- **인게임 검증(2026-08-28)**: 개인전술에서 지정 후 `item_tactics_sel.txt` 갱신 · `SEL맵` 19→23개 · 경기에서 아이템 4칸 반영 확인 + ★**조합테스트 화면에서도 정상 주입**(유저 확인) ⟹ **DONE(0.5.7)**. ★**검증 범위 = 일반 경기 + 조합테스트**(`REPORT\tfm2_item_tactics\02_구현정보.md` 검증표 **D1~D5 전항목 통과** — D5 = "조합테스트에서도 주입", 일반 경기만 보면 숨는 결함을 잡는 항목).

#### 4j.2 ★진짜 함정 — 공유모듈을 고쳐도 로컬 복사본은 안 고쳐진다 (버전무관 교훈)

`ui_kit\ui_kit.rs`의 `DD_SELECTED_OFF`를 `0x1790`으로 고쳤는데도 `tfm2_item_tactics`는 계속 고장나 있었다. **이 모드는 ui_kit을 링크하지 않고 `tfm2_scrim`에서 이식한 자체 복사본(`nat_dd_selected`)을 쓴다.**

0.5.7 시점 로컬 복사본 보유 활성 모드:

| 모드 | 위치 | 상태 |
|---|---|---|
| `tfm2_item_tactics` | `src\lib.rs:485` | ✅수정완(`0x1790`) |
| `tfm2_comptest_unlock` | `src\tfm2_comptest_unlock.rs:3570-3571` | ✅수정완(`0x1790`) |
| `tfm2_scrim` | `src\lib.rs:1298`·`:1327`·`:3865` | ⬜**미수정**(이번 회차 대상 아님 — 복구 시 함께 고칠 것) |

⟹ **규칙: 공유모듈 상수를 갱신하면 반드시 그 값을 전 모드에서 grep 해 로컬 복사본을 함께 고칠 것.**

#### 4j.3 ★증상의 비대칭이 용의 상수를 지목한다 (버전무관 교훈)

"옵션 목록은 정상인데 선택만 저장 안 됨" = **읽기 경로에만 쓰이는 상수**가 용의자다(`set_options`는 선택 오프셋을 안 쓰므로 멀쩡했다). 이 단서를 늦게 본 탓에 무관한 결함 4개(화면경기 retaddr / `MOD_ITEMS_VEC_OFF` / `athlete+0x440` / ui_kit 상수)를 원인으로 오인했다.

#### 4j.4 ⚠도구 안전 — `open(path,'wb')`는 즉시 truncate (버전무관 교훈)

- 인라인 파이썬 수정 스크립트에서 `open(p,'wb')` 호출 뒤 **같은 줄에서 예외**가 나 `tfm2_item_tactics\src\lib.rs`가 **0바이트**가 됐다(388KB 손실 · 직전 커밋 `e3413ea`에서 복구 · 실유실은 `LOG_ENABLED` 한 줄).
- `sed -i`는 이 프로젝트 소스(**CRLF**)를 **LF로 정규화**해 파일 전체가 diff로 잡힌다(lib.rs 5,222B 감소) ⟹ **바이트 단위 치환으로 EOL 보존**.

#### 4j.5 ⬜보류 — `FN_DD_SETOPT_RVA 0x1c31f0` ≠ ui_kit `DD_SETOPT_RVA 0x1c2e00`

- `tfm2_item_tactics`의 `FN_DD_SETOPT_RVA = 0x1c31f0`은 ui_kit의 `DD_SETOPT_RVA`(0.5.6 `0x1c1710` → 0.5.7 `0x1c2e00`)와 **다른 함수**다.
- `0x1c31f0`은 0.5.6 exe에서 **함수 시작조차 아니었으나**(`00 e8 ba d6`), 0.5.7에서는 프롤로그 `55 41 57 41 56 56 57 53 48 81 ec 88`인 **별개 함수 시작**이고 `dd_addr_valid()` 가드가 마침 이와 일치해 통과 · **옵션 주입도 인게임 정상 동작 확인**(2026-08-28).
- **판정 = 보류 · 적용 범위**: **현재 기능이 정상이므로 손대지 말 것.** 다음 패치에서 옵션 주입이 죽으면 그때 ghidra-re로 확인(후보 1순위 = ui_kit 값).

> 정본 = `REPORT\tfm2_item_tactics\02_구현정보.md §[2026-08-28 / 0.5.7]` · `03_시행착오.md §8` · [[tfm2-native-dropdown]] §0.5.7.
...
- ~~⬜빌드·배포 전량~~ → **✅28종 배포 완료**(§4·§4b·§4c). ~~⬜빌드스크립트 sdk_057 전환~~ → **✅6종 전환 완료**(§4). ~~⬜릴리스 zip 0.5.7 미생성~~ → **✅8종 생성·rel_verify 전건 OK**(§4i·⬜나머지 20종 미생성). ~~⬜인게임 검증 전량 미실시~~ → **✅검증 통과(§4i) — 크래시 0건·유저 확인**.
...
- ★★**현행 잔여(2026-08-27 기준·이 줄이 최신)**
  - ⬜**`tfm2_scrim` 드롭다운 선택 오프셋 미수정**(구 `0x1788` 3곳 = `src\lib.rs:1298`·`:1327`·`:3865`) — 0.5.7 대역 복구 시 `0x1790`으로 함께 고칠 것(2026-08-28 등재·§4j.2).
  - ⬜**item_tactics `FN_DD_SETOPT_RVA 0x1c31f0`은 ui_kit `DD_SETOPT_RVA 0x1c2e00`과 별개 함수** — 현재 옵션 주입 정상이라 **보류(손대지 말 것)**, 다음 패치에서 죽으면 ghidra-re(2026-08-28 등재·§4j.5).
  - ⚠**comptest 조합테스트 = "결과 정리 중 1/5"에서 멈춤**(유저 실측 2026-08-27). 경기 5개는 **전부 완료**되는데 결과 수집이 1건에서 진행되지 않는다. 정적 검증으로 **결과 수집 경로 프롤로그 3종(ARRIVE `0x1add430` 15B · HPUSH `0x17ae140` 12B · WARN `0x1aad370` 13B) 전부 일치** ⟹ 재핀 실수 가능성은 낮다. ★유력 가설 = **`collect` 훅의 "UI중복 → 등록용 distinct 치환"** 과 **0.5.7 패치노트 "서버·클라 결과 대조를 팀 합계→선수 단위로 확대"** 의 충돌. 판별법 = **중복 없는 로스터로 1회** 돌려보면 갈린다. ⚠comptest 는 릴리스 빌드라 `LOG_ENABLED=false` = 진단 로그 0줄 ⟹ 안 갈리면 진단 빌드 필요.
  - ⚠**elemental_serpen = "작동 잘 안한다"**(유저 실측 2026-08-27·미조사). 훅 5종은 **신주소로 설치 확인**(serpen `0x14a25f0`·렌더스텝 `0x90a090`·장로처형 `0x10af580`·파이프라인B `0x108f220`·증폭A `0x15912c0`) ⟹ **설치는 됐으나 기능이 안 산다**는 뜻. 첫 관측 로그(`serpen_probe.txt` 00:29)에서 **런처 발화 0 · 화면경기적중 0 · LIVE_SEED=0 · InGame=false** = 경기 미관전 상태였으므로 그 판으로는 판별 불가. → **경기를 한 판 관전한 뒤 `serpen_probe.txt` 재판독**이 첫 단계. ⚠0.5.6 회차의 동류 사고 2건 = riot 런처 **CALL식 후킹**에 의한 retaddr 오염(이번엔 riot이 `=0.5.6` 고정이라 비활성이므로 **다른 원인**) · 런처 훅 지연설치/체인 순서.
  - ⚠**ai_adjust = 기능 동작 미검증**(훅 설치·가드까지만 확인). 확인된 것 = 훅 스텁 11개 신주소 · `imm_guard checked=658/908 blocked=0`. **AI 판단이 실제로 모드 설정대로 바뀌는지는 별개** — 0.5.6 회차처럼 `class_verify.txt`(클래스 노브 실사용률)·`nexus_emg` 발화·`cs_lead` 비율 등으로 별도 판독 필요.
  - ⬜**sylas `EFF_VT_BASE`** stale 유지(자체 fail-safe 확인·유저 지시로 이번 회차 제외).
  - ⬜**릴리스 zip 나머지 20종**(생성분 8종 = Spectator_Chat·community_reaction_mod·daram2_viewplus·banpick_illust·comptest_unlock·elemental_serpen·level_cap·mod_order).
  - ⬜**serpen 경기 중 기능** 미확인(런처 발화 0 = 경기 미관전).
  - ⬜**ai_adjust 잔여**: 미재핀 243건(표에 구주소 = blocked 처리) · JT 베이스 · class_micro 18 · 본문변경 4훅(RETREAT·SUBPLAN·DISC18·DISC19) 재현 코드 정합성.
  - ⬜**지식베이스 4~6차 상세 미기록** — §4d~§4h(ai_adjust 665건·콜리 지문·CPS_OFF·스텁 4종)의 **재핀값·경위가 MIGRATION.md 에만** 있고 MEM·ANA 에는 요지만 반영됐다(record-keeper 지적 2026-08-27).
