...
#### 4g.3 ⛔SLOTSEL — 유일한 잔여
SIG `4d01d2488d35b5b6` vs 신 `4d01d2488d3508cb` ⟹ **불일치 = 자동 스킵(안전)**. 분기 수 34→32, 스텁이 재현하던 `mov rsi,[rbp+0x1338]` 소멸, join3(`0x256a10a`)은 구 분기에서 대응을 못 찾았다. **3-way 분기 규약 자체를 다시 확인해야** 하는 단계라 이번 회차에선 스킵 유지.
...
- ⬜**banpick_order SLOTSEL 스텁 재작성 1종만 잔여**(§4g.3 — 분기 34→32·`mov rsi,[rbp+0x1338]` 소멸·3-way 규약 재확인 필요). ~~HL·AITURN·DRAIN 계열~~ → **✅복구 완료(§4f·§4g)**. · ~~comptest 바이트패치 19~~ → **✅17/17 재핀 완료(§4c.1)** · **ai_adjust 바이트패치 908·JT 베이스**(665건 매핑은 확보·§4c.2).
