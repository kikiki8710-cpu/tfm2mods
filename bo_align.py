    ...
    for tag,i1,i2,j1,j2 in sm.get_opcodes():
        if tag=='equal':
            ...
            for k in range(i2-i1):
                m[oi[i1+k][0]] = ni[j1+k][0]
    if verbose:
        print(f"  align: old {len(oi)} ins, new {len(ni)} ins, matched {ratio_eq} ({ratio_eq/max(1,len(oi)):.3f})")
    return m, oi, ni
