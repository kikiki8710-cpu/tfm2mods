# tfm2_item_tactics — How It Works (v2.9.0 · game 0.5.5)

> Written in actual chronological order: game boots → you designate items → a match starts →
> purchases happen → the screen is drawn. RVA and struct-offset values are for **0.5.5** and change
> with every game patch — the constants in the source (`lib.rs` / `ui_inject.rs`) are canonical.
> 한국어판: `동작원리.md`.

## 0. When the game boots — mod preparation

1. **Version gate**: if the exe size does not match 0.5.5, the mod installs *no hooks at all* and
...
2. **Two byte patches** (when slots=4): ① the game's "stop buying at 3 items" comparison constant
   3→4 (`cmp [athlete+0x4a8], 3`) ② the slot-limit branch (`jbe`) inside the buy resolver. Together
...
stays inside the provider (`+0xec90`) as its initial value, so comparing it against the provider
...
|---|---|---|
| ① scene side | on-screen matches | read the match info from the db (two team ids + is_team1_blue), compute my side, compare with the athlete's side (`athlete+0x930`). Permanently cross-checked against ② on a 1/256 sample |
| ② athlete-id membership | background sims in general | the athlete's `athlete_id (+0x920)` ∈ `MY_ATHLETES` |
...
- **BG4 (new in v2.9.0)**: with slots=4, non-my athletes (i.e. every other team in background
  league sims) get their build **extended 3→4 slots** right before that early-out — so the whole
  league buys a 4th item and my team is no longer the only one with four. The 4th item is a
  **vanilla final item picked by an FNV hash of the champion name** (deterministic per champion =
  replay-safe). Designated-item injection and the neural net are never involved (the net is
  unverified under rayon's mass parallelism), so league statistics stay uncontaminated; the cost
  is a lightweight once-per-athlete check (only while build len==3).

**(c) Injection** — for a confirmed my-team athlete, overwrite the **goal-build array**
(build Vec at `+0x4e0/4e8/4f0`):
...
⚠ The per-player view-model stride is version-sensitive — it changed `0x260→0x2c0` in 0.5.5,
causing a "the effect applies but the icon doesn't show" defect, fixed in v2.9.0. A wrong stride
manifests as silent non-display, not a crash — which is why the slot-3 diagnostics in §6 exist.
...
Added in v2.9.0: a **`[slot3진단]` line** (GameView-hook installed / fired / PV captured / icons
set / miss) — fired=0 means the GameView-update RVA is wrong; icons-set=0 means the view-model
stride is off (regression watch for the §5 defect). BG4's first activation is logged once to
`4items_bg4.txt` as proof the background 4-slot extension actually ran.

## Appendix — hook & patch addresses (0.5.5)

| Site | RVA | | Site | RVA |
|---|---|---|---|---|
| UI loader / parser | `0x2e42d0` / `0x1a3e70` | | buy (injection) | `0xeb2c40` |
| dropdown set-options | `0x1c1a30` | | GameView update | `0x964350` |
| launcher | `0x14ac3e0` | | tooltip show | `0x2587990` |
| seed-ctor | `0x14c2380` | | item-net forward | `0x12624f0` |
| owned-cap patch | `0x15206b0` (imm) | | gate3 patch | `0xeb2fae` (jbe) |
...
`version_gate.txt` (version-gate log) · `4items_bg4.txt` (BG4 first-activation proof, once)
