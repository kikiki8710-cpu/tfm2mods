...
const RVA_TIP_SHOW: usize = 0x23da030; // 0.5.6(구0.5.5=0x2587990). head-UNIQUE·콜러 3개(UImega 0xab5339 포함) 동수·size 10237→10383. // 0.5.5(구0.5.4=0x236dc00). head-지문 유일후보·콜러 3개(UImega=0xaedc09 포함) 동수·size 10010→10237·ninsn 1936→2018. // 0.5.4(구0.5.3=0x1ab52f0). item_tooltip.rs panic-location 23개 동일 + 콜러 3개(UI메가 포함) 동수 + 명령수 1936 동일.
const RVA_TIP_MEASURE_VT: usize = 0x33bb690; // 0.5.6(구0.5.5=0x333b970). UImega tipshow call(0xab5339) 직전 lea r8,[rip+0x28979e8]@0xab5321 → 0x334cd10. // 0.5.5(구0.5.4=0x32602a0). UImega tipshow call(0xaedc09) 직전 lea r8,[rip+X]@0xaedbf1 = 4c 8d 05 78 dd 84 02 → 0x333b970. p3 = 텍스트 계측 ctx 의 vtable(상수)
...
const RVA_GAME_ALLOC: usize = 0x2b1b410;  // 0.5.6(구0.5.5=0x2a9bf30, skel/마스크 UNIQUE·BYTE=SAME). // 0.5.5(구0.5.4=0x29bb920). (rcx=무시, rdx=flags 0, r8=size) -> ptr
...
const RVA_GV_UPDATE: usize = 0x95cb80; // 0.5.6(구0.5.5=0x964350, skel UNIQUE·BYTE=SAME·size 4575·프롤로그 동일). // 0.5.5(구0.5.4=0xaa06c0, exe2exe 스켈레톤 확정). // 0.5.4(구0.5.3=0x960df0). 본문 니모닉 100% 동일(오프셋 시퀀스까지)·콜러 22개 동수.
...
unsafe extern "C" fn cap_launcher(saved: *mut u64, _e: usize) -> u64 {
    ...
    let is_comptest = rva == 0x23d5a52 || rva == 0x23aafce;
    if (rva == 0x893cb9 || rva == 0x898c0b || is_comptest) && seed != 0 {
...
const RA_TOURN_055: u64 = 0x200e3e5; // 0.5.6(구0.5.5=0x1c777d8). worker 0x1f16ea0 내 대회 배경 launcher retaddr(콜 0x1f24063, e8+5·launcher 9/9 전단사로 확정). // 0.5.5(구0.5.4=0x239f242). worker.rs:36 대회 배경 launcher retaddr(콜 0x1c777d3, e8+5)
...
const SEEDCTOR_RVA: usize = 0x132fc60; // 0.5.6(구0.5.5=0x14c2380). head-UNIQUE·seed 스토어 [rsi+0xec90]@0x10a4b0d 실측(provider 0xec90 불변)·프롤로그12 동일(chkstk imm 0x25438→0x256d8은 배열 밖). // 0.5.5(구0.5.4=0x14e16d0, 마스크시그 확정). seed 스토어 provider+0xeb28→+0xec90. // 0.5.4(구0.5.3=0x12b9ab0). 프롤로그 8push 동일·콜러 4개 동수·본문 sim 0.90·seed 스토어 1:1 대응. // 0.5.3(구0.5.2=0x22c1da0). 프롤로그 12B 완전동일(8push)·chkstk 프레임 0x11b58→0x11b98·launcher(0xeb8810) 내부 콜에서 rdx=저장된 r8(seed) 확인. ⚠seed 저장 오프셋은 provider+0xeab8→**+0xeaf8**로 이동(0x12ba92d 실측).
