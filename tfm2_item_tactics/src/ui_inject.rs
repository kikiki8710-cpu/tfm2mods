...
const LOADER_RVA: usize = 0x2e6f60; // 0.5.6(구0.5.5=0x2e42d0). 문자열-xref layout/main×17 수렴(clone family라 xref만 유효). // 0.5.5(구0.5.4=0x2e35d0). 문자열-xref 정본(layout/main×17·banpick 계열) 재현 OK, clone family라 xref만 유효. // 0.5.4(구0.5.3=0x2e1550). 문자열-xref 정본 경로 재도출: player_info 1 / wide 1 / strategy 2 / training 12 = 전부 이 함수로 수렴(0.5.3과 동수) + 본문 100% 동일. // 0.5.3(구0.5.2=0x5ac950). 진입 24B **바이트 완전동일**(push8+sub 0x98) → 12B 체이닝 install 무수정 동작.
const STRAT_LOADER_RVA: usize = 0x2e6f60; // 0.5.6: LOADER와 동일 copy(구0.5.5=0x2e42d0).
const PARSER_RVA: usize = 0x19ab40; // 0.5.6(구0.5.5=0x1a3e70, skel UNIQUE·BYTE=SAME). NT_SIZE 0x90 무변경. // 0.5.5(구0.5.4=0x1a3ce0, skel 유일). NT_SIZE 0x90 무변경. // 0.5.4(구0.5.3=0x1a6530). 본문 100% 동일·크기 0x890·콜러 5개 동수·mod.rs panic-loc 3개 동일 ⇒ NT_SIZE 0x90 무변경. // 0.5.3(구0.5.2=0x24b5a00). 3인자(out, ptr, len) 계약·`:`/`{`/`}` 파싱·에러 시 out[2]=-1·노드 stride 0x90 전부 동일 확인 ⟹ NT_SIZE 무변경.
...
const ALLOC_RVA: usize  = 0x2ab1670; // 0.5.6(구0.5.5=0x2a9bf30, skel/마스크 UNIQUE·BYTE=SAME). // 0.5.5(구0.5.4=0x29bb920, skel/마스크 유일). // 0.5.4(구0.5.3=0x28f7df0). 본문 100% 동일(IAT rip-rel만 이동)·콜러 32890→33708. // 0.5.3 힙 alloc 헬퍼(구0.5.2=0x25d9640에 대응. 구 __rust_alloc 심 0x25c4d30은 0.5.3에 없음).
