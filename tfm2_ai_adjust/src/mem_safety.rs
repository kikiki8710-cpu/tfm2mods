...
const TEXT_END_RVA: usize = 0x32d454e;   // ★0.5.8 실측(va 0x1000 + vsz 0x32d354f − 1). ~~0.5.6/0.5.7 0x32687ff~~ = 0.5.8 에선 **과소** → .text 후반 ~440KB 게터가 전부 차단(조용한 퇴화)였다.   // ★0.5.6(2026-08-20 실측 va 0x1000+vsz 0x3268800−1 — 0.5.4/0.5.5 미갱신이었음·진단 분류 전용). 구 ★0.5.3(was 0.5.2 0x2c087ff). PE .text va=0x1000 vsz=0x30a5200 → vsz_end=0x30a6200 실측(capstone/pefile 2026-07-29). ⚠구값을 두면 .text 후반 40% 게터가 전부 "범위밖"으로 차단돼 재현이 조용히 기본값으로 퇴화한다. ← 0.5.7 실측 va 0x1000+vsz 0x3267800-1 (0.5.6=0x32697ff, .text가 0x1000 줄었다)
