...
const RVA_RETREAT:  usize = 0xcead40;   // ★0.5.6(was 0.5.5 0xce0f70). retreat_engage. UNIQUE·size 11030 동일·BYTE=SAME·프롤로그 20B 동일. orig_len 12 유지.
...
const RVA_GENERIC_BUILD: usize = 0xccae00;  // ★0.5.6(was 0.5.5 0xcc1030). UNIQUE·size 27883 동일·BYTE=SAME·프롤로그 동일. orig_len 12 유지.

// ★facet#2 레인워크 waypoint 선택. install_replace_detour_rax 무조건설치 경로.
const RVA_FC59A0: usize = 0xd01950;  // ★0.5.6(was 0.5.5 0xcf7b80). recall_rng_score. UNIQUE·size 1459 동일·BYTE=SAME·프롤로그 20B 동일. orig_len 12.
...
const RVA_CONDGATE: usize = 0xe23190;   // ★0.5.6(was 0.5.5 0xe193b0). UNIQUE·size 513 동일·BYTE=SAME·프롤로그 20B 동일. orig_len 15 유지(경계 [0,1,2,3,7,15..] 동일).

const RVA_MOVEPRI: usize = 0xe23ad0;   // ★0.5.6(was 0.5.5 0xe19cf0). **Plan 디스패처**(SubPlan 아님).
...
const RVA_DISC18_HANDLER: usize = 0xd32b70;   // ★0.5.6(was 0.5.5 0xd28da0). UNIQUE·size 5891 동일·BYTE=SAME·프롤로그 동일. orig_len 12.

// ★SubPlan 디스패처(= disc18/19 핸들러를 직접 call 하는 그 함수).
//   ⚠2계층 주의: RVA_MOVEPRI 는 Plan 디스패처, 이건 SubPlan 디스패처. 번호공간이 다르다.
const RVA_SUBPLAN_DISPATCH: usize = 0xe55240;   // ★0.5.6(was 0.5.5 0xe4b460).
...
const RVA_DISC19_HANDLER: usize = 0xd426e0;   // ★0.5.6(was 0.5.5 0xd38910). UNIQUE·size 11221 동일·BYTE=SAME·프롤로그 동일. orig_len 12.
...
const RVA_ITEMNET_SCORER: usize = 0xf53de0;   // ★0.5.6(was 0.5.5 0x12624f0 — ⚠고대역 대이동 −0x30e710·저대역 델타 가정 금지 사례).
...
const RVA_AUCTION: usize = 0xe0e5e0;  // ★0.5.6(was 0.5.5 0xe04800). UNIQUE·size 19492 동일·BYTE=SAME.
