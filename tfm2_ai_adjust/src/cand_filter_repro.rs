// =====================================================================
//  ⛔ [0.5.7 휴면·DORMANT — 미배선·재RE 필요] ⛔
//    원본 FUN_141f4ec60/transition 0x1b64db0 = pre-0.5.7 stale(0.5.7 transition_engine=0xe723c0).
//    라이브 결정경로 미배선 + DIFF=0 미검증. ★0.5.7 transition과 구조 divergence:
//      - 0.5.7 로스터 후보루프(0xe72800~)는 레인당 gen_range 호출 전무(단순 null-skip push) — 이 재현의 레인당 RNG draw는 0.5.7 대응물 없음(구버전 잔재)
//      - cand+0x640은 0.5.7에서 우선도 비교 필드(cmp)이지 곱셈 가중치 아님
//    ⟹ 배선 전 반드시 0.5.7 재RE. (vt[0xe0]/[0xe8] 충돌은 action_score 전용, 본 모듈 무관.)
// =====================================================================
