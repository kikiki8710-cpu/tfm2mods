// ⚠⚠ STALE — 이 파일은 **0.5.6 재핀 이전** 상태다. 복원용으로 쓰지 말 것.
//   이걸로 되돌리면 0.5.7 마이그(DELAY/DRIVE/RUNNER_VT/TAKE/PAGE_IMM 재핀)가 통째로 날아간다.
//   진단↔릴리스 전환은 **현재 소스에서 `LOG_ENABLED` 플래그만** 토글할 것.
//   (2026-08-27 기록 — 필자가 실제로 이 파일로 복원할 뻔했다. 상세 = REPORT 03_시행착오)
...
const PATCHES: &[Patch] = &[
    ...
    Patch { name: "no_stamina_cost", rva: 0x22f9e6b,   // ★0.5.5(구0.5.4=0x20ecf0c) 마스크시그 k=1 유일·orig 05 실측·cont5=0x216e870 // 0.5.4(구0.5.3=0x17f6f44) `sub rax,5` imm
    ...
    Patch { name: "dr_inline_a", rva: 0x1abc656,   // ★0.5.5(구0.5.4=0x2306164) 마스크시그 k=1·orig 4c0f44e2·cont5=0x1a8aa10 // 0.5.4 재핀(구0.5.3=0x18d9436)
    ...
    Patch { name: "dr_inline_b", rva: 0x1ac7256,   // ★0.5.5(구0.5.4=0x2310c86) 마스크시그 k=1·orig 4120c5·cont5=0x1a95570(=CGATE) // 0.5.4 재핀(구0.5.3=0x18e3fd6)
    ...
    Patch { name: "dr_inline_d", rva: 0x1b3601c,   // ★0.5.5(구0.5.4=0x23ce6bc) 마스크시그 k=1·orig 4c0f44f8·cont5=0x1afcc20 // 0.5.4 재핀(구0.5.3=0x1987a3d)
    ...
    Patch { name: "panel_btn_daily_gate", rva: 0x1b36442,   // ★0.5.5(구0.5.4=0x23ceae2) 마스크시그 k=1·orig 20c1·cont5=0x1afcc20
    ...
    Patch { name: "daily_inc_gate", rva: 0x22f5619,   // ★0.5.5(구0.5.4=0x20e8246) 마스크시그 k=1·orig 04·cont5=0x216e870 // 0.5.4(구0.5.3=0x17f239c) `cmp rax,4` imm
    ...
    Patch { name: "server_pregate", rva: 0x22f2844,   // ★0.5.5(구0.5.4=0x20e5471) 마스크시그 k=1·orig 04·cont5=0x216e870 // 0.5.4 재핀(구0.5.3=0x17ef5f6) `cmp qword [rax+0x1d0],4`
            orig: &[0x04], fixed: &[0xff] },
    ...
    Patch { name: "server_dedup_real", rva: 0x2333e63,   // ★0.5.5(구0.5.4=0x2126f73) 2방법(마스크시그 k=1 + 컨테이너델타 off=0x473)·orig 0f85d3000000 실측 // 0.5.4(구0.5.3=0x1830df0)
    ...
    Patch { name: "allow_dup_players", rva: 0x1ac7701,   // ★0.5.5(구0.5.4=0x2311131) 컨테이너 difflib(cont 0x2310a90→0x1a95570)·orig 7547 실측 // 0.5.4(구0.5.3=0x18e4481)
    ...
    Patch { name: "server_dedup", rva: 0x22f162e,   // ★0.5.5(구0.5.4=0x20e42d1) 컨테이너 difflib·orig 7510 실측·no-op 유지 // 0.5.4(구0.5.3=0x17ee49c)
    ...
    Patch { name: "btn5v5_roster_min_a", rva: 0x1b36444,     // ★0.5.5(구0.5.4=0x23ceae4) 마스크시그 k=1·orig 4883fb0a0fb6f9b8·cont5=0x1afcc20 // 0.5.4(구0.5.3=0x1987e64) cmp rbx,0xa → 5
    ...
    Patch { name: "btn5v5_warn_text",    rva: 0x1b3605c,     // ★0.5.5(구0.5.4=0x23ce6fc) 마스크시그 k=1·orig 4883ff0ab8380000·cont5=0x1afcc20 // 0.5.4(구0.5.3=0x1987a7d) cmp rdi,0xa → 5
    ...
    Patch { name: "server_roster_min", rva: 0x2333dc0,  // ★0.5.5(구0.5.4=0x2126ed0) 2방법(마스크시그 k=1 + 컨테이너델타 off=0x3d0)·orig 4801db 실측·필요치 2×N → 1×N // 0.5.4(구0.5.3=0x1830d2e)
    ...
    Patch { name: "roster_count_gate", rva: 0x1ad4708,   // ★0.5.5(구0.5.4=0x231e155) 마스크시그 k=1·cont5=0x1aa2930(=RUN) // 0.5.4(구0.5.3=0x18f14ab)
    ...
    Patch { name: "collected_gate", rva: 0x1ad46fc,  // ★0.5.5(구0.5.4=0x231e142) 마스크시그 k=1·cont5=0x1aa2930 // 0.5.4(구0.5.3=0x18f149f)
    ...
    Patch { name: "collect_err_gate", rva: 0x1ad46df,  // ★0.5.5(구0.5.4=0x231e127) 마스크시그 k=1·cont5=0x1aa2930 // 0.5.4(구0.5.3=0x18f1484)
    ...
    Patch { name: "run_push_gate", rva: 0x1ad4e05,  // ★0.5.5(구0.5.4=0x231e838) 컨테이너 difflib(cont 0x231de30→0x1aa2930) // 0.5.4(구0.5.3=0x18f1b95)
...
const CT_REGION_LO: usize = 0x22e2e70; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x20d5bf0) 서버핸들러 .pdata 함수 시작(실측)       // 0.5.4(구0.5.3=0x17e0240)
...
const CT_CLIENT_LO: usize = 0x1a90000; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2300000) 클라 사이트 0x1a8ab76~0x1afe394 포괄(소비처 insert_detour inert)       // 0.5.4(구0.5.3=0x18c0000)
const CT_CLIENT_HI: usize = 0x1b40000; // 0.5.6(구0.5.5=0x1b00000, inert·신 클라사이트 max 0x1b0b054 커버)   // ★0.5.5(구0.5.4=0x23e0000)       // 0.5.4(구0.5.3=0x19a0000)
...
const RUN_RVA: usize = 0x1ad4410; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x231de30) 3방법(마스크시그 k=8 + 내부 3사이트 cont5 일치 + 콜그래프투표)·HOOK_PROLOGUE12 실측 MATCH // 0.5.4(구0.5.3=0x18f1180)
...
const CGATE_RVA: usize = 0x1ac7050; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2310a90) 마스크시그 k=12 + dr_inline_b/allow_dup 컨테이너·push8 확인
const SREG_RVA: usize = 0x23339f0; // 0.5.6 재핀    // ★0.5.5(구0.5.4=0x2126b00) skeleton UNIQUE size1400(§7.5 일치)
...
const RESULT_RVA: usize = 0x1b182e0; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x235b270) skeleton UNIQUE size540 (CONC_PROBE inert)
...
const CSEND_RVA: usize = 0x1ac2e80; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x230c910) PAGE_IMM owner·push8 12B 실측 MATCH
const HPUSH_RVA: usize = 0x17ae140; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x13006b0) skeleton UNIQUE size153·HPUSH_PROLOGUE 실측 MATCH // 커스텀 프롤로그(push rbp,rsi,rdi; sub rsp,0x60; lea rbp,[rsp+0x60])
...
const CLONE_CHAMP_RVA: usize = 0x1a3e450; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x193d560) RPLY_RESUME 사이트의 call 타깃으로 확정 // ChampionInfoSheet::clone(rcx=dst 0x7A90, rdx=&src)
const DROP_CHAMP_RVA: usize = 0x13557a0; // 0.5.6 재핀    // ⬜0.5.5 미확정(5후보 byte동일 monomorphized drop, RPLY_ON=false로 inert·0.5.4값 유지) // ChampionInfoSheet::drop_in_place(rcx=&val)
...
const RPLY2_RVA: usize = 0x1adcbd0; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x2326820) skeleton UNIQUE size1344 (RPLY2_ON=false inert) // 상세(.detail) 버튼 경로 — 대조군으로만 유지
...
const RPLY3_RVA: usize = 0x1ad9e50; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x2323aa0) 마스크시그 k=8·push8 확인 // 재시뮬 진입점(= 다시보기·상세 공통)
const LIVEB_RVA: usize = 0x1b18f90; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x235bf20) skeleton UNIQUE size3930·push8 확인 // 원본 경기 빌더(대조군, 읽기 전용)
...
const CTX_CLONE_RVA: usize = 0x1dec840; // 0.5.6 재핀     // ★0.5.5(구0.5.4=0x23e11e0) skeleton UNIQUE size5570·push8 확인 // (rcx=dst 0x20A0, rdx=src) -> rax=dst, 논리적 딥클론
...
const WARN_RVA: usize = 0x1aad370; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x22f6d00) skeleton UNIQUE size458·WARN_PROLOGUE 13B 실측 MATCH (CONC_PROBE inert)
...
const REFRESH_RVA: usize = 0x1abc4f0; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2306000) dr_inline_a 컨테이너·push8 확인(직접 CALL)
...
const FN_DD_SETOPT_RVA: usize = 0x1c1300; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x1bfc80) 다중앵커 투표 14/14 size2428 동일(자체 프롤로그 검증 있음)
...
const ITEMCONV_RVA: usize = 0x2345c00; // 0.5.6 재핀   // ★0.5.5 ghidra-re 2방법: movzx 앵커 0fb64325.. 전역유일 + HOOK_PROLOGUE12_ALT 실측·.pdata 엔트리. ⚠구값 ~~0x18429d0~~은 실은 0.5.3값(0.5.4 재핀 누락으로 0.5.4 내내 프롤로그검증 실패=죽어있었음)
...
const COLLECT_RVA: usize = 0x1ad5d70; // 0.5.6 재핀   // ★0.5.5 ghidra-re 2방법: TypeId+cmovb+mov rsi,[rsi+0x1788] 결합시그 유일 + push8 12B 실측·.pdata 엔트리. ⚠구값 ~~0x18f2b50~~은 실은 0.5.3값(0.5.4선 죽어있었음) // (구0.5.1=0x16203f0, 145 instr)
...
const ATH_GET_SC_RVA: usize = 0x12bdcf0; // 0.5.6 재핀           // ⬜0.5.5 미확정(SIM_PROBE_ON=false로 inert·0.5.4값 유지) // shadow-call: rcx=game_ctx+0x16b90, rdx=&id → rax(0=miss), athlete*=[rax]
...
const ORACLE_RVA: usize = 0x106bae0; // 0.5.6 재핀  // ★0.5.5(구0.5.4=0x13b3150) skeleton UNIQUE size5417·push8 확인 // 1틱 오케(run one tick), 프롤로그=HOOK_PROLOGUE12
...
const RUST_ALLOC_RVA: usize = 0x29f00e0; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x28f7df0) 다중앵커 투표 62/62 size927 동일 (game_alloc 소비처=slot/cosmetic OFF = inert)
