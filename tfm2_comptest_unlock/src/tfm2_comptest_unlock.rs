...
const PATCHES: &[Patch] = &[
    ...
    Patch { name: "panel_btn_daily_gate", rva: 0x23ceae2,
...
const SIMBODY_RVA: usize = 0x237c030;   // 0.5.4 sim 실행 본체(10회 롤아웃+본경기 완주+결과 조립)
...
const POLLER_RVA: usize = 0x148a7c0;    // 완주 폴러(구 0x206dc10) — 프롤로그 HOOK_PROLOGUE12 실측 MATCH
...
const G_OFF: usize = 0x1dc0;        // ctrl → game
const G_SCORE0: usize = 0xeb38;
const G_SCORE1: usize = 0xeb40;
const G_TICK: usize = 0xeb30;
const G_SEED: usize = 0xeb28;
...
const CGATE_RVA: usize = 0x2310a90;
const SREG_RVA: usize = 0x2126b00;
...
const MFORGE_RVA: usize = 0x2123590;
...
const RESULT_RVA: usize = 0x235b270;
...
const CSEND_RVA: usize = 0x230c910;   // push8 12B 실측 MATCH
const HPUSH_RVA: usize = 0x13006b0;   // 커스텀 프롤로그(push rbp,rsi,rdi; sub rsp,0x60; lea rbp,[rsp+0x60])
...
const RPLY_RVA: usize = 0x2323bb2;          // 스왑 훅(프레임 확보 후·첫 clone 직전)
...
const RPLY_RESUME_RVA: usize = 0x2323bc4;
const CLONE_CHAMP_RVA: usize = 0x193d560;   // ChampionInfoSheet::clone(rcx=dst 0x7A90, rdx=&src)
const DROP_CHAMP_RVA: usize = 0x182bf30;    // ChampionInfoSheet::drop_in_place(rcx=&val)
...
const RPLY2_RVA: usize = 0x2326820;         // 상세(.detail) 버튼 경로 — 대조군으로만 유지
...
const RPLY3_RVA: usize = 0x2323aa0;         // 재시뮬 진입점(= 다시보기·상세 공통)
const LIVEB_RVA: usize = 0x235bf20;         // 원본 경기 빌더(대조군, 읽기 전용)
...
const CTX_CLONE_RVA: usize = 0x23e11e0;     // (rcx=dst 0x20A0, rdx=src) -> rax=dst, 논리적 딥클론
const CTX_DROP_RVA: usize = 0x22df620;      // (rcx=&ctx) 정식 drop
...
const ARRIVE_RVA: usize = 0x2327080;
const ARRIVE_PROLOGUE: [u8; 15] = [0x55, 0x41, 0x57, 0x41, 0x56, 0x41, 0x54, 0x56, 0x57, 0x53,
                                   0xb8, 0xc0, 0x42, 0x00, 0x00];
const RUNNER_VT_RVA: usize = 0x33b91f8;   // node+0x238 == base+이 값 이어야 comp_test 러너
...
const WARN_RVA: usize = 0x22f6d00;
...
const TAKE_RVA: usize = 0x2311d43;      // imm32 = 기록탭 목록 take 한도(기본 0x14)
const TAKE_DEFAULT: u32 = 0x14;
const PAGE_IMM_RVA: usize = 0x230d0ec;  // imm8 = 기록 완료 후 이동할 페이지(기본 5=summary)
...
const REFRESH_RVA: usize = 0x2306000;
...
const DELAY_RVA: usize = 0x2327094;
const DELAY_ORIG: [u8; 15] = [0x48,0x29,0xc4, 0x48,0x8d,0xac,0x24,0x80,0x00,0x00,0x00,
                              0x48,0x83,0xe4,0x80];
const DELAY_RESUME_RVA: usize = 0x23270a3;
const EPILOG_RVA: usize = 0x232790a;     // xor eax,eax → lea rsp,[rbp+0x4240] → pop×7 → ret
const ARRIVE_FN_RVA: usize = 0x2327080;  // 재주입 시 직접 호출할 결과 도착 함수
const DETAIL_SZ2: usize = 0x20a0;
const ENTRY_SZ2: usize = 0xe0;
const DRIVE_RVA: usize = 0xa289c0;
...
const DRIVE_RESUME_RVA: usize = 0xa289d6;
const A15E20_RVA: usize = 0xa15e20;
...
const ORACLE_RVA: usize = 0x13b3150;  // 1틱 오케(run one tick), 프롤로그=HOOK_PROLOGUE12
