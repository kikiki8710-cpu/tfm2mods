...
//! ★0.6.0 stable ABI 껍데기(2026-09-17): 클래식 SDK 부재 → mod_api 의존부(Scene/GameUI/Node/ServerModContext)를 stable API 로 교체.
//!   · UI 노드(킬스코어 현황·범위 토글·runs 피커) = 로더 훅 템플릿 주입 → `ui_spawn_source(comp_test_popup 경로)` (ui_inject.rs ensure_spawned)
//!   · 로더 훅은 refresh_tick 용 Assets 포인터 캡처만 유지 · 아이템 드롭다운 오버레이는 클래식에서도 OFF(ITEM_DD_ENABLED=false) → 제거
//!   · 모드 아이템 목록 = 서버 `setting_get_json(ItemSetting, "mod_items")` (구 db+0x15d78 raw)
//!   · 훅/바이트패치/RVA·오프셋은 0.6.0 재핀(RE\2026-09-17_0.6.0-잔여재핀) 반영본 그대로. 클래식 판 백업 = src\tfm2_comptest_unlock.rs.bak_20260917_pre060
use mod_api_stable::{declare_stable_mod, SettingTargetV1, StableClient, StableExtension, StableHost, StableMod, StableServerCtx, StableServerExtension};
...
mod uk;  // 런타임 노드 조작(라벨·visible) = stable 경로 기반 키트
...
const PATCHES: &[Patch] = &[
    ...
    Patch { name: "no_stamina_cost", rva: 0x257772b,   // ★0.5.5(구0.5.4=0x20ecf0c) 마스크시그 k=1 유일·orig 05 실측·cont5=0x216e870 // 0.5.4(구0.5.3=0x17f6f44) `sub rax,5` imm
    ...
    Patch { name: "dr_inline_a", rva: 0x1d2e026,   // ★0.5.5(구0.5.4=0x2306164) 마스크시그 k=1·orig 4c0f44e2·cont5=0x1a8aa10 // 0.5.4 재핀(구0.5.3=0x18d9436)
    ...
    Patch { name: "dr_inline_b", rva: 0x1d38c86,   // ★0.5.5(구0.5.4=0x2310c86) 마스크시그 k=1·orig 4120c5·cont5=0x1a95570(=CGATE) // 0.5.4 재핀(구0.5.3=0x18e3fd6)
    ...
    Patch { name: "dr_inline_d", rva: 0x1da09df,   // ★0.5.5(구0.5.4=0x23ce6bc) 마스크시그 k=1·orig 4c0f44f8·cont5=0x1afcc20 // 0.5.4 재핀(구0.5.3=0x1987a3d)
    ...
    Patch { name: "panel_btn_daily_gate", rva: 0x1da0e02,   // ★0.5.5(구0.5.4=0x23ceae2) 마스크시그 k=1·orig 20c1·cont5=0x1afcc20
    ...
    Patch { name: "daily_inc_gate", rva: 0x2572708,   // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 구0.5.8=0x1a435d9 · cmp rax,4 전역 1히트 //   // ★0.5.5(구0.5.4=0x20e8246) 마스크시그 k=1·orig 04·cont5=0x216e870 // 0.5.4(구0.5.3=0x17f239c) `cmp rax,4` imm
    ...
    Patch { name: "server_pregate", rva: 0x256ee48,   // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 구0.5.8=0x1a40804 · `48 83 b8 d0 01 00 00 04 0f 86`+7 //   // ★0.5.5(구0.5.4=0x20e5471) 마스크시그 k=1·orig 04·cont5=0x216e870 // 0.5.4 재핀(구0.5.3=0x17ef5f6) `cmp qword [rax+0x1d0],4`
            orig: &[0x04], fixed: &[0xff] },
    ...
    Patch { name: "server_dedup_real", rva: 0x25b45d0,   // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 구0.5.8=0x1a82773 · SREG 0x2265f40 등록루프 · jne 변위 d3→d8 //   // ★0.5.5(구0.5.4=0x2126f73) 2방법(마스크시그 k=1 + 컨테이너델타 off=0x473)·orig 0f85d3000000 실측 // 0.5.4(구0.5.3=0x1830df0)
            orig: &[0x0f, 0x85, 0xd8, 0x00, 0x00, 0x00],
    ...
    Patch { name: "allow_dup_players", rva: 0x1d39131,   // ★0.5.5(구0.5.4=0x2311131) 컨테이너 difflib(cont 0x2310a90→0x1a95570)·orig 7547 실측 // 0.5.4(구0.5.3=0x18e4481)
    ...
    Patch { name: "server_dedup", rva: 0x1737a9e,   // ★0.5.5(구0.5.4=0x20e42d1) 컨테이너 difflib·orig 7510 실측·no-op 유지 // 0.5.4(구0.5.3=0x17ee49c)
    ...
    Patch { name: "btn5v5_roster_min_a", rva: 0x1da0e04,     // ★0.5.5(구0.5.4=0x23ceae4) 마스크시그 k=1·orig 4883fb0a0fb6f9b8·cont5=0x1afcc20 // 0.5.4(구0.5.3=0x1987e64) cmp rbx,0xa → 5
    ...
    Patch { name: "btn5v5_warn_text",    rva: 0x1da0a1f,     // ★0.5.5(구0.5.4=0x23ce6fc) 마스크시그 k=1·orig 4883ff0ab8380000·cont5=0x1afcc20 // 0.5.4(구0.5.3=0x1987a7d) cmp rdi,0xa → 5
    ...
    Patch { name: "server_roster_min", rva: 0x25b4531,  // ★0.5.5(구0.5.4=0x2126ed0) 2방법(마스크시그 k=1 + 컨테이너델타 off=0x3d0)·orig 4801db 실측·필요치 2×N → 1×N // 0.5.4(구0.5.3=0x1830d2e)
    ...
    Patch { name: "roster_count_gate", rva: 0x1d46130,   // ★0.5.5(구0.5.4=0x231e155) 마스크시그 k=1·cont5=0x1aa2930(=RUN) // 0.5.4(구0.5.3=0x18f14ab)
            // ⚠orig 바뀜: jae 변위 0x14d→0x161(0f8361010000). fixed jmp rel32 = disp+1 = 0x162(e962010000 90)로 재계산(타깃 0x1aa2d8f 동일).
            // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) jae 변위 0x161→0x172(0x1c65260 실측·타깃 0x1c653d8) → fixed jmp disp 0x173.
            orig: &[0x0f, 0x83, 0x72, 0x01, 0x00, 0x00],
            fixed: &[0xe9, 0x73, 0x01, 0x00, 0x00, 0x90] },
    ...
    Patch { name: "collected_gate", rva: 0x1d46124,  // ★0.5.5(구0.5.4=0x231e142) 마스크시그 k=1·cont5=0x1aa2930 // 0.5.4(구0.5.3=0x18f149f)
    ...
    Patch { name: "collect_err_gate", rva: 0x1d460fa,  // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 구0.5.8=0x23a513f · je 변위 52→5f //  // ★0.5.5(구0.5.4=0x231e127) 마스크시그 k=1·cont5=0x1aa2930 // 0.5.4(구0.5.3=0x18f1484)
            // ⚠orig 바뀜: je 변위 0x57→0x52(7452).
            orig: &[0x74, 0x5f], fixed: &[0x90, 0x90] },
    ...
    Patch { name: "run_push_gate", rva: 0x1d46865,  // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 구0.5.8=0x23a5865 · je 변위 fa05→f9d1 //  // ★0.5.5(구0.5.4=0x231e838) 컨테이너 difflib(cont 0x231de30→0x1aa2930) // 0.5.4(구0.5.3=0x18f1b95)
            // ⚠orig 바뀜: je 변위 fa12→fa05(0f8405faffff). fixed 6×NOP 불변.
            orig: &[0x0f, 0x84, 0xd1, 0xf9, 0xff, 0xff],
...
const INSERT_RVA: usize = 0xfc20d0; // ⬜죽은 상수(inert·함수시작 아님·0.5.1값 유지)
...
const CT_REGION_LO: usize = 0x255ce50; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x20d5bf0) 서버핸들러 .pdata 함수 시작(실측)       // 0.5.4(구0.5.3=0x17e0240)
const CT_REGION_HI: usize = 0x2589960; // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 서버 comp_test 핸들러 pdata 0x220f040..0x223b9bb 끝 (구0.5.7=0x230bc07) // 0.5.6(구0.5.5=0x219775c, inert·insert_detour 미발화·모노리스 0x2031a00+델타)   // ★0.5.5(구0.5.4=0x20ff156) = .pdata 함수 끝(실측)       // 0.5.4(구0.5.3=0x180920f)
const CT_CLIENT_LO: usize = 0x226d9f2; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2300000) 클라 사이트 0x1a8ab76~0x1afe394 포괄(소비처 insert_detour inert)       // 0.5.4(구0.5.3=0x18c0000)
const CT_CLIENT_HI: usize = 0x1daab50; // 0.5.6(구0.5.5=0x1b00000, inert·신 클라사이트 max 0x1b0b054 커버)   // ★0.5.5(구0.5.4=0x23e0000)       // 0.5.4(구0.5.3=0x19a0000)
...
const FORGE_CALLERS: &[usize] = &[0xf88280]; // 0.5.2(구0.5.1=0x1615480, 컨테이너 L1-UNIQUE 델타·retaddr 실측확인)
...
const ENQ_RVA: usize = 0xeb0e70; // ⬜죽은 상수(inert·0.5.1값 유지)
...
const RUN_RVA: usize = 0x1d45e20; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x231de30) 3방법(마스크시그 k=8 + 내부 3사이트 cont5 일치 + 콜그래프투표)·HOOK_PROLOGUE12 실측 MATCH // 0.5.4(구0.5.3=0x18f1180)
...
const SRV_RVA: usize = 0x1995180; // ⬜미확정(죽은 상수·훅 비활성·0.5.1값 유지)
...
const SIMBODY_RVA: usize = 0x25e3d10; // 0.5.6 재핀   // ⬜0.5.5 미확정(skeleton NONE=본문변경, CONC_PROBE_ON=false로 inert·install_hook12 자체검증 fail-safe·0.5.4값 유지) // 0.5.4 sim 실행 본체
...
const POLLER_RVA: usize = 0x142a1b0;    // ⬜0.5.5 미확정(skeleton NONE, CONC_PROBE_ON=false로 inert·fail-safe·0.5.4값 유지) // 완주 폴러
...
const PKT_DISC: usize = 0x1e;          // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) RUN 0x1c65b6c `mov qword [rax],0x1e` (구0.5.8=0x1c) — 틀리면 시드변조·match_register 전부 스킵          // 패킷 discriminant = comp_test RUN(검증용)
...
const CGATE_RVA: usize = 0x1d38a80; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2310a90) 마스크시그 k=12 + dr_inline_b/allow_dup 컨테이너·push8 확인
const SREG_RVA: usize = 0x25b4140; // 0.5.6 재핀    // ★0.5.5(구0.5.4=0x2126b00) skeleton UNIQUE size1400(§7.5 일치)
...
const MFORGE_RVA: usize = 0x27fff10;   // ★0.5.5(구0.5.4=0x2123590) skeleton UNIQUE size3101 (CONC_PROBE inert)
...
const RESULT_RVA: usize = 0x1d75b50; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x235b270) skeleton UNIQUE size540 (CONC_PROBE inert)
...
const CSEND_RVA: usize = 0x1d34850; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x230c910) PAGE_IMM owner·push8 12B 실측 MATCH
const HPUSH_RVA: usize = 0x10fd8c0; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x13006b0) skeleton UNIQUE size153·HPUSH_PROLOGUE 실측 MATCH // 커스텀 프롤로그(push rbp,rsi,rdi; sub rsp,0x60; lea rbp,[rsp+0x60])
...
fn fnv1a(addr: usize, len: usize) -> u64 {
    unsafe {
        if len == 0 || len > 0x202070 || addr < 0x10000 || !readable(addr, len) { return 0; }
...
const RPLY_RESUME_RVA: usize = 0xd0c14;   // ★0.5.5(구0.5.4=0x2323bc4) (RPLY_ON=false inert)
const CLONE_CHAMP_RVA: usize = 0x1e4fb50; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x193d560) RPLY_RESUME 사이트의 call 타깃으로 확정 // ChampionInfoSheet::clone(rcx=dst 0x7A90, rdx=&src)
const DROP_CHAMP_RVA: usize = 0x15e3ca0; // 0.5.6 재핀    // ⬜0.5.5 미확정(5후보 byte동일 monomorphized drop, RPLY_ON=false로 inert·0.5.4값 유지) // ChampionInfoSheet::drop_in_place(rcx=&val)
...
const RPLY2_RVA: usize = 0x1d4e7f0; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x2326820) skeleton UNIQUE size1344 (RPLY2_ON=false inert) // 상세(.detail) 버튼 경로 — 대조군으로만 유지
...
const RPLY3_RVA: usize = 0x1d4ba60; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x2323aa0) 마스크시그 k=8·push8 확인 // 재시뮬 진입점(= 다시보기·상세 공통)
const LIVEB_RVA: usize = 0x1d77430; // 0.5.6 재핀         // ★0.5.5(구0.5.4=0x235bf20) skeleton UNIQUE size3930·push8 확인 // 원본 경기 빌더(대조군, 읽기 전용)
...
const CTX_CLONE_RVA: usize = 0x2ac0c90; // 0.5.6 재핀     // ★0.5.5(구0.5.4=0x23e11e0) skeleton UNIQUE size5570·push8 확인 // (rcx=dst 0x20A0, rdx=src) -> rax=dst, 논리적 딥클론
const CTX_DROP_RVA: usize = 0x1cc9730;      // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) ARRIVE 2회 호출·size 0x376·maxoff 0x2080·헤드 [rcx+0x1dc0]/[+0x1dc8] = 0.5.8 0x1bc2e10 바이트동일 (자동값 0x1d07790 = CLONE_CHAMP 중간 → 크래시)      // ★0.5.5(구0.5.4=0x22df620) ghidra-re 2방법: ARRIVE(0x1aab950) 실호출 대상 + 러너 vtable drop 타입짝(원소drop FUN_141851730 공유)·최고접근오프셋 0x2080<0x20A0 // (rcx=&ctx) 정식 drop
...
const WARN_RVA: usize = 0x1d1ea30; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x22f6d00) skeleton UNIQUE size458·WARN_PROLOGUE 13B 실측 MATCH (CONC_PROBE inert)
...
extern "win64" fn warn_detour(rcx: usize, rdx: usize, r8: usize, r9: usize) -> usize {
    ...
    if n < 40 {
        ...
        let why = match (r8, rva) {
            ...
            (0x38, 0x3b0c0ed) => "★챔피언 미지정(champion_required)",
            (0x38, 0x3b0c125) => "선수 중복(duplicate_players)",
...
const TAKE_RVA: usize = 0x1d39d4a; // 0.5.7 재핀(match_mid UNIQUE, mov dword[rbp+0x278],0x14 주변16B 동일) // 0.5.6=0x1aafa9a      // ★0.5.5(구0.5.4=0x2311d43) 마스크시그 k=1·imm32 14000000 실측·cont5=0x1a96710 // imm32 = 기록탭 목록 take 한도(기본 0x14)
const TAKE_DEFAULT: u32 = 0x14;
const PAGE_IMM_RVA: usize = 0x1d3507c; // 0.5.7 재핀(패턴 41c6860c24000005 전역유일 0x1ac36a5+7) // 0.5.6=0x1aaae2c  // ★0.5.5(구0.5.4=0x230d0ec) 고정패턴 41c6860c24000005 전역유일·imm off+7 // imm8 = 기록 완료 후 이동할 페이지(기본 5=summary)
...
const REFRESH_RVA: usize = 0x1d2dec0; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x2306000) dr_inline_a 컨테이너·push8 확인(직접 CALL)
...
fn scope_tick(ctx: &mut StableClient<'_>, popup: Option<&str>) {
    if !RESULTVIEW_ON { return; }
    let Some(pp) = popup else { return };
    let _ = pp;
    uk::ensure_clicks(ctx, &[
        (ui_inject::HS_LAST_ID.to_string(), std::sync::Arc::new(|| { HIST_SCOPE.store(0, Ordering::Relaxed); REFRESH_REQ.store(1, Ordering::Relaxed); SCROLL_RESET.store(6, Ordering::Relaxed); }) as uk::ClickFn),
        (ui_inject::HS_ALL_ID.to_string(), std::sync::Arc::new(|| { HIST_SCOPE.store(1, Ordering::Relaxed); REFRESH_REQ.store(1, Ordering::Relaxed); SCROLL_RESET.store(6, Ordering::Relaxed); }) as uk::ClickFn),
        (ui_inject::RUNS_DEC_ID[0].to_string(), std::sync::Arc::new(runs_dec) as uk::ClickFn),
        (ui_inject::RUNS_INC_ID[0].to_string(), std::sync::Arc::new(runs_inc) as uk::ClickFn),
        (ui_inject::RUNS_DEC_ID[1].to_string(), std::sync::Arc::new(runs_dec) as uk::ClickFn),
        (ui_inject::RUNS_INC_ID[1].to_string(), std::sync::Arc::new(runs_inc) as uk::ClickFn),
    ]);
    if SCROLL_RESET.load(Ordering::Relaxed) > 0 {
        SCROLL_RESET.fetch_sub(1, Ordering::Relaxed);
        // stable 엔 스크롤 세터가 없다 — 목록 재생성(refresh_tick) 이 맨 위로 돌린다.
    }
    let page = unsafe {
    ...
    for i in 0..2 {
        ...
        uk::set_visible(ctx, ui_inject::RUNS_BOX_ID[i], want);
        if want { uk::label_set(ctx, ui_inject::RUNS_VAL_ID[i], &format!("한 번에 {}경기", runs)); }
    }
    let on_history = unsafe {
        let node = RV_NODE.load(Ordering::Relaxed);
        node != 0 && runner_of(node).map(|r| runner_state(r) == 1).unwrap_or(false)
    };
    uk::set_visible(ctx, ui_inject::HS_BOX_ID, on_history);
    let sc = HIST_SCOPE.load(Ordering::Relaxed);
    let show_wl = on_history && sc == 0 && M_N.load(Ordering::Relaxed) > 0;
    for id in [ui_inject::WL_BL_ID, ui_inject::WL_BN_ID, ui_inject::WL_C_ID, ui_inject::WL_RN_ID, ui_inject::WL_RL_ID] {
        uk::set_visible(ctx, id, show_wl);
    }
    if on_history {
        uk::toggle_set(ctx, ui_inject::HS_LAST_ID, sc == 0);
        uk::toggle_set(ctx, ui_inject::HS_ALL_ID, sc == 1);
    }
    if show_wl {
        let n = M_N.load(Ordering::Relaxed) as usize;
        ...
        for (id, txt, col) in [
            ...
            uk::label_set(ctx, id, &txt);
            uk::label_set_color(ctx, id, uk::Rgba::hex(col));
        }
    }
}
...
fn killscore_overlay(ctx: &mut StableClient<'_>, popup: Option<&str>) {
    if !KS_ON || !CONC_ON { return; }
    let n = M_N.load(Ordering::Relaxed) as usize;
    if n == 0 { return; }
    let Some(_pp) = popup else { return };
    if KS_LOGGED.fetch_add(1, Ordering::Relaxed) == 0 {
        let found = (0..ui_inject::KS_MAX).filter(|&i| uk::exists(ctx, &ui_inject::ks_id(i))).count();
        let msg_ok = uk::exists(ctx, ui_inject::KS_MSG_ID);
        log(&format!("[ks] 스폰={} 런타임노드 발견={}개 msg={} \n", ui_inject::KS_INJECTED.load(Ordering::Relaxed), found, msg_ok));
    }
    ...
    if show {
        uk::set_visible(ctx, "loading", false);
        uk::set_visible(ctx, "summary", false);
    }
    uk::set_visible(ctx, ui_inject::KS_MSG_ID, show);
    for i in 0..ui_inject::KS_MAX { uk::set_visible(ctx, &ui_inject::ks_id(i), show && i < n); }
    if !show { return; }
    ...
    } else { format!("결과 정리 중  {} / {}", HPUSH_HITS.load(Ordering::Relaxed).min(n as u64), n) };
    {
        ...
        if *last != msg { uk::label_set(ctx, ui_inject::KS_MSG_ID, &msg); *last = msg; }
    }
    for i in 0..n.min(ui_inject::KS_MAX) {
        ...
        let line = if t0 == 0 { format!("경기 {}   대기 중", i + 1) }
            else if fin { format!("경기 {}   {} : {}   완료", i + 1, b, r) }
            else { let pct = ((tick.min(KS_TICK_FULL) * 100) / KS_TICK_FULL).min(99); format!("경기 {}   {} : {}   {}%", i + 1, b, r, pct) };
        let col = match M_WIN[i].load(Ordering::Relaxed) { 2 => ui_inject::COL_BLUE, 1 => ui_inject::COL_RED, _ => ui_inject::COL_DIM };
        let id = ui_inject::ks_id(i);
        uk::label_set(ctx, &id, &line);
        uk::label_set_color(ctx, &id, uk::Rgba::hex(col));
    }
}
...
const DELAY_RVA: usize = 0x1d4f064; // 0.5.7 재핀(ARRIVE+0x14, ORIG 15B MATCH) // 0.5.6=0x1ac4bc4   // ★0.5.5(구0.5.4=0x2327094) 컨테이너 difflib(cont 0x2327080→0x1aab950)·DELAY_ORIG 15B 실측 MATCH
const DELAY_ORIG: [u8; 15] = [0x48,0x29,0xc4, 0x48,0x8d,0xac,0x24,0x80,0x00,0x00,0x00,
                              0x48,0x83,0xe4,0x80];
const DELAY_RESUME_RVA: usize = 0x1d4f073; // 0.5.7 재핀(mov rbx,rsp 확인) // 0.5.6=0x1ac4bd3   // ★0.5.5(구0.5.4=0x23270a3) 컨테이너 difflib(mov rbx,rsp)
const EPILOG_RVA: usize = 0x1d4f8da; // 0.5.7 재핀(xor eax,eax + lea rsp,[rbp+0x4240] 확인) // 0.5.6=0x1ac5409     // ★0.5.5(구0.5.4=0x232790a) 컨테이너 difflib(xor eax,eax) // → lea rsp,[rbp+..] → pop×7 → ret
const ARRIVE_FN_RVA: usize = 0x1d4f050; // 0.5.6 재핀  // ★0.5.5(구0.5.4=0x2327080) 투표 37/37 // 재주입 시 직접 호출할 결과 도착 함수
const DETAIL_SZ2: usize = 0x20a0;
const ENTRY_SZ2: usize = 0xe0;
const DRIVE_RVA: usize = 0x83db6a; // 0.5.7 재핀(owner 0x888cc0->0xb62c80 HEAD_UNIQUE, off=0x60) // 0.5.6=0x888d20   // ★0.5.5(구0.5.4=0xa289c0) 컨테이너델타 UNIQUE(off=0x60)·DRIVE_ORIG 22B 실측 MATCH
// ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 0.6.0 진입부 19B: mov [rbp+0x16e90],-1 (11B) + mov [rbp+0x18],-1 (8B; 구 [rbp+0x2070] 11B 가 8B 인코딩으로)
const DRIVE_ORIG: [u8; 19] = [0x48,0xc7,0x85,0x80,0x6e,0x01,0x00,0xff,0xff,0xff,0xff, // ★0.6.1: 프레임 0x18258→0x18248(−0x10) ⟹ [rbp+0x16e90]→[rbp+0x16e80] (2026-09-22 실측 +0x6a)
                              0x48,0xc7,0x45,0x18,0xff,0xff,0xff,0xff];
const DRIVE_RESUME_RVA: usize = 0x83db7d; // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) = DRIVE+0x13 (구 +0x16 규칙 폐기; 구0.5.8=0x8e6a96) // 0.5.7 재핀(DRIVE+0x16) // 0.5.6=0x888d36   // ★0.5.5(구0.5.4=0xa289d6) 컨테이너델타(off=0x76)
const A15E20_RVA: usize = 0x799b60;   // ★0.5.5(구0.5.4=0xa15e20) 다중앵커 투표 50/50 size974→988
...
unsafe extern "win64" fn par_drive_shim() {
    ...
    core::arch::naked_asm!(
        ...
        "mov r8, [rbp+0x18240]",   // ★0.6.1 frame: sub rsp,0x18248 → p4 = [rbp+0x18240] (0.6.0 0x18258→0x18250 · 구 0x17480)
        "sub rsp, 0x20",
        "call {drv}",
        "add rsp, 0x20",
        "pop r11", "pop r10", "pop r9", "pop r8", "pop rdx", "pop rcx",
        // 스틸한 원본 2명령 재현
        "mov qword ptr [rbp+0x16e80], -1",   // ★0.6.1 (0.6.0 0x16e90 · 구 0x16bb8)
        "mov qword ptr [rbp+0x18], -1",      // ★0.6.0 (구 0x2070)
        "jmp qword ptr [rip + {res}]",
...
const DEDUP_INS_RVA: usize = 0xe0e210; // ⬜미확정(죽은 상수·inert·0.5.1값 유지)
const SPAWN_CP_RVA: usize = 0x13f7c7d;   // ⬜미확정(죽은 상수·inert 확인필·0.5.1값 유지)
...
const PUSH_RVA: usize = 0x1122d38;   // ⬜미확정(죽은 상수·프로브 호출 비활성 상태·0.5.1값 유지)
...
const FN_DD_SETOPT_RVA: usize = 0x1e5d80; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x1bfc80) 다중앵커 투표 14/14 size2428 동일(자체 프롤로그 검증 있음)
...
// (0.6.0 stable) Node 기반 헬퍼(runner_base/find_rb/find_node/find_mut/node_box/set_node_box_all_states/dd_set_options/dd_selected) 제거 — 클래식 백업 참조
...
const ITEMCONV_RVA: usize = 0x25c6dc0; // ★0.6.0(ghidra-re 2026-09-17, RE6-09-17_0.6.0-잔여재핀) 콜그래프(0x2262a60 내 2회 연속 호출)·HOOK_PROLOGUE12_ALT·pdata 0x2278dc0..0x22791a9 (구0.5.8=0x1a96640) // // 0.5.6 재핀   // ★0.5.5 ghidra-re 2방법: movzx 앵커 0fb64325.. 전역유일 + HOOK_PROLOGUE12_ALT 실측·.pdata 엔트리. ⚠구값 ~~0x18429d0~~은 실은 0.5.3값(0.5.4 재핀 누락으로 0.5.4 내내 프롤로그검증 실패=죽어있었음)
...
const COLLECT_RVA: usize = 0x1d47980; // 0.5.6 재핀   // ★0.5.5 ghidra-re 2방법: TypeId+cmovb+mov rsi,[rsi+0x1788] 결합시그 유일 + push8 12B 실측·.pdata 엔트리. ⚠구값 ~~0x18f2b50~~은 실은 0.5.3값(0.5.4선 죽어있었음) // (구0.5.1=0x16203f0, 145 instr)
...
const ATH_GET_SC_RVA: usize = 0x12971b0; // 0.5.6 재핀           // ⬜0.5.5 미확정(SIM_PROBE_ON=false로 inert·0.5.4값 유지) // shadow-call: rcx=game_ctx+0x16b90, rdx=&id → rax(0=miss), athlete*=[rax]
...
const ORACLE_RVA: usize = 0x1a81910; // 0.5.6 재핀  // ★0.5.5(구0.5.4=0x13b3150) skeleton UNIQUE size5417·push8 확인 // 1틱 오케(run one tick), 프롤로그=HOOK_PROLOGUE12
...
const SLOT_RVA: usize = 0x18ad390;   // ⬜0.5.5 미확정(SIM_PROBE_ON=false로 inert·install_hook12 fail-safe·0.5.4값 유지)
...
const RUST_ALLOC_RVA: usize = 0x2e9ee80; // 0.5.6 재핀   // ★0.5.5(구0.5.4=0x28f7df0) 다중앵커 투표 62/62 size927 동일 (game_alloc 소비처=slot/cosmetic OFF = inert)
...
const ATH_GET_RVA: usize = 0x45d460; // ⬜죽은 상수(0.4.x 잔재·훅 비활성·실체=ATH_GET_SC_RVA)
const ATH_GET_JE_TARGET_RVA: usize = 0x45d51b; // ⬜죽은 상수(위와 동일)
const ATH_GET_PROLOGUE: [u8; 17] = [0x56, 0x57, 0x48, 0x83, 0xec, 0x28, 0x48, 0x83, 0x79, 0x18, 0x00, 0x0f, 0x84, 0xaa, 0x00, 0x00, 0x00];
const CT_ARM_LO: usize = 0x1500000;              // ⬜미확정(0.5.0_2 기준·HYBRID 비활성이라 inert)
const CT_ARM_HI: usize = 0x186de40;              // ⬜미확정(위와 동일)
...
fn init(host: &StableHost) -> StableMod {
    ...
    log(&format!("[{}ms] === tfm2_comptest_unlock INIT ({} patches) game {}.{}.{} host_abi={} ===\n", now_ms(), PATCHES.len(), v.major, v.minor, v.patch, host.abi_level()));
    unsafe {
    ...
    d.set_extension(CompTestExt);
    d.set_server_extension(CompTestServerExt);
    d
}

// 클라 확장 = UI 주입 훅 설치(매프레임 멱등). ★로더 훅은 "늦게" 설치해야 다른 모드 훅 위에 체인된다.
struct CompTestExt;
impl StableExtension for CompTestExt {
    fn post_update(&self, ctx: &mut StableClient<'_>, _dt: u64) {
        ...
        // ★0.6.0 stable: 로더 훅은 Assets 포인터 캡처(refresh_tick 용)만 — UI 노드는 아래 스폰으로.
        if UI_INJECT_ON { unsafe { let _ = uinj::install(); } }
        let popup = ct_popup_path(ctx);
        if let Some(pp) = popup.as_deref() { ui_inject::ensure_spawned(ctx, pp); }
        scope_tick(ctx, popup.as_deref());              // (29) 범위 토글·runs 피커
        killscore_tick();                               // (24) 실시간 킬스코어 현황판 = **기능**(진단 아님)
        killscore_overlay(ctx, popup.as_deref());       // (27) 킬스코어 현황 오버레이
        }));
    }
}

/// comp_test 팝업 런타임 경로(`comp_test_popup` id, `main` 아래 얕은 DFS). 없으면 None. 60프레임마다 재탐색.
...
// 서버 확장 = 모드 아이템 목록(최종템) — ★0.6.0 stable: `setting_get_json(ItemSetting, "mod_items")`.
struct CompTestServerExt;
impl StableServerExtension for CompTestServerExt {
    fn on_server_start(&self, ctx: &mut StableServerCtx<'_>) {
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| load_mod_items_stable(ctx)));
    }
}
fn load_mod_items_stable(ctx: &StableServerCtx<'_>) {
    if MODITEMS_DONE.swap(true, Ordering::Relaxed) { return; }
    let Some(j) = ctx.setting_get_json(SettingTargetV1::ItemSetting, "mod_items") else { log("[moditem] ItemSetting.mod_items 조회 실패\n"); return };
    // 의존 없는 최소 파싱: 배열 원소마다 "key"/"id"/"name" 후보 + "next_tier" 배열 비어있음 = 최종템. serde_json 없이 문자열 스캔.
    let mut finals: Vec<(u64, String)> = Vec::new();
    let mut idx = 0usize;
    let mut depth = 0i32; let mut start: Option<usize> = None;
    let b = j.as_bytes();
    for (i, &c) in b.iter().enumerate() {
        match c {
            b'{' => { if depth == 1 { start = Some(i); } depth += 1; }
            b'}' => { depth -= 1; if depth == 1 { if let Some(s) = start.take() {
                let e = &j[s..=i];
                let key = ["\"key\":\"", "\"id\":\"", "\"name\":\""].iter().find_map(|k| e.find(k).map(|p| { let r = &e[p + k.len()..]; r[..r.find('"').unwrap_or(0)].to_string() })).unwrap_or_default();
                let nt_empty = e.find("\"next_tier\":").map(|p| e[p + 12..].trim_start().starts_with("[]")).unwrap_or(false);
                log(&format!("[moditem] id={:<4} next_empty={} {}{}\n", 30 + idx, nt_empty, key, if nt_empty { " ★최종템" } else { "" }));
                if nt_empty && !key.is_empty() { finals.push(((30 + idx) as u64, key)); }
                idx += 1;
            } } }
            _ => {}
        }
    }
    log(&format!("[moditem] 모드 아이템 {}개 / 최종템 {}개 (stable ItemSetting.mod_items)\n", idx, finals.len()));
    *MOD_FINALS.lock().unwrap_or_else(|e| e.into_inner()) = finals;
}
