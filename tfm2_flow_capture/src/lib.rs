...
const RUN_TICK_RVA: usize = 0x14aa160; // 0.5.5(구0.5.4=0x13b3150). disp-masked 구조skel UNIQUE + comptest ORACLE 교차 + 프롤로그14/size 0x1529 동일
...
const SCENE_STEP_RVA: usize = 0x196c2c0; // 0.5.5(구0.5.4=0x1dad900). banpick_order PHASE_SCENE 마이그 동일 함수 + movzx[rcx+0xce]@0x196c2e2 시그 확인
...
const RECO_INNER_RVA: usize = 0x12ae860; // 0.5.5(구0.5.4=0x14a0240). skel UNIQUE(disp 포함, delta -0x1f19e0) + 프롤로그12 동일 = 본문 무변경
...
const CTOR_RVA: usize = 0x14ac3e0; // 0.5.5(구0.5.4=0x13b53d0). 구조skel LCP 342(압도) + item_tactics/serpen LAUNCHER 교차 + 프롤로그[13] 0x68→0x38(chkstk imm 0x25168→0x25438). 첫12B=HOOK_PROLOGUE12 불변(설치체크 통과)
