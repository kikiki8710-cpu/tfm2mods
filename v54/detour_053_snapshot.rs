...
unsafe fn apply_objective_imm() {
    ...
    if let Some(p) = pth("obj_imm.txt") {
        let _ = fs::write(p, format!("nx_enable={} applied={}/14 cg={}=DEAD nh={} hc={} hl={} lm={} an={}=DEAD near={} pred={} fh={} cull={} @base{:#x}\n",
...
//   ⚠disc17 시야창은 기배선 nx_dn_vision_mem(0xdf9513·disc17/19 공용 헬퍼) 소관 — 이중패치 금지로 여기서 제외.
...
unsafe fn apply_visshort_imm() {
    ...
    for rva in [0xd80b2eusize, 0xd80bf5, 0xd80cb7, 0xd80d79, 0xd80e42] {
        ok += patch_imm_bytes(base + rva, &[0x48,0x83,0xc7], 3, 1, enc(lane)) as u32;
    }
    // ── jungle: disc4 컨테이너(0xd71630) — add rdi ×4 + add rsi ×1 ──
    for rva in [0xd71d27usize, 0xd71dec, 0xd71eac, 0xd71f6c] {
        ok += patch_imm_bytes(base + rva, &[0x48,0x83,0xc7], 3, 1, enc(jungle)) as u32;
    }
    ok += patch_imm_bytes(base + 0xd7202e, &[0x48,0x83,0xc6], 3, 1, enc(jungle)) as u32;
    // ── check: 공용 bool 헬퍼 0x12b6e20 — add rbx ──
    ok += patch_imm_bytes(base + 0x12b6e78, &[0x48,0x83,0xc3], 3, 1, enc(check)) as u32;
    // ── nexus: 헬퍼 0xc8e4e0(add r13 — ⚠disc19 관찰 shadow-CALL 대상과 동일 함수=값 일관 자동 유지)·0xd10d00(add r14) ──
    ok += patch_imm_bytes(base + 0xc8e55d, &[0x49,0x83,0xc5], 3, 1, enc(nexus)) as u32;
    ok += patch_imm_bytes(base + 0xd10e33, &[0x49,0x83,0xc6], 3, 1, enc(nexus)) as u32;
    // ── threat: sev[A] 헬퍼 0xd36b00(add r12, 비트마스크형)·sev[B] 헬퍼 0xc4d6f0(add r15) ──
    ok += patch_imm_bytes(base + 0xd36ca2, &[0x49,0x83,0xc4], 3, 1, enc(threat)) as u32;
    ok += patch_imm_bytes(base + 0xc4d871, &[0x49,0x83,0xc7], 3, 1, enc(threat)) as u32;
    // ── score: sev[E] 본체(0xc7f640) add rsi ×5 + 헬퍼 0xcc8060 add rbx ×1·add rsi ×4 ──
    for rva in [0xc806d9usize, 0xc807b1, 0xc80885, 0xc80959, 0xc80a29] {
        ok += patch_imm_bytes(base + rva, &[0x48,0x83,0xc6], 3, 1, enc(score)) as u32;
    }
    ok += patch_imm_bytes(base + 0xcc834c, &[0x48,0x83,0xc3], 3, 1, enc(score)) as u32;
    for rva in [0xcc8495usize, 0xcc85ce, 0xcc8720, 0xcc8845] {
        ok += patch_imm_bytes(base + rva, &[0x48,0x83,0xc6], 3, 1, enc(score)) as u32;
    }
...
unsafe fn apply_gank_imm() {
    ...
    const A_SITES: [(usize, u8, [u8; 3], usize, u8, [u8; 3], bool); 5] = [
        (0xe0237d, 0x89, [0x48,0x8d,0x0c], 0xe02381, 0x48, [0x48,0x8d,0x04], false), // A1 passive_jungle 10초
        (0xe02cba, 0x49, [0x48,0x8d,0x0c], 0xe02cbe, 0x88, [0x48,0x8d,0x04], false), // A2 passive_jungle 12초
        (0xd45967, 0x89, [0x48,0x8d,0x0c], 0xd4596b, 0x49, [0x48,0x8d,0x0c], true),  // A3 GankPlan 수락 15초
        (0xd557ae, 0x89, [0x48,0x8d,0x0c], 0xd557b2, 0x49, [0x48,0x8d,0x3c], true),  // A4 핸들러 15초
        (0xd55cda, 0x89, [0x48,0x8d,0x0c], 0xd55cde, 0x48, [0x48,0x8d,0x1c], false), // A5 핸들러 10초
    ];
    ...
    // ── B. 라인개입(jng=1) 리드액션 HP 게이트 base (mov dl,imm8 @0xe01e53, 원본 0x46=70) ──
    let bv = if hpb >= 0 { hpb.min(100) as u64 } else { 0x46 };
    ok += patch_imm_bytes(base + 0xe01e53, &[0xb2], 1, 1, bv) as u32;
    ...
    for rva in [0xe01ef7usize, 0xe01f9e, 0xe020d4] {
...
unsafe fn apply_exec_imm() {
    ...
    let wdst = tune("ex_wait_dist", -1);     // 대기 경로 전환 거리(유닛, 원본 180000)
    let wbak = tune("ex_wait_back", -1);     // 목적지에서 뒤로 물러날 거리(원본 180000)
    let hold = tune("ex_order_hold", -1);    // 오더 유지 최소 경과 틱(원본 10)
    let mut sig = 0u64;
    for v in [jcap, jslp, jflr, wdst, wbak, hold] { sig = sig.wrapping_mul(0x100000001b3) ^ (v as u64); }
    ...
    ok += patch_imm_bytes(base + 0xc60837, &[0x48,0x83,0xfa], 3, 1, b1(jcap, 100)) as u32;   // cmp rdx,100
    ok += patch_imm_bytes(base + 0xc60844, &[0x6b,0xc0], 2, 1, b1(jslp, 85)) as u32;         // imul eax,eax,85
    ok += patch_imm_bytes(base + 0xc60853, &[0x05], 1, 4, b4(jflr, 150)) as u32;             // add eax,150
    // ── ② 대기 위치 (line_wait) ──
    ok += patch_imm_bytes(base + 0xd971b6, &[0x48,0xb9], 2, 8,
                          if wdst < 0 { 0x78B30C401 } else { sq1(wdst) }) as u32;            // movabs rcx, 180000²+1
    ok += patch_imm_bytes(base + 0xd974c9, &[0x48,0x2d], 2, 4, b4(wbak, 0x2bf20)) as u32;    // sub rax, 180000
    // ── ③ 오더 유지 최소 경과 (lib.rs 유닛 AI 틱) ──
    ok += patch_imm_bytes(base + 0xd0a4fc, &[0x48,0x83,0xc0], 3, 1, b1(hold, 10)) as u32;    // add rax,10
    EXECIMM_SIG.store(sig, Ordering::Relaxed);
    if let Some(p) = pth("exec_imm.txt") {
        let _ = fs::write(p, format!("applied={}/6 judge=[cap{} slope{} floor{}] wait=[dist{} back{}] order_hold={} (-1=원본: 100/85/150/180000/180000/10) @base{:#x}\n",
            ok, jcap, jslp, jflr, wdst, wbak, hold, base));
    }
}
...
unsafe fn apply_cast_imm() {
    ...
    // ── ① 시전 후보 생성기 (0xcb3ab0) ──
    p!(base + 0xcb3efd, &[0x41,0xbc], 2, 4, b4(la, 30));
    p!(base + 0xcb440a, &[0xb9],      1, 4, b4(la, 30));
    p!(base + 0xcb4781, &[0xb9],      1, 4, b4(ls, 30));
    p!(base + 0xcb5a7d, &[0xbb],      1, 4, b4(ls2, 30));
    p!(base + 0xcb6936, &[0xba],      1, 4, b4(lst, 30));
    p!(base + 0xcb8036, &[0x48,0x6b,0x85,0x60,0x02,0x00,0x00], 7, 1, b1(lu, 60));
    p!(base + 0xcb81d7, &[0x48,0x6b,0x86,0x28,0x06,0x00,0x00], 7, 1, b1(lu, 60));
    p!(base + 0xcb7ac9, &[0xb9],      1, 4, sq(ur, 36_000_000));          // 6000²
    p!(base + 0xcb7ae1, &[0xb8],      1, 4, sq(ur, 36_000_000));
    p!(base + 0xcb7ae6, &[0x48,0xb9], 2, 8, sq(urg, 8_100_000_000));      // 90000²
    p!(base + 0xcb7bfa, &[0xb9],      1, 4, b4(umk, 0x6f));
    for a in [0xcb6a26usize, 0xcb6a36] { p!(base + a, &[0x48,0x83,0xf8], 3, 1, b1(shp, 20)); }
    for a in [0xcb4317usize, 0xcb4331] { p!(base + a, &[0x48,0x83,0xf8], 3, 1, b1(uh, 2)); }
    for a in [0xcb4ee6usize, 0xcb61b6] { p!(base + a, &[0x48,0x83,0xf8], 3, 1, b1(ahp, 79)); }
    for a in [0xcb4d9fusize, 0xcb4e12, 0xcb4fd2, 0xcb6080, 0xcb60eb, 0xcb629b] {   // 120000²+1
        pany!(base + a, [[0x48,0xb9],[0x48,0xba],[0x49,0xb9],[0x49,0xba]], 2, 8, sqp(arad, 14_400_000_001));
    }
    for a in [0xcb4e9ausize, 0xcb616a] {                                           // 120000²
        pany!(base + a, [[0x49,0xbb],[0x48,0xbb],[0x49,0xb9],[0x48,0xb9]], 2, 8, sq(arad, 14_400_000_000));
    }
    p!(base + 0xcb756b, &[0x48,0x83,0xc3], 3, 1, b1(mvis, 120));
    for a in [0xcb3f07usize, 0xcb4874, 0xcb5a86, 0xcb7cb4] {
        pany!(base + a, [[0x41,0xbd],[0x41,0xb9],[0x41,0xbc],[0x41,0xbb],[0x41,0xba]], 2, 4, b4(ccm, 0x3B8));
    }
    // ── ② 행동 실행층 ──
    p!(base + 0xcc3489, &[0x49,0x83,0xbe,0xb0,0x05,0x00,0x00], 7, 1, b1(sk2l, 3));
    p!(base + 0xcc36b9, &[0x49,0x83,0xbe,0xb0,0x05,0x00,0x00], 7, 1, b1(ultl, 5));
    p!(base + 0xc87c76, &[0xba], 1, 4, b4(am, 15000));
    p!(base + 0xc87c71, &[0xb8], 1, 4, b4(ams, 2000));
    p!(base + 0xc87a81, &[0x83,0xc2], 2, 1, b1(asek, 100));
    p!(base + 0xd0a85b, &[0x48,0x83,0xf9], 3, 1, b1(fmt, 119));
    p!(base + 0xd0cb6b, &[0x48,0x81,0xc1], 3, 4, b4(tmin, 400));
    p!(base + 0xd0cb74, &[0x48,0x8d,0x04,0x85], 4, 4, b4(tmax, 800));
...
unsafe fn apply_score_imm() {
    ...
    // ── ① 수적우세 배율 (코드 즉치 2개) ──
    p!(base + 0xc7d300, &[0xb9], 1, 4, b4(ahi, 200));
    p!(base + 0xc7d30f, &[0xb9], 1, 4, b4(alo, 30));
    ...
    // ── ② 인식 반경·보너스 ──
    p!(base + 0xc7ccde, &[0x49,0xbe], 2, 8, sq(arad, 22_500_000_000));      // 150000²
    for a in [0xc7d189usize, 0xc7d599] {
        p!(base + a, &[0x48,0xba], 2, 8, sqp(erad, 10_000_000_000));        // 100000²+1
    }
    p!(base + 0xc7d5a6, &[0xbb], 1, 4, b4(nbon, 10));
    p!(base + 0xd9bac1, &[0xb8], 1, 4, b4(obon, 10));
    // ── ③ 라인 수비 후보 유지 하한 (두 인코딩이 한 임계를 이룸: N, N−1) ──
    p!(base + 0xc3cf8d, &[0x48,0x83,0xf8], 3, 1, s1(if rthr_orig { -30 } else { rthr }));
    p!(base + 0xc3cf9e, &[0x48,0x83,0xf8], 3, 1, s1(if rthr_orig { -31 } else { rthr - 1 }));
    // ── ④ 대기(line_wait) / 라인 안전(line_safe) ──
    p!(base + 0xd971b6, &[0x48,0xb9], 2, 8, sqp(lwd, 32_400_000_001));      // 180000²+1
    p!(base + 0xd974c9, &[0x48,0x2d], 2, 4, b4(lwb, 180_000));
    for a in [0xd96dd0usize, 0xd9743b, 0xd97a76] {
        p!(base + a, &[0x48,0xc7,0x85], 7, 4, b4(lwr, 80_000));
    }
    for a in [0xdefd8ausize, 0xdefe65] {
        p!(base + a, &[0x48,0xc7,0x85], 7, 4, b4(lsr, 80_000));
    }
    // ── ⑤ 이동 실행층 ──
    p!(base + 0xd87f22, &[0x48,0x81,0xfa], 3, 4, sqp(mvb, 256_000_001));    // 16000²+1
    p!(base + 0xd89ad0, &[0x48,0x81,0xfa], 3, 4, sq(mvh, 144_000_000));     // 12000²
    p!(base + 0xc7b4a5, &[0x48,0xb9], 2, 8, sqp(mvt, 14_400_000_001));      // 120000²+1
...
unsafe fn apply_score2_imm() {
    ...
    const CMP3: [[u8;3];4] = [[0x48,0x81,0xfe],[0x49,0x81,0xfa],[0x48,0x81,0xfa],[0x49,0x81,0xf9]];
    const CMP1: [[u8;3];4] = [[0x48,0x83,0xf8],[0x49,0x83,0xf8],[0x48,0x83,0xff],[0x49,0x83,0xff]];
    ...
    for a in [0xc7f8b9usize, 0xc7f939, 0xc7f9ac, 0xc7fa1f, 0xc7faa0, 0xc7fcaf,
              0xc80289, 0xc80309, 0xc80397, 0xc80418, 0xc8048b, 0xc80513] {
        pany!(base + a, CMP3, 3, 4, tr_a);
    }
    for a in [0xc7fd67usize, 0xc80be7] { pany!(base + a, CMP3, 3, 4, tr_a.wrapping_sub(1)); }
    // ── ② 적 챔피언 근접 인식 (d²>>9) ──
    for a in [0xc80679usize, 0xc80751, 0xc80825, 0xc808f9, 0xc809cd] {
        pany!(base + a, CMP3, 3, 4, dsh(erad2, 0x1BF08EA, 9));
    }
    // ── ③ 셀 위협 진입 거리 (d²+1) ──
    pany!(base + 0xc80e02, CMP3, 3, 4, sqp(cdst, 0x49040441));
    // ── ④ 다이브 여유분 ──
    pany!(base + 0xc7fe82, [[0x48,0x05]], 2, 4, b4(dvm, 15000));
    // ── ⑤ 위험 판정 사다리 ──
    for (a, v, o) in [(0xc82224usize, rd0, 49u64), (0xc8222a, rh1, 65), (0xc82230, rd1, 29),
                      (0xc82236, rh2, 40), (0xc8223c, rd2, 17), (0xc82244, rh3, 25),
                      (0xc8224a, rd3, 10)] {
        pany!(base + a, CMP1, 3, 1, b1(v, o));
    }
    // ── ⑥ 보너스 상한·기준 ──
    pany!(base + 0xc827c4, CMP1, 3, 1, b1(fcap, 80));
    pany!(base + 0xc827c8, [[0x41,0xbe]], 2, 4, b4(fcap, 80));
    ...
    pany!(base + 0xc82f27, CMP1, 3, 1, b1(kcap, 80));
    pany!(base + 0xc82f2b, [[0xb8]], 1, 4, b4(kcap, 80));
    pany!(base + 0xc82f76, CMP1, 3, 1, b1(kcap, 80));
    pany!(base + 0xc82f7a, [[0xb8]], 1, 4, b4(kcap, 80));
    // ⚠`c83391`은 `mov`가 아니라 **`cmp rax,80`** 이다(짝이 되는 `mov ecx,80`이 c83395). 08-03 정정.
    pany!(base + 0xc83391, CMP1, 3, 1, b1(kcap, 80));
    pany!(base + 0xc83395, [[0xb8],[0xb9],[0xbb]], 1, 4, b4(kcap, 80));
    pany!(base + 0xc82f69, CMP1, 3, 1, b1(kpct, 60));
    // ── ⑦ 점수 계산의 시야 기억 ──
    // ⚠[08-05 감사] 같은 축이 5곳인데 여기서 잡는 건 1곳뿐이다. 나머지 4곳(`0xc807b1`·`0xc80885`·
    //   `0xc80959`·`0xc80a29`)은 **`vw_score`가 10사이트로 이미 소유**한다 — 여기서 또 잡으면 두 노브가
    //   같은 바이트를 다투게 되므로(마지막에 쓴 쪽이 이김) **의도적으로 1곳만 유지**한다.
    //   사용자에겐 `vw_score`를 쓰라고 안내한다(편집기 설명 반영).
    for a in [0xc806d9usize] { pany!(base + a, [[0x48,0x83,0xc6],[0x49,0x83,0xc6],[0x48,0x83,0xc5]], 3, 1, b1(svis, 120)); }
    // ── ⑧ 본체 0일 때 대체 점수(음수) ──
    let nv = if nul_orig { -10i64 } else { nul };
    for a in [0xc83620usize, 0xc838e1] {
...
unsafe fn apply_move_imm() {
    ...
    let mvm  = tune("mv_vision_mem", -1);   // 이동 점수의 적 "최근 목격" 유효틱(원본 120)
    let mut sig = 0u64;
    for v in [m0hi, m0lo, m0m1, m0z, m0p1, m0rs, m0es, m0nb, m0ng, m0bp,
              mtm, mtc, m2gs, met, mvm] {
        sig = sig.wrapping_mul(0x100000001b3) ^ (v as u64);
    }
    ...
    // ── ① cat0 배율 (코드 즉치 2개) ──
    p!(base + 0xc7c7ae, &[0x41,0xb8], 2, 4, b4(m0hi, 300));
    p!(base + 0xc7c7be, &[0x41,0xb8], 2, 4, b4(m0lo, 40));
    ...
    p!(base + 0xc7c7f5, &[0x49,0xc1,0xf9], 3, 1, b1(m0rs, 2));    // sar r9, 2   (÷4)
    p!(base + 0xc7c803, &[0x48,0xc1,0xfa], 3, 1, b1(m0es, 9));    // sar rdx, 9  (÷800)
    {   // add r12, −2  → imm8 그대로. 센티널 이하면 원본 유지.
        tot += 1;
        let want = if m0bp_orig { 0xfeu64 } else { (m0bp as i8) as u8 as u64 };
        ok += patch_imm_bytes(base + 0xc7c80e, &[0x49,0x83,0xc4], 3, 1, want) as u32;
    }
    p!(base + 0xc7d5a6, &[0xbb], 1, 4, b4(m0nb, 10));
    p!(base + 0xc7d4f0, &[0x48,0x81,0x7d,0x68], 4, 4, b4(m0ng, 950));
    // ── ③ 포탑 사거리 감산 / 상한 3쌍 ──
    for a in [0xc7c664usize, 0xc7ca21] {
        p!(base + a, &[0x48,0x81,0xee], 3, 4, b4(mtm, 30000));
    }
    //   상한은 `cmp rax,100` 과 `mov reg,100` **두 곳을 같이** 고쳐야 의미가 맞는다.
    for (ca, mv, mpre) in [(0xc7c8edusize, 0xc7c8f1usize, &[0x41,0xbe][..]),
                           (0xc7cb06,      0xc7cb0a,      &[0xb9][..]),
                           (0xc7d392,      0xc7d396,      &[0x41,0xbc][..])] {
        p!(base + ca, &[0x48,0x83,0xf8], 3, 1, b1(mtc, 100));
        p!(base + mv, mpre, mpre.len(), 4, b4(mtc, 100));
    }
    // ── ④ cat2 goal_gain 가중 / 예상 피격 필터 / 시야 기억 ──
    p!(base + 0xc7d3d0, &[0x48,0xc1,0xfa], 3, 1, b1(m2gs, 7));    // sar rdx, 7 (÷200)
    for a in [0xc7c09busize, 0xc7c3ea, 0xc7cb7f] {
    ...
    for a in [0xc7cd8ausize, 0xc7ce72, 0xc7cf58, 0xc7d03e] {
        p!(base + a, &[0x48,0x83,0xc7], 3, 1, b1(mvm, 120));
    }
    p!(base + 0xc7d126, &[0x48,0x83,0xc6], 3, 1, b1(mvm, 120));
...
unsafe fn apply_db_imm() {
    ...
    for a in [0xdafa40usize, 0xdafab6, 0xdafb2c, 0xdafba2, 0xdafc14] {
        p!(base + a, &[0x48,0xb8], 2, 8, sqp(dna, 0x5_3D1A_C101));
    }
    for a in [0xdafce1usize, 0xdafd68, 0xdafdef, 0xdafe76, 0xdafefd] {
        p!(base + a, &[0x48,0xb8], 2, 8, sqp(dne, 0x5_3D1A_C101));
    }
    // ── ② 사거리 판정 선행 틱수 (speed × N) ──
    for a in [0xdaff72usize, 0xdb09f5, 0xdb1de3, 0xdb2def] {
        p!(base + a, &[0xb9], 1, 4, b4(dla, 30));
    }
    p!(base + 0xdb05b0, &[0x41,0xb9], 2, 4, b4(dla, 30));
    p!(base + 0xdb4aac, &[0x48,0x6b,0x8d], 7, 1, b1(dul, 60));
    p!(base + 0xdb4c5a, &[0x49,0x6b,0x83], 7, 1, b1(dul, 60));
    // ── ③ 처형·막타·스킬 허용 HP% ──
    ...
    p!(base + 0xdb2ed6, &[0x48,0x83,0xf8], 3, 1, b1(dex, 20));
    p!(base + 0xdb2ee6, &[0x48,0x83,0xf8], 3, 1, b1(dex, 20));
    p!(base + 0xdb04be, &[0x48,0x83,0xf8], 3, 1, b1(dlh, 2));
    p!(base + 0xdb04d0, &[0x48,0x83,0xf8], 3, 1, b1(dlh, 2));
    for a in [0xdb11e6usize, 0xdb25a6] {
        p!(base + a, &[0x48,0x83,0xf8], 3, 1, b1(dsh_, 79));
    }
    // ── ④ 궁 게이트 ──
    p!(base + 0xdb43bd, &[0xb9], 1, 4, sq(dur, 36_000_000));      // 6000²
    p!(base + 0xdb43d5, &[0xb8], 1, 4, sq(dur, 36_000_000));
    p!(base + 0xdb43da, &[0x48,0xb9], 2, 8, sqp(dur2, 0x1_E2CC_3100)); // 90000²+1
    p!(base + 0xdb441e, &[0x48,0x3d], 2, 4, b4(durg, 150_000));
    p!(base + 0xdb4699, &[0xb9], 1, 4, b4(dm1, 0x6f));
    p!(base + 0xdb492c, &[0xb9], 1, 4, b4(dm2, 0x4e));
    p!(base + 0xdb4953, &[0xb9], 1, 4, b4(dm3, 0x21));
    // ── ⑤ 스킬2·궁 해금 레벨 ──
    p!(base + 0xdaf9ab, &[0x48,0x83,0xfa], 3, 1, b1(ds2l, 3));
    p!(base + 0xdaf9cf, &[0x48,0x83,0xfa], 3, 1, b1(dulv, 5));
    // ── ⑥ 안전판정 `0xc8b560` — 위 값들과 같이 안 바꾸면 효과가 상쇄된다 ──
    p!(base + 0xc8b99b, &[0x48,0x05], 2, 4, b4(dsm, 15_000));
    // ★[08-05 감사] 3곳 → **5곳**(같은 함수의 5슬롯 언롤). 짝인 `sf_mem`이 5/5인 것과 대조해 확정.
    for a in [0xc8baf9usize, 0xc8bbe6, 0xc8bcc2, 0xc8bd9e, 0xc8be7a] {
        p!(base + a, &[0x48,0xb8], 2, 8, sq(dsr, 0x3_5A4E_9000));  // 120000²
    }
    for a in [0xc8bb5eusize, 0xc8bc4b, 0xc8bd27, 0xc8be03, 0xc8becc] {
...
unsafe fn apply_pe_imm() {
    ...
    for a in [0xcca385usize, 0xcca3c0, 0xcca454, 0xcca490, 0xcca523, 0xcca560, 0xcca5f7, 0xcca62d] {
        p!(base + a, &[0x48,0xb8], 2, 8, sqp(pcol, 0x9_502F_9001));
    }
    for a in [0xcca6c3usize, 0xcca6f8, 0xccaa2e, 0xccaa60] {
        p!(base + a, &[0x48,0xb8], 2, 8, sq(pcol, 0x9_502F_9000));
    }
    for a in [0xccbc19usize, 0xccbda6, 0xccbf36, 0xccc0a6] {
        p!(base + a, &[0x48,0x81,0xf9], 3, 4, dsh(pflt, 87_890_625, 8, 1));
    }
    for a in [0xcccfc8usize, 0xccd068] {
        p!(base + a, &[0x48,0x81,0xfa], 3, 4, dsh(pflt, 87_890_625, 8, 1));
    }
    p!(base + 0xccaeef, &[0x48,0x3d], 2, 4, dsh(pflt, 87_890_624, 8, 0));
    p!(base + 0xccd108, &[0x48,0x81,0xfa], 3, 4, dsh(pflt, 87_890_624, 8, 0));
    for a in [0xccd76eusize, 0xccd78e, 0xccdad8] {
        p!(base + a, &[0x49,0xbb], 2, 8, sq(pflt, 0x5_3D1A_C100));
    }
    for a in [0xccbfaausize, 0xccc11a] {
        p!(base + a, &[0x49,0x81,0xfe], 3, 4, dsh(pnea, 19_140_625, 8, 1));
    }
    for a in [0xccbc86usize, 0xccbe16] {
        p!(base + a, &[0x49,0x81,0xfe], 3, 4, dsh(pnea, 19_140_624, 8, 0));
    }
    p!(base + 0xccd3f3, &[0xb8], 1, 4, sq(pmin, 4_096_000_000));
    p!(base + 0xccefaa, &[0x48,0x81,0xf9], 3, 4, dsh(pcha, 9_765_625, 10, 0));
    p!(base + 0xccdcf1, &[0x49,0x81,0xf8], 3, 4, dsh(pfld, 244_140_624, 8, 0));
    p!(base + 0xcd01f5, &[0x48,0xb8], 2, 8, sqp(pcnt, 0x3_5A4E_9001));
    p!(base + 0xcd0aab, &[0x48,0xb8], 2, 8, sq(pcnt, 0x3_5A4E_9000));
    // ── ② 선형 반경·여유 ──
    for a in [0xcce53cusize, 0xcce574, 0xcce6dc, 0xcce714, 0xcce8ac, 0xcce8e4, 0xccea4c,
              0xccea84, 0xccec2c, 0xccec64, 0xccedcc, 0xccee04, 0xcd059b, 0xcd05cf] {
        p!(base + a, &[0x49,0x81,0xc0], 3, 4, b4(prea, 80_000));
    }
    for a in [0xccf4dausize, 0xccf758, 0xccfe3f] {
        p!(base + a, &[0x48,0x81,0xc1], 3, 4, b4(prea, 80_000));
    }
    for a in [0xcd0720usize, 0xcd0754, 0xcd0890, 0xcd08c8] {
        p!(base + a, &[0x49,0x81,0xc1], 3, 4, b4(prea, 80_000));
    }
    for a in [0xccb5afusize, 0xccdac4] { p!(base + a, &[0x48,0x05], 2, 4, b4(pband, 32_000)); }
    p!(base + 0xccfd38, &[0x48,0x05], 2, 4, b4(pshot, 20_000));
    for a in [0xcd00beusize, 0xcd0190, 0xcd0b1c] {
        p!(base + a, &[0x48,0x81,0xc2], 3, 4, b4(pblk, 28_000));
    }
    p!(base + 0xccccc4, &[0x48,0x05], 2, 4, b4(ptwr, 18_000));
    p!(base + 0xccde6f, &[0x49,0x81,0xc1], 3, 4, b4(ptwr, 18_000));
    // ── ③ 캡·비율 ──
    //   ⚠150 캡은 `cmp`와 `mov`가 **쌍**이라 둘 다 안 고치면 의미가 어긋난다.
    //   실측된 cmp 인코딩 5종·mov 인코딩 2종. `48 3d`(cmp rax)는 prefix가 2바이트라 imm 오프셋도 2다.
    const CAPCMP: [(&[u8], usize); 5] = [(&[0x48,0x81,0xf9], 3), (&[0x49,0x81,0xfc], 3),
                                         (&[0x48,0x81,0xff], 3), (&[0x48,0x81,0xfb], 3),
                                         (&[0x48,0x3d], 2)];
    const CAPMOV: [(&[u8], usize); 3] = [(&[0xb8], 1), (&[0xb9], 1), (&[0xbb], 1)];
    for (ca, mv) in [(0xccafffusize, 0xccb006usize), (0xccb1d6, 0xccb1dd), (0xccc611, 0xccc618),
        ...
        pmulti!(base + ca, CAPCMP, 4, b4(pcap, 150));
        pmulti!(base + mv, CAPMOV, 4, b4(pcap, 150));
    }
    p!(base + 0xccd5c5, &[0x48,0x3d], 2, 4, b4(ppcap, 140));
    p!(base + 0xccd5cb, &[0xbb], 1, 4, b4(ppcap, 140));
    for a in [0xccce81usize, 0xcccef2] { p!(base + a, &[0x69,0xc0], 2, 4, b4(pfar, 656)); }
    p!(base + 0xcd0e8e, &[0x41,0xbe], 2, 4, b4(pna2, 1000));
    p!(base + 0xcd0e9e, &[0xb9], 1, 4, b4(pna, 2000));
    p!(base + 0xcd0e38, &[0x48,0x81,0xf9], 3, 4, b4(pnex, 100_000));
    p!(base + 0xcd0db9, &[0x48,0x6b,0xcf], 3, 1, b1(pks, 120));
    p!(base + 0xcd0ddc, &[0x49,0x6b,0x48,0x10], 4, 1, b1(pks, 120));
    for a in [0xccb681usize, 0xccd609] { p!(base + a, &[0xba], 1, 4, b4(pmsk, 0x1a1)); }
    p!(base + 0xccd654, &[0xb9], 1, 4, b4(pkm, 0x303));
    p!(base + 0xcc9eaf, &[0x48,0xc7,0x00], 3, 4, b4(pwall, 9999));
    p!(base + 0xcca0a6, &[0x48,0xc7,0x02], 3, 4, b4(pwell, 9999));
    p!(base + 0xcca0ad, &[0x48,0xc7,0x42,0x08], 4, 4, b4(pwell, 9999));
    p!(base + 0xcca0b5, &[0xb8], 1, 4, b4(pwell, 9999));
    p!(base + 0xcd0a31, &[0x48,0x81,0xbd,0x28,0x04,0x00,0x00], 7, 4, b4(pagc, 1200));
    //   세 사이트가 **읽는 구조체 오프셋도 레지스터도 다르다**(+0xb8/rdi, +0xc0/rdi, +0xc8/rcx).
    const STGATE: [(&[u8], usize); 3] = [(&[0x48,0x81,0xbf,0xb8,0x00,0x00,0x00], 7),
                                         (&[0x48,0x81,0xbf,0xc0,0x00,0x00,0x00], 7),
                                         (&[0x48,0x81,0xb9,0xc8,0x00,0x00,0x00], 7)];
    for a in [0xcce400usize, 0xcce77d, 0xcceaf4] {
        pmulti!(base + a, STGATE, 4, b4(pst, 180));
    }
...
unsafe fn apply_move2_imm() {
    ...
    p!(base + 0xc8694b, &[0x48,0x3d], 2, 4, b4(snap, 2000));
    p!(base + 0xc86a36, &[0x48,0x69,0xc2], 3, 4, b4(acf, 400));
    p!(base + 0xc86a77, &[0x48,0x83,0xc2], 3, 1, b1(50, 50));   // 위 계수 입력 보정(고정)
    p!(base + 0xc86a86, &[0x48,0x05], 2, 4, b4(amg, 6000));
    p!(base + 0xc86f23, &[0x48,0x3d], 2, 4, b4(abi, 1500));
    // ── 우물 탈출(두 함수 미러) ──
    for a in [0xd94766usize, 0xc86807] {
        p!(base + a, &[0x48,0xb8], 2, 8, sq(wr, 67_600_000_000));
    }
    p!(base + 0xd94863, &[0x49,0x69,0xc5], 3, 4, b4(wd, 260_000));
    p!(base + 0xd9486a, &[0x4d,0x69,0xce], 3, 4, b4(wd, 260_000));
    p!(base + 0xc8689a, &[0x49,0x69,0xc7], 3, 4, b4(wd, 260_000));
    p!(base + 0xc868a1, &[0x4d,0x69,0xce], 3, 4, b4(wd, 260_000));
    p!(base + 0xd87c92, &[0x48,0x83,0x7f,0x18], 4, 1, b1(apm, 10));
...
unsafe fn apply_bv_imm() {
    ...
    for (c, m) in [(0xcc5fcfusize, 0xcc5fd6usize), (0xcc6598, 0xcc659f), (0xcc9004, 0xcc900b)] {
        p!(base + c, &[0x48,0x81,0xf9], 3, 4, b4(cap160, 160));
        p!(base + m, &[0xb8], 1, 4, b4(cap160, 160));
    }
    for (c, m) in [(0xcc690busize, 0xcc690fusize), (0xcc6f83, 0xcc6f87)] {
        p!(base + c, &[0x48,0x83,0xf9], 3, 1, b1(cap80, 80));
        p!(base + m, &[0xb8], 1, 4, b4(cap80, 80));
    }
    // ── 집중포화 ──
    p!(base + 0xcc71dd, &[0x48,0x83,0xfa], 3, 1, b1(focus, 3));
    p!(base + 0xcc71e1, &[0xb8], 1, 4, b4(focus, 3));
    p!(base + 0xcc54ee, &[0x49,0x83,0xf8], 3, 1, b1(focus, 3));
    p!(base + 0xcc54f2, &[0xb8], 1, 4, b4(focus, 3));
    //   ⚠반경 10사이트 중 2곳(`cc71c9`·`cc5b78`)은 **REX가 없어 imm 오프셋이 1**이다.
    for a in [0xcc7092usize, 0xcc70e2, 0xcc7131, 0xcc7180] {
        p!(base + a, &[0x41,0xb9], 2, 4, sqp(frad, 3_600_000_000));
    }
    p!(base + 0xcc71c9, &[0xb9], 1, 4, sqp(frad, 3_600_000_000));
    for a in [0xcc5a32usize, 0xcc5a84, 0xcc5ad3, 0xcc5b22] {
        p!(base + a, &[0x41,0xbb], 2, 4, sqp(frad, 3_600_000_000));
    }
    p!(base + 0xcc5b78, &[0xba], 1, 4, sqp(frad, 3_600_000_000));
    // ── cc9100 하드코딩 점수표 ──
    p!(base + 0xcc9341, &[0xbf], 1, 4, b4(aflat, 10));
    p!(base + 0xcc939c, &[0x48,0x83,0xf8], 3, 1, b1(acap, 90));
    p!(base + 0xcc93a0, &[0xbf], 1, 4, b4(acap, 90));
    p!(base + 0xcc93c3, &[0xba], 1, 4, b4(aout, 5));
    p!(base + 0xcc9254, &[0xb8], 1, 4, b4(bin, 25));
    p!(base + 0xcc9259, &[0xba], 1, 4, b4(bout, 8));
    p!(base + 0xcc9332, &[0xb8], 1, 4, b4(din, 90));
    p!(base + 0xcc9337, &[0xba], 1, 4, b4(dout, 30));
    p!(base + 0xcc949e, &[0x48,0x83,0xf9], 3, 1, b1(ccap, 60));
    p!(base + 0xcc94a2, &[0xba], 1, 4, b4(ccap, 60));
    {   // mov rdx, −100
        tot += 1;
        let want = if cnone_orig { (-100i32) as u32 as u64 } else { (cnone as i32) as u32 as u64 };
        ok += patch_imm_bytes(base + 0xcc92b7, &[0x48,0xc7,0xc2], 3, 4, want) as u32;
    }
...
unsafe fn apply_ae_imm() {
    ...
    p!(base + 0xdf58de, &[0x41,0xb8], 2, 4, b4(msk, 0x1F863));
    p!(base + 0xdf5b96, &[0x49,0xc1,0xfe], 3, 1, b1(rsh, 6));
    p!(base + 0xdf5bcf, &[0x48,0xc1,0xf8], 3, 1, b1(tsh, 6));
    p!(base + 0xdf5cfe, &[0x48,0xc1,0xfa], 3, 1, b1(gsh, 7));
    // ⚠루프 A와 B는 **같은 값인데 인코딩이 다르다**(A=`41 bc` off 2 / B=`ba` off 1).
    p!(base + 0xdf5f01, &[0x41,0xbc], 2, 4, b4(bsoon, 25));
    p!(base + 0xdf63b7, &[0xba], 1, 4, b4(bsoon, 25));
    p!(base + 0xdf5fbc, &[0x41,0xbc], 2, 4, b4(bkill, 140));
    p!(base + 0xdf636e, &[0xba], 1, 4, b4(bkill, 140));
    p!(base + 0xdf5fdc, &[0x41,0xbc], 2, 4, b4(bnear, 70));
    p!(base + 0xdf638a, &[0xba], 1, 4, b4(bnear, 70));
    p!(base + 0xdf6430, &[0x48,0x8d,0x4a], 3, 1, b1(bstru, 80));
    for a in [0xdf5b51usize, 0xdf5b9d] {
...
unsafe fn apply_th_imm() {
    ...
    for a in [0xd07d1ausize, 0xd07e06, 0xd07ee6] {
        p!(base + a, &[0x48,0x05], 2, 4, b4(smg, 18_000));
    }
    p!(base + 0xd08501, &[0x48,0x81,0xc1], 3, 4, b4(smg, 18_000));
    p!(base + 0xd0850d, &[0x49,0x81,0xc3], 3, 4, b4(amg, 50_000));
    // ── +32000 여유 13사이트 (레지스터가 6종으로 갈리지만 prefix 길이는 전부 3) ──
    const LEA32: [(&[u8], usize); 6] = [(&[0x48,0x8d,0x8d], 3), (&[0x48,0x8d,0x8b], 3),
                                        (&[0x49,0x8d,0x8a], 3), (&[0x49,0x8d,0x8f], 3),
                                        (&[0x4c,0x8d,0xb7], 3), (&[0x4d,0x8d,0x82], 3)];
    const LEA32B: [(&[u8], usize); 6] = [(&[0x49,0x8d,0xaf], 3), (&[0x48,0x8d,0xbb], 3),
                                         (&[0x48,0x8d,0x8a], 3), (&[0x4c,0x8d,0xa2], 3),
                                         (&[0x49,0x8d,0xbe], 3), (&[0x49,0x8d,0xab], 3)];
    for a in [0xd0851cusize, 0xd08533, 0xd0854a, 0xd085a1, 0xd085c5, 0xd086e4, 0xd086f9,
              0xd09295, 0xd092ab, 0xd092c8, 0xd09394, 0xd093ab, 0xd093c3] {
        tot += 1;
        let mut done = false;
        for (pre, off) in LEA32.iter().chain(LEA32B.iter()) {
            if !done && patch_imm_bytes(base + a, pre, *off, 4, b4(band, 32_000)) { done = true; }
        }
        ok += done as u32;
    }
    // ── 150 캡 12쌍 (cmp + mov, mov는 레지스터별 4종) ──
    const CAPMOV2: [(&[u8], usize); 4] = [(&[0xb9], 1), (&[0x41,0xba], 2), (&[0x41,0xbc], 2), (&[0x41,0xbb], 2)];
    for (c, m) in [(0xd082f2usize, 0xd082f8usize), (0xd0833f, 0xd08345), (0xd0843f, 0xd08445),
        ...
        p!(base + c, &[0x48,0x3d], 2, 4, b4(cap, 150));
        pm!(base + m, CAPMOV2, 4, b4(cap, 150));
    }
...
unsafe fn apply_rt_imm() {
    ...
    {   // A식 기울기만 원본이 음수(−800)라 "음수 = 원본" 규약을 못 쓴다 → 값 그대로 적용.
        ...
        ok += patch_imm_bytes(base + 0xd64318, &[0x69,0xc1], 2, 4, want) as u32;
    }
    p!(base + 0xd6431e, &[0x05], 1, 4, b4(ia, 80_000));
    p!(base + 0xd6432e, &[0x83,0xc0], 2, 1, b1(oa, 80));
    p!(base + 0xd64335, &[0x69,0xc1], 2, 4, b4(kb, 450));
    p!(base + 0xd6434a, &[0x83,0xc0], 2, 1, b1(ib, 45));
    p!(base + 0xd64351, &[0x69,0xc1], 2, 4, b4(kc, 350));
    p!(base + 0xd64366, &[0x83,0xc0], 2, 1, b1(ic, 15));
    //   deadline 하한은 `cmp 61` + `mov 60` 쌍이라 비교값을 +1로 맞춰야 한다.
    p!(base + 0xd654d5, &[0x48,0x83,0xf9], 3, 1, if dl < 0 { 61 } else { (dl.max(0).min(0x7e) + 1) as u64 });
    p!(base + 0xd654d9, &[0xba], 1, 4, b4(dl, 60));
    // ── 정글 진행 HP% ──
    p!(base + 0xdffebc, &[0x48,0x83,0xf8], 3, 1, b1(jf, 21));
    p!(base + 0xdfff00, &[0x48,0x83,0xf8], 3, 1, b1(jn, 41));
...
unsafe fn apply_ldsc_imm() {
    ...
    for a in [0xc66ec3usize, 0xc66f33, 0xc66f9e] {
        p!(base + a, &[0x49,0x83,0xc6], 3, 1, b1(lsv, 120));
    }
    p!(base + 0xc66b95, &[0x83,0xc2], 2, 1, b1(lfac, 100));
    p!(base + 0xdf58de, &[0x41,0xb8], 2, 4, b4(lem, 0x1F863));
    {   // mov rcx, −99999 (48 c7 c1 imm32)
        tot += 1;
        let want = if lnul_orig { (-99999i32) as u32 as u64 } else { (lnul as i32) as u32 as u64 };
        ok += patch_imm_bytes(base + 0xc66be3, &[0x48,0xc7,0xc1], 3, 4, want) as u32;
    }
...
unsafe fn apply_new_imm() {
    ...
    // ① 적 위치 추정
    p!(base + 0xc43101, &[0x81,0xc2], 2, 4, b4(epsb, 3000));
    p!(base + 0xc43126, &[0x48,0x8d,0x9e], 3, 4, b4(epdr, 40000));
    // ⚠짝 사이트 — 여기는 **반경+1**(gen_range 상한). 하나만 바꾸면 난수 범위가 어긋난다.
    p!(base + 0xc43379, &[0x48,0x81,0xc1], 3, 4, if epdr < 0 { 40001 } else { b4(epdr, 40000) + 1 });
    p!(base + 0xc4312d, &[0x48,0x81,0xfb], 3, 4, b4(eprc, 300000));
    // ② 시전 후보 2차 검열
    p!(base + 0xc65f3e, &[0x48,0x83,0xf9], 3, 1, b1(cfrn, 9));
    p!(base + 0xc65ef1, &[0x48,0x83,0xf9], 3, 1, b1(cfrf, 25));
    p!(base + 0xc65ec9, &[0x4c,0x6b,0xc0], 3, 1, b1(cfdp, 35));
    p!(base + 0xc65793, &[0xb9], 1, 4, b4(cfrp, 15000));
    p!(base + 0xc657be, &[0xb8], 1, 4, b4(cfru, 150000));
    // ⚠12 미만은 JT 인덱스가 음수가 되어 크래시한다 — 켜기(17)/원본(12) 두 값만 허용한다.
    p!(base + 0xc65788, &[0x3c], 1, 1, if cfoff == 1 { 17 } else { 12 });
    // `jne +0x0d`(= 도주중이면 이 후보 제거) → NOP 2바이트면 그 조건만 꺼진다.
    let flee_ok = patch_toggle_bytes(base + 0xc65f3c, &[0x75,0x0d], &[0x90,0x90], cffk == 1);
    // ③ 1차 점수컷 — 두 사이트가 (−v, −v−1) 쌍
    let (f0, f1) = if cssf < 0 { (0xe2u64, 0xe1u64) } else {
        let v = cssf.clamp(0, 120) as u64; ((256 - v) & 0xff, (256 - v - 1) & 0xff)
    };
    p!(base + 0xc3cf8d, &[0x48,0x83,0xf8], 3, 1, f0);
    p!(base + 0xc3cf9e, &[0x48,0x83,0xf8], 3, 1, f1);
    // ④ 경매 재선택
    p!(base + 0xd61e4b, &[0x3c], 1, 1, b1(recp, 2));
    p!(base + 0xd6220d, &[0x48,0x05], 2, 4, b4(retp, 25000));
    p!(base + 0xd621d3, &[0x83,0x38], 2, 1, b1(regs, 1));
    // ⑤ 전역 궁 오버라이드
    p!(base + 0xd5f93b, &[0x48,0x83,0xb9,0xb0,0x05,0x00,0x00], 7, 1, b1(gulv, 5));
    p!(base + 0xd5faa7, &[0x48,0x83,0xc3], 3, 1, b1(gumem, 120));
    // ⚠거리 그대로가 아니라 **d²+1**로 인코딩된 자리다(원본 150000 → 22,500,000,001).
    p!(base + 0xd5f9fa, &[0x48,0xb9], 2, 8,
...
unsafe fn apply_auction_imm() {
    ...
    let bvis = tune("bt_vision_mem", -1);     // battle 마지막 목격 유효 틱(원본 120, 7곳)
    ...
    let noise_ok = patch_toggle_bytes(base + 0xd5fecd,
                                      &[0x48,0x0f,0x45,0xf2], &[0x0f,0x1f,0x40,0x00], noff == 1);
    p!(base + 0xd5febc, &[0xba], 1, 4, if namp < 0 { 900 } else { (namp.max(900) as u64) & 0xffff_ffff });
    p!(base + 0xd5fefc, &[0xbb], 1, 4, b4(ctr, 1000));               // mov ebx,1000  (lo 중심)
    p!(base + 0xd5ff04, &[0x48,0x81,0xc6], 3, 4, b4(ctr, 1000));     // add rsi,1000  (hi 중심)
    // ── ② battle.rs (0xca8a10) ──
    p!(base + 0xcab663, &[0x48,0x83,0xf8], 3, 1, b1(bhpf, 21));
    ...
    p!(base + 0xcab1ef, &[0x48,0x83,0xf8], 3, 1, b1(bhpg, 41));     // cmp rax,41 — HP%≥41이면 아군수 +2
    ...
    for a in [0xca9cf9usize, 0xca9da1, 0xca9fb3, 0xcaa323, 0xcab77e, 0xcabb22] {
    ...
    p!(base + 0xcac3be, &[0x48,0x81,0xc1], 3, 4, b4(bstp, 15000));
    // ★[08-05] 여기도 1곳이 아니라 2곳 — `0xcac136`(emit#18, arm4 근접 처리)이 빠져 있었다.
    for a in [0xcaba72usize, 0xcac136] {
    ...
    for a in [0xca9930usize, 0xcaa5cf, 0xcaaf7e, 0xcac8dd, 0xcacbac, 0xcaceac, 0xcad097] {
        tot += 1;
        let mut done = false;
        for pre in [[0x49u8,0x83,0xc5], [0x48,0x83,0xc6], [0x49,0x83,0xc6], [0x49,0x83,0xc7]].iter() {
    ...
    for a in [0xc624cdusize, 0xc62269, 0xc6236c, 0xc62422] {
        p!(base + a, &[0x48,0xc7,0x85], 7, 4, b4(lstp, 15000));
    }
    for a in [0xc619d2usize, 0xc61a3b, 0xc61aa7, 0xc61b13, 0xc61b77] {
        tot += 1;
    ...
    for a in [0xc61784usize, 0xc5f059] {                                   // ← c5f059 = 1회차(신규)
        p!(base + a, &[0x48,0x81,0xfa], 3, 4, dsh(livn, 9765625, 8));      // (50000²)>>8
    }
    p!(base + 0xc5eee7, &[0x49,0x83,0xc6], 3, 1, b1(lvis, 120));           // ← 1회차(신규)
    p!(base + 0xc63b87, &[0x49,0x83,0xc4], 3, 1, b1(lvis, 120));
    // c61667 = `add rdi,0x78` — 08-03 exe 정적 검증으로 바이트열 확정(같은 120틱 사이트, 세 번째).
    p!(base + 0xc61667, &[0x48,0x83,0xc7], 3, 1, b1(lvis, 120));
    p!(base + 0xc61cb4, &[0x83,0xc1], 2, 1, b1(lest, 10));                 // add ecx,10
    // ── ③-b line_defense 1회차 전용 상수 (08-03 신규 노출) ──
    //   ★`ld_around_range`는 1·2회차 합쳐 **7사이트** — 지금까지 완전 미노출이던 값이다.
    for a in [0xc5f2a8usize, 0xc5f6d7, 0xc5fbdc, 0xc62ec4, 0xc62fd4, 0xc633a5, 0xc63488] {
        p!(base + a, &[0x48,0xc7,0x85], 7, 4, b4(larn, 80000));
    }
    // ★[08-05 감사] 3곳만 잡고 있었다 — 실제는 **7곳**이고, 바로 위 `ld_around_range` 7사이트의 **+11B 짝**이다.
    for a in [0xc5f2b3usize, 0xc5f6e2, 0xc5fbe7, 0xc62ecf, 0xc62fdf, 0xc633b0, 0xc63493] {
    ...
    p!(base + 0xc5e61a, &[0xba], 1, 4, b4(lmsk, 0x1a1));
    p!(base + 0xc5e9b3, &[0x41,0xb8], 2, 4, b4(lmsk, 0x1a1));
    p!(base + 0xc5f664, &[0xba], 1, 4, b4(lmsk, 0x1a1));
    for a in [0xc60c10usize, 0xc60f80, 0xc6451d] {
        p!(base + a, &[0xba], 1, 4, b4(lmsk, 0x1a1));
    }
    // ★[08-05 감사] 4곳 → **10곳**. 전부 동일 관용구 `movsxd rax,[r+0x458]; add rax,0x64; imul [r+0x668]`.
    for a in [0xc5e31ausize, 0xc5e55e, 0xc5fe08, 0xc5fe56,
              0xc60427, 0xc6045a, 0xc60ca9, 0xc60ed0, 0xc625de, 0xc626e4] {
        p!(base + a, &[0x48,0x83,0xc0], 3, 1, b1(lmvp, 100));
    }
    // ★[08-05 감사] `0xc60d6c`는 `0xc5e3cc`의 완전 클론(`cmp qword[r?+0x68],13; sete`)인데 미배선이었다.
    p!(base + 0xc5e3cc, &[0x48,0x83,0x7b,0x68], 4, 1, b1(lthr, 13));
    p!(base + 0xc60d6c, &[0x48,0x83,0x7e,0x68], 4, 1, b1(lthr, 13));
    p!(base + 0xc60a0c, &[0x48,0x83,0xfa], 3, 1, b1(lrnd, 2));
    // ── ④ 팀모드 자동취소 마스크 (chat.rs / modes.rs) ──
    for a in [0xd55bacusize, 0xd4c4c9] {
...
unsafe fn apply_plan_imm() {
    ...
    ok += patch_imm_bytes(base + 0xd45321, &[0x49,0x83,0xff], 3, 1, b1(role, 1)) as u32;
    ok += patch_imm_bytes(base + 0xd4555f, &[0x41,0x83,0xff], 3, 1, b1(role, 1)) as u32;
    // ── 세르펜 허용 페이즈 비트마스크 2사이트 ──
    ok += patch_imm_bytes(base + 0xd45360, &[0xb9], 1, 4, b4(smask, 0x1a1)) as u32;
    ok += patch_imm_bytes(base + 0xd453b4, &[0xb9], 1, 4, b4(smask, 0x1a1)) as u32;
    // ── 에픽 허용 페이즈 경계 2사이트 (⬜의미 미확정·실험용) ──
    ok += patch_imm_bytes(base + 0xd453f2, &[0x3c], 1, 1, if ephase < 0 { 0xf9 } else { (ephase & 0xff) as u64 }) as u32;
    ok += patch_imm_bytes(base + 0xd45418, &[0x3c], 1, 1, if ephase < 0 { 0xf9 } else { (ephase & 0xff) as u64 }) as u32;
    // ── 갱 plan 게이트 1사이트 ──
    ok += patch_imm_bytes(base + 0xd4592e, &[0x3c], 1, 1, b1(gank, 0x0b)) as u32;
    PLANIMM_SIG.store(sig, Ordering::Relaxed);
    if let Some(p) = pth("plan_imm.txt") {
        let _ = fs::write(p, format!("applied={}/7 obj_role={} serpen_mask={} epic_phase={} ganker_gate={} (-1=원본: 1/0x1a1/0xf9/0x0b) @base{:#x}\n",
...
unsafe fn apply_sev_imm() {
    ...
    // ★[08-03 신설] [C] branch A "소극 경로" 별도 4임계 배선 (구 ⬜미배선 후보 = MIGRATION §7.2-A14 §7 / 0.5.2 0x22f0067~79 → 0.5.3 0xcd4cd7~e9, ghidra-re 08-03 확정·함수 내부 오프셋 보존·바이트 지문 검증).
    ...
    ok += patch_imm_bytes(base + 0xcd4cd7, &[0x48,0x83,0xf9], 3, 1, q_hh) as u32;   // A1 hp%>25 → 1단 스킵
    ok += patch_imm_bytes(base + 0xcd4cdd, &[0x48,0x83,0xf8], 3, 1, q_th) as u32;   // A2 tr>34 → 통과
    ok += patch_imm_bytes(base + 0xcd4ce3, &[0x48,0x83,0xf9], 3, 1, q_hl) as u32;   // A3 hp%>15 → 차단
    ok += patch_imm_bytes(base + 0xcd4ce9, &[0x48,0x83,0xf8], 3, 1, q_tl) as u32;   // A4 tr>=20 → 통과 (jb 인코딩)
...
unsafe fn apply_hd_imm() {
    ...
    p!(base + 0xdc2dcc, &[0x48,0xba], 2, 8, v_hd_detect_max_h);
    ...
    p!(base + 0xdc2d76, &[0x49,0x83,0xc6], 3, 1, v_hd_vision_mem);
    // ★[08-04 신설] Phase 0 이동 오더 유효기간
    let v_hd_ph0_ttl: u64 = { if hd_ph0_ttl < 0 { 5u64 } else { hd_ph0_ttl.max(0) as u64 } };
    p!(base + 0xca4727, &[0x48,0xc7,0x45,0x10], 4, 4, v_hd_ph0_ttl);
    p!(base + 0xca4882, &[0x48,0xc7,0x45,0x10], 4, 4, v_hd_ph0_ttl);
    ...
    p!(base + 0xc55d3b, &[0x66,0xc7,0x46,0x10], 4, 2, v_hd_phase);
    p!(base + 0xc7a72e, &[0x66,0xc7,0x40,0x10], 4, 2, v_hd_phase);
    p!(base + 0xd81b6e, &[0x66,0xc7,0x40,0x10], 4, 2, v_hd_phase);
