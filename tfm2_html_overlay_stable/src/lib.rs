...
const LOG_ENABLED: bool = true; // ⬜인게임 검증 전이라 ON — 릴리스 zip 전 OFF (오류 로그 elog 는 게이트 무관 항상 남음)
...
/// file:/// URL → 로컬 경로(간단 %XX 디코드, UTF-8 보존). http(s) 등은 None.
fn file_url_to_path(url: &str) -> Option<String> {
    ...
    Some(String::from_utf8_lossy(&out).replace('/', "\\"))
}
fn html_escape(s: &str) -> String {
    s.replace('&', "&amp;").replace('<', "&lt;").replace('>', "&gt;")
}
/// 경로가 없는 탭에 띄우는 내장 안내 페이지(파일 의존 없음 — NavigateToString 으로 직접 렌더).
fn error_page_html(url: &str, path: &str) -> String {
    format!(
        r#"<!doctype html><meta charset="utf-8"><title>페이지를 찾을 수 없음</title>
<style>
  body{{margin:0;font:14px/1.7 'Malgun Gothic',sans-serif;background:#14161c;color:#dfe3ec;
       display:flex;align-items:center;justify-content:center;min-height:100vh}}
  .card{{max-width:560px;padding:28px 32px;background:#1a1e28;border:1px solid #2a3040;border-radius:12px}}
  h1{{font-size:17px;margin:0 0 10px;color:#ffb74d}}
  code{{display:block;background:#232735;padding:8px 10px;border-radius:6px;margin:10px 0;
       word-break:break-all;color:#9fc3e8;font-size:12px}}
  li{{margin:6px 0;color:#aab6c8}} b{{color:#dfe3ec}}
</style>
<div class="card">
  <h1>⚠ 페이지를 찾을 수 없습니다</h1>
  <div>지정한 경로에 파일이 없습니다:</div>
  <code>{path}</code>
  <ul>
    <li><b>html_overlay.cfg</b>의 <b>url=</b> / <b>presetN=</b> 경로가 맞는지 확인하세요.</li>
    <li>파일을 옮겼거나 지웠다면 경로를 고친 뒤 스트립의 <b>[↻]</b> 버튼을 누르세요.</li>
    <li>토큰: <b>{{mod}}</b> = 이 모드 폴더, <b>{{mods}}</b> = 게임 mods 폴더.</li>
  </ul>
  <code>{url}</code>
</div>"#,
...
unsafe fn navigate_wv(wv: usize, url: &str) {
    ...
    // file:// 대상은 사전 존재 검사 → 없으면 전용 안내 페이지(WebView2 기본 에러 대체)
    if let Some(path) = file_url_to_path(url) {
        if !std::path::Path::new(&path).exists() {
            // ICoreWebView2 vtable: 6 = NavigateToString(LPCWSTR)
            let f: FnHrWstr = vslot(wv as *mut c_void, 6);
            let w = wstr(&error_page_html(url, &path));
            let hr = f(wv as *mut c_void, w.as_ptr());
            elog(&format!("경로 없음 → 안내 페이지 표시: {path} (hr={hr:#x})"));
            return;
        }
    }
...
// 오버레이 창
// ===========================================================================
fn strip_buttons(client_w: i32) -> (RECT, RECT, RECT, RECT) {
    // (back, forward, reload, collapse) — 스트립 오른쪽 끝에서부터 [←][→][↻][—]
    ...
    let forward = RECT {
        left: client_w - BTN_W * 3 - 12,
        top: 2,
        right: client_w - BTN_W * 2 - 12,
        bottom: STRIP_H - 2,
    };
    let back = RECT {
        left: client_w - BTN_W * 4 - 16,
        top: 2,
        right: client_w - BTN_W * 3 - 16,
        bottom: STRIP_H - 2,
    };
    (back, forward, reload, collapse)
}
/// 현재 탭 웹뷰 히스토리 이동 (Can* 확인 후).
/// ICoreWebView2 vtable: 38 get_CanGoBack / 39 get_CanGoForward / 40 GoBack / 41 GoForward
fn history_nav(forward: bool) {
    let tag = current_tag();
    let wv = {
        let ctls = CTLS.lock().unwrap_or_else(|e| e.into_inner());
        ctls.iter().find(|s| s.tag == tag).map(|s| s.wv)
    };
    let Some(wv) = wv else { return };
    if wv == 0 {
        return;
    }
    let (can_slot, go_slot) = if forward { (39, 41) } else { (38, 40) };
    unsafe {
        type FnHrOutBool = unsafe extern "system" fn(*mut c_void, *mut i32) -> HRESULT;
        let can_fn: FnHrOutBool = vslot(wv as *mut c_void, can_slot);
        let mut can: i32 = 0;
        if can_fn(wv as *mut c_void, &mut can) >= 0 && can != 0 {
            let go_fn: FnHr1 = vslot(wv as *mut c_void, go_slot);
            let _ = go_fn(wv as *mut c_void);
        }
    }
}
fn go_back() {
...
unsafe fn paint_strip(hwnd: HWND) {
    ...
    if hdc != 0 {
        ...
        let (r_bk, r_fw, r_re, r_co) = strip_buttons(rc.right);
        SetTextColor(hdc, 0x003E_D2FF);
        SelectObject(hdc, f_sym); // 기호는 Segoe UI Symbol (기본 폰트엔 글리프 없음)
        if !COLLAPSED.load(Ordering::Relaxed) {
            let bk = wstr("←");
            TextOutW(hdc, r_bk.left + 6, 5, bk.as_ptr(), (bk.len() - 1) as i32);
            let fw = wstr("→");
            TextOutW(hdc, r_fw.left + 6, 5, fw.as_ptr(), (fw.len() - 1) as i32);
            let re = wstr("↻");
        ...
        match msg {
            ...
            WM_NCHITTEST => {
                ...
                if hit == HTCLIENT {
                    ...
                    if pt[1] < STRIP_H {
                        ...
                        GetClientRect(hwnd, &mut rc);
                        let (r_bk, r_fw, r_re, r_co) = strip_buttons(rc.right);
                        if pt_in(&r_bk, pt[0], pt[1])
                            || pt_in(&r_fw, pt[0], pt[1])
                            || pt_in(&r_re, pt[0], pt[1])
                            || pt_in(&r_co, pt[0], pt[1])
                        {
            ...
            WM_LBUTTONUP => {
                ...
                if pt_in(&r_co, x, y) {
                    ...
                    do_reload();
                } else if !COLLAPSED.load(Ordering::Relaxed) && pt_in(&r_fw, x, y) {
                    go_forward();
                } else if !COLLAPSED.load(Ordering::Relaxed) && pt_in(&r_bk, x, y) {
                    go_back();
                } else {
                    let presets = PRESETS.lock().unwrap_or_else(|e| e.into_inner()).clone();
                    if let Some(i) =
...
// ===========================================================================
// 모드 등록 (stable ABI — 클래식의 post_update 는 스레드 1회 spawn 용도뿐이었으므로
// init 에서 바로 spawn. 오버레이 스레드가 게임 창 출현을 자체 대기하므로 시점 무관.)
// ===========================================================================
fn init(host: &StableHost) -> StableMod {
    host.log(
        LogLevel::Info,
        &format!("{MOD_ID} stable init (game v{:?})", host.game_version()),
    );
