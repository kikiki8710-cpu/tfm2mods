...
//! ★2026-09-20: tfm2_banpick_illust v1.4.0(쇼케이스 전용 껍데기) 에서 banpick_view_plus 로 **물리 통합**(유저 결정 "3").
//!   · 아트 = view_plus 의 일러 인덱스(`crate::showcase_art` = 픽 슬롯과 같은 팩/선택 · 레드 = 엔진 flip, `red_noflip` 존중) — 구 `keys.rs`(blue/red/red_noflip 폴더 팩) 폐기
//!   · 설정 = `crate::cfg().showcase`(ModData\config.txt `banpick_showcase`) · 로그 = `crate::log`(DBG)
//!   · ⚠이 파일의 RVA 29개(훅 3 + FFI 14 + 기하 12)는 패치마다 재핀 대상 — view_plus 가 "재핀 필요 모드"가 됨. 원본 RE = REPORT tfm2_banpick_illust.
//! ===========================================================================
...
const RVA_FX_SET: usize = 0x2438a90; // 0.5.6(구값 갱신) // 훅 A: 연출 상태 세팅 (진영 스태시) — 0.5.5 프롤로그 20B 완전일치  ⟵0.5.4 0x1d92980
const RVA_CARD_DRAW: usize = 0x244e410; // 0.5.6(구값 갱신) // 훅 B: 카드 드로우 헬퍼 — 0.5.5 프롤로그 일치 + ILLUST_GET 콜러 size2979 대응  ⟵0.5.4 0x1da8410
const RVA_ILLUST_GET: usize = 0x239bc00; // 0.5.6(구값 갱신) // 훅 C: 밴픽 일러 에셋 조회 — 0.5.5 프롤로그 일치 + 콜러 3/3 프로필 완전대응  ⟵0.5.4 0x1ffd970
const RVA_SUBMIT: usize = 0x1c5fe0; // 0.5.6(구값 갱신) // (list, &cmd) 일반 제출 — §7.5 skel유일  ⟵0.5.4 0x181400
const RVA_SUBMIT_TEXT: usize = 0x1c6260; // 0.5.6(구값 갱신) // (list, &cmd) 텍스트 전용 — §7.5 skel유일  ⟵0.5.4 0x181680
const RVA_IMG_BUILD: usize = 0x1c7430; // 0.5.6(구값 갱신) // (&cmd, key, len, x, y, layer, w, h, 0,0,0,0) — §7.5 skel유일  ⟵0.5.4 0x182b20
const RVA_IMG_UV: usize = 0x1c7290; // 0.5.6(구값 갱신) // (&out, &in, &uv) — §7.5 skel유일  ⟵0.5.4 0x182980
const RVA_IMG_FLAG: usize = 0x1c7740; // 0.5.6(구값 갱신) // (&out, &in, 샘플링: 1=nearest) — §7.5 skel유일  ⟵0.5.4 0x182e30
const RVA_IMG_COLOR: usize = 0x2182b0; // 0.5.6(구값 갱신) // (&out, &in, "color", 5, &rgba) — §7.5 콜슬롯8/19 정렬  ⟵0.5.4 0x1c2f8d0
const RVA_IMG_SHADER: usize = 0x1c8da0; // 0.5.6(구값 갱신) // (&out, &in, shader_key, len) — §7.5 skel유일  ⟵0.5.4 0x184430
const RVA_TEXT_BUILD: usize = 0x1c6cb0; // 0.5.6(구값 갱신) // (...) 텍스트 cmd — §7.5 skel유일  ⟵0.5.4 0x182010
const RVA_NAME_GET: usize = 0x24789b0; // 0.5.6(구값 갱신) // 챔프 표시명 String — ABI(sret GStr,ctx,key,len) 동일  ⟵0.5.4 0x1dd4240
...
const RVA_ASSET_GET: usize = 0x16cdb0; // 0.5.6(구값 갱신) // 키→텍스처 에셋 (obj,vtbl) — §7.5 ILLUST_GET 콜슬롯1 정렬  ⟵0.5.4 0x143d50
const RVA_ANIM_GET: usize = 0x220a00; // 0.5.6(구값 갱신) // 키→애님 리소스 — §7.5 lea→call 1표 + CARD_DRAW 콜슬롯13  ⟵0.5.4 0x74c010
const RVA_SPRITE_CALC: usize = 0x247dd80; // 0.5.6(구값 갱신) // idle 시트키+UV+크기 계산기(무부작용) — §7.5 skel유일  ⟵0.5.4 0x1dd9170
...
const RVA_GAME_ALLOC: usize = 0x2f64f90; // 0.5.6(구값 갱신) // (무시, flags, size) → ptr / GetProcessHeap→HeapAlloc tail-jmp 래퍼 — §7.5 skel/마스크 유일  ⟵0.5.4 0x29bb920
...
const RVA_C_CARD_RECT: usize = 0x3c12d90; // 0.5.6(구값 갱신) // {-180,-240,360,480} 카드 로컬 rect(밴·픽 공용)  ⟵0.5.4 0x338b480
const RVA_C_SNAP_RECT: usize = 0x3c12db0; // 0.5.6(구값 갱신) // {0,0,360,480} 스냅샷 내부 rect(좌상단 원점)  ⟵0.5.4 0x338b4b0
const RVA_C_LINE_DIR: usize = 0x3c12de0; // 0.5.6(구값 갱신) // {360,340} 취소선 방향  ⟵0.5.4 0x338b4e0
const RVA_C_LINE_START: usize = 0x3c12df0; // 0.5.6(구값 갱신) // {-180,170} 취소선 시작  ⟵0.5.4 0x338b4f0
const RVA_C_LINE_ANCHOR: usize = 0x3c12e00; // 0.5.6(구값 갱신) // {0,170} 앵커(블록 내부 LINE_START<x<ZIG 유일)  ⟵0.5.4 0x338b500
const RVA_C_NORMAL: usize = 0x3c12dc0; // 0.5.6(구값 갱신) // {0.6866,0.727} 분리 법선(블록 내부 SNAP<x<LINE_DIR 유일)  ⟵0.5.4 0x338b4c0
...
const RVA_I_SNAP_H: usize = 0x24c1360; // 0.5.6(구값 갱신) // mov dword [rsp+0x20], 480.0 의 imm4 (스냅샷 타깃 높이)  ⟵0.5.4 0x1e17510
const RVA_D_SNAP_W: usize = 0x24c1376; // 0.5.6(구값 갱신) // disp4 → 360.0 (스냅샷 폭, 광공유)  ⟵0.5.4 0x1e17526
const RVA_D_CUT_LO: usize = 0x24585c8; // 0.5.6(구값 갱신) // disp4 → -70.0 (컨테이너 0x1970f40 하단 컷)  ⟵0.5.4 0x1db23f8
const RVA_D_CUT_HI: usize = 0x24585d6; // 0.5.6(구값 갱신) // disp4 → +70.0 (〃 상단 컷)  ⟵0.5.4 0x1db2406
// ZIG 2건은 컨테이너 안에 -180.0 참조가 정확히 2곳(0.5.2 와 동수)이고 **둘 다 slots+12 로 리타겟**
//   하므로 X1/X2 배정 순서가 결과에 영향을 주지 않는다(주소 순으로 배정).
const RVA_D_ZIG_X1: usize = 0x24c21c2; // 0.5.6(구값 갱신) // disp4 → -180.0 (지그재그 x)  ⟵0.5.4 0x1e18372
const RVA_D_ZIG_X2: usize = 0x24c28a0; // 0.5.6(구값 갱신) // disp4 → -180.0 (〃 두 번째 블록)  ⟵0.5.4 0x1e18a50
// 0.5.5 .rdata 최장 0런 = **0x40c41c4..0x40db530(95,084B)** 내부의 4K 정렬 지점(±64B 전부 0 실측).
//   precheck 가 "슬롯 4개 전부 0.0" 을 요구하므로 오염 시 자동 SKIP(안전).
const RVA_SLOTS: usize = 0x484f000; // ★0.5.7 재검증 OK(값 변경 불필요 — 2026-08-27 실측: 0.5.7 .rdata 에서도 ±64B 전부 0). 0.5.6(구값 갱신) // .rdata 패딩 슬롯 [w, cut_lo, cut_hi, zig_x]  ⟵0.5.4 0x3fe2000
...
/// 설정 어댑터(구 config::get() 대체).
struct ScCfg { showcase: bool, debug: bool }
mod config {
    pub fn get() -> super::ScCfg { super::ScCfg { showcase: crate::cfg().showcase, debug: crate::DBG } }
}
fn slog(msg: &str) {
    if !config::get().debug {
        return;
...
unsafe fn resolve_art(store: usize, champ: &str, is_blue: bool) -> Option<(&'static str, bool, f32, f32)> {
    // view_plus 인덱스에서 픽 슬롯과 같은 아트를 고른다(팩 순환 선택 반영). 키는 게임 String 에 넘기므로 'static 으로 인턴.
    let (key, flip) = crate::showcase_art(champ, is_blue)?;
    let key: &'static str = intern(&key);
...
fn intern(k: &str) -> &'static str {
    ...
    if let Some(s) = g.iter().find(|s| **s == k) { return s; }
...
unsafe extern "win64" fn hook_fx_set(
    ...
    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if ptr_ok(self_) {
            // ★0.6.0: 0x3d0 → 0x3f8 (FX_SET 0x23c0b40 진입부 `mov rax,[rcx+0x3f8]; cmp rax,r9` — 0.5.8 0x2140ebc `[rcx+0x3d0]` 와 같은 자리, 2026-09-16 fieldmap)
            let blue_id = *((self_ + 0x3f8) as *const u64);
