...
static TABS: &[Tab] = &[
  ...
  Tab{ id:"disc8", title:"• [8] 갱커버 (disc8 · ⛔死)", keys:&[
      "§⛔ 死레버 — 원본이 무조건 7(대기) 고정·옛 슬롯 로직 폐기","d8_slot_thr"], note:
    "<b>subplan 8 (실명 Jungle)</b> — ⛔원본이 <b>무조건 7(대기)</b>만 내놓음(플랜2와 동일 처리·게임 원본과 비트동일 확인). 옛 슬롯 우선순위 로직은 코드가 '사용 안 함' 처리로 막혀 있어 <b>d8_slot_thr는 도달 불가 = 값 무반영</b>(07-23 확정)." },
  ...
  Tab{ id:"disc17", title:"• [15·16·17] 세르펜", keys:&[
      ...
      "§⛔ [15]세르펜 교전판단 — 죽은 틀(게임이 생성 안 함 = 켜도 미발화)","d15_repl","d15_engage_hp_pct"], note:
    "<b>17 (SerpenPoke·세르펜 견제)</b> — 진척/근접 게이트 공방(재배선 완료). <b>16 (SerpenHunt·사냥)</b>=disc16_home_hp(부상 홈대기 HP%, 기본100=원본). ★16·17 대체는 [엔진]탭 <b>d1617_repl</b>(기본 ON·0.5.3 리플레이 A/B로 실효 확인). ⛔<b>15 (SerpenCheck·교전판단)=죽은 틀</b> — 게임이 이 subplan을 생성하지 않아(세르펜 분기에 15를 만드는 경우 없음·07-23 확정) <b>d15_repl을 켜도 실행되지 않음</b>.<br>\
     진척(progress) 게이트와 근접/예측 거리²로 DEFEND/진격 결정.<br>\
     ✅<b>[정정 2026-07-31]</b> 세르펜 전용 전력회피(<b>numbers_threat_sp15/16/17</b>)는 <b>死레버가 아니었습니다</b> — 감사 도구가 접두 파싱 키를 못 봐서 생긴 오판정.<br>\
  ...
  Tab{ id:"engage", title:"• [공통] 전력·포탑 회피", keys:&[
      ...
      "§◇ 교전판단 (라인전) — 비우거나 -1이면 한타 수치 따름","numbers_threat_move","numbers_range_move","ally_tower_hp_move","ally_tower_dps_move","ally_tower_range_move","numbers_min_enemy_move",
      "§◈ subplan별 개별 임계 — 라인·정글 (-1=위 공통값 따름 / 0=그 판단선 후퇴안함)","numbers_threat_sp0","numbers_threat_sp1","numbers_threat_sp3","numbers_threat_sp2","numbers_threat_sp4",
      "§◈ subplan별 개별 임계 — 귀환·갱","numbers_threat_sp5","numbers_threat_sp6","numbers_threat_sp7","numbers_threat_sp8",
      "§◈ subplan별 개별 임계 — 오브젝트·모르가드","numbers_threat_sp9","numbers_threat_sp10","numbers_threat_sp11","numbers_threat_sp12","numbers_threat_sp13","numbers_threat_sp14",
      "§◈ subplan별 개별 임계 — 세르펜","numbers_threat_sp15","numbers_threat_sp16","numbers_threat_sp17"], note:
    "게임 원본에 <b>없던 추가 항</b>(초록=신규, 전부 <b>0이면 원본과 동일</b>). <b>여러 subplan의 교전판단에 공유</b>되는 공통 회피 시스템.<br>\
     ...
     <b>포탑회피</b>(tower_threat): 적 포탑 사거리 안 + tower_threat &ge; 승산이면 후퇴. <b>전력회피</b>(numbers_threat): numbers_threat &ge; 승산이면 후퇴(100=이길싸움만). <b>tower_dps</b>=아군 포탑 DPS를 아군 전력에 반영하는 힘 단위(라이너/전 disc 공용, ally_tower_dps>0일때).<br>\
     ★<b>subplan별 개별 임계 (numbers_threat_spN)</b> — 판단 종류마다 겁을 다르게 먹입니다. <b>-1(기본)=위 공통값 그대로</b>, <b>0=그 판단에선 전력 때문에 후퇴하지 않음</b>, <b>N=그 판단 전용 임계</b>(↑=겁쟁이 / ↓=대담).<br>\
     &nbsp;&nbsp;예) 라인전은 자유롭게(sp3=0) + 세르펜 사냥은 신중하게(sp16=65) 같은 조합이 가능합니다.<br>\
     &nbsp;&nbsp;✅<b>[정정 2026-07-31] sp16·sp17은 死레버가 아닙니다</b> — 07-30 감사에서 '코드에 read site 없음'으로 폐기 처리됐으나, 이 키는 <b>접두 파싱</b>(numbers_threat_sp + 숫자)이라 리터럴 스캔 도구에 안 잡혔을 뿐이고 실제로는 정상 동작합니다.<br>\
     &nbsp;&nbsp;✅<b>[수정 2026-07-31] sp0·sp1 부활</b> — 라인전 경로가 disc를 3으로 고정해 넘기고 있어 그 둘이 영구 무반영이었습니다.<br>\
     &nbsp;&nbsp;★<b>[인게임 실측 1경기 · 2026-07-31] 어디를 만져야 효과가 큰가</b> — 후퇴 발동 횟수 순:<br>\
     &nbsp;&nbsp;<b>sp0 = 38,060</b>(라인전 주력·압도적 1위) &gt; <b>sp14 = 2,395</b>(모르가드 견제) &gt; <b>sp12 = 1,660</b>(모르가드 교전판단) &gt; <b>sp17 = 853</b>(세르펜 견제) &gt; <b>sp1 = 540</b> &gt; <b>sp16 = 524</b>(세르펜 사냥) &gt; <b>sp8 = 448</b>(갱커버) &gt; <b>sp10 = 48</b>. 나머지(sp2·3·4·5·6·7·9·11·13·15)는 <b>발동 0회</b>.<br>\
     &nbsp;&nbsp;⚠따라서 <b>라인전 성향을 바꾸려면 sp3이 아니라 sp0</b>입니다(구 설명이 sp3을 가리켰던 것은 코드가 disc를 3으로 고정해 넘기던 시절의 잔재).<br>\
     &nbsp;&nbsp;적용 확인 = 모드 폴더 <b>sp_seen.txt</b>(subplan별 후퇴 발동 누적, log=1 필요)." },
  ...
  Tab{ id:"vision", title:"• [공통] 시야", keys:&[
    ...
    "적이 시야에서 사라져도 <b>일정 틱동안 '아는 적'으로 기억</b>해 판단에 반영. 값↑=더 오래 경계.<br>\
     ...
     <b>vis_window</b> = ✅[07-16 부활] <b>비-라인 전반</b> 기억창(기본 600≈10초). 단일 공유 byte-patch(0.5.3 재핀완 applied=1/1)라 <b>여러 판단에 한꺼번에</b> 걸림.<br>\
  ...
  Tab{ id:"engine", title:"• [공통] 엔진·대체스택 (고급)", keys:&[
      ...
      "§◆ 대체 게이트 (전부 OFF=원본)","enabled","team","mp_repl","dd7_repl","poke_repl","recall_repl","engage_repl","cond_repl","d4_repl","e9jt","replay_reset",
      "§🆕 대체 게이트 — subplan 개별 (2026-07-31 노출)","d1617_repl","d12_repl","d14_repl","gbskip",
      "§◆ 속도","skip_untuned",
      "§◆ engage 정적패치 (재빌드 없이 적용)","engage_base","engage_thr_mult",
      "§🔬 계측 (평소 0)","perf_measure","read_bench",
      "§📊 subplan 후퇴발동 누적 측정 (전술 비교용)","sp_seen","sp_seen_tag"], note:
    "<b>고급/개발용</b> — AI judge 메모리 read 방식(fast_read)과 각 judge를 우리 코드로 대체할지 여부(대체 게이트).<br>\
     ...
     ⚠★<b>관전 즉시보기 결과가 실행마다 바뀐다면 여기부터 의심</b> — 대체 함수의 난수 소비가 원본과 어긋나면 <b>배경 시뮬(확정) ↔ 관전 재시뮬</b>의 결정성이 깨집니다. 결정성 확정 조합 = <b>mp·dd7·recall·engage·cond ON</b>(실측 발산 0). <b>d12_repl·d14_repl</b>은 07-23 RNG 배선 + 07-29 구조결함 2건 수정 후 <b>현행 프로덕션 기본 ON</b>(스코어 발산 0% 실측·위치해시 ~20% 잔차만=결과 불변). <b>poke_repl</b>만 RNG-sync 미구현 = <b>상시 OFF</b>. 증상 재발 시 poke→d12→d14 순으로 0 롤백.<br>\
...
fn is_toggle(k: &str) -> bool {
  matches!(k, "cond_repl"|"gbskip"|"mp_repl"|"dd7_repl"|"poke_repl"|"recall_repl"|"engage_repl"
    ...
    |"d1617_repl"|"d12_repl"|"d14_repl"|"d15_repl"|"skip_untuned"|"sp_seen")   // ★07-31 노출분 (sp_seen_tag는 자유 문자열이라 제외)
}
...
fn desc_static(k: &str) -> Option<&'static str> {
  Some(match k {
    ...
    "vis_window" => "✅LIVE[07-16 부활]. 비-라인 전반 적 시야기억창(틱). 적이 사라져도 이 틱동안 '아는 적'으로 추적. 단일 공유 byte-patch(13개 호출처 공용·0.5.3 재핀완 vis_imm.txt applied=1/1)라 <b>여러 판단에 한꺼번에</b> 걸림 — 영향: <b>넥서스 커밋(18·19)·오브젝트 평가·모르가드(12~14)·세르펜(15~17)·CONDGATE</b>. ↑=더 오래 경계, ↓=빨리 잊음. 기본 600(≈10초, =원본). 0=즉시 망각(주의).",
    ...
    "d8_slot_thr" => "⛔DEAD(07-23 확정·도달 불가) — disc8은 원본이 무조건 7(대기) 고정이라 이 임계를 읽는 코드에 도달하지 않음. 값 무반영. (구 설명: 슬롯값 <5/≥5 분기. 기본 5)",
    ...
    "hang_diag" => "✅[07-16] 행(멈춤) 진단 워치독. 게임이 멈추면(일정 넘기다 연산 안 끝남 등) 자동으로 hang_diag.txt에 원인 덤프 — STALL(스레드 갇힘: 전 스레드 위치+스택) / RUNAWAY(시뮬이 계속 돎: 경기 교착 의심). ★0(기본)=OFF(opt-in) — 켜면 judge 콜마다 마킹 비용. 멈춤 재현 조사할 때만 1 (07-19: '기본 1' 함정 수정 — 프리셋/초기화 시 진단 자동 ON 방지)",
    ...
    "adv_prof" => "✅[07-16] 활동창 프로파일러. 큰 연산(일정넘김 등)이 돌면 자동으로 구간을 감지해 adv_prof.txt에 기록 — 소요시간·CPU·judge 콜분포·<b>어느 게임코드(exe+RVA)에서 시간이 갔는지 TOP20</b>. ★0(기본)=OFF — 켜면 활동 중 100ms마다 전 활성스레드 Suspend/Resume = rayon 워커 주기 정지(게임 느려짐). 성능 조사할 때만 1, 끝나면 0. <b>⚠배포(zip)용 cfg는 0 필수</b>",
    ...
    "poke_repl" => "disc9/11 견제 대체(Battle/Hide) — mp_repl 켜져야 동작. ⚠RNG-sync 미구현 = 켜면 관전 즉시보기 결정성 깨짐(desync) → 상시 0 권장(현행 기본 0)",
    ...
    "d7_repl" => "✅[0.5.3 재가동 07-30] disc7(귀환 이동판단) 대체 스위치. 1(기본 ON)이면 아래 d7_* 반영 — 실경기 발화·대조 396/400 DIFF=0 검증완(잔여 4건은 드문 scan2 경로 원본위임 = 그 블록엔 노브가 없어 손실 0). 0=게임 원본.",
    ...
    "enabled" => "⚠이름과 달리 **모드 전체 스위치가 아니다**(모드를 끄려면 게임 모드메뉴에서 비활성화). 실제 동작 = 개발용 디버그 — 켜면 대상 팀의 모든 이동 명령을 아래 x/y 좌표로 강제 치환한다(SDK Move override). 평소 0.",
    ...
    "numbers_threat_sp3"  => "⚠[07-31 실측 정정] disc3은 **실제 경기에서 발동 0회**다(1경기 측정). 구 설명 '라인전에서 멀뚱멀뚱 고치려면 여기를 0'은 코드가 disc를 3으로 고정해 넘기던 시절 이야기 — **지금 라인전 노브는 numbers_threat_sp0**이다. 이 키는 폴백만 남겨두면 된다. 기본 -1",
    "numbers_threat_sp0" => "★★[07-31 부활·라인전 주력] 라인전(dd7700) 판단의 실제 좌석. **실측 1경기 38,060회 발동**으로 전 subplan 중 압도적 1위 = 라인전 성향을 바꾸려면 여기를 만진다. 0=라인전선 전력 때문에 후퇴 안 함(미니언/딜교 자유), -1=numbers_threat 폴백. ※그간 코드가 disc를 3으로 고정해 넘겨 이 키는 영구 무반영이었다(수정됨). 기본 -1",
    "numbers_threat_sp1" => "★[07-31 부활] disc1 전용 전력임계. sp0과 같은 라인전(dd7700) 계열이지만 발동은 훨씬 적다(실측 540회). sp0과 동일 사유로 그간 무반영이었다. -1=폴백. 기본 -1",
    "numbers_threat_sp2" => "★신규. disc2(실명 LineDefense) 전용 전력임계. ⚠정규 경기에서 이 subplan은 미발화로 관측돼 값이 무반영일 수 있다. -1=폴백. 기본 -1",
    "numbers_threat_sp5" => "★신규. disc5(귀환) 전용 전력임계. -1=폴백. 기본 -1",
    "numbers_threat_sp6" => "★신규. disc6(교전) 전용 전력임계. -1=폴백. 기본 -1",
    "numbers_threat_sp10" => "★신규. disc10(오브젝트 배틀) 전용 전력임계. 실경기 발동 확인됨(sp_seen.txt). -1=폴백. 기본 -1",
    "numbers_threat_sp15" => "★신규. disc15(SerpenCheck=세르펜 교전판단) 전용 전력임계. ⚠플랜 15는 게임이 생성하지 않는 죽은 틀 → 실측 발동 0회 = 사실상 무의미. -1=폴백. 기본 -1",
    ...
    "numbers_threat_sp16" => "✅✅[정정 07-31] LIVE — 07-30 감사의 '死레버(read site 없음)' 판정은 오판이었다(접두 파싱 키라 리터럴 스캔에 안 잡혔을 뿐). **인게임 실측으로 발동 524회 확인**. 세르펜 사냥(16) 전용 전력회피 임계. 세르펜 사냥 중 전력승산이 이값 미만이면 후퇴. ↑=아슬한 싸움도 뺌(겁많음)/0=세르펜 사냥 땐 후퇴안함. -1(기본)=공통 numbers_threat 따름. ※유리할 땐(팀과 함께) 안 걸림. '혼자 세르펜 가서 죽는' 억제용은 60~70 권장.",
    "numbers_threat_sp17" => "✅[07-31 실측 확증] 발동 853회. 세르펜 견제(17) 전용 전력회피 임계. 세르펜 견제 중 불리하면 후퇴. ↑=겁많음/0=견제 땐 후퇴안함. -1(기본)=공통 따름. ※유리할 땐 안 걸림.",
    "numbers_threat_sp11" => "★신규. disc11(실명 Hide) 전용 전력임계. -1=폴백. 기본 -1",
    "numbers_threat_sp13" => "★신규. disc13(EpicHunt) 전용 전력임계. ⚠플랜 13은 게임이 생성하지 않는 죽은 틀 → 실측 발동 0회 = 사실상 무의미. -1=폴백. 기본 -1",
    ...
    "d19_retreat_hp" => "✅실작동 byte-patch — ★<b>d19i_enable=1 켜야 게임에 반영</b>(0이면 원본 45 유지. 구 설명 '게이트무관 항상반영'은 오류 — 그건 은퇴한 재현부 static 얘기였음). 넥서스 후퇴 HP%문턱. 위협이 애매할 때 넥서스HP%가 이 값 이하면 후퇴. ↑=높은 HP에도 후퇴(수비적), ↓=저HP까지 안 후퇴(공격적). 원본 45",
    ...
    "d1617_repl" => "[16·17 세르펜] 대체 스위치. 1(기본)=우리 재현 사용 / 0=그 둘만 게임 원본 그대로(passthrough). ★리플레이 A/B 실험용 — 같은 다시보기를 이 값만 바꿔 두 번 돌리면 개입 효과를 직접 확인할 수 있다(실측 예: OFF 16:1 ↔ ON 14:2). 결정성에는 영향 없음.",
    "d12_repl" => "[12 모르가드 교전판단] 대체 스위치. 1(현행 기본)=우리 재현 / 0=게임 원본. 이력상 결정성(관전 즉시보기) 용의군이었으나 07-29 구조결함 2건 수정 후 스코어 발산 0% 실측 → 기본 ON 운용. 증상 재발 시 poke→d12→d14 순으로 0 롤백.",
    "d14_repl" => "[14 모르가드 견제] 대체 스위치. 1(현행 기본)=우리 재현 / 0=게임 원본(passthrough·대조 캡처용). 07-23 RNG 홀 배선 수정 + 400/400 재검증 통과, 07-29 이후 발산 0% 실측. 증상 재발 시 롤백 대상(poke→d12→d14 순).",
    "sp_seen" => "★누적 측정 스위치. 1=어느 subplan(판단 종류)에서 '전력 때문에 후퇴'가 몇 번 발동했는지 sp_seen.txt 에 기록. log과 독립이라 이것만 켜도 되고(여러 판 연속 측정 시 권장), 게임을 재시작해도 sp_seen_acc.txt 에서 누적을 이어받는다. 부하는 사실상 0(원자 카운터 + 5초마다 작은 파일 1개). 측정 안 할 땐 0.",
    "sp_seen_tag" => "★구간 라벨(자유 문자열 — 예: A_세르펜무조건, B_세르펜포기). 이 값을 바꾸는 순간 직전 구간의 발동수가 sp_seen_hist.txt 에 한 줄로 확정되고 카운터가 리베이스된다 ⟹ 전술을 바꿔가며 돌리면 전술별 비교표가 쌓인다. ⚠게임 실행 중에 바꿔도 ~0.5초 안에 반영된다(경기 도중에 바꾸면 그 경기가 두 구간에 걸쳐 쪼개지니, 경기 사이에 바꿀 것). ⛔`auto`는 무효 — 조합 테스트 경기는 DB(match_replays)에 저장되지 않아 전술을 읽어올 수 없다(리그 경기 값만 잡힘).",
    ...
    "d15_repl" => "⛔DEAD(07-23 확정) — [15 SerpenCheck]는 게임이 생성하지 않는 죽은 subplan(생성 사이트 부재·영구 미발화)이라 1로 켜도 재현이 실행될 일이 없음. 값 무의미·재시도 금지.",
    "d15_engage_hp_pct" => "⛔DEAD(07-23 확정) — 플랜 15 자체가 영구 미발화(죽은 틀)라 값 무반영. (구 설명: self 체력%<이값이면 교전 개시. 기본 51)",
    ...
    "oi_enable" => "objective 원본상수 대체 마스터 스위치. 0(기본)=무개입(원본 복원). 1=oi_* 값으로 넥서스 방어/공격 원본상수 byte-patch. ✅0.5.3 재핀 완료(07-30)·인게임 applied=14/14 검증(obj_imm.txt·FAIL 0). 기본 0",
    ...
    "oi_an_finish_hp" => "[oi·확증07-16] 적 넥서스 <b>마무리오더</b> 발행 HP% 게이트(disc18·0.5.2부터 두 갈래 다 패치). 적넥서스 HP%가 <b>이값 이상</b>이면 아군 합류수 무관 즉시 마무리오더, 미만이면(13분 이후) 아군 2명↑ 도달가능할 때만 발행. ↓낮추면 더 낮은 넥서스 HP%에서도 단독 압박=<b>공격적</b>, ↑높이면 스쿼드 대기=신중. 원본 56",
    "oi_an_cull_dist" => "[oi·확증07-16] 넥서스 공격후보 <b>거리 컬링</b>(disc18). 아군↔적넥서스 거리²(>>14 스케일)가 이값 초과면 그 아군은 넥서스 공격후보에서 제외. ↑높이면 더 먼 아군도 넥서스 공세 동원(탐색범위↑), ↓낮추면 근접 아군만. 원본 390624(≈넥서스 2.5셀 반경). ★0.5.3: 패치 자리 <b>1곳→3곳</b>(게임이 후보를 세 묶음으로 나눠 훑음)·비교방향 반전 — 모드가 내부 보정하므로 <b>의미·체감 동일</b>",
    ...
    "gb_enable" => "✅[07-16 경로A] GenericBuild 로밍 byte-patch <b>마스터 스위치</b>. 0(기본)=게임 원본 그대로. 1이면 아래 gb_* 로밍/게이트 값이 실제 게임 판단상수에 덮어써짐(각 값 -1이면 그 항목만 원본유지). 인게임 gb_imm.txt에 applied=N/<b>9</b> 기록. ★0.5.3 재핀 완료(07-30, 10곳→9곳: gb_scout_radius 2곳→1곳 병합·인게임 9/9 실증). 구 이력: 0.5.2 재핀(07-23) 전까진 주소가 어긋나 전량 무반영이었음. gb_join_phase는 0.5.2부터 死.",
    ...
    "gb_scout_radius" => "✅[07-16] 거점/타겟 <b>후보수집 반경</b>(유닛, 원본 120000)=로밍 탐색범위. 후보 유닛/거점을 이 반경 안에서만 채택. ↑키우면 더 먼 거점까지 로밍 대상=로밍 범위 확장, ↓줄이면 근처만. -1=원본. ★0.5.3: 패치 자리 <b>2곳→1곳 병합</b>(게임이 같은 기준값을 루프 앞뒤 2곳에 복사하던 것을 루프 안 1회 사용으로 변경)·비교방향 반전은 모드가 보정 = <b>의미·체감 동일</b>. 적용확인=gb_imm.txt.",
    ...
    "gb_reach_cap" => "⚠[07-16] <b>전역 사거리 상한</b>(유닛, 원본≈140052). ★GenericBuild 전용 아님 — 전 AI가 공유하는 reach 헬퍼라 켜면 <b>모든 판단의 사거리 상한</b>이 바뀜(신중). ↑키우면 전반적으로 더 먼 대상 도달가능 판정. -1=원본(무개입 권장).",
