...
const SERPEN_RVA: usize = 0x21f8ca0; // 0.5.2 (구0.5.1=0x1f8d0c0) kind6. ★migrator 오답 0x1c70e90=kind5 Epic이었음. 프롤로그 8push+sub rsp,0x358.
...
static CFG_FIXED_HASH: AtomicBool = AtomicBool::new(false);
...
const RENDER_RVA: usize = 0x1136600; // ⬜0.5.2 미확정(구0.5.1 값·render_probe=0이라 미사용, 0.5.1서도 미설치)
...
const SHEET_RVA: usize = 0x51bbc0; // ⬜0.5.2 미확정(구0.5.1 값·CFG_SHEET_PROBE=false라 미사용)
...
const SEED_OFF: usize = 0xeab8; // 세르펜 detour rcx(provider) + 0xeab8 = 경기 시드(u64, 불변). 0.5.2 델타0 확인.
...
fn pick_live<'a>(m: &'a HashMap<(u64, u64), WorldState>, ls: u64) -> Option<&'a WorldState> {
    ...
    let first = it.next()?;
    if it.next().is_some() { None } else { Some(first) } // 복수 파티션 + fp 미확정 = 조회 포기(오염보다 안전)
}
...
const MOBATICK_RVA: usize = 0x230c290; // 0.5.2 (구0.5.1=0x21fcf90). 마스크시그 UNIQUE + 본문 L1=0.976/L2=0.987(경미 리인라인)·프롤로그 12B 동일.
...
const DB_SCENE_GAME_OFF: usize = 0x1340; // db + 0x1340 = *mut Game (ClientScene payload) ⬜0.5.2 런타임검증 대기
...
const SPAWN_HOOKS: [usize; 2] = [0x53aae0, 0x539f40]; // 0.5.2 (구0.5.1=0x50edd0/0x50e230). 둘 다 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
const LAUNCHER_RVA: usize = 0x1d96870; // 0.5.2 (구0.5.1=0x20588a0). 콜그래프 앵커 7/7 만장일치(전 콜사이트 EQ 정렬).
...
const LAUNCHER_RET_A: usize = 0x759c36; // 0.5.2 화면 경기 경로 A (구0.5.1=0x72f507)
const LAUNCHER_RET_B: usize = 0x75e5cf; // 0.5.2 화면 경기 경로 B (구0.5.1=0x733e9f)
...
const ARG_STR_RVA: usize = 0xfef190; // 0.5.2 (구0.5.1=0xb4fda0). 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
const UILOADER_RVA: usize = 0x5ac950;  // 0.5.2 (구0.5.1=0x40f3d0) 제네릭 asset-get(main/ingame 계열). ⚠item_tactics 등과 공유 → 체이닝 필수
const UIPARSER_RVA: usize = 0x24b5a00; // 0.5.2 (구0.5.1=0x24b4590) .ui 텍스트 → NodeTemplate. 스켈레톤 L1 UNIQUE.
const UIALLOC_RVA: usize = 0x25c4d30;  // 0.5.2 (구0.5.1=0x25c5a40) 게임 힙 alloc(size, align). 스켈레톤 L1 UNIQUE.
    ...
    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        if new_rva && CFG_PROBE_LOG.load(Ordering::Relaxed) {
            log_push(format!("[{}ms] ◆런처 신규 retaddr rva={:#x} seed={:#x}{}", now_ms(), rva, seed,
                if rva == LAUNCHER_RET_A || rva == LAUNCHER_RET_B { " ★게이트" } else { "" }));
        }
...
const RENDER_STEP_RVA: usize = 0x811500; // 0.5.2 (구0.5.1=0x872950). 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
const RUNNER_CTOR_RVA: usize = 0x1d981e0; // 0.5.2 (구0.5.1=0x205a2f0). 콜그래프 앵커 3/3 만장일치(EQ 정렬)·프롤로그 12B 동일.
...
const DISP_RVA: usize = 0x9f3090; // 0.5.2 (구0.5.1=0x9f40a0, -0x1010 RVA-only). 스켈레톤 L1 UNIQUE·프롤로그 동일. (현재 producer_seam=1이라 미사용 경로)
...
const BUILD_RVA: usize = 0x5b1d80; // 0.5.2 (구0.5.1=0x414800). 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
const PUSH_RVA: usize = 0x2217600; // 0.5.2 (구0.5.1=0x1f15940, +0x301cc0). 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
const BUFFAPPLY_RVA: usize = 0x1daa7b0; // 0.5.2 (구0.5.1=0x21df4f0). 스켈레톤 L1 UNIQUE·프롤로그 동일.
...
",
        ...
        s.push_str(&format!("[◆진단2차] 타임라인모순={} 킬재기록={} (0이 아니면 같은 seed에 복수 타임라인 혼입 = 제보 버그 기전)\n",
            TL_ANOMALY_N.load(Ordering::Relaxed), KILLDIFF_LOG_N.load(Ordering::Relaxed)));
        s.push_str(&format!("[결정론실험] 고정해시키={} 훅설치={} detour발화={} (실험: fixed_hash_seed=1이면 킬재기록이 0으로 떨어져야 가설 확증)\n",
            CFG_FIXED_HASH.load(Ordering::Relaxed), RNGGET_INSTALLED.load(Ordering::Relaxed), FIXED_HASH_N.load(Ordering::Relaxed)));
        s.push_str(&format!("[스레드분석] 킬재기록 같은스레드(재계산)={} 다른스레드(동시)={} — 다른스레드>0이면 FP 병렬 비결정 가설 성립가능, 같은스레드뿐이면 재계산 문제\n",
            KILLDIFF_SAME_TID.load(Ordering::Relaxed), KILLDIFF_DIFF_TID.load(Ordering::Relaxed)));
        s.push_str(&format!("[salt동기] on={} 훅설치={} 셋업발화={}회 비셋업통과={}회 resolve={}회 ★공유(둘째인스턴스)={}회 (킬재기록↓+공유>0 = 관전=결과 고침 발화)\n",
            CFG_FIXED_SALT.load(Ordering::Relaxed), SALT_INSTALLED.load(Ordering::Relaxed),
            SALT_HIT_N.load(Ordering::Relaxed), SALT_PASS_N.load(Ordering::Relaxed),
            SALT_RESOLVE_N.load(Ordering::Relaxed), SALT_SHARED_N.load(Ordering::Relaxed)));
        {
            let mut fp = String::new();
            for i in 0..8 {
                let t = MXCSR_TIDS[i].load(Ordering::Relaxed);
                if t == 0 { break; }
                let v = MXCSR_VALS[i].load(Ordering::Relaxed);
                fp.push_str(&format!("tid{}=MXCSR{:#06x}/CW{:#06x} ", t, (v >> 16) as u32, (v & 0xffff) as u16));
            }
            s.push_str(&format!("[FP환경] fix_mxcsr={} 통일발화={}회 | {} (스레드별 값 상이 = FP환경 비결정 후보)\n",
                CFG_FIX_MXCSR.load(Ordering::Relaxed), FP_FIX_N.load(Ordering::Relaxed),
                if fp.is_empty() { "미수집".into() } else { fp }));
        }
...
fn load_cfg() {
    ...
    for line in txt.lines() {
        ...
        match k {
            ...
            "stat_write" => CFG_STAT_WRITE.store(on, Ordering::Relaxed), // 대조실험: 템플릿 write만 억제
            "fixed_hash_seed" => CFG_FIXED_HASH.store(on, Ordering::Relaxed), // 결정론 실험: 해시키 고정
            "fixed_salt" => CFG_FIXED_SALT.store(on, Ordering::Relaxed), // 결정론 실험2: 매치셋업 salt 동기
            "fix_mxcsr" => CFG_FIX_MXCSR.store(on, Ordering::Relaxed),   // 결정론 실험3: FP 환경 통일
...
unsafe fn probe_buffapply(n: u64, r8: u64, r9: u64) {
    ...
    if sn >= 40 { return; } // 07-24 제보 진단: 다세트 관전 동안 실제 뿌려진 버프 스탯 전수 확보 12→40
        ...
        if data > 0x10000 && vt > 0x10000 {
            if let Some(m) = safe_read_u64(vt as usize + 0x48) {
                if m > 0x10000 {
                    ...
                    let known = { let wg = WORLDS.lock().unwrap_or_else(|e| e.into_inner()); wg.as_ref().map(|w| w.keys().any(|k| k.0 == seed)).unwrap_or(false) };
...
// ══ 결정론 실험 2탄(07-24): 매치셋업 salt 고정 ══
//   ★근원 확정(RE 07-24): 매치 구축 함수 0xd5f380의 mode==3 분기가 thread_rng로 salt(u64)를 뽑아
//   런처 0x1d96870 param_3으로 넘겨 매치를 시드한다. thread_rng=스레드별 OS엔트로피 → 같은 표시seed
//   경기라도 스레드마다 다른 salt → 다른 전개 → "관전 화면 ≠ 확정 결과". next_u64 콜사이트 6개 중
//   **retaddr 0xd6480b(셋업 salt)만** 격리해 반환 salt를 고정 → 두 인스턴스 수렴 여부로 근원 확증.
//   ⚠call-then-override: 원본 next_u64를 호출해 thread-local rng 스트림을 정상 진행시킨 뒤 rax만 덮음
//   (건너뛰면 그 스레드의 이후 draw가 밀려 셋업 오작동). 나머지 5개 retaddr는 무손상 통과.
const NEXTU64_RVA: usize = 0x496110;      // ThreadRng::next_u64 (07-24 RE)
const NEXTU64_PROLOGUE: [u8; 12] = [0x56, 0x57, 0x48, 0x83, 0xec, 0x28, 0x48, 0x89, 0xce, 0x48, 0x8b, 0x81];
const SALT_SETUP_RETADDR_RVA: usize = 0xd6480b; // 셋업 salt 콜사이트 다음 명령(격리 게이트)
static CFG_FIXED_SALT: AtomicBool = AtomicBool::new(false);
static SALT_TRAMP_ADDR: AtomicU64 = AtomicU64::new(0);   // 원본 next_u64 트램폴린 주소
static SALT_SETUP_RET_ADDR: AtomicU64 = AtomicU64::new(0); // EXE_BASE+0xd6480b
static SALT_HIT_N: AtomicU64 = AtomicU64::new(0);   // 셋업 salt 발화 수(asm 참조)
static SALT_PASS_N: AtomicU64 = AtomicU64::new(0);  // 비-셋업 통과 수(asm 참조)
static SALT_RESOLVE_N: AtomicU64 = AtomicU64::new(0); // resolve 콜백 수
static SALT_SHARED_N: AtomicU64 = AtomicU64::new(0);  // 캐시 공유(둘째 인스턴스) 히트 수 = 고침 발화
static SALT_INSTALLED: AtomicBool = AtomicBool::new(false);
// ★매치 salt 캐시 (유저 착안 07-24): salt를 결정론화하지 않고 **같은 경기의 두 인스턴스가 첫 salt를
//   공유**만 하게 함. 키 = (매치id, 홈팀id, 어웨이id). salt 값은 랜덤 그대로 = 리그 밸런스 무변경.
//   첫 인스턴스: thread_rng 랜덤 저장 / 둘째: 저장값 반환 ⟹ 화면=결과 수렴, 관전≠결과 해소.
static SALT_CACHE: Mutex<Option<HashMap<u64, u64>>> = Mutex::new(None);
// ★resolve 콜백(win64): 원본 salt(orig)를 받아, 매치 레코드(r13)의 id로 캐시 조회. 첫 등록이면 orig
//   저장·반환, 이미 있으면 저장값 반환(=둘째 인스턴스가 첫 salt 공유). id 무효면 orig(바닐라).
extern "win64" fn salt_resolve(record: usize, orig: u64) -> u64 {
    let out = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if record < 0x10000 { return orig; }
        let m = unsafe { safe_read_i32(record.wrapping_sub(0x40)) }.unwrap_or(-1);
        let ta = unsafe { safe_read_i32(record.wrapping_sub(0x28)) }.unwrap_or(-1);
        let tb = unsafe { safe_read_i32(record.wrapping_sub(0x14)) }.unwrap_or(-1);
        if ta < 0 || tb < 0 { return orig; } // 팀 id 무효 → 바닐라(안전판)
        // 키 = 매치id<<32 ^ 홈팀<<16 ^ 어웨이 (매치id가 두 인스턴스 동일이면 경기별 고유 = 정확)
        let key = ((m as u32 as u64) << 32) ^ ((ta as u32 as u64) << 16) ^ (tb as u32 as u64);
        let (salt, first) = {
            let mut g = SALT_CACHE.lock().unwrap_or_else(|e| e.into_inner());
            let cache = g.get_or_insert_with(HashMap::new);
            match cache.get(&key) {
                Some(&v) => (v, false),
                None => { cache.insert(key, orig); (orig, true) }
            }
        };
        if !first { SALT_SHARED_N.fetch_add(1, Ordering::Relaxed); }
        if CFG_PROBE_LOG.load(Ordering::Relaxed) {
            let n = SALT_RESOLVE_N.fetch_add(1, Ordering::Relaxed);
            if n < 48 { log_push(format!("[{}ms] ◆salt동기 rec={:#x} m={} ta={} tb={} key={:#x} orig={:#x}→{:#x} {}",
                now_ms(), record, m, ta, tb, key, orig, salt, if first { "[첫등록]" } else { "★[공유]" })); }
        } else { SALT_RESOLVE_N.fetch_add(1, Ordering::Relaxed); }
        salt
    }));
    out.unwrap_or(orig)
}
// naked detour: [rsp]=retaddr. 셋업(==SALT_SETUP_RET_ADDR)이면 원본 next_u64 call(rng 스트림 진행) 후
//   salt_resolve(record=r13, orig=rax)로 캐시 공유 salt 획득해 반환. 아니면 트램폴린 tail-jmp.
//   ⚠심볼 참조 = sym operand 필수(07-24 크래시: [rip+이름] 직접참조는 Windows서 잘못된 relocation).
//   ⚠r13 = 매치 레코드(셋업 프레임, callee-saved라 call 2회 넘어 보존). rsp%16: 진입8 → sub0x28 → 0.
core::arch::global_asm!(
    ".globl salt_detour",
    "salt_detour:",
    "  mov r11, qword ptr [rip + {setup_ret}]",
    "  cmp qword ptr [rsp], r11",
    "  jne 2f",
    "  lea r11, [rip + {hit}]",
    "  lock inc qword ptr [r11]",
    "  sub rsp, 0x28",                     // 32B shadow + 8 align
    "  call qword ptr [rip + {tramp}]",    // 원본 next_u64 → rax=랜덤 salt
    "  mov rcx, r13",                       // arg1 = 매치 레코드
    "  mov rdx, rax",                       // arg2 = 원본 salt
    "  call {resolve}",                     // rax = 캐시 공유 salt
    "  add rsp, 0x28",
    "  ret",
    "2:",
    "  lea r11, [rip + {pass}]",
    ...
    setup_ret = sym SALT_SETUP_RET_ADDR,
    hit = sym SALT_HIT_N,
    tramp = sym SALT_TRAMP_ADDR,
    pass = sym SALT_PASS_N,
    resolve = sym salt_resolve,
);
extern "C" { fn salt_detour(); }
fn install_salt_hook() {
    if !CFG_FIXED_SALT.load(Ordering::Relaxed) { return; }
    if SALT_INSTALLED.swap(true, Ordering::Relaxed) { return; }
    unsafe {
        let base = GetModuleHandleW(core::ptr::null());
        if base == 0 { SALT_INSTALLED.store(false, Ordering::Relaxed); return; }
        EXE_BASE.store(base, Ordering::Relaxed);
        let fn_addr = base + NEXTU64_RVA;
        for i in 0..12 {
            if *((fn_addr + i) as *const u8) != NEXTU64_PROLOGUE[i] {
                log_push(format!("[{}ms] salt(next_u64) prologue mismatch @+{}: {:#x}", now_ms(), i, *((fn_addr + i) as *const u8)));
                SALT_INSTALLED.store(false, Ordering::Relaxed); return;
            }
        }
        // 트램폴린 = 원본 16B + jmp [rip] abs(fn+0x10)
        let stub = VirtualAlloc(0, 64, 0x3000, 0x40);
        if stub == 0 { SALT_INSTALLED.store(false, Ordering::Relaxed); return; }
        let mut tb: Vec<u8> = Vec::new();
        tb.extend_from_slice(core::slice::from_raw_parts(fn_addr as *const u8, 16)); // 원본 16B(경계 맞음)
        tb.extend_from_slice(&[0xff, 0x25, 0, 0, 0, 0]);                             // jmp qword [rip+0]
        tb.extend_from_slice(&((fn_addr + 0x10) as u64).to_le_bytes());              // → fn+0x10
        core::ptr::copy_nonoverlapping(tb.as_ptr(), stub as *mut u8, tb.len());
        SALT_TRAMP_ADDR.store(stub as u64, Ordering::Relaxed);
        SALT_SETUP_RET_ADDR.store((base + SALT_SETUP_RETADDR_RVA) as u64, Ordering::Relaxed);
        // FIXED_VAL: 0 = 전 인스턴스 동일 salt. (표시seed 유래로 바꾸려면 여기 설정)
        // 엔트리 12B 패치
        let mut patch = [0u8; 12];
        patch[0] = 0x48; patch[1] = 0xb8;
        patch[2..10].copy_from_slice(&(salt_detour as usize).to_le_bytes());
        patch[10] = 0xff; patch[11] = 0xe0;
        let mut old = 0u32;
        if VirtualProtect(fn_addr, 12, 0x40, &mut old) == 0 { SALT_INSTALLED.store(false, Ordering::Relaxed); return; }
        core::ptr::copy_nonoverlapping(patch.as_ptr(), fn_addr as *mut u8, 12);
        VirtualProtect(fn_addr, 12, old, &mut old);
        FlushInstructionCache(GetCurrentProcess(), fn_addr, 12);
        log_push(format!("[{}ms] ★★salt고정 훅 installed fn={:#x} tramp={:#x} setup_ret={:#x}",
            now_ms(), fn_addr, stub, base + SALT_SETUP_RETADDR_RVA));
    }
}
// ══ 결정론 실험(07-24): thread-local RandomState 게터 전량 대체 ══
//   게임은 화면재생/결과확정/되감기 sim을 별도 워커 스레드로 병렬 실행한다. 각 tick은 내부에서
//   Rust std HashMap을 만드는데, 그 SipHash 키를 **스레드-로컬 RandomState**(ProcessPrng seed)에서
//   뽑는다. 키가 스레드마다 달라 → HashMap 순회 순서가 스레드 의존 → 같은 seed·같은 World여도
//   "동률 판정"(우선순위 같은 타깃 2개 등)의 첫 순간에 인스턴스별로 다른 선택 → 킬 타이밍/팀 분기
//   → 화면(뷰 인스턴스)과 확정 결과(다른 인스턴스)가 어긋남. (RE @0x23147c4·대조실험 07-24로 확정.)
//   ★게터(0x25c47d0)는 값이 아니라 TLS 셀 **포인터**를 반환(cell: k0@+0, k1@+8, init_flag@+0x10).
//   detour = 그 셀을 (0,0,flag=1) 강제 후 포인터 반환 ⟹ 전 스레드가 동일 해시키 = HashMap 순회
//   결정론화. flag=1이 필수(안 하면 콜러가 ProcessPrng init 경로로 빠져 난수로 덮음). 스레드-로컬
//   대상이라 rayon 병렬에도 레이스 없음. lock/alloc/format 없는 순수 write뿐(tick당 다수 호출 대비).
//   ⚠전역 훅 = UI/로딩 포함 프로그램 전체 HashMap이 (0,0) → 순회순서 결정론화(정상 Rust 로직엔
//   무해, repro엔 이로움). cfg fixed_hash_seed 게이트, 기본 OFF = 검증 전용(배포본 무영향).
const RNGGET_RVA: usize = 0x25c47d0; // 0.5.2 TLS RandomState 게터 (07-24 RE)
const RNGGET_PROLOGUE: [u8; 12] = [0x8b, 0x05, 0x4a, 0x10, 0xab, 0x01, 0x65, 0x48, 0x8b, 0x0c, 0x25, 0x58];
const TLS_INDEX_RVA: usize = 0x4075820; // _tls_index (런타임 값 read)
const RNG_CELL_OFF: usize = 0x4c8;      // TLS 블록 내 RandomState 키셀 오프셋
static RNGGET_INSTALLED: AtomicBool = AtomicBool::new(false);
static FIXED_HASH_N: AtomicU64 = AtomicU64::new(0); // detour 발화 수(진단)
unsafe extern "system" fn rngget_detour() -> *mut u64 {
    let base = EXE_BASE.load(Ordering::Relaxed);
    let tls_index = *((base + TLS_INDEX_RVA) as *const u32) as usize;
    let tls_array: *const usize;
    core::arch::asm!("mov {}, gs:[0x58]", out(reg) tls_array, options(nostack, preserves_flags));
    let block = *tls_array.add(tls_index);
    if block < 0x1000 { return core::ptr::null_mut(); } // 방어(원본은 null 미체크지만 안전측)
    let cell = (block + RNG_CELL_OFF) as *mut u64;
    *cell = 0;                                  // k0 = 0
    *cell.add(1) = 0;                            // k1 = 0
    *((block + RNG_CELL_OFF + 0x10) as *mut u8) = 1; // init_flag=1 → 콜러 ProcessPrng init 스킵
    FIXED_HASH_N.fetch_add(1, Ordering::Relaxed);
    cell
}
fn install_rngget_hook() {
    if !CFG_FIXED_HASH.load(Ordering::Relaxed) { return; } // 게이트: 켜졌을 때만 설치
    if RNGGET_INSTALLED.swap(true, Ordering::Relaxed) { return; }
    unsafe {
        let base = GetModuleHandleW(core::ptr::null());
        if base == 0 { RNGGET_INSTALLED.store(false, Ordering::Relaxed); return; }
        EXE_BASE.store(base, Ordering::Relaxed);
        let fn_addr = base + RNGGET_RVA;
        for i in 0..12 {
            if *((fn_addr + i) as *const u8) != RNGGET_PROLOGUE[i] {
                log_push(format!("[{}ms] rngget prologue mismatch @+{}: {:#x} (RVA {:#x})",
                    now_ms(), i, *((fn_addr + i) as *const u8), RNGGET_RVA));
                RNGGET_INSTALLED.store(false, Ordering::Relaxed); return;
            }
        }
        let mut patch = [0u8; 12];
        patch[0] = 0x48; patch[1] = 0xb8;
        patch[2..10].copy_from_slice(&(rngget_detour as usize).to_le_bytes());
        patch[10] = 0xff; patch[11] = 0xe0;
        let mut old = 0u32;
        if VirtualProtect(fn_addr, 12, 0x40, &mut old) == 0 { return; }
        core::ptr::copy_nonoverlapping(patch.as_ptr(), fn_addr as *mut u8, 12);
        VirtualProtect(fn_addr, 12, old, &mut old);
        FlushInstructionCache(GetCurrentProcess(), fn_addr, 12);
        log_push(format!("[{}ms] ★★결정론 훅(고정해시키) installed fn={:#x} (RVA {:#x})", now_ms(), fn_addr, RNGGET_RVA));
    }
}
// ══ 결정론 실험 3탄(07-24): FP 환경(MXCSR/x87 CW) 스레드 의존 조사 ══
//   salt 기각 후 남은 사실: 같은 seed(World rng) 두 인스턴스가 다른 전개 = World rng 밖 스레드 의존.
//   유력 후보 = 부동소수점 제어 레지스터가 sim 워커 스레드마다 달라(rounding/FTZ/DAZ) 같은 FP 연산이
//   미세하게 다른 결과 → 접전(동률)서 승부 갈림 → "가끔" 분기. (단일스레드 결정론과 무모순 = FP'환경'.)
//   관찰: sim tick 스레드별 MXCSR/x87CW 로그 → 상이하면 가설 지지. 통일(cfg fix_mxcsr): tick 전 표준값
//   강제 → 킬재기록 줄면 확증. 표준 MXCSR=0x1f80(all masked·round-nearest·no FTZ/DAZ), x87CW=0x027f.
static CFG_FIX_MXCSR: AtomicBool = AtomicBool::new(false);
static MXCSR_TIDS: [AtomicU64; 8] = [const { AtomicU64::new(0) }; 8];
static MXCSR_VALS: [AtomicU64; 8] = [const { AtomicU64::new(0) }; 8]; // (mxcsr<<16)|x87cw
static MXCSR_N: AtomicU64 = AtomicU64::new(0);
static FP_FIX_N: AtomicU64 = AtomicU64::new(0);
unsafe fn get_mxcsr() -> u32 {
    let mut v: u32 = 0;
    core::arch::asm!("stmxcsr [{}]", in(reg) &mut v as *mut u32, options(nostack, preserves_flags));
    v
}
unsafe fn get_x87cw() -> u16 {
    let mut v: u16 = 0;
    core::arch::asm!("fnstcw [{}]", in(reg) &mut v as *mut u16, options(nostack, preserves_flags));
    v
}
unsafe fn set_std_fp() {
    let m: u32 = 0x1f80;
    core::arch::asm!("ldmxcsr [{}]", in(reg) &m as *const u32, options(nostack, preserves_flags));
    let c: u16 = 0x027f;
    core::arch::asm!("fldcw [{}]", in(reg) &c as *const u16, options(nostack, preserves_flags));
    FP_FIX_N.fetch_add(1, Ordering::Relaxed);
}
// sim 스레드별 FP 환경 최초 1회 기록(원래 값). tid 8개까지.
fn record_fp_env() {
    ...
    let idx = MXCSR_N.fetch_add(1, Ordering::Relaxed) as usize;
    if idx < 8 {
        let (m, c) = unsafe { (get_mxcsr(), get_x87cw()) };
        MXCSR_TIDS[idx].store(tid, Ordering::Relaxed);
        MXCSR_VALS[idx].store(((m as u64) << 16) | c as u64, Ordering::Relaxed);
        log_push(format!("[{}ms] ◆FP환경 tid={} MXCSR={:#06x} x87CW={:#06x}", now_ms(), tid, m, c));
    }
}
extern "win64" fn mobatick_detour(a: u64, b: u64, c: u64, d: u64) {
    let t = MOBATICK_TRAMP.load(Ordering::Relaxed);
    if t == 0 { return; }
    // FP 환경: 원래 값 기록(통일 전) → 그 다음 통일(cfg on). 원본 tick이 통일된 FP로 계산되게 tick 전에.
    if CFG_PROBE_LOG.load(Ordering::Relaxed) {
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(record_fp_env));
    }
    if CFG_FIX_MXCSR.load(Ordering::Relaxed) { unsafe { set_std_fp(); } }
...
const DMGA_RVA: usize = 0x22164a0;   // 0.5.2 (구0.5.1=0x1f147e0, +0x301cc0=PUSH와 동일 델타) 최종 HP 감산 어플라이어 (모든 실드-경유 피해 수렴점)
...
const DMGB_RVA: usize = 0x22d2b20;   // 0.5.2 (구0.5.1=0x21e2400) 딜 파이프라인 (r8=World) — TLS world 캡처 전용. 스켈레톤 L1 UNIQUE.
...
const ENTBUILD_RVA: usize = 0xc33950; // 0.5.2 (구0.5.1=0x13c4e90). 마스크시그 UNIQUE + L2=0.9892·프롤로그 동일.
...
const ANIM_LOOKUP_RVA: usize = 0xeb0880;  // ⬜0.5.2 미확정(구0.5.1 값·anim_probe=0이라 미사용)
const SHEET_LOOKUP_RVA: usize = 0xeb0420; // ⬜0.5.2 미확정(구0.5.1 값·anim_probe=0이라 미사용)
...
const KEYRES_RVA: usize = 0xc2f990; // 0.5.2 (구0.5.1=0x13c0e90). 콜그래프 앵커 7/7 만장일치(전 콜사이트 EQ 정렬).
...
fn ensure_setup() {
    ...
    // ★스프라이트 교체 = 생산자 seam(키 리졸버) 전담. 디스패처(0x9f40a0) 훅은 2026-07-19 제거 —
    //   커맨드당 발화·체인 후킹·매 프레임 진입부 재검증이 전부 불필요해졌다.
    ...
    // ★결정론 실험 훅(cfg fixed_hash_seed=1일 때만) — 병렬 sim HashMap 해시키 고정
    install_rngget_hook();
    // ★결정론 실험2 훅(cfg fixed_salt=1일 때만) — 매치셋업 thread_rng salt 고정
    install_salt_hook();
...
fn update_tip_panel(ui: &mut GameUI) {
    ...
    if !b.is_empty() { text.push_str(&format!("[{}]{}\n", tr("ui.blue_team"), b)); }
    if !r.is_empty() { text.push_str(&format!("[{}]{}", tr("ui.red_team"), r)); }
    if text.is_empty() { text = tr("ui.no_kills"); }
...
fn resolve_color_from_played() {
    ...
    let wg = WORLDS.lock().unwrap_or_else(|e| e.into_inner());
    let Some(ws) = wg.as_ref().and_then(|w| pick_live(w, ls)) else { return; };
...
impl ModExtension for ElementalSerpenExt {
    ...
    fn post_update(&self, scene: &mut Scene, ui: &mut GameUI, _assets: &mut Assets, _dt: f32) {
        ...
        resolve_render_fp(); // ★(07-24) 화면 세트 fp — 미확정이면 프레임마다 재시도(확정 후 no-op)
