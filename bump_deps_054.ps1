# bump_deps_054.ps1 - raise base dependency of ALREADY-REBUILT mods to the 0.5.4 band.
#
# Policy (user directive 2026-07-30, carried into 0.5.4): base dep = ">=<current>, <<next>>"
#   so that when the next patch lands, stale dlls auto-disable at the loader and cannot crash the game.
# WARNING: apply ONLY to mods whose dll has already been rebuilt against sdk_054.
#          The upper bound (<0.5.4) on a not-yet-rebuilt mod IS the safety net.
...
#
# usage: powershell -File bump_deps_054.ps1 -ModIds a,b,c
#        powershell -File bump_deps_054.ps1 -ModIds a -WhatIf
param(
...
# The band is not written uniformly across mods: some use ">=0.5.3, <0.5.4" and some
# ">=0.5.3,<0.5.4" (no space, e.g. tfm2_item_tactics). Match on a regex, not a literal.
$OLDRX = '>=\s*0\.5\.3\s*,\s*<\s*0\.5\.4'
$NEWDEP = '>=0.5.4, <0.5.5'
...
foreach ($id in $ModIds) {
  ...
  if ($raw -notmatch $OLDRX) {
    if ($raw -match [regex]::Escape($NEWDEP)) {
      Write-Output ("{0,-28} already 0.5.4 band" -f $id)
    } else {
