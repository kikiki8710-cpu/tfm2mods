...
| `tfm2_bancard_keep` | 환경설정 "밴 카드 수"(1~5장) 리셋 방지 핫픽스 — 룸 설정 커밋 새니타이저의 풀 부족 클램프가 GPO+0x720(`room_practice_ban_count`)을 0으로 store하는 사이트를 **NOP×11 바이트패치**(init 1회·orig 11B 실측 검증·멱등) + 진단 로그 `bancard_keep.txt`(GPO+0x720 폴링 — (A)클램프/(B)커밋누락 판별용) | **비-T1** ⚠**하드코딩 RVA 보유(패치 사이트+새니타이저) = 패치 시 재핀 필요(정본=`MIGRATION.md §7.5 §10`)**. v0.1.0 배포완(0.5.5·dll 143,872B @2026-08-12 19:42:12·deps `>=0.5.5, <0.5.6`)·**⬜인게임 미검증(A/B 판별 대기)·⬜릴리스 zip = 판별 후**. 정본=`REPORT\tfm2_bancard_keep\` |
