...
| `tfm2_champ_pos_lock` | 특정 챔피언을 특정 포지션에만 쓰게 제한(챔피언별 허용 포지션 cfg) — 축1 = SDK `ModDraftScoreHook` score_pick에서 허용 포지션 매칭(홀 조건) 깨지는 후보 Replace(-1e9)(RVA 0) + 축2 Hook A = champ→eligible-positions 비트마스크 산출기 트램폴린 detour 1사이트(마스크 AND 교정·★범위=AI 평가/추천/자동배정만 — 유저 수동 스왑 미지배, 3차 RE 08-19) | **비-T1** ⚠**축2 하드코딩 RVA 1개 = 패치 시 재핀 필요(정본=~~`MIGRATION.md §7.5 §12`~~ → **`MIGRATION.md §7.6 §3`**(0.5.6 현행))**. v0.1.0 배포완(0.5.5·dll ~~166,400B @23:31:09~~ → **166,400B @2026-08-19 23:43:00** 문구 정정 재빌드)·**⬜인게임 미검증·⬜릴리스 zip = 검증 후**. ⛔인접 함정(0x1a636c0≠참가자레코드 writer)=`MEM\DONE.md` 08-19 행. 정본=`REPORT\tfm2_champ_pos_lock\` + `MEM\tfm2-champ-pos-lock-mod.md`. 2026-08-19 등재 |
| `tfm2_champion_exclude` | 인게임 시즌 패치의 **신챔프 추가 대상에서 특정 챔피언(바닐라·모드) 영구 제외** — cfg 기반(`'*'`=전면 차단)·후보 Vec 생성 함수 트램폴린 detour 1사이트(원본 호출 후 out Vec 사후 필터) + ★**v0.2.0 인게임 설정 UI**(환경설정 게임플레이 탭 '추가 챔피언 설정' 행 + 미출시 챔피언 그리드 팝업 → cfg 저장·후보 = 슈퍼셋+멤버십 검증) | **비-T1** ⚠**하드코딩 RVA = 패치마다 재핀 필요(정본=`MIGRATION.md §7.6 §3`)**. ~~0.5.6 배포완(dll 163,840B @2026-08-20 20:56:42·커밋 `0da8885`)~~ → ★**v0.2.0 배포완(2026-08-23·dll 2,654,720B @05:50:08·deploy-verify PASS·커밋 `80c853b` — ⚠사이즈가드 초과 전환 = rustc 직접+Copy-Item, banpick_illust 류)**·deps `>=0.5.6, <0.5.7`·**⬜인게임 미검증(기존 훅 + v0.2.0 UI = REPORT 02 §6 표 1~13)·⬜릴리스 zip = 검증 후**. 정본=`REPORT\tfm2_champion_exclude\` + `MEM\tfm2-champion-exclude-mod.md`. 2026-08-20 등재 |
