...
// heap ptr이 String{ptr,?,len} 또는 record{+8=char*,+0x10=len}인지 시도 → id 문자열.
unsafe fn try_id_at(p: usize) -> Option<String> {
    // (a) p 자체가 char* + 다음워드 len? — 호출측서 (p,len) 판단
    // (b) p = master record 후보: +8=char*, +0x10=len
    let cptr = rd_u64(p + 8)? as usize;
    let clen = rd_u64(p + 0x10)? as usize;
    if let Some(s) = read_id(cptr, clen) { return Some(s); }
    None
}
...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m3_log.txt") {
...
unsafe fn apply_common(saved: usize, e: usize) {
    let action = *((saved + 0x28) as *const u64) as usize; // rcx = action POD
    let world  = *((saved + 0x20) as *const u64) as usize; // rdx = world(엔티티 스토리지)
    let cctx   = rd_u64(e + 0x28).unwrap_or(0) as usize;    // [orig_rsp+0x28] = casting_ctx(r15)
    if cctx < 0x10000 { return; }
    // casting_ctx 키 후보 오프셋들로 resolve → 어느 게 caster/target entity인지 실측
    let mut ents: Vec<(usize, u64, usize)> = Vec::new(); // (key_off, key, entity)
    for koff in [0usize, 8, 0x10, 0x18, 0x20] {
        let key = match rd_u64(cctx + koff) { Some(v) => v, None => continue };
        if key >= 0x100000 { continue; } // 슬롯 key는 작은 인덱스
        let ent = resolve(world, key);
        if ent != 0 { ents.push((koff, key, ent)); }
    }
    if ents.is_empty() { return; }

    // entity dedup + 상한
    let first_ent = ents[0].2;
    let is_new = {
    ...
    if !is_new { return; }
    if N.fetch_add(1, Ordering::Relaxed) >= 120 { return; }

    // action POD 시그니처 (궁 판별자 후보)
    let disc = rd_u64(action).unwrap_or(0);
    let gate = rd_u64(action + 0x48).unwrap_or(0);
    let a188 = rd_u64(action + 0x188).unwrap_or(0);
    let a198 = rd_u64(action + 0x198).unwrap_or(0);
    let cty = a198 & 0xffff_ffff; // casting_type 후보

    let mut s = format!(
        "\n[m3#{}] world={:#x} cctx={:#x} action={:#x} disc={:#x} gate={:#x} a188={:#x} cty={}\n",
        N.load(Ordering::Relaxed), world, cctx, action, disc, gate, a188, cty);
    for (koff, key, ent) in &ents {
        let k68 = rd_u64(ent + 0x68).unwrap_or(0);        // kind/state disc
        let x = rd_u64(ent + 0x648).unwrap_or(0);
        let y = rd_u64(ent + 0x650).unwrap_or(0);
        s.push_str(&format!("  key@+{:#x}={} → ent={:#x} k68={:#x} pos=({},{})\n",
            koff, key, ent, k68, x as i64 / 32000, y as i64 / 32000));
        let _ = entity_find_id(*ent, &mut s); // id 위치 찾기(있으면 줄 추가)
    }
    m3log(&s);
}
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v3 (resolve재구현+entity id실측) 시작 base={:#x} =====\n", base));
