...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m6_log.txt") {
...
// ★v6: 발화 dispatch FUN_1423a76e0 훅 → caster=sylas일 때 발화되는 effect apply(+대상) 로깅.
//   base/data 균일 발화점이므로 stolen 궁이 실제 발화되면 여기에 대상 궁 apply가 찍힌다.
unsafe fn apply_common(saved: usize, e: usize) {
    let _ = e;
    let regs = [
        *((saved + 0x28) as *const u64) as usize, // rcx = p1
        *((saved + 0x20) as *const u64) as usize, // rdx = p2 (sim)
        *((saved + 0x18) as *const u64) as usize, // r8  = p3 (effect fatptr)
        *((saved + 0x10) as *const u64) as usize, // r9  = p4 (caster)
    ];
    // caster = reg 중 name@+0x250 이 유효 champ인 것. sylas만 관심.
    let mut caster = 0usize; let mut cname = String::new();
    for &r in &regs {
        if let Some(n) = ent_name(r) { caster = r; cname = n; break; }
    }
    if cname != "sylas" { return; }

    // 발화 apply: effect fatptr는 caster 아닌 reg. [reg+8]=vtable, [vtable+0xd0]=apply(in_exe).
    let mut apply = 0u64; let mut fatptr = 0usize;
    for &r in &regs {
        if r == caster || r < 0x10000 { continue; }
        let vt = rd_u64(r + 8).unwrap_or(0);
        if in_exe(vt) {
            let ap = rd_u64(vt as usize + 0xd0).unwrap_or(0);
            if in_exe(ap) { apply = ap; fatptr = r; break; }
        }
    }
    if apply == 0 { return; }

    // 강탈 대상(caster+0x308) champ 이름
    let target = ent_name(caster) // (자기 이름 이미 sylas, 아래는 +0x308 별도)
        .and(None::<String>)
        .or_else(|| {
            let tp = rd_u64(caster + 0x308).unwrap_or(0) as usize;
            let tl = rd_u64(caster + 0x310).unwrap_or(0) as usize;
            read_id(tp, tl)
        })
        .unwrap_or_default();

    // dedup: (apply, target) 조합별 1회
    let key = apply as usize ^ (target.len() << 40) ^ target.bytes().map(|b| b as usize).sum::<usize>();
    let is_new = {
    ...
    // 대상 궁 apply set 표시 (0x20b0460=lightning_mage / 0x1e60ce0=fighter / 0x21dcb20=exorcist
    //   / 0x1e5e1b0=executioner / 0x1fe5cc0=base공용). 이게 찍히면 = 방법2 궁 발화 확인.
    let apply_rva = rva_of(apply);
    let ult_hint = matches!(apply_rva, 0x20b0460|0x1e60ce0|0x21dcb20|0x1e5e1b0|0x1fe5cc0);
    let mut s = format!("[fire#{}] sylas 발화 apply=RVA:{:#x}{} fatptr={:#x} 대상(+0x308)=\"{}\"\n",
        N.load(Ordering::Relaxed), apply_rva, if ult_hint {" ★궁후보"} else {""}, fatptr, target);
    // fatptr 내부 몇 워드 (effect 종류 파악)
    for k in 0..4 { s.push_str(&format!("  f[{}]={:#x}", k, rd_u64(fatptr + k*8).unwrap_or(0))); }
    s.push('\n');
    m3log(&s);
}
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v6 (발화dispatch 훅 sylas 궁발화 확인) 시작 base={:#x} =====\n", base));
