...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m8_log.txt") {
...
// ★v8: 사일러스 caster 발화만 상세 덤프. effect fatptr 이중역참조(vtable=[[reg]+8], apply=[vtable+0xd0])
//   포함 각 reg 다중깊이 apply 추출 → param_3(effect) 위치·구조·궁 apply 확정. option C 배선용.
unsafe fn apply_common(saved: usize, e: usize) {
    let _ = e;
    let regs = [
        *((saved + 0x28) as *const u64) as usize, // rcx p1
        *((saved + 0x20) as *const u64) as usize, // rdx p2
        *((saved + 0x18) as *const u64) as usize, // r8  p3
        *((saved + 0x10) as *const u64) as usize, // r9  p4
    ];
    // 사일러스 caster 찾기 (어느 reg든 name=="sylas")
    let mut sci = 9usize;
    for (i, &r) in regs.iter().enumerate() {
        if ent_name(r).as_deref() == Some("sylas") { sci = i; break; }
    }
    if sci == 9 { return; } // 사일러스 발화만

    if N.load(Ordering::Relaxed) >= 150 { return; }
    let n = N.fetch_add(1, Ordering::Relaxed);

    let mut s = format!("[syl#{}] caster=r{} sylas={:#x}\n", n, sci, regs[sci]);
    for (i, &r) in regs.iter().enumerate() {
        if r < 0x1000 { s.push_str(&format!("  r{}={:#x}\n", i, r)); continue; }
        let w0 = rd_u64(r).unwrap_or(0);       // [r]
        let w1 = rd_u64(r + 8).unwrap_or(0);   // [r+8]
        s.push_str(&format!("  r{}={:#x} [r]={:#x} [r+8]={:#x}", i, r, w0, w1));
        // A) fatptr 이중역참조: fatptr=[r], vtable=[fatptr+8], apply=[vtable+0xd0]
        if w0 >= 0x10000 {
            let fvt = rd_u64(w0 as usize + 8).unwrap_or(0);
            if in_exe(fvt) {
                let ap = rd_u64(fvt as usize + 0xd0).unwrap_or(0);
                if in_exe(ap) { s.push_str(&format!(" ★[[r]+8]vt=RVA:{:#x}→apply=RVA:{:#x}", rva_of(fvt), rva_of(ap))); }
            }
        }
        // B) [r+8]=vtable 직접
        if in_exe(w1) {
            let ap = rd_u64(w1 as usize + 0xd0).unwrap_or(0);
            if in_exe(ap) { s.push_str(&format!(" [r+8]vt→apply=RVA:{:#x}", rva_of(ap))); }
        }
        // C) [r]=vtable 직접
        if in_exe(w0) {
            let ap = rd_u64(w0 as usize + 0xd0).unwrap_or(0);
            if in_exe(ap) { s.push_str(&format!(" [r]vt→apply=RVA:{:#x}", rva_of(ap))); }
        }
        s.push('\n');
    }
    m3log(&s);
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v8 (sylas 발화 effect 상세) 시작 base={:#x} =====\n", base));
