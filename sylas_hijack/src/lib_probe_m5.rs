...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m5_log.txt") {
...
// ★v5: 0x1fe5cc0(base apply) 진입 시 world 전체 entity 순회 → sylas entity 찾아 궁슬롯 apply 덤프.
//   사일러스는 이 apply를 안 통과(native effect 경로)하지만 world엔 존재 → 궁슬롯 vtable→apply(+0xd0) 실측.
unsafe fn apply_common(saved: usize, e: usize) {
    ...
    for i in 0..dense_len as usize {
        ...
        // name@+0x250(char*)/+0x258(len)
        let np = match rd_u64(ent + 0x250) { Some(v) => v as usize, None => continue };
        let nl = rd_u64(ent + 0x258).unwrap_or(0) as usize;
        let name = match read_id(np, nl) { Some(v) => v, None => continue };
        if name != "sylas" { continue; }

        // dedup: sylas ent별 1회 (상한)
        let is_new = {
        ...
        let k68 = rd_u64(ent + 0x68).unwrap_or(0);
        let x = rd_u64(ent + 0x648).unwrap_or(0) as i64 / 32000;
        let y = rd_u64(ent + 0x650).unwrap_or(0) as i64 / 32000;
        let mut s = format!("\n★[SYLAS] ent={:#x} k68={:#x} pos=({},{})\n", ent, k68, x, y);
        // 슬롯 영역(+0x100~0x170) raw + 각 워드의 master id / vtable / apply(+0xd0) 후보
        let mut off = 0x100usize;
        while off < 0x170 {
            let w = rd_u64(ent + off).unwrap_or(0);
            s.push_str(&format!("  +{:#x} = {:#x}", off, w));
            if is_heap(w) {
                let wu = w as usize;
                if let Some(id) = try_id_at(wu) { s.push_str(&format!(" id=\"{}\"", id)); }
                let vt0 = rd_u64(wu).unwrap_or(0);            // w의 첫 워드(vtable?)
                if in_exe(vt0) {
                    let ap = rd_u64(vt0 as usize + 0xd0).unwrap_or(0); // vtable+0xd0 = apply
                    s.push_str(&format!(" vt=RVA:{:#x} apply=RVA:{:#x}", rva_of(vt0), rva_of(ap)));
                }
                let ap2 = rd_u64(wu + 0xd0).unwrap_or(0);     // w+0xd0 직접
                if in_exe(ap2) { s.push_str(&format!(" w+0xd0=RVA:{:#x}", rva_of(ap2))); }
            }
            s.push('\n');
            off += 8;
        }
        let _ = entity_find_id(ent, &mut s); // 광범위 id 스캔(궁이 다른 오프셋일 때)
        m3log(&s);
    }
}
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v5 (world순회 sylas 궁슬롯apply 실측) 시작 base={:#x} =====\n", base));
