...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m7_log.txt") {
...
// ★v7 무필터 진단: FUN_1423a76e0 진입 시 4 인자 각각에서 name(+0x250)과 apply([reg]/[reg+8]→+0xd0)를
//   시도해 caster 위치·발화 apply를 실측. sylas 필터 없이 (name 조합+apply 조합)별 dedup.
unsafe fn apply_common(saved: usize, e: usize) {
    ...
    // 각 reg name(+0x250) 시도 — entity 후보 식별
    let mut names = String::new();
    let mut has_sylas = false;
    for (i, &r) in regs.iter().enumerate() {
        if let Some(n) = ent_name(r) {
            if n == "sylas" { has_sylas = true; }
            names.push_str(&format!("r{}=\"{}\" ", i, n));
        }
    }
    // 각 reg의 effect fatptr 후보: [reg+8]/[reg] = vtable, [vtable+0xd0] = apply
    let mut applies = String::new();
    for (i, &r) in regs.iter().enumerate() {
        if r < 0x10000 { continue; }
        for (lbl, vo) in [("v8", 8usize), ("v0", 0usize)] {
            let vt = rd_u64(r + vo).unwrap_or(0);
            if in_exe(vt) {
                let ap = rd_u64(vt as usize + 0xd0).unwrap_or(0);
                if in_exe(ap) { applies.push_str(&format!("r{}{}→{:#x} ", i, lbl, rva_of(ap))); }
            }
        }
    }
    if names.is_empty() && applies.is_empty() { return; }

    // dedup: (names + applies) 조합별 1회. sylas 포함 발화는 우선(cap 넉넉).
    let combo = format!("{}|{}", names, applies);
    let key: usize = combo.bytes().fold(0usize, |a, b| a.wrapping_mul(131).wrapping_add(b as usize));
    let is_new = {
    ...
    let s = format!("[fire#{}]{} names=[{}] applies=[{}]\n  regs=[{:#x} {:#x} {:#x} {:#x}]\n",
        n, mark, names.trim_end(), applies.trim_end(), regs[0], regs[1], regs[2], regs[3]);
    m3log(&s);
}
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v7 (발화dispatch 무필터 진단) 시작 base={:#x} =====\n", base));
