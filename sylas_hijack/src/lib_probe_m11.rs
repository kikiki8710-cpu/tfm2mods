...
const APPLY_RVA: usize = 0x1e267b0;
const APPLY_SIG: [u8; 12] = [0x41,0x57,0x41,0x56,0x41,0x54,0x56,0x57,0x53,0x48,0x83,0xec];
const APPLY_LEN: usize = 13;
...
fn m3log(s: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true)
        .open("C:\\tfm2mods\\sylas_hijack\\probe_m11_log.txt") {
...
// ★v11: Grab effect apply(0x1e267b0, 사일러스 궁 전용) 훅 = 궁 시전 확실 감지.
//   진입 시 rcx/rdx/r8/r9 + 스택 인자 무필터 덤프 → caster/world/target/casting_ctx 위치 실측.
//   Grab apply ABI(ghidra-re): rcx=effect_def, r8=sim, r9=WorldOps vtable, [rsp+0x28+]=casting_ctx. world는?
//   각 reg/스택워드에서 entity name(sylas 찾기) + sim_const(=base+0x38c5d78) 확인.
unsafe fn apply_common(saved: usize, e: usize) {
    let base = exe_base();
    let regs = [
        *((saved + 0x28) as *const u64) as usize, // rcx
        *((saved + 0x20) as *const u64) as usize, // rdx
        *((saved + 0x18) as *const u64) as usize, // r8
        *((saved + 0x10) as *const u64) as usize, // r9
    ];
    // 스택 인자 (e=orig_rsp): [e+0x28..+0x60]
    let mut stack = [0u64; 8];
    for k in 0..8 { stack[k] = rd_u64(e + 0x28 + k * 8).unwrap_or(0); }

    // 각 reg/스택워드에서 entity name 시도(sylas 찾기). resolve로도 시도.
    let mut names = String::new();
    let mut has_sylas = false;
    let named = |v: usize, lbl: &str, out: &mut String, hs: &mut bool| {
        if let Some(n) = ent_name(v) {
            if n == "sylas" { *hs = true; }
            out.push_str(&format!("{}=\"{}\" ", lbl, n));
        }
    };
    for (i, &r) in regs.iter().enumerate() { named(r, &format!("r{}", i), &mut names, &mut has_sylas); }
    for (k, &sv) in stack.iter().enumerate() { named(sv as usize, &format!("s{}", k), &mut names, &mut has_sylas); }
    // resolve 시도: 각 (world후보, key후보) 조합 — casting_ctx[8] key로
    // stack[0]=casting_ctx 후보 → [casting_ctx+8]=key
    let cctx = stack[0] as usize;
    let ckey = if cctx >= 0x10000 { rd_u64(cctx + 8).unwrap_or(u64::MAX) } else { u64::MAX };
    // world 후보(regs/stack 중 dense_base 있는 것)로 resolve
    let mut resolved = String::new();
    if ckey < 0x100000 {
        for (i, &w) in regs.iter().enumerate() {
            let ent = resolve(w, ckey);
            if ent != 0 { if let Some(n) = ent_name(ent) { resolved.push_str(&format!("r{}→{} ", i, n)); if n=="sylas"{has_sylas=true;} } }
        }
        for (k, &sv) in stack.iter().enumerate() {
            let ent = resolve(sv as usize, ckey);
            if ent != 0 { if let Some(n) = ent_name(ent) { resolved.push_str(&format!("s{}→{} ", k, n)); if n=="sylas"{has_sylas=true;} } }
        }
    }

    // sim_const 위치
    let mut simpos = String::new();
    for (i, &r) in regs.iter().enumerate() { if r == base + 0x38c5d78 { simpos.push_str(&format!("r{} ", i)); } }
    for (k, &sv) in stack.iter().enumerate() { if sv as usize == base + 0x38c5d78 { simpos.push_str(&format!("s{} ", k)); } }

    if N.load(Ordering::Relaxed) >= 80 { return; }
    // dedup: names 조합별
    let dkey: usize = names.bytes().fold(0usize, |a, b| a.wrapping_mul(131).wrapping_add(b as usize));
    let is_new = {
        let mut g = SEEN.lock().unwrap_or_else(|x| x.into_inner());
        if g.is_none() { *g = Some(HashSet::new()); }
        g.as_mut().unwrap().insert(dkey ^ (has_sylas as usize) << 40)
    };
    if !is_new { return; }
    let n = N.fetch_add(1, Ordering::Relaxed);

    let mark = if has_sylas { "★SYLAS" } else { "" };
    let mut s = format!("[grab#{}]{} names=[{}] resolved=[{}] sim_const=[{}] cctx={:#x} ckey={}\n",
        n, mark, names.trim_end(), resolved.trim_end(), simpos.trim_end(), cctx, ckey as i64);
    s.push_str(&format!("  regs=[{:#x} {:#x} {:#x} {:#x}]\n", regs[0], regs[1], regs[2], regs[3]));
    s.push_str("  stack=[");
    for sv in &stack { s.push_str(&format!("{:#x} ", sv)); }
    s.push_str("]\n");
    m3log(&s);
}
...
fn init(_ctx: &GameCtx) -> ModRegistration {
    std::thread::spawn(|| {
        ...
        veh_install();
        let base = exe_base();
        m3log(&format!("\n===== sylas_hijack PROBE v11 (Grab apply 훅 궁시전 인자실측) 시작 base={:#x} =====\n", base));
