...
# 20차 배치 D 도시에 — 담당 `109`~`109` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 20 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `109` | BattlePlan::update_v32 | in:game_ai::plan_legacy::old::battle | 108 | 39 | 20 | 9 | 167 |
...
| # | 이름 | 파일 |
|---|---|---|
| `109` | BattlePlan::update_v32 | `MIG\_verify20\D\spec_109_BattlePlan__update_v32.md` |
...
**[G16 params.role 대조] 6건**
  [109] BattlePlan::update_v32                          sig.params[6] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[6] team_plan `readonly` — IR %5 = ptr noundef nonnull align 8 %5
  [109] BattlePlan::update_v32                          sig.params[6] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[6] team_plan `noalias` — IR %5 = ptr noundef nonnull align 8 %5
```
...
```json
{
  "round": 20, "batch": "D",
  "errors": [
    {"path": "/specs[109]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[109]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 20 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify20/D 만 써라**

```
MIG/_verify20/D/patch.json      ← 기계 적용분(필수)
MIG/_verify20/D/REPORT.md       ← 산문 보고서
MIG/_verify20/D/oracle/         ← 오라클 .rs · 출력 로그
```
