...
# 20차 배치 E 도시에 — 담당 `110`~`110` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 20 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `110` | LegacyPlanHandler::handle_interact_battle | in:game_ai | 77 | 70 | 23 | 13 | 167 |
...
| # | 이름 | 파일 |
|---|---|---|
| `110` | LegacyPlanHandler::handle_interact_battle | `MIG\_verify20\E\spec_110_LegacyPlanHandler__handle_interact_battle.md` |
...
**[G12 src_line 대조] 35건**
  [110] LegacyPlanHandler::handle_interact_battle       consts[23] src_line=447 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [193, 217, 234, 245, 258, 270, 311, 320]

**[G19 knobs.value 값] 5건**
  [110] LegacyPlanHandler::handle_interact_battle       knobs[10] 주장한 값을 IR 관측에서 못 찾는다
  value=14400000000 · 관측(창)=[-24, -16, -8, -1, 0, 1, 2, 3, 4, 5]
  [110] LegacyPlanHandler::handle_interact_battle       knobs[15] 주장한 값을 IR 관측에서 못 찾는다
  value=40000000000 · 관측(창)=[0, 1, 2, 3, 4, 5, 8, 16, 24, 32]
  [110] LegacyPlanHandler::handle_interact_battle       knobs[16] 주장한 값을 IR 관측에서 못 찾는다
  value=22500000000 · 관측(창)=[0, 6, 328, 1632, 1640, 14400000001]
```
...
```json
{
  "round": 20, "batch": "E",
  "errors": [
    {"path": "/specs[110]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[110]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 20 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify20/E 만 써라**

```
MIG/_verify20/E/patch.json      ← 기계 적용분(필수)
MIG/_verify20/E/REPORT.md       ← 산문 보고서
MIG/_verify20/E/oracle/         ← 오라클 .rs · 출력 로그
```
