...
# 19차 배치 C 도시에 — 담당 `91`~`97` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 19 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `91` | PassiveJunglePlan::sub_plan | pub | 36 | 10 | 5 | 6 | 51 |
| `92` | v46_flee_gate_check | in:game_ai | 41 | 17 | 7 | 5 | 65 |
| `93` | check_serpen_giveup | pub | 20 | 10 | 5 | 6 | 35 |
| `94` | check_serpen_hunt | pub | 25 | 15 | 8 | 6 | 48 |
| `95` | check_serpen_setup | pub | 30 | 17 | 9 | 7 | 56 |
| `96` | serpen_passive_plan | pub | 29 | 24 | 6 | 6 | 58 |
| `97` | SerpenHuntAndPokePlan::sub_plan | pub | 42 | 18 | 6 | 6 | 66 |
...
| # | 이름 | 파일 |
|---|---|---|
| `91` | PassiveJunglePlan::sub_plan | `MIG\_verify19\C\spec_91_PassiveJunglePlan__sub_plan.md` |
| `92` | v46_flee_gate_check | `MIG\_verify19\C\spec_92_v46_flee_gate_check.md` |
| `93` | check_serpen_giveup | `MIG\_verify19\C\spec_93_check_serpen_giveup.md` |
| `94` | check_serpen_hunt | `MIG\_verify19\C\spec_94_check_serpen_hunt.md` |
| `95` | check_serpen_setup | `MIG\_verify19\C\spec_95_check_serpen_setup.md` |
| `96` | serpen_passive_plan | `MIG\_verify19\C\spec_96_serpen_passive_plan.md` |
| `97` | SerpenHuntAndPokePlan::sub_plan | `MIG\_verify19\C\spec_97_SerpenHuntAndPokePlan__sub_plan.md` |
...
**[G12 src_line 대조] 21건**
  [97] SerpenHuntAndPokePlan::sub_plan                 consts[9] src_line=103 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [79, 102]
  [97] SerpenHuntAndPokePlan::sub_plan                 consts[10] src_line=103 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [79, 102]

**[G16 params.role 대조] 6건**
  [93] check_serpen_giveup                             sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 6개 ⟹ i 는 [1, 2, 3, 4, 5, 6] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5]. 규약 정본 = 07·15
  [94] check_serpen_hunt                               sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 8개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7, 8] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5, 6, 7]. 규약 정본 = 07·15
  [95] check_serpen_setup                              sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 6개 ⟹ i 는 [1, 2, 3, 4, 5, 6] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5]. 규약 정본 = 07·15

**[G18 logic↔표 상호참조] 1건**
  [95] check_serpen_setup                              logic/P2 logic 이 필드 `strategy.early_serpen_top` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `early_serpen_top` 는 distruct 의 실재 필드명

**[G19 knobs.value 값] 2건**
  [91] PassiveJunglePlan::sub_plan                     knobs[0] 주장한 값을 IR 관측에서 못 찾는다
  value=21 · 관측(창)=[100, 266]
```
...
```json
{
  "round": 19, "batch": "C",
  "errors": [
    {"path": "/specs[91]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[91]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 19 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify19/C 만 써라**

```
MIG/_verify19/C/patch.json      ← 기계 적용분(필수)
MIG/_verify19/C/REPORT.md       ← 산문 보고서
MIG/_verify19/C/oracle/         ← 오라클 .rs · 출력 로그
```
