...
# 19차 배치 B 도시에 — 담당 `84`~`90` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 19 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `84` | battle_check_with_list | pub | 19 | 12 | 4 | 5 | 35 |
| `85` | should_disengage_object_hunt | pub | 12 | 6 | 3 | 3 | 21 |
| `86` | FightSituation::build | pub | 48 | 10 | 7 | 5 | 64 |
| `87` | check_kill | pub | 38 | 14 | 7 | 5 | 59 |
| `88` | v2_apply_assign_commit | in:game_ai::plan_legacy::handler | 26 | 20 | 3 | 4 | 49 |
| `89` | TeamPlan::v24_objective_setup_should_check_camp | pub | 24 | 19 | 6 | 7 | 48 |
| `90` | v3_epicops_defer_serpen | in:game_ai | 20 | 13 | 5 | 7 | 36 |
...
| # | 이름 | 파일 |
|---|---|---|
| `84` | battle_check_with_list | `MIG\_verify19\B\spec_84_battle_check_with_list.md` |
| `85` | should_disengage_object_hunt | `MIG\_verify19\B\spec_85_should_disengage_object_hunt.md` |
| `86` | FightSituation::build | `MIG\_verify19\B\spec_86_FightSituation__build.md` |
| `87` | check_kill | `MIG\_verify19\B\spec_87_check_kill.md` |
| `88` | v2_apply_assign_commit | `MIG\_verify19\B\spec_88_v2_apply_assign_commit.md` |
| `89` | TeamPlan::v24_objective_setup_should_check_camp | `MIG\_verify19\B\spec_89_TeamPlan__v24_objective_setup_should_check_camp.md` |
| `90` | v3_epicops_defer_serpen | `MIG\_verify19\B\spec_90_v3_epicops_defer_serpen.md` |
...
**이 배치 몫 = 14건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G10 class 오분류] 1건**
  [90] v3_epicops_defer_serpen                         class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  필드가 '경과 틱' 저장 방식인지 '절대 틱' 인지는 이 함수만으로 확정 불가(미탐색 = goal_data.rs:387 SerpenStanceData 갱신 지점 IR). 명세는 IR 그대로 적었다

**[G12 src_line 대조] 21건**
  [84] battle_check_with_list                          consts[11] src_line=448 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [449, 482, 493, 502, 512]
  [88] v2_apply_assign_commit                          consts[5] src_line=50 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [554, 2325]
  [88] v2_apply_assign_commit                          consts[6] src_line=46 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [526]
  [88] v2_apply_assign_commit                          consts[7] src_line=46 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [526, 527, 565]
  [88] v2_apply_assign_commit                          consts[16] src_line=25 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [526]
  [89] TeamPlan::v24_objective_setup_should_check_cam  consts[2] src_line=258 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [91, 100, 105, 110, 117, 123, 160, 162]
  [89] TeamPlan::v24_objective_setup_should_check_cam  consts[3] src_line=258 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [91, 95, 110, 123, 152, 160, 165]
  [90] v3_epicops_defer_serpen                         consts[1] src_line=210 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [276, 295]

**[G15 consts.kind 관측] 2건**
  [90] v3_epicops_defer_serpen                         consts[1] kind=임계 NEG
  `meaning` 이 「[usize;2] bounds check(game.rs:210) — 배열 길이, 임계 아님」라 적었는데 kind=임계

**[G16 params.role 대조] 6건**
  [85] should_disengage_object_hunt                    sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 6개(sret보정 -0) > params 5개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[3, 4], 덮은 레지스터 ['%3', '%4', '%5']). 자리 매핑 보류

**[G7 과열림] 9건**
  [86] FightSituation::build                           ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  :209 v54_fs_unseen_picks 는 필터(:205)가 이미 is_recent_visible 을 요구하므로 논리상 발화 불가로 보이나, is_recent_visible 이 상태(tick)에 의존하면 두 호
  [86] FightSituation::build                           ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  exe 0xdfe2a0 대조: exe 는 is_enemy_well_danger(상수 160001/64001 관측)·is_recent_visible(vtable 간접호출 8곳) 을 인라인해 call 수가 IR(직접 c
  [89] TeamPlan::v24_objective_setup_should_check_cam  ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard::is_recent_visible(&blackboard[1-team], game, player, c) 의 내부(어느 last_visible 배열을 보는지)는 _gcbc g07.ll:157005 미
```
...
```json
{
  "round": 19, "batch": "B",
  "errors": [
    {"path": "/specs[84]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[84]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 19 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify19/B 만 써라**

```
MIG/_verify19/B/patch.json      ← 기계 적용분(필수)
MIG/_verify19/B/REPORT.md       ← 산문 보고서
MIG/_verify19/B/oracle/         ← 오라클 .rs · 출력 로그
```
