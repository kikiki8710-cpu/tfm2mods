...
# 19차 배치 A 도시에 — 담당 `77`~`83` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `5e5307f8570bd6bd` | 2026-09-13 20:34:34 |
| `_spec/specs20.json` (손이 닿는 정본) | `c096d3d8d86c9463` | 2026-09-13 20:34:09 |

| `mkdossier.py`(이 파일을 만든 도구) | `81d9294dd98fa641` | 2026-09-13 12:49:42 |

이 파일 생성 = `2026-09-13 20:36:17`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 19 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `77` | DefenseNexusPlan::sub_plan | pub | 24 | 7 | 5 | 6 | 36 |
| `78` | objective_defense_role | pub | 23 | 9 | 3 | 5 | 35 |
| `79` | engage::can_battle_triggered_filtered | pub | 22 | 5 | 3 | 5 | 29 |
| `80` | try_engage | in:game_ai | 24 | 9 | 4 | 5 | 37 |
| `81` | check_epic_hunt | pub | 30 | 17 | 10 | 9 | 56 |
| `82` | epic_passive_plan | pub | 32 | 27 | 9 | 8 | 66 |
| `83` | EpicHuntAndPokePlan::sub_plan | pub | 41 | 23 | 7 | 7 | 69 |
...
| `MIG\TOOLS.md` | 271 | `009b0c1949909b7d` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `77` | DefenseNexusPlan::sub_plan | `MIG\_verify19\A\spec_77_DefenseNexusPlan__sub_plan.md` |
| `78` | objective_defense_role | `MIG\_verify19\A\spec_78_objective_defense_role.md` |
| `79` | engage::can_battle_triggered_filtered | `MIG\_verify19\A\spec_79_engage__can_battle_triggered_filtered.md` |
| `80` | try_engage | `MIG\_verify19\A\spec_80_try_engage.md` |
| `81` | check_epic_hunt | `MIG\_verify19\A\spec_81_check_epic_hunt.md` |
| `82` | epic_passive_plan | `MIG\_verify19\A\spec_82_epic_passive_plan.md` |
| `83` | EpicHuntAndPokePlan::sub_plan | `MIG\_verify19\A\spec_83_EpicHuntAndPokePlan__sub_plan.md` |
...
**[G12 src_line 대조] 21건**
  [78] objective_defense_role                          consts[6] src_line=433 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [384, 389, 394, 395, 399, 407, 414, 415]
  [83] EpicHuntAndPokePlan::sub_plan                   consts[9] src_line=107 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [79, 106]
  [83] EpicHuntAndPokePlan::sub_plan                   consts[10] src_line=107 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [79, 106]
  [83] EpicHuntAndPokePlan::sub_plan                   consts[16] src_line=148 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [79, 149, 151, 155, 157]
  [83] EpicHuntAndPokePlan::sub_plan                   consts[21] src_line=89 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [87]

**[G16 params.role 대조] 6건**
  [80] try_engage                                      sig.params[1] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[1] self `readonly` — IR %1 = ptr noundef nonnull align 8 %1
  [81] check_epic_hunt                                 sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 8개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7, 8] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5, 6, 7]. 규약 정본 = 07·15

**[G19 knobs.value 값] 2건**
  [79] engage::can_battle_triggered_filtered           knobs[0] 주장한 값을 IR 관측에서 못 찾는다
  value=480 · 관측(창)=[120, 123]

**[G20 명세간 공유사실] 32건**
  [82] epic_passive_plan                               [R1] EpicHuntAndPokePlan 0x0 — 같은 (base,offset) 을 2가지 이름으로 부른다: v46_flee_threats / v46_flee_threats·focus_epic_only·vision_only·v46_flee
  [82]`v46_flee_threats·focus_epic_only·vision_only·v46_flee` · [83]`v46_flee_threats`
  [82] epic_passive_plan                               [R1] Option<BigPlan> 0x0 — 같은 (base,offset) 을 2가지 이름으로 부른다: @tag / tag
  [82]`tag` · [96]`@tag`
  [82] epic_passive_plan                               [R1] Strategy 0x0 — 같은 (base,offset) 을 2가지 이름으로 부른다: object_buildup / object_buildup@tag
  [82]`object_buildup@tag` · [83]`object_buildup` · [96]`object_buildup@tag` · [97]`object_buildup`

**[G5 sig 정본 대조] 1건**
  [80] try_engage                                      params[1](self) 의 가변성이 tcx 와 다르다 — 명세 `&mut LegacyPlanHandler` / tcx `&game_ai::plan_legacy::handler::LegacyPlanHandler`

**[G7 과열림] 9건**
  [79] engage::can_battle_triggered_filtered           open 항목의 답이 **shared.전투_위협모델.난수** 에 이미 있다 (7/11 토큰 겹침) — 과열림 의심
  judge_battle_latency 의 필드 +152(stat.judgement)·+720(judgement_mental_ratio) 는 tcxdict AthleteParameter 0x98/0x2d0 로 확인,
  [81] check_epic_hunt                                 ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible 내부(120틱=2초 유예, _gcbc g07.ll:157005~157049: is_visible(team,id) || last_visible[pos]+120 >= tick) — 범위
  [82] epic_passive_plan                               ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible 내부(120틱 = 2초 유예, _gcbc g07.ll:157005)는 담당 범위 밖 — knobs 에 미등재
  [83] EpicHuntAndPokePlan::sub_plan                   ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  exe 0xdefcd0 대조: 상수 0x38b8·0x6d70·64000·892000(891999+1)·896000·960000·40000000001(+0x508)·22500000001(+0xbbe) 전부 exe 본문
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 2374 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 1188 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 621 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 1513 | ⚠**348행만 IR 앵커**(`ev3`). 나머지 1165행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 19, "batch": "A",
  "errors": [
    {"path": "/specs[77]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[77]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 19 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify19/A 만 써라**

```
MIG/_verify19/A/patch.json      ← 기계 적용분(필수)
MIG/_verify19/A/REPORT.md       ← 산문 보고서
MIG/_verify19/A/oracle/         ← 오라클 .rs · 출력 로그
```
