...
# 19차 배치 D 도시에 — 담당 `98`~`101` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 19 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `98` | steal::evaluate_steal_for_target | in:game_ai::plan_legacy::steal | 30 | 28 | 10 | 8 | 68 |
| `99` | steal::should_steal_now | pub | 30 | 20 | 6 | 5 | 56 |
| `100` | TeamPlan::update_steal | pub | 66 | 23 | 4 | 6 | 91 |
| `101` | line_backfight_support_focus | pub | 26 | 10 | 4 | 5 | 40 |
...
| # | 이름 | 파일 |
|---|---|---|
| `98` | steal::evaluate_steal_for_target | `MIG\_verify19\D\spec_98_steal__evaluate_steal_for_target.md` |
| `99` | steal::should_steal_now | `MIG\_verify19\D\spec_99_steal__should_steal_now.md` |
| `100` | TeamPlan::update_steal | `MIG\_verify19\D\spec_100_TeamPlan__update_steal.md` |
| `101` | line_backfight_support_focus | `MIG\_verify19\D\spec_101_line_backfight_support_focus.md` |
...
**[G20 명세간 공유사실] 32건**
  [99] steal::should_steal_now                         [R1] TeamPlan 0x1ba — 같은 (base,offset) 을 2가지 이름으로 부른다: current_steal_session@Some.0.enemy_jungler_alive_at_start / current_steal_session@tag
  [99]`current_steal_session@tag` · [100]`current_steal_session@Some.0.enemy_jungler_alive_at_start` · [100]`current_steal_session@tag`
```
...
```json
{
  "round": 19, "batch": "D",
  "errors": [
    {"path": "/specs[98]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[98]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 19 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify19/D 만 써라**

```
MIG/_verify19/D/patch.json      ← 기계 적용분(필수)
MIG/_verify19/D/REPORT.md       ← 산문 보고서
MIG/_verify19/D/oracle/         ← 오라클 .rs · 출력 로그
```
