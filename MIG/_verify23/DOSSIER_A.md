...
# 23차 배치 A 도시에 — 담당 `134`~`141` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `134` | around::check_cell | in:game_ai::small_action::around | 6 | 7 | 3 | 5 | 16 |
| `135` | v3_lethal_tower_position | pub | 13 | 6 | 2 | 4 | 20 |
| `136` | v23_should_break_objective_hunt_anchor | pub | 4 | 7 | 6 | 2 | 17 |
| `137` | v48_on_cast_line | in:game_ai::plan_legacy::sub_plan::battle | 12 | 4 | 1 | 2 | 14 |
| `138` | EpicPokeSubPlan::score | pub | 27 | 8 | 3 | 4 | 32 |
| `139` | SerpenPokeSubPlan::score | pub | 27 | 9 | 3 | 4 | 37 |
| `140` | SmallActionAroundBush::new_with_target | pub | 15 | 5 | 2 | 3 | 21 |
| `141` | SmallActionPlay::get_input | pub | 28 | 14 | 3 | 5 | 45 |
...
| # | 이름 | 파일 |
|---|---|---|
| `134` | around::check_cell | `MIG\_verify23\A\spec_134_around__check_cell.md` |
| `135` | v3_lethal_tower_position | `MIG\_verify23\A\spec_135_v3_lethal_tower_position.md` |
| `136` | v23_should_break_objective_hunt_anchor | `MIG\_verify23\A\spec_136_v23_should_break_objective_hunt_anchor.md` |
| `137` | v48_on_cast_line | `MIG\_verify23\A\spec_137_v48_on_cast_line.md` |
| `138` | EpicPokeSubPlan::score | `MIG\_verify23\A\spec_138_EpicPokeSubPlan__score.md` |
| `139` | SerpenPokeSubPlan::score | `MIG\_verify23\A\spec_139_SerpenPokeSubPlan__score.md` |
| `140` | SmallActionAroundBush::new_with_target | `MIG\_verify23\A\spec_140_SmallActionAroundBush__new_with_target.md` |
| `141` | SmallActionPlay::get_input | `MIG\_verify23\A\spec_141_SmallActionPlay__get_input.md` |
...
**[G12 src_line 대조] 16건**
  [140] SmallActionAroundBush::new_with_target          consts[4] src_line=1180 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1181]

**[G16 params.role 대조] 9건**
  [136] v23_should_break_objective_hunt_anchor          sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 6개(sret보정 -0) > params 5개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[4], 덮은 레지스터 ['%4', '%5']). 자리 매핑 보류

**[G20 명세간 공유사실] 120건**
  [138] EpicPokeSubPlan::score                          [R1] SmallActionPlay 0x10 — 같은 (base,offset) 을 4가지 이름으로 부른다: AroundPositionBush y / AroundRegion x / goal_y · Around/AroundHide/LaneMinionPosition.goal_x / variant goal_y · Around/AroundHide/LaneMinionPosition.goal_x
  [138]`AroundRegion x/AroundPositionBush y` · [139]`AroundRegion x/AroundPositionBush y` · [141]`goal_y · Around/AroundHide/LaneMinionPosition.goal_x`
  [139] SerpenPokeSubPlan::score                        [R4] 보 너 스   적 용   거 리   게 이 트 — 같은 노브가 값 22500000001 / 39999999999 를 동시에 갖는다
  [139]`22500000001` · [143]`39999999999`
  [141] SmallActionPlay::get_input                      [R1] Input 0x0 — 같은 (base,offset) 을 2가지 이름으로 부른다: @tag / tag
  [141]`@tag` · [179]`tag`
```
...
```json
{
  "round": 23, "batch": "A",
  "errors": [
    {"path": "/specs[134]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[134]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify23/A 만 써라**

```
MIG/_verify23/A/patch.json      ← 기계 적용분(필수)
MIG/_verify23/A/REPORT.md       ← 산문 보고서
MIG/_verify23/A/oracle/         ← 오라클 .rs · 출력 로그
```
