...
# 23차 배치 B 도시에 — 담당 `142`~`149` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `142` | path_needs_tower_escape | in:game_ai::small_action | 22 | 9 | 3 | 4 | 34 |
| `143` | LineDefenseSubPlan::score | pub | 29 | 12 | 5 | 4 | 45 |
| `144` | SmallActionLaneMinionPosition::has_current_explicit_minion_action | in:game_ai | 18 | 4 | 0 | 3 | 22 |
| `145` | SmallActionPlay::evaluation_position | pub | 25 | 5 | 1 | 4 | 31 |
| `146` | SmallActionAroundBush::new_with_out_line | pub | 15 | 5 | 2 | 4 | 21 |
| `147` | line_action_economy_adjustment | pub | 23 | 8 | 4 | 4 | 34 |
| `148` | serpen_action_score | pub | 31 | 14 | 4 | 4 | 47 |
| `149` | epic_action_score | pub | 31 | 14 | 4 | 4 | 47 |
...
| # | 이름 | 파일 |
|---|---|---|
| `142` | path_needs_tower_escape | `MIG\_verify23\B\spec_142_path_needs_tower_escape.md` |
| `143` | LineDefenseSubPlan::score | `MIG\_verify23\B\spec_143_LineDefenseSubPlan__score.md` |
| `144` | SmallActionLaneMinionPosition::has_current_explicit_minion_action | `MIG\_verify23\B\spec_144_SmallActionLaneMinionPosition__has_current_explicit_minion_action.md` |
| `145` | SmallActionPlay::evaluation_position | `MIG\_verify23\B\spec_145_SmallActionPlay__evaluation_position.md` |
| `146` | SmallActionAroundBush::new_with_out_line | `MIG\_verify23\B\spec_146_SmallActionAroundBush__new_with_out_line.md` |
| `147` | line_action_economy_adjustment | `MIG\_verify23\B\spec_147_line_action_economy_adjustment.md` |
| `148` | serpen_action_score | `MIG\_verify23\B\spec_148_serpen_action_score.md` |
| `149` | epic_action_score | `MIG\_verify23\B\spec_149_epic_action_score.md` |
...
**이 배치 몫 = 4건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G19 knobs.value 값] 6건**
  [143] LineDefenseSubPlan::score                       knobs[1] 주장한 값을 IR 관측에서 못 찾는다
  value=100 · 관측(창)=[13]
  [143] LineDefenseSubPlan::score                       knobs[2] 주장한 값을 IR 관측에서 못 찾는다
  value=5 · 관측(창)=[1168]

**[G20 명세간 공유사실] 120건**
  [143] LineDefenseSubPlan::score                       [R1] ActionContext 0x10 — 같은 (base,offset) 을 2가지 이름으로 부른다: anchor@tag / anchor@tag = 1
  [143]`anchor@tag = 1` · [165]`anchor@tag`
  [143] LineDefenseSubPlan::score                       [R1] ActionContext 0x11 — 같은 (base,offset) 을 2가지 이름으로 부른다: anchor@Lane.line / anchor@Lane.line = self.line
  [143]`anchor@Lane.line = self.line` · [165]`anchor@Lane.line`
```
...
```json
{
  "round": 23, "batch": "B",
  "errors": [
    {"path": "/specs[142]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[142]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify23/B 만 써라**

```
MIG/_verify23/B/patch.json      ← 기계 적용분(필수)
MIG/_verify23/B/REPORT.md       ← 산문 보고서
MIG/_verify23/B/oracle/         ← 오라클 .rs · 출력 로그
```
