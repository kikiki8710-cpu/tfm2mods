...
# 23차 배치 E 도시에 — 담당 `165`~`171` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `112cf801b9634e1b` | 2026-09-14 12:20:36 |
| `_spec/specs20.json` (손이 닿는 정본) | `c2be3647820cf756` | 2026-09-14 12:17:27 |

| `mkdossier.py`(이 파일을 만든 도구) | `802add3b146ab88e` | 2026-09-14 02:13:55 |

이 파일 생성 = `2026-09-14 12:27:31`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `165` | action_eval::evaluate_action | pub | 48 | 21 | 10 | 6 | 79 |
| `166` | utils::can1v1win | pub | 13 | 6 | 2 | 5 | 21 |
| `167` | SmallActionLaneMinionPosition::get_input | in:game_ai | 29 | 10 | 3 | 10 | 40 |
| `168` | SmallActionAroundBush::get_input | in:game_ai | 26 | 10 | 3 | 5 | 39 |
| `169` | calculate_serpen_action_score | in:game_ai::plan_legacy::sub_plan::serpen_hunt | 15 | 13 | 7 | 3 | 35 |
| `170` | calculate_epic_action_score | in:game_ai::plan_legacy::sub_plan::epic_hunt | 14 | 13 | 7 | 3 | 34 |
| `171` | line_projected_punish_damage_at | in:game_ai | 34 | 8 | 3 | 7 | 45 |
...
| `MIG\TOOLS.md` | 273 | `a9b71f84bbbca24a` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `165` | action_eval::evaluate_action | `MIG\_verify23\E\spec_165_action_eval__evaluate_action.md` |
| `166` | utils::can1v1win | `MIG\_verify23\E\spec_166_utils__can1v1win.md` |
| `167` | SmallActionLaneMinionPosition::get_input | `MIG\_verify23\E\spec_167_SmallActionLaneMinionPosition__get_input.md` |
| `168` | SmallActionAroundBush::get_input | `MIG\_verify23\E\spec_168_SmallActionAroundBush__get_input.md` |
| `169` | calculate_serpen_action_score | `MIG\_verify23\E\spec_169_calculate_serpen_action_score.md` |
| `170` | calculate_epic_action_score | `MIG\_verify23\E\spec_170_calculate_epic_action_score.md` |
| `171` | line_projected_punish_damage_at | `MIG\_verify23\E\spec_171_line_projected_punish_damage_at.md` |
...
**이 배치 몫 = 16건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 16건**
  [165] action_eval::evaluate_action                    consts[5] src_line=112 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [78, 81, 110]
  [169] calculate_serpen_action_score                   consts[2] src_line=933 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [939, 945]
  [170] calculate_epic_action_score                     consts[2] src_line=939 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [945, 951]
  [171] line_projected_punish_damage_at                 consts[4] src_line=132 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [116, 119, 141]
  [171] line_projected_punish_damage_at                 consts[6] src_line=131 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [134]
  [171] line_projected_punish_damage_at                 consts[7] src_line=113 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [119, 141]

**[G16 params.role 대조] 9건**
  [167] SmallActionLaneMinionPosition::get_input        sig.params[0] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[0] (sret) `sret` — IR %0 = ptr dead_on_unwind noalias noundef nonnull writable align 8 captures(none) dereferenceable(32) %0
  [169] calculate_serpen_action_score                   sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 4개(sret보정 -0) vs params 7개 ⟹ 소거 3개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류
  [169] calculate_serpen_action_score                   sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 7개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7] 여야 한다(sret=0, 소스 1..n). 현재 [2, 3, 6, 7, 1, 4, 5]. 규약 정본 = 07·15
  [170] calculate_epic_action_score                     sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 4개(sret보정 -0) vs params 7개 ⟹ 소거 3개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류
  [170] calculate_epic_action_score                     sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 7개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7] 여야 한다(sret=0, 소스 1..n). 현재 [2, 3, 6, 7, 1, 4, 5]. 규약 정본 = 07·15

**[G5 sig 정본 대조] 4건**
  [169] calculate_serpen_action_score                   params[2](player) 의 가변성이 tcx 와 다르다 — 명세 `&PlayerState(2528B)` / tcx `&mut rand::rngs::std::StdRng`
  [169] calculate_serpen_action_score                   params[1](rnd) 의 가변성이 tcx 와 다르다 — 명세 `&mut StdRng` / tcx `&std::boxed::Box<dyn [Binder { value: Trait(game_core::Actio`
  [170] calculate_epic_action_score                     params[2](player) 의 가변성이 tcx 와 다르다 — 명세 `&PlayerState(2528B)` / tcx `&mut rand::rngs::std::StdRng`
  [170] calculate_epic_action_score                     params[1](rnd) 의 가변성이 tcx 와 다르다 — 명세 `&mut StdRng` / tcx `&std::boxed::Box<dyn [Binder { value: Trait(game_core::Actio`

**[G7 과열림] 7건**
  [171] line_projected_punish_damage_at                 ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard::is_recent_visible(_gcbc g07.ll:157005) 계약: (&Blackboard 744B, game data ptr, vtable ptr(816B), &PlayerState,
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 5061 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 2334 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 1113 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 3176 | ⚠**1066행만 IR 앵커**(`ev3`). 나머지 2110행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 23, "batch": "E",
  "errors": [
    {"path": "/specs[165]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[165]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify23/E 만 써라**

```
MIG/_verify23/E/patch.json      ← 기계 적용분(필수)
MIG/_verify23/E/REPORT.md       ← 산문 보고서
MIG/_verify23/E/oracle/         ← 오라클 .rs · 출력 로그
```
