...
# 23차 배치 D 도시에 — 담당 `158`~`164` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `158` | line_minion_action_candidates | pub | 32 | 10 | 3 | 6 | 44 |
| `159` | battle_ally_action | pub | 37 | 16 | 4 | 7 | 55 |
| `160` | get_die_tick_player | pub | 30 | 13 | 5 | 8 | 48 |
| `161` | lane_minion_position_action | pub | 23 | 9 | 2 | 6 | 32 |
| `162` | v57_summon_command_score | in:game_ai | 19 | 20 | 7 | 5 | 44 |
| `163` | abstract_input::skill | pub | 24 | 9 | 2 | 5 | 35 |
| `164` | abstract_input::skill2 | pub | 24 | 9 | 3 | 6 | 36 |
...
| # | 이름 | 파일 |
|---|---|---|
| `158` | line_minion_action_candidates | `MIG\_verify23\D\spec_158_line_minion_action_candidates.md` |
| `159` | battle_ally_action | `MIG\_verify23\D\spec_159_battle_ally_action.md` |
| `160` | get_die_tick_player | `MIG\_verify23\D\spec_160_get_die_tick_player.md` |
| `161` | lane_minion_position_action | `MIG\_verify23\D\spec_161_lane_minion_position_action.md` |
| `162` | v57_summon_command_score | `MIG\_verify23\D\spec_162_v57_summon_command_score.md` |
| `163` | abstract_input::skill | `MIG\_verify23\D\spec_163_abstract_input__skill.md` |
| `164` | abstract_input::skill2 | `MIG\_verify23\D\spec_164_abstract_input__skill2.md` |
...
**[G20 명세간 공유사실] 120건**
  [158] line_minion_action_candidates                   [R2] Entity skill2_effect.growth_range — 같은 이름이 오프셋 0x18 / 0x518 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [158]`0x518` · [159]`0x18`
  [158] line_minion_action_candidates                   [R2] Entity skill2_effect.range — 같은 이름이 오프셋 0x10 / 0x510 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [158]`0x510` · [159]`0x10`
  [158] line_minion_action_candidates                   [R2] Entity skill2_effect.target — 같은 이름이 오프셋 0x28 / 0x528 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [158]`0x528` · [159]`0x28`

**[G7 과열림] 7건**
  [162] v57_summon_command_score                        ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Effect::expected_damage_target / Entity::attack_cooltime / Blackboard::is_recent_visible 내부는 game_core 경계 — 시그니처만 기록(tcx
```
...
```json
{
  "round": 23, "batch": "D",
  "errors": [
    {"path": "/specs[158]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[158]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify23/D 만 써라**

```
MIG/_verify23/D/patch.json      ← 기계 적용분(필수)
MIG/_verify23/D/REPORT.md       ← 산문 보고서
MIG/_verify23/D/oracle/         ← 오라클 .rs · 출력 로그
```
