...
# 23차 배치 C 도시에 — 담당 `150`~`157` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `150` | v22_lane_tower_pressure_attack_allowed | pub | 21 | 13 | 6 | 8 | 40 |
| `151` | SmallActionSkill::get_input | in:game_ai | 15 | 5 | 0 | 5 | 20 |
| `152` | line_champion_trade_allowance | in:game_ai | 7 | 7 | 5 | 4 | 19 |
| `153` | SmallActionSkill2::get_input | in:game_ai | 16 | 5 | 1 | 6 | 22 |
| `154` | SmallActionUlt::get_input | in:game_ai | 16 | 6 | 1 | 6 | 23 |
| `155` | LineGankerPlan::make_gank_battle | in:game_ai::plan_legacy::old::line_gank::ganker | 15 | 7 | 3 | 6 | 24 |
| `156` | v27_objective_discipline_action | pub | 16 | 7 | 3 | 5 | 26 |
| `157` | SmallActionAroundPositionBush::get_input | in:game_ai | 14 | 5 | 2 | 4 | 21 |
...
| # | 이름 | 파일 |
|---|---|---|
| `150` | v22_lane_tower_pressure_attack_allowed | `MIG\_verify23\C\spec_150_v22_lane_tower_pressure_attack_allowed.md` |
| `151` | SmallActionSkill::get_input | `MIG\_verify23\C\spec_151_SmallActionSkill__get_input.md` |
| `152` | line_champion_trade_allowance | `MIG\_verify23\C\spec_152_line_champion_trade_allowance.md` |
| `153` | SmallActionSkill2::get_input | `MIG\_verify23\C\spec_153_SmallActionSkill2__get_input.md` |
| `154` | SmallActionUlt::get_input | `MIG\_verify23\C\spec_154_SmallActionUlt__get_input.md` |
| `155` | LineGankerPlan::make_gank_battle | `MIG\_verify23\C\spec_155_LineGankerPlan__make_gank_battle.md` |
| `156` | v27_objective_discipline_action | `MIG\_verify23\C\spec_156_v27_objective_discipline_action.md` |
| `157` | SmallActionAroundPositionBush::get_input | `MIG\_verify23\C\spec_157_SmallActionAroundPositionBush__get_input.md` |
...
**[G15 consts.kind 관측] 4건**
  [151] SmallActionSkill::get_input                     consts[1] kind=임계 NEG
  `meaning` 이 「team 인덱스 bounds(len 2) — 임계 아님」라 적었는데 kind=임계
  [153] SmallActionSkill2::get_input                    consts[1] kind=임계 NEG
  `meaning` 이 「team 인덱스 bounds(len 2) — 임계 아님. 같은 2 가 L204 `」라 적었는데 kind=임계
  [154] SmallActionUlt::get_input                       consts[1] kind=임계 NEG
  `meaning` 이 「team 인덱스 bounds(len 2) — 임계 아님」라 적었는데 kind=임계

**[G16 params.role 대조] 9건**
  [155] LineGankerPlan::make_gank_battle                sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 10개(sret보정 -0) vs params 11개 ⟹ 소거 1개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류

**[G7 과열림] 7건**
  [150] v22_lane_tower_pressure_attack_allowed          ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard::is_recent_visible(&Blackboard(744B), &dyn AbstractGame, &PlayerState, &Entity)->bool 내부(_gcbc/g07.ll:157005)
  [156] v27_objective_discipline_action                 ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_ignored_well_enemy(usize,&PlayerState,&Entity)->bool (fight_model.rs:754) · Blackboard::is_recent_visible · new_with_
```
...
```json
{
  "round": 23, "batch": "C",
  "errors": [
    {"path": "/specs[150]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[150]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify23/C 만 써라**

```
MIG/_verify23/C/patch.json      ← 기계 적용분(필수)
MIG/_verify23/C/REPORT.md       ← 산문 보고서
MIG/_verify23/C/oracle/         ← 오라클 .rs · 출력 로그
```
