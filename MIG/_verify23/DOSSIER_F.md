...
# 23차 배치 F 도시에 — 담당 `172`~`179` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 23 F      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `172` | SmallActionPositioning::get_input | in:game_ai | 27 | 16 | 5 | 6 | 48 |
| `173` | SmallActionAroundHide::get_input | in:game_ai | 33 | 21 | 6 | 6 | 59 |
| `174` | EpicPokeSubPlan::action_candidates_old | pub | 57 | 17 | 6 | 6 | 76 |
| `175` | SerpenPokeSubPlan::action_candidates_old | pub | 55 | 16 | 7 | 5 | 78 |
| `176` | SmallActionTrace::get_input | in:game_ai | 52 | 19 | 6 | 8 | 77 |
| `177` | SmallActionAroundRegion::get_input | in:game_ai | 38 | 18 | 7 | 7 | 62 |
| `178` | SmallActionAroundPosition::get_input | pub | 42 | 19 | 8 | 8 | 68 |
| `179` | SmallActionAround::get_input | in:game_ai | 51 | 23 | 9 | 7 | 82 |
...
| # | 이름 | 파일 |
|---|---|---|
| `172` | SmallActionPositioning::get_input | `MIG\_verify23\F\spec_172_SmallActionPositioning__get_input.md` |
| `173` | SmallActionAroundHide::get_input | `MIG\_verify23\F\spec_173_SmallActionAroundHide__get_input.md` |
| `174` | EpicPokeSubPlan::action_candidates_old | `MIG\_verify23\F\spec_174_EpicPokeSubPlan__action_candidates_old.md` |
| `175` | SerpenPokeSubPlan::action_candidates_old | `MIG\_verify23\F\spec_175_SerpenPokeSubPlan__action_candidates_old.md` |
| `176` | SmallActionTrace::get_input | `MIG\_verify23\F\spec_176_SmallActionTrace__get_input.md` |
| `177` | SmallActionAroundRegion::get_input | `MIG\_verify23\F\spec_177_SmallActionAroundRegion__get_input.md` |
| `178` | SmallActionAroundPosition::get_input | `MIG\_verify23\F\spec_178_SmallActionAroundPosition__get_input.md` |
| `179` | SmallActionAround::get_input | `MIG\_verify23\F\spec_179_SmallActionAround__get_input.md` |
...
**이 배치 몫 = 7건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 16건**
  [173] SmallActionAroundHide::get_input                consts[7] src_line=347 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [325, 327, 333, 334, 353, 362, 364, 368]
  [174] EpicPokeSubPlan::action_candidates_old          consts[3] src_line=437 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [450, 460, 462, 465, 472, 474, 480, 483]
  [175] SerpenPokeSubPlan::action_candidates_old        consts[3] src_line=429 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [442, 453, 460, 462, 467, 470, 477, 481]
  [175] SerpenPokeSubPlan::action_candidates_old        consts[4] src_line=431 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [451]

**[G19 knobs.value 값] 6건**
  [172] SmallActionPositioning::get_input               knobs[0] 주장한 값을 IR 관측에서 못 찾는다
  value=5000 / 96000 · 관측(창)=[-9216000000, -9190999999, -1, 8, 16, 1632, 1640]

**[G20 명세간 공유사실] 120건**
  [172] SmallActionPositioning::get_input               [R1] bumpalo Vec 0x0 — 같은 (base,offset) 을 2가지 이름으로 부른다: candidates / candidates ptr/+0x8 bump/+0x10 cap/+0x18 len
  [172]`candidates ptr/+0x8 bump/+0x10 cap/+0x18 len` · [173]`candidates`
  [177] SmallActionAroundRegion::get_input              [R1] 후보 튜플 0x10 — 같은 (base,offset) 을 2가지 이름으로 부른다: .4 = key = score*10000 + d/1000 / .4 = score
  [177]`.4 = score` · [179]`.4 = key = score*10000 + d/1000`
```
...
```json
{
  "round": 23, "batch": "F",
  "errors": [
    {"path": "/specs[172]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[172]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 23 --only F --dry
```
...
## §6 출력 디렉터리 — **_verify23/F 만 써라**

```
MIG/_verify23/F/patch.json      ← 기계 적용분(필수)
MIG/_verify23/F/REPORT.md       ← 산문 보고서
MIG/_verify23/F/oracle/         ← 오라클 .rs · 출력 로그
```
