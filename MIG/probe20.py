...
#   ⟹ 명세 밖 보조 항목은 idx **250~** 를 쓴다(idx 는 u8 · 09-13 밤: 명세가 102 개가 되어 옛 90 이 i=90 과 충돌 — 두 번째 이동).
AUX = {
    250: ("caf9f0", u"BigPlan::sub_plan", u"plan_legacy/types",
...
NO_ENTRY = {
    2: u"exe 에 독립 함수가 없다 — `BigPlan::sub_plan`(0xcaf9f0) 안 **점프테이블 idx14 arm**"
       ...
       u"★호스트는 `AUX[250]`(~~20~~→~~90~~ · 명세 i 와 두 번 충돌해 이동) 으로 따로 계측한다 — ~~「#02 = 2,048만 회」~~ 는 **디스패처 호출수**였다. "
