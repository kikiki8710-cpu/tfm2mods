...
# 10차 배치 B 도시에 — 담당 `05`~`09` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `9a1edfa8fc364b08` | 2026-09-11 21:02:21 |
| `_spec/specs20.json` (손이 닿는 정본) | `cfad6faecd9d1003` | 2026-09-11 21:02:15 |

| `mkdossier.py`(이 파일을 만든 도구) | `af0a7efd074c9da7` | 2026-09-11 21:16:15 |

이 파일 생성 = `2026-09-11 21:16:34`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 10 B      # ← 이것만 돌리면 된다
```
...
게이트 19개는 **각자 한 축만** 본다. 아래는 **어떤 게이트도 보지 않는 칸**이다:
...
| `MIG\SPEC_RUNBOOK.md` | 739 | `57b6f77822950dd6` | ★라운드 절차. **§S5-b 게이트표 · §S5-c 「검사받지 않는 축」 · §S5-d 파이프라인 4계약**. |
| `MIG\SPEC_GUIDE.md` | 364 | `0b45fe38ad5ebf34` | 명세 필드의 뜻과 채우는 법. |
| `MIG\TOOLS.md` | 219 | `9b8f41cb1a6045fe` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `05` | v50_fold_dive_episode | `MIG\_verify10\B\spec_05_v50_fold_dive_episode.md` |
| `06` | v2_response_retreat_stance | `MIG\_verify10\B\spec_06_v2_response_retreat_stance.md` |
| `07` | sub_plan | `MIG\_verify10\B\spec_07_sub_plan.md` |
| `08` | is_end | `MIG\_verify10\B\spec_08_is_end.md` |
| `09` | check_favorable_engage_formation | `MIG\_verify10\B\spec_09_check_favorable_engage_formation.md` |
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 452 | ⚠**무검사** — IR 의 load/store 와 대조된 적이 없다 |
| `consts.kind`(상수 종류) | 186 | ⚠**무검사** |
| `sig.params.role`(인자 역할) | 126 | ⚠**무검사** |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 10 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify10/B 만 써라**

```
MIG/_verify10/B/patch.json      ← 기계 적용분(필수)
MIG/_verify10/B/REPORT.md       ← 산문 보고서
MIG/_verify10/B/oracle/         ← 오라클 .rs · 출력 로그
```
