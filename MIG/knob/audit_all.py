...
print("파싱 성공 %d 사이트 (그중 prefix 후보 여럿 = 이전 감사에서 빠져 있던 것 %d)"
      % (len(rows), sum(1 for r in rows if r['multi'])))
print("여전히 판정 불가 %d건\n" % len(unparsed))

# ★같은 주소를 여러 (prefix, off) 변형으로 시도하는 형태가 있다:
#   `patch_imm_bytes(a, &[..], 3, 4, v) || patch_imm_bytes(a, &[..], 2, 4, v)`
#   두 호출을 따로 세면 안 맞는 쪽이 **죽음으로 오탐**된다(2026-09-04 실측 5건).
#   ⟹ (파일, 주소)로 묶어 **변형 중 하나라도 맞으면 살아있음**으로 본다.
variants = defaultdict(list)
for r in rows:
    variants[(r['fn'], r['rva'])].append(r)

dead, aliv = [], 0
for key, grp in variants.items():
    ok = False
    for r in grp:
...
print("\n=== 죽은 사이트 전체 (소스행별) ===")
for (f,l),n in Counter((r['fn'],r['line']) for r in dead).most_common(40):
    ex=next(r for r in dead if r['fn']==f and r['line']==l)
    print("   %-14s:%-5d %2d사이트  후보%d종  val=%s" % (f,l,n,len(ex['pres']),ex['val']))
newly = [r for r in dead if r['multi']]
print("★죽은 사이트 %d건 (그중 **이전 감사가 아예 안 보던 것** %d건)" % (len(dead), len(newly)))
c = Counter((r['fn'], r['line']) for r in newly)
for (f, l), n in c.most_common(20):
    ex = next(r for r in newly if r['fn'] == f and r['line'] == l)
    print("   %-16s:%-5d %2d사이트  후보 %d종  val=%s"
          % (f, l, n, len(ex['pres']), ex['val']))
print("\n살아있음 %d" % aliv)

print("\n--- 판정 불가 사유 ---")
for k, n in Counter(w.split(':')[0] for _, _, w in unparsed).most_common(8):
    print("   %-34s %d" % (k, n))
for f, l, w in unparsed[:10]:
    print("     %-16s:%-5d %s" % (f, l, w))
