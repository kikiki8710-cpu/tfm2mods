...
| ★**exe 진입 ABI 확정**(재현체 `my_*` 착수 전 · ⚠「동치」(`mig060_same` 명령열 동일)는 **본문** 기준이라 인자 승격은 따로 — 09-20 target_bush_v41 (line,team,position,cache,ctx)→(cl line, rdx &PlayerState, r8 &OperationData) · count_nearby_enemies %2 &PlayerState→(x,y,team i64,cache,[rsp+0x28] bb)) | **`dis060.py <rva> [old]`**(capstone · pdata 경계 · 0.6.0/0.5.8 exe · Ghidra 불요 · 진입부 레지스터↔인자 실독) ⑧ | `argscan.py` ⑧ | ghidra-re · 정본 = `REPORT\tfm2_judge_verify\03_시행착오.md §61` 교훈 23 · 런북 `05 §3-A` |
...
| ★**패치 후 함수 정체 이음(전면 재컴파일 · 「분석한 함수가 새 exe 에서 어디·얼마나 바뀌었나」 · 바이트/skel 해시는 25%만 대응)** | **`mig060.py [--only 264\|map]`**(panic Location 집합 지문 6단 → 등급 동일/이동/오프셋/소폭/변경/미발견 · 신뢰 A/B/C · `_next\mig060_judge.{md,json}` · ⚠OLD/NEW exe 경로 상수 하드코딩 · 진단 `mig060_diag.py` · 0.5.8→0.6.0 769 표본) ⑧ | `fnprobe`/`argscan` 개별 대조 · `repin`/`sitealign`(RVA 재핀) | Ghidra 콜러 추적 ⑧(Location 0 소형 래퍼) — 「무엇이 달라졌나」는 IR 없이는 디컴 대조뿐 |
| ★**패치 후 「동일」 확정**(mig060 등급 동일/이동/오프셋을 믿기 전 · ⚠skel 해시(숫자→`I`)는 즉치·변위·rip-rel·콜리가 달라도 같은 해시 = 동일≠동일) | **`mig060_same.py`**(명령열 정렬 + 피연산자 실비교 + 콜리 짝 + rip-rel 데이터 4축 → ✅확정/enum 태그 재번호/실변경 · `_next\mig060_same.{md,json}` · 0.6.0 147 표본 = ✅119·enum11·실변경17) ⑧ + **`mig060_fix.py`**(skel·크기 동일 다중 후보의 짝 정정 = 호출부 실측 · 「크기 최근접」은 틀렸다) | `fnprobe`/`argscan` 개별 대조 | 「오프셋만」도 필드 의미 보장 아님 → 구조체 재검증 별도 · ICF/디스패처 블록 오정렬은 보류(judge 03 §49 함정 6) |
| ★**패치 후 enum 변화**(「어느 enum 이 변형을 잃었/얻었나 · 태그 재번호」 · SDK 없이 exe 만으로 · ⚠점프테이블 정렬은 「몇 개」만 말한다 — 09-16 「case #2 제거·13→12」 오독) | **`enumdiff.py`**(두 exe `.rdata` 의 `#[derive(Debug)]` 변형 이름 연접 문자열 → CamelCase 분절 → 구/신 대조 · `_next\enumdiff.md` · 0.5.8→0.6.0 표본 = `SmallActionPlay` −`AroundHide` · SubPlan −AttackNexusSubPlan +ObjContest · BigPlan +Dive/MorgardTrait/JoinTrait) ⑧ → 구 DI 인덱스표(`dienum` ② / IR DI VariantNames ①) 로 구 번호 → switch 기본 인덱스·태그 즉치(`mig060_same.py` enum 태그 재번호 행) 로 구조 확인 = **3축 합쳐야 닫힌다** | `tcxdict --enum` ⑦(구 SDK 만) | Ghidra 디컴 태그 상수 대조 ⑧ · 정본 = `REPORT\tfm2_judge_verify\03_시행착오.md §50` |
| ★**0.6.0 런타임 실측**(클래식 SDK 없음 · 「게임 실행 필요」 보류 항목 = 횡단 변수 값·필드 상시값·씬 발생·Option 인자 Some 여부·콜러 리턴 히스토그램) | **stable 껍데기 모드 + 진입부 12B 캡처 detour**(`C:\tfm2mods\tfm2_ver_probe` = `declare_stable_mod!` + item_tactics `install_detour_generic` 축약판 · 카운터 5초 기록 · 세이브 로드 후 관리 화면 **배경 리그 sim** 이 AI 전체를 돌려 경기 화면 없이 5분에 수천만 히트 · 09-17 표본 = version=3 3.7M+29.7M 히트) ⑧ | sweep(game==mine) 하네스를 같은 껍데기로 이식(다음 갈림길) | ⚠새 모드는 모드 관리 「활성화 → 코드 모드 경고 승인 → 재시작」 뒤에야 적재(첫 실행은 INIT 로그 없이 조용히 미적재) · 세이브 「이어하기」 구성 불일치 → 「강제로 불러오기」 · 정본 = `REPORT\tfm2_judge_verify\03_시행착오.md §56` 교훈 17 |
