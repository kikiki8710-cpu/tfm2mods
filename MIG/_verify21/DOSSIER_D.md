...
# 21차 배치 D 도시에 — 담당 `111`~`111` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 21 D      # ← 이것만 돌리면 된다
```
...
| # | 이름 | 파일 |
|---|---|---|
| `111` | LegacyPlanHandler::update | `MIG\_verify21\D\spec_111_LegacyPlanHandler__update.md` |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 21 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify21/D 만 써라**

```
MIG/_verify21/D/patch.json      ← 기계 적용분(필수)
MIG/_verify21/D/REPORT.md       ← 산문 보고서
MIG/_verify21/D/oracle/         ← 오라클 .rs · 출력 로그
```
