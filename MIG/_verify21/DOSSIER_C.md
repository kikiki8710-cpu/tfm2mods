...
# 21차 배치 C 도시에 — 담당 `111`~`111` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `700ce7bc2a56894c` | 2026-09-14 04:26:35 |
| `_spec/specs20.json` (손이 닿는 정본) | `0d741740ac21d856` | 2026-09-14 04:25:33 |

| `mkdossier.py`(이 파일을 만든 도구) | `802add3b146ab88e` | 2026-09-14 02:13:55 |

이 파일 생성 = `2026-09-14 05:06:16`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 21 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `111` | LegacyPlanHandler::update | pub | 241 | 105 | 19 | 34 | 355 |
...
| # | 이름 | 파일 |
|---|---|---|
| `111` | LegacyPlanHandler::update | `MIG\_verify21\C\spec_111_LegacyPlanHandler__update.md` |
...
**[G12 src_line 대조] 7건**
  [111] LegacyPlanHandler::update                       consts[30] src_line=698 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [704, 709, 763, 767, 947, 1013, 1126, 1130]
  [111] LegacyPlanHandler::update                       consts[46] src_line=1013 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [763, 767, 1126, 1130, 1672]
  [111] LegacyPlanHandler::update                       consts[64] src_line=1133 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [477, 478, 479, 480, 483, 613, 615, 689]
  [111] LegacyPlanHandler::update                       consts[93] src_line=1349 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1352]
  [111] LegacyPlanHandler::update                       consts[96] src_line=1453 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1454]
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 3346 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 1631 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 690 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 2154 | ⚠**650행만 IR 앵커**(`ev3`). 나머지 1504행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 21, "batch": "C",
  "errors": [
    {"path": "/specs[111]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[111]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 21 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify21/C 만 써라**

```
MIG/_verify21/C/patch.json      ← 기계 적용분(필수)
MIG/_verify21/C/REPORT.md       ← 산문 보고서
MIG/_verify21/C/oracle/         ← 오라클 .rs · 출력 로그
```
