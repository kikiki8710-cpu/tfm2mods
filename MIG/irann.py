...
사용:
    python MIG\\irann.py <module.ll> <함수이름조각> [-o out.ll] [--no-dbg]
...
def loc_chain(meta, mid, depth=0):
    ...
    l = meta.get('!' + mid)
    if l is None or 'DILocation' not in l or depth > 6:
        return ''
...
def annotate(path, frag, out, keep_dbg=True):
    meta = load_meta(path)
    ...
    with io.open(path, encoding='utf-8', errors='ignore') as f:
        for i, line in enumerate(f, 1):
            ...
            if m:
                ...
                res.append('    ;; %s%s = %s' % (nm, frag_ex, val.strip()))
                continue
...
if __name__ == '__main__':
    ...
    sys.exit(annotate(a[0], a[1], out, keep))
