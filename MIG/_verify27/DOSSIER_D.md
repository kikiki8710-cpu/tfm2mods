...
# 27차 배치 D 도시에 — 담당 `255`~`257` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 27 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `255` | end_check | in:game_ai | 42 | 16 | 7 | 7 | 64 |
| `256` | LegacyPlanHandler::update_on_dead | pub | 48 | 14 | 3 | 5 | 64 |
| `257` | v30_wave_danger_chase_guard | in:game_ai | 29 | 18 | 8 | 5 | 54 |
...
| # | 이름 | 파일 |
|---|---|---|
| `255` | end_check | `MIG\_verify27\D\spec_255_end_check.md` |
| `256` | LegacyPlanHandler::update_on_dead | `MIG\_verify27\D\spec_256_LegacyPlanHandler__update_on_dead.md` |
| `257` | v30_wave_danger_chase_guard | `MIG\_verify27\D\spec_257_v30_wave_danger_chase_guard.md` |
...
**[G12 src_line 대조] 38건**
  [257] v30_wave_danger_chase_guard                     consts[14] src_line=2195 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2089, 2157, 2161, 2181]

**[G16 params.role 대조] 42건**
  [256] LegacyPlanHandler::update_on_dead               sig.params[0] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[0] self `readnone` — IR %0 = ptr noalias noundef align 8 dereferenceable(6168) %0
  [256] LegacyPlanHandler::update_on_dead               sig.params[0] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[0] self `readonly` — IR %0 = ptr noalias noundef align 8 dereferenceable(6168) %0
```
...
```json
{
  "round": 27, "batch": "D",
  "errors": [
    {"path": "/specs[255]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[255]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 27 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify27/D 만 써라**

```
MIG/_verify27/D/patch.json      ← 기계 적용분(필수)
MIG/_verify27/D/REPORT.md       ← 산문 보고서
MIG/_verify27/D/oracle/         ← 오라클 .rs · 출력 로그
```
