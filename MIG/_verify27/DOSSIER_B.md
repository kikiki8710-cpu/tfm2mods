...
# 27차 배치 B 도시에 — 담당 `250`~`251` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 27 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `250` | next_plan | pub | 48 | 18 | 7 | 13 | 68 |
| `251` | battle_action | pub | 94 | 26 | 9 | 16 | 126 |
...
| # | 이름 | 파일 |
|---|---|---|
| `250` | next_plan | `MIG\_verify27\B\spec_250_next_plan.md` |
| `251` | battle_action | `MIG\_verify27\B\spec_251_battle_action.md` |
...
**[G12 src_line 대조] 38건**
  [250] next_plan                                       consts[13] src_line=109 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [127]

**[G14 mem.dir 방향] 2건**
  [250] next_plan                                       mem[25] dir=r 인데 그 자리는 w 로만 쓰인다(base 동정 루트 %0 shift +0)
  DebugFrameData infos +0xa0

**[G7 과열림] 26건**
  [250] next_plan                                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 G) game_core 콜리(get_start_position·lane_path·near_jungle_bush·is_recent_visible·add_text·get_entity_by_id) 본체 미독 — 시
```
...
```json
{
  "round": 27, "batch": "B",
  "errors": [
    {"path": "/specs[250]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[250]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 27 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify27/B 만 써라**

```
MIG/_verify27/B/patch.json      ← 기계 적용분(필수)
MIG/_verify27/B/REPORT.md       ← 산문 보고서
MIG/_verify27/B/oracle/         ← 오라클 .rs · 출력 로그
```
