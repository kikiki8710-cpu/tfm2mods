...
# 27차 배치 A 도시에 — 담당 `249`~`249` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 27 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `249` | base_battle_action | in:game_ai::plan_legacy::sub_plan::battle | 188 | 58 | 21 | 35 | 263 |
...
| # | 이름 | 파일 |
|---|---|---|
| `249` | base_battle_action | `MIG\_verify27\A\spec_249_base_battle_action.md` |
...
**[G12 src_line 대조] 38건**
  [249] base_battle_action                              consts[40] src_line=1798 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1372, 1391, 1401, 1428, 1779, 1813, 1849]

**[G16 params.role 대조] 42건**
  [249] base_battle_action                              sig.params[4] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[4] data `readonly` — IR %4 = i64 %4
  [249] base_battle_action                              sig.params[4] P5 「미사용/전달만」인데 call 밖에서 쓰인다
  p[4] data %4 — 절 「SmallActionUlt/Skill2::new · is_dash_worth 에 그」 / call 밖 사용 5회(스필 0 제외) · 총 5회 · 오프셋 []
```
...
```json
{
  "round": 27, "batch": "A",
  "errors": [
    {"path": "/specs[249]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[249]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 27 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify27/A 만 써라**

```
MIG/_verify27/A/patch.json      ← 기계 적용분(필수)
MIG/_verify27/A/REPORT.md       ← 산문 보고서
MIG/_verify27/A/oracle/         ← 오라클 .rs · 출력 로그
```
