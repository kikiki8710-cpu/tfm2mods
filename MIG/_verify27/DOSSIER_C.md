...
# 27차 배치 C 도시에 — 담당 `252`~`254` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 27 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `252` | v16_knight_ult_zone_bonus | in:game_ai | 27 | 30 | 19 | 13 | 72 |
| `253` | LineSafeSubPlan::action_candidates | pub | 43 | 10 | 5 | 7 | 56 |
| `254` | noncombat_steroid_window | in:game_ai | 23 | 13 | 5 | 8 | 40 |
...
| # | 이름 | 파일 |
|---|---|---|
| `252` | v16_knight_ult_zone_bonus | `MIG\_verify27\C\spec_252_v16_knight_ult_zone_bonus.md` |
| `253` | LineSafeSubPlan::action_candidates | `MIG\_verify27\C\spec_253_LineSafeSubPlan__action_candidates.md` |
| `254` | noncombat_steroid_window | `MIG\_verify27\C\spec_254_noncombat_steroid_window.md` |
...
**이 배치 몫 = 9건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 38건**
  [252] v16_knight_ult_zone_bonus                       consts[7] src_line=222 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [258]
  [252] v16_knight_ult_zone_bonus                       consts[16] src_line=254 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [258]
  [254] noncombat_steroid_window                        consts[12] src_line=413 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [436]

**[G13 knobs.where 대조] 1건**
  [252] v16_knight_ult_zone_bonus                       knobs[6] 인용한 명령이 그 줄(±2)에 없다
  g07.ll:157025  인용=add 120

**[G16 params.role 대조] 42건**
  [253] LineSafeSubPlan::action_candidates              sig.params[3] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[3] rnd `readnone` — IR %3 = ptr noalias noundef align 16 dereferenceable(320) %3

**[G5 sig 정본 대조] 6건**
  [253] LineSafeSubPlan::action_candidates              params[1](self) 의 가변성이 tcx 와 다르다 — 명세 `&LineSafeSubPlan (1B: +0x0 line: LineTyp` / tcx `&mut game_ai::plan_legacy::sub_plan::LineSafeSubPlan`

**[G7 과열림] 26건**
  [252] v16_knight_ult_zone_bonus                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 I) data.blackboard[1 - team](적팀 blackboard)를 아군 player 와 함께 is_recent_visible 에 넘기는 것이 의도인지(자기팀 blackboard 오타인지) — I
  [254] noncombat_steroid_window                        ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  395~404 사이 소스 줄의 정확한 형태(`if enemy_near { return None; }` 추정) — IR 은 is_recent_visible true → 즉시 {2,undef} 반환으로 접혀 있고 그 구
  [254] noncombat_steroid_window                        ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard 인덱스 의미: is_recent_visible 은 blackboard[enemy_team](관측 대상 팀), team_action_hits 는 blackboard[my_team].small_act
```
...
```json
{
  "round": 27, "batch": "C",
  "errors": [
    {"path": "/specs[252]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[252]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 27 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify27/C 만 써라**

```
MIG/_verify27/C/patch.json      ← 기계 적용분(필수)
MIG/_verify27/C/REPORT.md       ← 산문 보고서
MIG/_verify27/C/oracle/         ← 오라클 .rs · 출력 로그
```
