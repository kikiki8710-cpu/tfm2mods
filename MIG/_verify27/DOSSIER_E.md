...
# 27차 배치 E 도시에 — 담당 `258`~`261` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 27 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `258` | SmallActionLaneMinionPosition::choose_goal | in:game_ai | 22 | 20 | 7 | 8 | 48 |
| `259` | v48_projectile_profile | pub | 41 | 14 | 5 | 7 | 54 |
| `260` | resolve_fight_stake_roster | in:game_ai | 17 | 3 | 4 | 5 | 24 |
| `261` | kite_reposition_point | in:game_ai::plan_legacy::sub_plan::battle | 11 | 9 | 5 | 3 | 24 |
...
| # | 이름 | 파일 |
|---|---|---|
| `258` | SmallActionLaneMinionPosition::choose_goal | `MIG\_verify27\E\spec_258_SmallActionLaneMinionPosition__choose_goal.md` |
| `259` | v48_projectile_profile | `MIG\_verify27\E\spec_259_v48_projectile_profile.md` |
| `260` | resolve_fight_stake_roster | `MIG\_verify27\E\spec_260_resolve_fight_stake_roster.md` |
| `261` | kite_reposition_point | `MIG\_verify27\E\spec_261_kite_reposition_point.md` |
...
**이 배치 몫 = 10건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G1 자기모순] 1건**
  [259] v48_projectile_profile                          자기모순(Option 겹수): 같은 명세에 'Option<Option<' 와 'Option<Effect>' 가 동시에 있다

**[G12 src_line 대조] 38건**
  [259] v48_projectile_profile                          consts[10] src_line=208 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [199, 218, 223]
  [259] v48_projectile_profile                          consts[11] src_line=208 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [199, 218, 223]

**[G14 mem.dir 방향] 2건**
  [259] v48_projectile_profile                          mem[38] dir=w 인데 그 자리는 r 로만 쓰인다(base 동정 루트 %124 shift +8)
  RefCell<V48ProfileCache>(TLS) V48ProfileCache.seed +0x38

**[G16 params.role 대조] 42건**
  [258] SmallActionLaneMinionPosition::choose_goal      sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 8개(sret 행 있음) ⟹ 9행이어야 하는데 10행. 현재 i = [0, 1, 1, 2, 3, 4, 5, 6, 7, 8]
  [260] resolve_fight_stake_roster                      sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 8개(sret 행 있음) ⟹ 9행이어야 하는데 11행. 현재 i = [0, 1, 2, 3, 4, 4, 5, 5, 6, 7, 8]
  [261] kite_reposition_point                           sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 8개(sret보정 -0) > params 7개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[], 덮은 레지스터 []). 자리 매핑 보류

**[G19 knobs.value 값] 10건**
  [258] SmallActionLaneMinionPosition::choose_goal      knobs[1] 주장한 값을 IR 관측에서 못 찾는다
  value=14000 · 관측(창)=[-14000]

**[G5 sig 정본 대조] 6건**
  [258] SmallActionLaneMinionPosition::choose_goal      sig.tcx 인자 8개(팻포인터 확장 8) vs params 9개 — 개수가 다르다
  &game_ai::SmallActionLaneMinionPos | usize | &game_core::PlayerState | &game_core::OperationData | &game_core::Entity | &game_core::Entity | &game_cor

**[G9 callees 오염] 1건**
  [259] v48_projectile_profile                          ★`p.shape.radius` — 중간 `shape` 는 enum(game_core::simulation::projectile::Proje) 이라 필드 경로를 이을 수 없다. `radius` 는 **메서드**이고 callees 자동수집에서 빠진다
  logic 표기를 `radius()` 로 고칠 것
```
...
```json
{
  "round": 27, "batch": "E",
  "errors": [
    {"path": "/specs[258]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[258]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 27 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify27/E 만 써라**

```
MIG/_verify27/E/patch.json      ← 기계 적용분(필수)
MIG/_verify27/E/REPORT.md       ← 산문 보고서
MIG/_verify27/E/oracle/         ← 오라클 .rs · 출력 로그
```
