...
|---|---|---|---|---|---|---|---|
| `05` | v50_fold_dive_episode | in:game_ai | 30 | 14 | 7 | 1 | 7 |
| `06` | v2_response_retreat_stance | in:game_ai | 13 | 4 | 5 | 0 | 5 |
| `07` | sub_plan | pub | 28 | 7 | 14 | 1 | 6 |
| `08` | is_end | pub | 21 | 6 | 9 | 0 | 4 |
| `09` | check_favorable_engage_formation | pub | 14 | 9 | 11 | 1 | 5 |
...
| # | 이름 | 파일 |
|---|---|---|
| `05` | v50_fold_dive_episode | `MIG\_verify7\B\spec_05_v50_fold_dive_episode.md` |
| `06` | v2_response_retreat_stance | `MIG\_verify7\B\spec_06_v2_response_retreat_stance.md` |
| `07` | sub_plan | `MIG\_verify7\B\spec_07_sub_plan.md` |
| `08` | is_end | `MIG\_verify7\B\spec_08_is_end.md` |
| `09` | check_favorable_engage_formation | `MIG\_verify7\B\spec_09_check_favorable_engage_formation.md` |
...
**이 배치 몫 = 18건** (게이트 머리줄의 `G12=47`·`G13=8` 은 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 47건**
  [05] v50_fold_dive_episode                           consts[2] src_line=146 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [156, 157, 167]
  [05] v50_fold_dive_episode                           consts[3] src_line=147 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [139, 156, 157]
  [05] v50_fold_dive_episode                           consts[5] src_line=149 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [145, 146, 155, 156, 157]
  [05] v50_fold_dive_episode                           consts[6] src_line=150 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131, 156]
  [05] v50_fold_dive_episode                           consts[7] src_line=151 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131, 157]
  [05] v50_fold_dive_episode                           consts[8] src_line=152 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131, 139]
  [05] v50_fold_dive_episode                           consts[11] src_line=146 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131]
  [05] v50_fold_dive_episode                           consts[12] src_line=147 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [145]
  [05] v50_fold_dive_episode                           consts[13] src_line=151 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131, 139]
  [06] v2_response_retreat_stance                      consts[2] src_line=36 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [17, 21, 30, 31]
  [06] v2_response_retreat_stance                      consts[3] src_line=38 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [31]
  [07] sub_plan                                        consts[2] src_line=36 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [29, 33]
  [07] sub_plan                                        consts[3] src_line=43 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [35]
  [07] sub_plan                                        consts[4] src_line=47 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [29, 33]
  [08] is_end                                          consts[0] src_line=164 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [169, 174, 179, 180, 193]
  [09] check_favorable_engage_formation                consts[4] src_line=1283 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1203, 1286]
  [09] check_favorable_engage_formation                consts[5] src_line=1280 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1233]

**[G9 callees 오염] 4건**
  [07] sub_plan                                        손확인 1개 — logic 이 필드처럼 적었고 tcx 에 동명 함수가 있다
  target_bush(동명 필드 있음)
```
...
```json
{
  "round": 7, "batch": "B",
  "errors": [
    {"path": "/specs[5]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[5]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 7 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify7/B 만 써라**

```
MIG/_verify7/B/patch.json      ← 기계 적용분(필수)
MIG/_verify7/B/REPORT.md       ← 산문 보고서
MIG/_verify7/B/oracle/         ← 오라클 .rs · 출력 로그
```
