# -*- coding: utf-8 -*-
u"""배치 C 의 `_verify7/C/REPORT.md` 를 생성한다.
(`auditrounds.py` 가 라운드 커버리지를 셀 때 읽는 파이프라인 입력이라 파일로 있어야 한다.)"""
import io, os, sys
sys.stdout.reconfigure(encoding="utf-8", errors="replace")

BODY = r"""# 7차 반증검증 — 배치 C (담당 `10`~`14`) 보고
...
out = r"C:\tfm2mods\MIG\_verify7\C\REPORT.md"
io.open(out, "w", encoding="utf-8").write(BODY)
print(u"%s  %d bytes" % (out, os.path.getsize(out)))
