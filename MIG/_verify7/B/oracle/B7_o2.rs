...
//! B7_o2 — 7차 배치B 오라클 ②: **09 의 미니언 위험 조회 창(window) 계수가 정말 `tick_per_second × 2` 인가**
//!
//! ## 무엇을 내리려 하나 (ev4 → ev2)
...
fn main() {
    ...
    // 적(팀1) 미니언 위치를 target_enemy 자리로 쓴다 = 미니언 라인 한복판
    let (base, pts) = {
        let cache = AbstractGameWithCache::new(&game as &dyn AbstractGame, &ctx);
        let b = cache.player_champion[0][0].unwrap().clone();
    ...
    // ── ①창 스윕: 같은 데이터에서 window 를 0..400 으로 올리며 danger 가 언제 켜지나 ──
    //    (계수 후보를 가를 T 구간을 여기서 고른다)
    {
        let mut ce = base.clone();
        ce.hp = 300; ce.stat_cached.hp = 1000; ce.x = ex; ce.y = ey;
        ...
        for w in 0..=400usize {
            let d = game_ai::enemy_minion_line_action_danger_damage_at(0, &data, &ce, ex, ey, w, true, false);
            if prev != Some(d) { println!("W\t{}\t{}", w, d); prev = Some(d); }
        }
    }

    // ── ②T 스윕: tick_per_second 를 바꿔 09 의 판정과 K별 직접호출을 나란히 찍는다 ──
    println!("\n#T\ttps\tr09\tg1\tg2\tg3\tg4");
    let mut rows: Vec<(usize, bool, bool, bool, bool, bool)> = Vec::new();
    for tps in [1usize, 2, 5, 10, 15, 20, 25, 30, 40, 50, 60, 80, 100, 150, 200] {
        ...
        // 자기 챔피언을 미니언 라인 위에 놓고 HP 를 낮춰 게이트가 갈릴 여지를 준다
        let mut ce = base.clone();
        ce.hp = 300; ce.stat_cached.hp = 1000; ce.x = ex; ce.y = ey;
        // target_enemy — 적팀 엔티티가 필요하다. 적 챔피언을 복제해 같은 지점에 둔다.
        let mut te = {
            let c0 = AbstractGameWithCache::new(&game as &dyn AbstractGame, &ctx2);
            c0.player_champion[1][0].unwrap().clone()
        };
        te.x = ex; te.y = ey;
        ...
        let g = |k: usize| game_ai::enemy_minion_line_action_danger_damage_at(
            0, &data, &ce, te.x, te.y, tps * k, true, false) != 0;
        let (g1, g2, g3, g4) = (g(1), g(2), g(3), g(4));
        println!("T\t{}\t{}\t{}\t{}\t{}\t{}", tps, r09, g1, g2, g3, g4);
        rows.push((tps, r09, g1, g2, g3, g4));
    }
    // ── ③계수 판정: `r09==false` 를 전 구간 설명하는 K ──
    println!("\n#K\tK\tagree\tdisagree\tverdict");
    for (ki, name) in [(1usize, "K=1"), (2, "K=2"), (3, "K=3"), (4, "K=4")] {
        let mut ag = 0; let mut dis = 0;
        for &(t, r09, g1, g2, g3, g4) in &rows {
            let gk = [g1, g2, g3, g4][ki - 1];
            // 게이트가 켜지면(dmg!=0) 09 는 반드시 false 여야 한다
            if gk { if !r09 { ag += 1 } else { dis += 1 } } else { ag += 1 }
        }
        println!("K\t{}\t{}\t{}\t{}", name, ag, dis, if dis == 0 { "가능" } else { "기각" });
    }
    println!("\n#NOTE\tK 가 여럿 '가능' 이면 이 데이터로는 못 가른다 — g1..g4 가 갈리는 tps 를 찾아야 한다");
}
