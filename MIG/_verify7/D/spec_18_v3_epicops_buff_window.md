...
| 15 | TeamPlan | 0xc8 | chats.ptr(push 대상 버퍼) | w | 원소 24B. 태그 @+0, Press/PressChange 는 LineType @+1, 공통 usize 필드 @+8 에 항상 0 을 넣는다 ★**행 분리 **: 한 행에 오프셋을 묶으면 `tcxaudit` 기계 검사가 무력화된다. 14 는 3차에 같은 이유로 분리됐는데 **18 만 남아 있었다.** | 3 |
...
| 4 | 25 | 654 | 태그 | Chat::SerpenSetup 의 태그값 · 오라클 실행 확증( Chat::SerpenSetup(0) 태그바이트 직독 = 25 (o1.txt)) | 2 |
...
| 6 | 수리 우선순위1 부상 HP% 임계 | m09.ll:65212 (icmp ult .., 41) | 41 | 올리면 더 멀쩡한 팀에도 '수리' 선점이 걸린다 | 4 |
| 7 | 수리 우선순위1 부상자 수 임계 | m09.ll:65301 (ugt .., 1) | > 1 (2명 이상) | 0 으로 내리면 1명만 다쳐도 수리 | 4 |
| 8 | 수리 우선순위2 '건강' HP% 임계 | m09.ll:65273 (ugt .., 29) | 29 | 올리면 우선순위2 가 훨씬 자주 발동 | 4 |
