...
|---|---|---|---|---|---|---|---|
| `15` | single_try_engage | in:game_ai | 18 | 8 | 9 | 1 | 12 |
| `16` | max_range_nearly_can_use | pub | 31 | 7 | 9 | 0 | 4 |
| `17` | new | pub | 43 | 4 | 5 | 0 | 1 |
| `18` | v3_epicops_buff_window | in:game_ai | 18 | 8 | 27 | 1 | 25 |
| `19` | best_jungle_goal | pub | 16 | 5 | 8 | 0 | 1 |
...
| # | 이름 | 파일 |
|---|---|---|
| `15` | single_try_engage | `MIG\_verify7\D\spec_15_single_try_engage.md` |
| `16` | max_range_nearly_can_use | `MIG\_verify7\D\spec_16_max_range_nearly_can_use.md` |
| `17` | new | `MIG\_verify7\D\spec_17_new.md` |
| `18` | v3_epicops_buff_window | `MIG\_verify7\D\spec_18_v3_epicops_buff_window.md` |
| `19` | best_jungle_goal | `MIG\_verify7\D\spec_19_best_jungle_goal.md` |
...
**이 배치 몫 = 11건** (게이트 머리줄의 `G12=47`·`G13=8` 은 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 47건**
  [15] single_try_engage                               consts[5] src_line=256 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [245]
  [15] single_try_engage                               consts[6] src_line=256 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [242, 245, 248, 249, 253]
  [15] single_try_engage                               consts[7] src_line=256 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [245]
  [16] max_range_nearly_can_use                        consts[0] src_line=2401 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2399, 2403, 2407, 2410, 2414, 2417, 2421, 2424]
  [16] max_range_nearly_can_use                        consts[5] src_line=2398 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2399, 2402, 2403, 2407, 2409, 2410, 2414, 2416]
  [18] v3_epicops_buff_window                          consts[0] src_line=635 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [637, 638, 642, 652, 653, 654, 658, 667]
  [18] v3_epicops_buff_window                          consts[1] src_line=635 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [664, 666, 667, 668, 669, 670, 673, 679]
  [18] v3_epicops_buff_window                          consts[4] src_line=654 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [638, 658]
  [18] v3_epicops_buff_window                          consts[5] src_line=674 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [638]
  [18] v3_epicops_buff_window                          consts[6] src_line=676 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [638]

**[G9 callees 오염] 4건**
  [15] single_try_engage                               손확인 1개 — logic 이 필드처럼 적었고 tcx 에 동명 함수가 있다
  chats(동명 필드 있음)
```
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 7 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify7/D 만 써라**

```
MIG/_verify7/D/patch.json      ← 기계 적용분(필수)
MIG/_verify7/D/REPORT.md       ← 산문 보고서
MIG/_verify7/D/oracle/         ← 오라클 .rs · 출력 로그
```
