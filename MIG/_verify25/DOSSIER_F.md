...
# 25차 배치 F 도시에 — 담당 `199`~`199` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 F      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `199` | EpicPokeSubPlan::action_candidates | pub | 59 | 36 | 14 | 8 | 106 |
...
| # | 이름 | 파일 |
|---|---|---|
| `199` | EpicPokeSubPlan::action_candidates | `MIG\_verify25\F\spec_199_EpicPokeSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [199] EpicPokeSubPlan::action_candidates              consts[24] src_line=419 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [24, 25, 37, 38, 47, 48, 62, 63]

**[G18 logic↔표 상호참조] 8건**
  [199] EpicPokeSubPlan::action_candidates              logic/P2 logic 이 필드 `cache.jungles` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `jungles` 는 distruct 의 실재 필드명

**[G5 sig 정본 대조] 1건**
  [199] EpicPokeSubPlan::action_candidates              params[1](self) 의 가변성이 tcx 와 다르다 — 명세 `&EpicPokeSubPlan(0B ZST)` / tcx `&mut game_ai::plan_legacy::sub_plan::EpicPokeSubPlan`
```
...
```json
{
  "round": 25, "batch": "F",
  "errors": [
    {"path": "/specs[199]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[199]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only F --dry
```
...
## §6 출력 디렉터리 — **_verify25/F 만 써라**

```
MIG/_verify25/F/patch.json      ← 기계 적용분(필수)
MIG/_verify25/F/REPORT.md       ← 산문 보고서
MIG/_verify25/F/oracle/         ← 오라클 .rs · 출력 로그
```
