...
# 25차 배치 M 도시에 — 담당 `195`~`195` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 M      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `195` | LineWaitSubPlan::action_candidates | pub | 49 | 11 | 5 | 5 | 65 |
...
| # | 이름 | 파일 |
|---|---|---|
| `195` | LineWaitSubPlan::action_candidates | `MIG\_verify25\M\spec_195_LineWaitSubPlan__action_candidates.md` |
...
**[G10 class 오분류] 4건**
  [195] LineWaitSubPlan::action_candidates              class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  라인(effect.rs:26)에 range+stat_buff.range+(level-1)*growth 가 있고 move_speed*30·tower.radius 는 L114 로 추정. 합 자체는 확정
```
...
```json
{
  "round": 25, "batch": "M",
  "errors": [
    {"path": "/specs[195]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[195]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only M --dry
```
...
## §6 출력 디렉터리 — **_verify25/M 만 써라**

```
MIG/_verify25/M/patch.json      ← 기계 적용분(필수)
MIG/_verify25/M/REPORT.md       ← 산문 보고서
MIG/_verify25/M/oracle/         ← 오라클 .rs · 출력 로그
```
