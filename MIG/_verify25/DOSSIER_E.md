...
# 25차 배치 E 도시에 — 담당 `194`~`194` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `194` | HideSubPlan::action_candidates | pub | 59 | 30 | 14 | 16 | 101 |
...
| # | 이름 | 파일 |
|---|---|---|
| `194` | HideSubPlan::action_candidates | `MIG\_verify25\E\spec_194_HideSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [194] HideSubPlan::action_candidates                  consts[19] src_line=81 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [63]

**[G15 consts.kind 관측] 3건**
  [194] HideSubPlan::action_candidates                  consts[2] kind=임계 NEG
  `meaning` 이 「eck(20547/20551 · 20881/20885 · 21727/21731). 임계 아님」라 적었는데 kind=임계

**[G16 params.role 대조] 7건**
  [194] HideSubPlan::action_candidates                  sig.params[1] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[1] self `readonly` — IR %1 = ptr noalias noundef align 8 captures(none) dereferenceable(16) %1
```
...
```json
{
  "round": 25, "batch": "E",
  "errors": [
    {"path": "/specs[194]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[194]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify25/E 만 써라**

```
MIG/_verify25/E/patch.json      ← 기계 적용분(필수)
MIG/_verify25/E/REPORT.md       ← 산문 보고서
MIG/_verify25/E/oracle/         ← 오라클 .rs · 출력 로그
```
