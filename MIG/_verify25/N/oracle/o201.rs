...
fn main() {
    ...
    let (target, desc): (&Entity, &str) = match case {
        ...
        25 => { unsafe { std::ptr::write_volatile(&mut (*np).visible_state[0], VisibleState::Visible) }; println!("pred_split	tower(tag {}): is_tower={} is_nexus={} is_structure={}	nexus(tag {}): is_tower={} is_nexus={} is_structure={}", r64(tp,0x68), etower.ty.is_tower(), etower.ty.is_nexus(), etower.ty.is_structure(), r64(np,0x68), enexus.ty.is_tower(), enexus.ty.is_nexus(), enexus.ty.is_structure()); (enexus, "적 넥서스 · pub 술어 3종 분리(is_tower/is_nexus/is_structure) → L173 마스크 14 의 정체") }
        26 => { w64(ep, 0x660, champ.x + 5000); w64(ep, 0x668, champ.y); unsafe { std::ptr::write_volatile(&mut (*ep).visible_state[0], VisibleState::Visible); std::ptr::write_volatile(&mut (*cp).attack_effect, Some(mkeffect(CastingType::Position))); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xab8) as *mut u64, 10); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xac0) as *mut u64, 10) }; (enemy, "TLS 도달: auto_target=true · casting=Position · ps.cx=cy=10(셀 7..13 유효) → get_input_target L534→L539 position_score_at_cell→position_eval_at→POS_EVAL_CACHE.with 기대") }
        27 => { w64(ep, 0x660, champ.x + 5000); w64(ep, 0x668, champ.y); unsafe { std::ptr::write_volatile(&mut (*ep).visible_state[0], VisibleState::Visible); std::ptr::write_volatile(&mut (*cp).attack_effect, Some(mkeffect(CastingType::Direction))); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xab8) as *mut u64, 10); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xac0) as *mut u64, 10) }; (enemy, "TLS 도달: auto_target=true · casting=Direction · ps.cx=cy=10 → L379→L384 루프 → POS_EVAL_CACHE 기대") }
        28 => { w64(ep, 0x660, champ.x + 5000); w64(ep, 0x668, champ.y); unsafe { std::ptr::write_volatile(&mut (*ep).visible_state[0], VisibleState::Visible); std::ptr::write_volatile(&mut (*cp).attack_effect, Some(mkeffect(CastingType::Targeting))); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xab8) as *mut u64, 10); std::ptr::write_volatile((&ps as *const PositioningScoreData as *mut u8).add(0xac0) as *mut u64, 10) }; (enemy, "대조군: casting=Targeting · ps.cx=cy=10 → L377 switch 0 → L652 Target 즉시 → TLS 무접촉 기대") }
        29 => { w64(cp, 0x0, 1); w64(ep, 0x660, champ.x); w64(ep, 0x668, champ.y); unsafe { std::ptr::write_volatile(&mut (*ep).visible_state[0], VisibleState::Unknown) }; (enemy, "champ.team=Neutral · 같은 좌표(sz=0) · 대상 Unknown · Targeting → get_input_target None(L354) 인데 is_visible_from=true → L179 sdiv sz=0 → panic div_by_zero 기대(reach 'live' 실증)") }
        _ => (enemy, "-"),
    };
    ...
    let pred: Option<Input> = (|| {
        ...
        let margin: u64 = if target.ty.is_tower() || target.ty.is_nexus() { 2000 } else { 15000 };
