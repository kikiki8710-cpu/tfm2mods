...
# 25차 배치 C 도시에 — 담당 `197`~`197` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `197` | SerpenHuntSubPlan::action_candidates | pub | 123 | 80 | 30 | 30 | 228 |
...
| # | 이름 | 파일 |
|---|---|---|
| `197` | SerpenHuntSubPlan::action_candidates | `MIG\_verify25\C\spec_197_SerpenHuntSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [197] SerpenHuntSubPlan::action_candidates            consts[12] src_line=270 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [24, 271, 286, 301, 428, 558, 570, 615]
  [197] SerpenHuntSubPlan::action_candidates            consts[48] src_line=445 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [59]
  [197] SerpenHuntSubPlan::action_candidates            consts[49] src_line=445 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [59, 85]

**[G7 과열림] 11건**
  [197] SerpenHuntSubPlan::action_candidates            ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 H) range_misjudge_roll 이 rnd 를 몇 번 소비하는지·max_range_can_use / range_adjust / is_recent_visible / positioning_effectiv
```
...
```json
{
  "round": 25, "batch": "C",
  "errors": [
    {"path": "/specs[197]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[197]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify25/C 만 써라**

```
MIG/_verify25/C/patch.json      ← 기계 적용분(필수)
MIG/_verify25/C/REPORT.md       ← 산문 보고서
MIG/_verify25/C/oracle/         ← 오라클 .rs · 출력 로그
```
