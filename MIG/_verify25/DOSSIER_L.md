...
# 25차 배치 L 도시에 — 담당 `192`~`193` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 L      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `192` | SerpenCheckSubPlan::action_candidates | pub | 41 | 15 | 5 | 5 | 59 |
| `193` | JungleSubPlan::action_candidates | pub | 53 | 17 | 5 | 6 | 75 |
...
| # | 이름 | 파일 |
|---|---|---|
| `192` | SerpenCheckSubPlan::action_candidates | `MIG\_verify25\L\spec_192_SerpenCheckSubPlan__action_candidates.md` |
| `193` | JungleSubPlan::action_candidates | `MIG\_verify25\L\spec_193_JungleSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [193] JungleSubPlan::action_candidates                consts[9] src_line=117 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [29, 64, 66, 76, 78, 90, 92, 147]
  [193] JungleSubPlan::action_candidates                consts[10] src_line=117 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [20, 29, 31, 34, 40, 42, 56, 64]
  [193] JungleSubPlan::action_candidates                consts[11] src_line=117 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [29, 90, 111, 114, 118, 147, 148]

**[G16 params.role 대조] 7건**
  [192] SerpenCheckSubPlan::action_candidates           sig.params[7] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[7] team_plan `readonly` — IR %7 = ptr noundef nonnull align 8 %7
```
...
```json
{
  "round": 25, "batch": "L",
  "errors": [
    {"path": "/specs[192]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[192]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only L --dry
```
...
## §6 출력 디렉터리 — **_verify25/L 만 써라**

```
MIG/_verify25/L/patch.json      ← 기계 적용분(필수)
MIG/_verify25/L/REPORT.md       ← 산문 보고서
MIG/_verify25/L/oracle/         ← 오라클 .rs · 출력 로그
```
