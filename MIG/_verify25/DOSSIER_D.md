...
# 25차 배치 D 도시에 — 담당 `198`~`198` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `198` | EpicHuntSubPlan::action_candidates | pub | 109 | 74 | 32 | 15 | 211 |
...
| # | 이름 | 파일 |
|---|---|---|
| `198` | EpicHuntSubPlan::action_candidates | `MIG\_verify25\D\spec_198_EpicHuntSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [198] EpicHuntSubPlan::action_candidates              consts[7] src_line=232 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [107, 144, 234, 446]
  [198] EpicHuntSubPlan::action_candidates              consts[36] src_line=446 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [59]
  [198] EpicHuntSubPlan::action_candidates              consts[37] src_line=446 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [59, 85]
  [198] EpicHuntSubPlan::action_candidates              consts[70] src_line=481 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [107, 144, 234, 446]

**[G16 params.role 대조] 7건**
  [198] EpicHuntSubPlan::action_candidates              sig.params[3] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[3] rnd `readnone` — IR %3 = ptr noalias noundef align 16 dereferenceable(320) %3

**[G7 과열림] 11건**
  [198] EpicHuntSubPlan::action_candidates              ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 K) range_misjudge_rng/roll · max_range_can_use/nearly · position_score_at_position/at_cell · epic_action_score · Epi
```
...
```json
{
  "round": 25, "batch": "D",
  "errors": [
    {"path": "/specs[198]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[198]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify25/D 만 써라**

```
MIG/_verify25/D/patch.json      ← 기계 적용분(필수)
MIG/_verify25/D/REPORT.md       ← 산문 보고서
MIG/_verify25/D/oracle/         ← 오라클 .rs · 출력 로그
```
