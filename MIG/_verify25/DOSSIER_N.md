...
# 25차 배치 N 도시에 — 담당 `201`~`201` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 N      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `201` | abstract_input::attack | pub | 29 | 7 | 2 | 4 | 37 |
...
| # | 이름 | 파일 |
|---|---|---|
| `201` | abstract_input::attack | `MIG\_verify25\N\spec_201_abstract_input__attack.md` |
...
```json
{
  "round": 25, "batch": "N",
  "errors": [
    {"path": "/specs[201]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[201]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only N --dry
```
...
## §6 출력 디렉터리 — **_verify25/N 만 써라**

```
MIG/_verify25/N/patch.json      ← 기계 적용분(필수)
MIG/_verify25/N/REPORT.md       ← 산문 보고서
MIG/_verify25/N/oracle/         ← 오라클 .rs · 출력 로그
```
