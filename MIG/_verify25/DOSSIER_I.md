...
# 25차 배치 I 도시에 — 담당 `202`~`202` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 I      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `202` | LineGankerPlan::next_plan | pub | 46 | 19 | 10 | 9 | 71 |
...
| # | 이름 | 파일 |
|---|---|---|
| `202` | LineGankerPlan::next_plan | `MIG\_verify25\I\spec_202_LineGankerPlan__next_plan.md` |
...
**[G16 params.role 대조] 7건**
  [202] LineGankerPlan::next_plan                       sig.params[3] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[3] rnd `readnone` — IR %3 = ptr noalias noundef align 16 dereferenceable(320) %3

**[G7 과열림] 11건**
  [202] LineGankerPlan::next_plan                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible 의 self 가 왜 blackboard[1-team](적팀 슬롯)인지 — IR 사실(96020·113034·14604 전부 1-team). Blackboard.last_visible[
```
...
```json
{
  "round": 25, "batch": "I",
  "errors": [
    {"path": "/specs[202]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[202]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only I --dry
```
...
## §6 출력 디렉터리 — **_verify25/I 만 써라**

```
MIG/_verify25/I/patch.json      ← 기계 적용분(필수)
MIG/_verify25/I/REPORT.md       ← 산문 보고서
MIG/_verify25/I/oracle/         ← 오라클 .rs · 출력 로그
```
