...
# 25차 배치 H 도시에 — 담당 `189`~`189` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 H      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `189` | DefenseNexusSubPlan::action_candidates | pub | 46 | 20 | 7 | 9 | 71 |
...
| # | 이름 | 파일 |
|---|---|---|
| `189` | DefenseNexusSubPlan::action_candidates | `MIG\_verify25\H\spec_189_DefenseNexusSubPlan__action_candidates.md` |
...
**[G12 src_line 대조] 26건**
  [189] DefenseNexusSubPlan::action_candidates          consts[19] src_line=40 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [259]

**[G18 logic↔표 상호참조] 8건**
  [189] DefenseNexusSubPlan::action_candidates          logic/P1 logic 이 `+0x31` 를 인용하는데 mem 표에 그 오프셋이 없다
  L274 let on_trajectory = position_score.on_trajectory(+0x30) || position_score.on_periodic_trajectory(+0x31)
  [189] DefenseNexusSubPlan::action_candidates          logic/P1 logic 이 `+0x31` 를 인용하는데 mem 표에 그 오프셋이 없다
  // 46299~46306 (IR: +0x30 참 → last_stand 만 판정 / 거짓 → last_stand || !(windup || +0x31))
```
...
```json
{
  "round": 25, "batch": "H",
  "errors": [
    {"path": "/specs[189]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[189]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only H --dry
```
...
## §6 출력 디렉터리 — **_verify25/H 만 써라**

```
MIG/_verify25/H/patch.json      ← 기계 적용분(필수)
MIG/_verify25/H/REPORT.md       ← 산문 보고서
MIG/_verify25/H/oracle/         ← 오라클 .rs · 출력 로그
```
