...
# 25차 배치 B 도시에 — 담당 `196`~`196` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `196` | LineDefenseSubPlan::action_candidates | pub | 146 | 73 | 24 | 24 | 235 |
...
| # | 이름 | 파일 |
|---|---|---|
| `196` | LineDefenseSubPlan::action_candidates | `MIG\_verify25\B\spec_196_LineDefenseSubPlan__action_candidates.md` |
...
**[G10 class 오분류] 4건**
  [196] LineDefenseSubPlan::action_candidates           class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  define 미확인(미탐색: `grep define.*is_in_skill /c/tfm2mods/_gcbc`). 동작(casting∈{1,2} 일 때만 is_in_range 검사)은 IR 로 확정

**[G12 src_line 대조] 26건**
  [196] LineDefenseSubPlan::action_candidates           consts[14] src_line=390 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [472]

**[G16 params.role 대조] 7건**
  [196] LineDefenseSubPlan::action_candidates           sig.params[1] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[1] self `readonly` — IR %1 = ptr noalias noundef dereferenceable(3) %1
```
...
```json
{
  "round": 25, "batch": "B",
  "errors": [
    {"path": "/specs[196]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[196]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify25/B 만 써라**

```
MIG/_verify25/B/patch.json      ← 기계 적용분(필수)
MIG/_verify25/B/REPORT.md       ← 산문 보고서
MIG/_verify25/B/oracle/         ← 오라클 .rs · 출력 로그
```
