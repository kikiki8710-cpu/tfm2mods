...
# 25차 배치 A 도시에 — 담당 `188`~`188` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `188` | BattleSubPlan::action_candidates | pub | 111 | 61 | 30 | 28 | 201 |
...
| # | 이름 | 파일 |
|---|---|---|
| `188` | BattleSubPlan::action_candidates | `MIG\_verify25\A\spec_188_BattleSubPlan__action_candidates.md` |
...
**[G20 명세간 공유사실] 135건**
  [188] BattleSubPlan::action_candidates                [R1] res 0x18 — 같은 (base,offset) 을 2가지 이름으로 부른다: len / truncate
  [188]`truncate` · [194]`len`
  [188] BattleSubPlan::action_candidates                [R2] res extend — 같은 이름이 오프셋 0x0 -> vec 원소 / 0x0 -> 원소[len..] 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [188]`0x0 -> vec 원소` · [194]`0x0 -> 원소[len..]`
  [188] BattleSubPlan::action_candidates                [R2] res push runaway — 같은 이름이 오프셋 0x0 -> vec 원소[0] / 0x0 -> 원소[len] 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [188]`0x0 -> vec 원소[0]` · [194]`0x0 -> 원소[len]`

**[G7 과열림] 11건**
  [188] BattleSubPlan::action_candidates                ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 A) `data.blackboard[1 - player.team]`(적 팀 인덱스) 를 is_recent_visible 첫 인자로 넘기는 이유(블랙보드 배열의 인덱스 규약)는 game_core 몫 — IR 사
  [188] BattleSubPlan::action_candidates                ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 B) blackboard 인덱스가 `1 - player.team`(적 팀 슬롯)인 이유(Blackboard 의미론) 미확인 — is_recent_visible 자식 명세(game_core) 소관
```
...
```json
{
  "round": 25, "batch": "A",
  "errors": [
    {"path": "/specs[188]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[188]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify25/A 만 써라**

```
MIG/_verify25/A/patch.json      ← 기계 적용분(필수)
MIG/_verify25/A/REPORT.md       ← 산문 보고서
MIG/_verify25/A/oracle/         ← 오라클 .rs · 출력 로그
```
