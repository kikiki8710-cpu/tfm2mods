...
# 25차 배치 K 도시에 — 담당 `190`~`191` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `c966cbd00622bae0` | 2026-09-16 01:39:13 |
| `_spec/specs20.json` (손이 닿는 정본) | `7ec124ea1e3922f3` | 2026-09-16 01:36:52 |

| `mkdossier.py`(이 파일을 만든 도구) | `232c166a31001480` | 2026-09-16 00:21:13 |

이 파일 생성 = `2026-09-16 01:45:35`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 K      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `190` | AttackNexusSubPlan::action_candidates | pub | 39 | 20 | 5 | 8 | 62 |
| `191` | EpicCheckSubPlan::action_candidates | pub | 38 | 15 | 6 | 8 | 58 |
...
| # | 이름 | 파일 |
|---|---|---|
| `190` | AttackNexusSubPlan::action_candidates | `MIG\_verify25\K\spec_190_AttackNexusSubPlan__action_candidates.md` |
| `191` | EpicCheckSubPlan::action_candidates | `MIG\_verify25\K\spec_191_EpicCheckSubPlan__action_candidates.md` |
...
**[G10 class 오분류] 4건**
  [190] AttackNexusSubPlan::action_candidates           class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  er_towers 136B 의 array 6칸 + 슬라이스 + Option 구성이 각각 무엇(레인 타워 6 · 추가 타워 · 넥서스?)인지 — game_core 경계. dloc 사슬로 타입만 확정.

**[G15 consts.kind 관측] 3건**
  [190] AttackNexusSubPlan::action_candidates           consts[3] kind=태그 NEG
  `meaning` 이 「5번째 usize 인자=5(rs:149) · PositionEvalPurpose 태그가 아님」라 적었는데 kind=태그

**[G18 logic↔표 상호참조] 8건**
  [191] EpicCheckSubPlan::action_candidates             logic/P2 logic 이 필드 `data.cache.game` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `game` 는 distruct 의 실재 필드명

**[G20 명세간 공유사실] 135건**
  [191] EpicCheckSubPlan::action_candidates             [R1] ObjectivePosture 0x8 — 같은 (base,offset) 을 2가지 이름으로 부른다: focus_enemy.0 / focus_enemy@Some.0
  [191]`focus_enemy@Some.0` · [192]`focus_enemy.0` · [197]`focus_enemy@Some.0` · [198]`focus_enemy@Some.0`
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 6708 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 3197 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 1315 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 4544 | ⚠**1643행만 IR 앵커**(`ev3`). 나머지 2901행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 25, "batch": "K",
  "errors": [
    {"path": "/specs[190]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[190]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only K --dry
```
...
## §6 출력 디렉터리 — **_verify25/K 만 써라**

```
MIG/_verify25/K/patch.json      ← 기계 적용분(필수)
MIG/_verify25/K/REPORT.md       ← 산문 보고서
MIG/_verify25/K/oracle/         ← 오라클 .rs · 출력 로그
```
