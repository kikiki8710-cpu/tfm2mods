...
# 25차 배치 J 도시에 — 담당 `203`~`203` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 J      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `203` | PassiveJunglePlan::next_plan | pub | 53 | 49 | 25 | 10 | 126 |
...
| # | 이름 | 파일 |
|---|---|---|
| `203` | PassiveJunglePlan::next_plan | `MIG\_verify25\J\spec_203_PassiveJunglePlan__next_plan.md` |
...
**[G12 src_line 대조] 26건**
  [203] PassiveJunglePlan::next_plan                    consts[21] src_line=316 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [285, 307, 369, 370, 417, 418, 525, 655]

**[G15 consts.kind 관측] 3건**
  [203] PassiveJunglePlan::next_plan                    consts[21] kind=임계 NEG
  `meaning` 이 「· ⚠`shl i8 line, 5`(=*32 타워 슬롯 stride, L307)는 임계 아님」라 적었는데 kind=임계

**[G18 logic↔표 상호참조] 8건**
  [203] PassiveJunglePlan::next_plan                    logic/P2 logic 이 필드 `ctx.pool` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `pool` 는 distruct 의 실재 필드명
```
...
```json
{
  "round": 25, "batch": "J",
  "errors": [
    {"path": "/specs[203]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[203]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only J --dry
```
...
## §6 출력 디렉터리 — **_verify25/J 만 써라**

```
MIG/_verify25/J/patch.json      ← 기계 적용분(필수)
MIG/_verify25/J/REPORT.md       ← 산문 보고서
MIG/_verify25/J/oracle/         ← 오라클 .rs · 출력 로그
```
