...
# 25차 배치 G 도시에 — 담당 `200`~`200` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 25 G      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `200` | SerpenPokeSubPlan::action_candidates | pub | 58 | 30 | 14 | 8 | 98 |
...
| # | 이름 | 파일 |
|---|---|---|
| `200` | SerpenPokeSubPlan::action_candidates | `MIG\_verify25\G\spec_200_SerpenPokeSubPlan__action_candidates.md` |
...
**[G18 logic↔표 상호참조] 8건**
  [200] SerpenPokeSubPlan::action_candidates            logic/P2 logic 이 필드 `t.tower` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `tower` 는 distruct 의 실재 필드명
```
...
```json
{
  "round": 25, "batch": "G",
  "errors": [
    {"path": "/specs[200]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[200]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 25 --only G --dry
```
...
## §6 출력 디렉터리 — **_verify25/G 만 써라**

```
MIG/_verify25/G/patch.json      ← 기계 적용분(필수)
MIG/_verify25/G/REPORT.md       ← 산문 보고서
MIG/_verify25/G/oracle/         ← 오라클 .rs · 출력 로그
```
