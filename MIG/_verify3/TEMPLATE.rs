...
//!     (`Effect` 도 전 필드 pub).
//!
//!     ⚠**TLS 메모는 이것만이 아니다** — `MapDef::camp_pos` 도 `CAMP_POS_MEMO`(thread_local RefCell)를 쓴다
//!     (5차 배치 D). 그 배치는 의심 → `MapDef.camps` 직독 우회 → 프로세스 분리 재측정 → **기각**
//!     (좌표가 정말 맵 설정 무관)까지 갔다. **이게 올바른 처리 순서다** — 의심만으로 "무관"이라 쓰지 말고,
//!     프로세스를 갈라 확인한 뒤에 판정하라. 새 함수를 재기 전에 `tlsscan.py` 로 훑는 것이 싸다.
//!
//!  ⑥ ★**default 챔프의 `stat_cached.move_speed == 1` 이다**(0 이 아니다 — 5차 배치 D).
//!     `distance / move_speed` 형 함수의 판별력을 죽이고, 0 으로 만들면 **div-by-zero 패닉**이다.
//!     실전 값을 넣어야 그 계열 함수가 갈린다.
//!
//!  ⑦ ★**입력 판별력이 없을 때 여는 일반 수법**(5차 배치 D·A 공통):
//!     `Entity` **41필드 전부 `pub`** + `cache.player_champion` 도 `pub` ⟹
//!     `ptr::read` 로 복제 → `ptr::write` 로 원하는 값 조립 → **캐시에 다시 꽂아** 판단함수에 먹인다.
//!     그리고 **`derive(Debug)` 가 private 필드까지 찍는다**(`DeathMatchBattle` 36필드 중 15개가 private)
//!     ⟹ 그 경우 `transmute` 가 아예 불필요하다. **`transmute` 전에 `{:?}` 를 먼저 시도하라.**
//!
...
//!
//! ## ★수법 — 5차에 각 배치가 따로 발명했다. **이제 전부 네 것이다**
//! 5차에는 아래 넷이 **한 배치씩만** 썼다(서로 몰랐다). 그래서 같은 일을 네 번 다시 발명했고,
//! 어떤 배치는 그게 없어서 「판별력 부재」로 닫았다. 여기 모아 둔다.
//!
//!  ⓐ ★**`std::mem::offset_of!` 일괄 대조**(5차 배치A) — `game_core` 필드가 거의 전부 `pub`
//!     (`Entity` 41필드 전부)이라 **오프셋 표 전체를 한 프로세스에서** 대조할 수 있다.
//!     배치 A 가 이걸로 `mem` 45행 + 구조체 크기 20건을 MISMATCH 0 으로 검증했다.
//!     ⚠**단 이건 `ev2` 가 아니라 `ev3` 이다** — 오프셋의 정본은 `tcx`(컴파일러)이고
//!     `offset_of!` 는 **같은 등급의 교차검증**이다(`mkspec3.ev_mem` 주석 참조).
//!     ⚠**적용 범위: `pub` 필드만.** private 필드에 쓰면 **E0616** 이 난다
//!     (`DeathMatchBattle` 은 36필드 중 18개가 private). 그 경우는 ⓑ(`derive(Debug)`)로 가라 —
//!     `{:?}` 는 private 까지 찍는다. **ⓐ 로 안 되면 곧장 ⓑ 다.**
//!
//!  ⓑ ★**`derive(Debug)` 로 private 필드 관통**(5차 배치D) — `{:?}` 가 private 까지 찍는다
//!     (`DeathMatchBattle` 36필드 중 15개가 private). **`transmute` 전에 `{:?}` 를 먼저 써라.**
//!
//!  ⓒ ★**구조체 통째 바이트 diff**(5차 배치C) — 호출 전후로 대상 구조체를 그대로 떠서
//!     (예: 6168B) 비교하면 **`writes` 표를 기계 대조**할 수 있다. 필드를 하나씩 읽는 것보다 강하다.
//!     ⚠`tick=0` 으로만 재면 **초기값과 같은 write 를 놓친다**(`mf_swap.1`). `set_tick`(pub)으로 0 아니게.
//!
//!  ⓓ ★**독립 재구현 대조**(5차 배치B) — 명세를 읽고 Rust 로 다시 구현해 난수 입력으로 돌린다
//!     (3000/3000). 오라클이 **명세→구현** 방향을 검증하므로, 구현→명세 방향인 진리표보다 강하다.
//!
//!  ⓔ **게임모드 게이트 직접 구동**(5차 배치C) — `SingleLaneGame::new` / `DeathMatchGame::new` 가
//!     `pub` 이라 `Moba` 아닌 모드의 분기를 직접 탈 수 있다.
//!
//!  ⓕ **같은 팀이면 `CastingTarget::check` 가 먼저 거부한다**(5차 배치C) — `can_enemy_hit_objective`
//!     류 프로브가 전 케이스 false 로 죽는 흔한 원인이다.
//!
