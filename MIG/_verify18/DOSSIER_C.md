...
# 18차 배치 C 도시에 — 담당 `67`~`71` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 18 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `67` | v24_objective_setup_lane_pressure_ready | pub | 19 | 13 | 8 | 7 | 40 |
| `68` | base_sub_goal | pub | 13 | 7 | 2 | 5 | 21 |
| `69` | try_engage_dive | in:game_ai | 19 | 8 | 4 | 6 | 31 |
| `70` | tower_dive_is_viable | pub | 29 | 14 | 8 | 7 | 49 |
| `71` | should_recall_to_shop | in:game_ai | 24 | 6 | 2 | 7 | 29 |
...
| # | 이름 | 파일 |
|---|---|---|
| `67` | v24_objective_setup_lane_pressure_ready | `MIG\_verify18\C\spec_67_v24_objective_setup_lane_pressure_ready.md` |
| `68` | base_sub_goal | `MIG\_verify18\C\spec_68_base_sub_goal.md` |
| `69` | try_engage_dive | `MIG\_verify18\C\spec_69_try_engage_dive.md` |
| `70` | tower_dive_is_viable | `MIG\_verify18\C\spec_70_tower_dive_is_viable.md` |
| `71` | should_recall_to_shop | `MIG\_verify18\C\spec_71_should_recall_to_shop.md` |
...
**이 배치 몫 = 12건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G1 자기모순] 1건**
  [69] try_engage_dive                                 constants[0] src_line=112 인데 meaning 은 같은 파일 engage.rs:973 를 가리킨다
  `shl i64 %tps, 2` = tps*4 (dive_rejoin_cd, engage.rs:973). 소스 수준 값은 folded_from 참조

**[G10 class 오분류] 3건**
  [68] base_sub_goal                                   class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  evaluate_gank_opportunity_with_score 에서도 같은 패턴. Blackboard 갱신 코드 미탐색이라 의미(적 팀 판에 '적이 우리에게 보였던 시각' 이 기록되는지) 미확정

**[G12 src_line 대조] 5건**
  [67] v24_objective_setup_lane_pressure_ready         consts[12] src_line=75 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [74]
  [70] tower_dive_is_viable                            consts[1] src_line=959 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [960, 962, 1017, 1018]

**[G16 params.role 대조] 13건**
  [67] v24_objective_setup_lane_pressure_ready         sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 7개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5, 6]. 규약 정본 = 07·15
  [68] base_sub_goal                                   sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 4개 ⟹ i 는 [1, 2, 3, 4] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3]. 규약 정본 = 07·15
  [69] try_engage_dive                                 sig.params[0] P2 sret 규약 — params[0] 가 sret out-ptr 인데 `(sret` 표식이 없다
  IR m13.ll:34298 첫 인자 `ptr dead_on_unwind noalias noundef nonnull writable writeonly align 8 ` 가 반환 out-ptr 인데 params[0].name=`sret`. 규약 = name 을 `(sre
  [69] try_engage_dive                                 sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 8개(sret 행 없음) ⟹ 8행이어야 하는데 9행. 현재 i = [0, 1, 2, 3, 4, 5, 6, 7, 8]

**[G19 knobs.value 값] 4건**
  [70] tower_dive_is_viable                            knobs[5] 주장한 값을 IR 관측에서 못 찾는다
  value=40000000001 · 관측(창)=[0, 5, 8, 16, 24, 32, 64, 192, 296]
  [70] tower_dive_is_viable                            knobs[6] 주장한 값을 IR 관측에서 못 찾는다
  value=22500000001 · 관측(창)=[0, 5, 8, 16, 24, 32, 40, 48, 64, 128]

**[G20 명세간 공유사실] 12건**
  [71] should_recall_to_shop                           [R1] GoalData 0x88 — 같은 (base,offset) 을 2가지 이름으로 부른다: epic.epic_enemy_tick / epic.epic_enemy_tick · epic.stance@0xa8 · serpen.stance@0xe0
  [71]`epic.epic_enemy_tick` · [74]`epic.epic_enemy_tick · epic.stance@0xa8 · serpen.stance@0xe0`

**[G7 과열림] 3건**
  [70] tower_dive_is_viable                            ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  check_kill_die_tick 내부(m?)·resolve_fight_stake 내부·max_range_cached·is_recent_visible·is_enemy_well_danger·is_unreasonabl
```
...
```json
{
  "round": 18, "batch": "C",
  "errors": [
    {"path": "/specs[67]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[67]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 18 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify18/C 만 써라**

```
MIG/_verify18/C/patch.json      ← 기계 적용분(필수)
MIG/_verify18/C/REPORT.md       ← 산문 보고서
MIG/_verify18/C/oracle/         ← 오라클 .rs · 출력 로그
```
