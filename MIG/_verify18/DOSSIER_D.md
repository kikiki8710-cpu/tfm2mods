...
# 18차 배치 D 도시에 — 담당 `72`~`76` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 18 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `72` | SerpenHuntAndPokePlan::is_end | pub | 19 | 7 | 2 | 5 | 25 |
| `73` | wave_priority_clearer_position | in:game_ai | 11 | 12 | 1 | 4 | 23 |
| `74` | GoalData::update | pub | 29 | 8 | 3 | 5 | 39 |
| `75` | SerpenStanceData::update_plan | pub | 48 | 22 | 11 | 8 | 81 |
| `76` | EpicStanceData::update_plan | pub | 36 | 15 | 9 | 7 | 60 |
...
| # | 이름 | 파일 |
|---|---|---|
| `72` | SerpenHuntAndPokePlan::is_end | `MIG\_verify18\D\spec_72_SerpenHuntAndPokePlan__is_end.md` |
| `73` | wave_priority_clearer_position | `MIG\_verify18\D\spec_73_wave_priority_clearer_position.md` |
| `74` | GoalData::update | `MIG\_verify18\D\spec_74_GoalData__update.md` |
| `75` | SerpenStanceData::update_plan | `MIG\_verify18\D\spec_75_SerpenStanceData__update_plan.md` |
| `76` | EpicStanceData::update_plan | `MIG\_verify18\D\spec_76_EpicStanceData__update_plan.md` |
...
**[G16 params.role 대조] 13건**
  [74] GoalData::update                                sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 6개 ⟹ i 는 [1, 2, 3, 4, 5, 6] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5]. 규약 정본 = 07·15
  [75] SerpenStanceData::update_plan                   sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 7개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5, 6]. 규약 정본 = 07·15
  [76] EpicStanceData::update_plan                     sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 7개 ⟹ i 는 [1, 2, 3, 4, 5, 6, 7] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5, 6]. 규약 정본 = 07·15

**[G20 명세간 공유사실] 12건**
  [75] SerpenStanceData::update_plan                   [R2] MapDef region_dist[][] — 같은 이름이 오프셋 0x54e8 / 0x5510 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [75]`0x54e8` · [76]`0x5510`

**[G5 sig 정본 대조] 4건**
  [75] SerpenStanceData::update_plan                   params[2]((rnd)) 의 가변성이 tcx 와 다르다 — 명세 `ptr` / tcx `&mut rand::rngs::std::StdRng`
  [76] EpicStanceData::update_plan                     params[2]((rnd)) 의 가변성이 tcx 와 다르다 — 명세 `ptr` / tcx `&mut rand::rngs::std::StdRng`
  [76] EpicStanceData::update_plan                     params[6]((debug)) 의 가변성이 tcx 와 다르다 — 명세 `ptr` / tcx `&mut game_core::DebugFrameData`
```
...
```json
{
  "round": 18, "batch": "D",
  "errors": [
    {"path": "/specs[72]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[72]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 18 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify18/D 만 써라**

```
MIG/_verify18/D/patch.json      ← 기계 적용분(필수)
MIG/_verify18/D/REPORT.md       ← 산문 보고서
MIG/_verify18/D/oracle/         ← 오라클 .rs · 출력 로그
```
