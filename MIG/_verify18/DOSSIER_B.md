...
# 18차 배치 B 도시에 — 담당 `62`~`66` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `ececdafa0a5606fe` | 2026-09-13 18:14:26 |
| `_spec/specs20.json` (손이 닿는 정본) | `08a0f4cf4707c8c3` | 2026-09-13 18:14:07 |

| `mkdossier.py`(이 파일을 만든 도구) | `81d9294dd98fa641` | 2026-09-13 12:49:42 |

이 파일 생성 = `2026-09-13 18:15:37`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 18 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `62` | v23_enemy_object_pressure | pub | 12 | 8 | 3 | 4 | 22 |
| `63` | should_keep_object_for_contested_wave_priority | in:game_ai::plan_legacy::team_plan | 7 | 9 | 2 | 4 | 18 |
| `64` | v25_objective_splitter_can_stay | pub | 9 | 9 | 4 | 5 | 21 |
| `65` | check_epic_kill_time_with_hp | in:game_ai::goal_data | 9 | 5 | 2 | 5 | 16 |
| `66` | evaluate_gank_opportunity_with_score | pub | 19 | 29 | 10 | 7 | 58 |
...
| # | 이름 | 파일 |
|---|---|---|
| `62` | v23_enemy_object_pressure | `MIG\_verify18\B\spec_62_v23_enemy_object_pressure.md` |
| `63` | should_keep_object_for_contested_wave_priority | `MIG\_verify18\B\spec_63_should_keep_object_for_contested_wave_priority.md` |
| `64` | v25_objective_splitter_can_stay | `MIG\_verify18\B\spec_64_v25_objective_splitter_can_stay.md` |
| `65` | check_epic_kill_time_with_hp | `MIG\_verify18\B\spec_65_check_epic_kill_time_with_hp.md` |
| `66` | evaluate_gank_opportunity_with_score | `MIG\_verify18\B\spec_66_evaluate_gank_opportunity_with_score.md` |
...
**이 배치 몫 = 11건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G10 class 오분류] 3건**
  [66] evaluate_gank_opportunity_with_score            class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  ) (m01.ll:30760). 적팀 판 last_visible 을 자기 시점 가시성으로 쓰는 것이 의도인지는 Blackboard 갱신 코드(미탐색)로만 확정 가능 — IR 로는 인덱스 사실만 확정

**[G12 src_line 대조] 5건**
  [66] evaluate_gank_opportunity_with_score            consts[1] src_line=713 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [715, 716, 759]

**[G15 consts.kind 관측] 3건**
  [63] should_keep_object_for_contested_wave_priority  consts[5] kind=임계 NEG
  `meaning` 이 「팀 수 — player.info.team 의 bounds check 상한(임계 아님)」라 적었는데 kind=임계

**[G16 params.role 대조] 13건**
  [65] check_epic_kill_time_with_hp                    sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 5개(sret보정 -0) > params 4개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[0], 덮은 레지스터 ['%0']). 자리 매핑 보류
  [66] evaluate_gank_opportunity_with_score            sig.params[0] P2 sret 규약 위반 — sret out-ptr 이 `params[]` 에 없다
  IR m04.ll:62981 첫 인자 `ptr dead_on_unwind noalias noundef writable writeonly sret([12 x i8]) ` 가 반환 out-ptr 인데 params[0] 은 `rnd`. 8차 확정 규약 = sret 를 par

**[G18 logic↔표 상호참조] 4건**
  [62] v23_enemy_object_pressure                       logic/P1 logic 이 `+0x40` 를 인용하는데 mem 표에 그 오프셋이 없다
  let moba = game.get_game_mode()[vtable+0x40].as_moba().unwrap()
  [62] v23_enemy_object_pressure                       logic/P1 logic 이 `+0x1f0` 를 인용하는데 mem 표에 그 오프셋이 없다
  let Some(ent) = id_opt.and_then(|&id| game.get_entity_by_id(id)[vtable+0x1f0]) else
  [62] v23_enemy_object_pressure                       logic/P1 logic 이 `+0xf8` 를 인용하는데 mem 표에 그 오프셋이 없다
  if !game.is_visible(player.info.team, ent.id)[vtable+0xf8]
  [62] v23_enemy_object_pressure                       logic/P2 logic 이 필드 `data.cache.game` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `game` 는 distruct 의 실재 필드명

**[G20 명세간 공유사실] 12건**
  [63] should_keep_object_for_contested_wave_priority  [R4] 건 강   판 정   h p %   하 한 — 같은 노브가 값 39 / 40 를 동시에 갖는다
  [63]`39` · [64]`40`

**[G7 과열림] 3건**
  [66] evaluate_gank_opportunity_with_score            ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  blackboard 인덱스 비대칭: closure#0 의 is_recent_visible 은 blackboard[1-team](적 팀 판) (m04.ll:66367~66370), line_allies 의 in_big
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 1611 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 805 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 435 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 1083 | ⚠**204행만 IR 앵커**(`ev3`). 나머지 879행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 18, "batch": "B",
  "errors": [
    {"path": "/specs[62]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[62]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 18 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify18/B 만 써라**

```
MIG/_verify18/B/patch.json      ← 기계 적용분(필수)
MIG/_verify18/B/REPORT.md       ← 산문 보고서
MIG/_verify18/B/oracle/         ← 오라클 .rs · 출력 로그
```
