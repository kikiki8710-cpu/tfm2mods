...
# 15차 배치 A 도시에 — 담당 `20`~`24` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 15 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `20` | v27_active_objective_discipline | pub | 18 | 6 | 2 | 4 | 24 |
| `21` | upgrade_item | pub | 22 | 5 | 4 | 5 | 30 |
| `22` | can_tower_focused | pub | 24 | 8 | 4 | 6 | 36 |
| `23` | buy_item | pub | 20 | 20 | 6 | 6 | 45 |
| `24` | v23_healthy_allies_near_point | pub | 7 | 3 | 2 | 3 | 12 |
...
| # | 이름 | 파일 |
|---|---|---|
| `20` | v27_active_objective_discipline | `MIG\_verify15\A\spec_20_v27_active_objective_discipline.md` |
| `21` | upgrade_item | `MIG\_verify15\A\spec_21_upgrade_item.md` |
| `22` | can_tower_focused | `MIG\_verify15\A\spec_22_can_tower_focused.md` |
| `23` | buy_item | `MIG\_verify15\A\spec_23_buy_item.md` |
| `24` | v23_healthy_allies_near_point | `MIG\_verify15\A\spec_24_v23_healthy_allies_near_point.md` |
...
**[G1 자기모순] 2건**
  [20] v27_active_objective_discipline                 constants[1] src_line=490 인데 meaning 은 같은 파일 objective_discipline.rs:326 를 가리킨다
  GameMode 태그 0 = Moba. 아니면 unwrap_failed(v27_objective_entity 인라인, objective_discipline.rs:326/328)

**[G12 src_line 대조] 10건**
  [22] can_tower_focused                               consts[6] src_line=25 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [17, 52]
  [22] can_tower_focused                               consts[7] src_line=21 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [17, 52]

**[G16 params.role 대조] 10건**
  [21] upgrade_item                                    sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 5개(sret 행 있음) ⟹ 6행이어야 하는데 7행. 현재 i = [0, 1, 2, 3, 4, 5, 6]
  [22] can_tower_focused                               sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 5개 ⟹ i 는 [1, 2, 3, 4, 5] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4]. 규약 정본 = 07·15
  [23] buy_item                                        sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 5개(sret 행 없음) ⟹ 5행이어야 하는데 6행. 현재 i = [0, 1, 2, 3, 4, 5]
  [24] v23_healthy_allies_near_point                   sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 6개 ⟹ i 는 [1, 2, 3, 4, 5, 6] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5]. 규약 정본 = 07·15

**[G5 sig 정본 대조] 2건**
  [21] upgrade_item                                    sig.tcx 인자 5개 vs params 6개 — 개수가 다르다
  &mut game_ai::AgentVerHamster | &mut rand::rngs::std::StdRng | &game_core::PlayerState | &dyn [Binder { value: Trait(game_c | &game_core::GameContext
  [23] buy_item                                        sig.tcx 인자 5개 vs params 6개 — 개수가 다르다
  &mut game_ai::AgentVerHamster | &mut rand::rngs::std::StdRng | &game_core::PlayerState | &dyn [Binder { value: Trait(game_c | &game_core::GameContext
```
...
```json
{
  "round": 15, "batch": "A",
  "errors": [
    {"path": "/specs[20]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[20]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 15 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify15/A 만 써라**

```
MIG/_verify15/A/patch.json      ← 기계 적용분(필수)
MIG/_verify15/A/REPORT.md       ← 산문 보고서
MIG/_verify15/A/oracle/         ← 오라클 .rs · 출력 로그
```
