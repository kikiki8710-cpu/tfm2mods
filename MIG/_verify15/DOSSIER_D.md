...
# 15차 배치 D 도시에 — 담당 `35`~`39` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `03d55ff89412a202` | 2026-09-13 07:57:52 |
| `_spec/specs20.json` (손이 닿는 정본) | `7f19696a4a246f02` | 2026-09-13 07:57:20 |

| `mkdossier.py`(이 파일을 만든 도구) | `501a2f51605bec42` | 2026-09-13 08:00:10 |

이 파일 생성 = `2026-09-13 08:00:49`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 15 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `35` | resolve_fight_stake | in:game_ai | 14 | 4 | 3 | 4 | 21 |
| `36` | calculate_nexus_defense_count | pub | 19 | 14 | 5 | 5 | 38 |
| `37` | i_am_chosen_defender | pub | 26 | 19 | 5 | 5 | 50 |
| `38` | can_tower_focused_when_battle | pub | 19 | 7 | 3 | 2 | 29 |
| `39` | is_wave_priority_start_line | in:game_ai | 11 | 10 | 2 | 2 | 23 |
...
| 파일 | 줄 | sha256[:16] | 왜 |
|---|---|---|---|
| `MIG\METHOD_MAP.md` | 213 | `f4614b98b93915e1` | ★**「불가」를 쓰기 전에 §0 라우팅표를 보라.** 09-11 에 「원리적 불가」가 7건 뒤집혔고 원인은 매번 「가진 재료의 한계를 문제의 한계로 착각」이었다. |
...
| `MIG\TOOLS.md` | 266 | `e9ad51c9c0fe778b` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `35` | resolve_fight_stake | `MIG\_verify15\D\spec_35_resolve_fight_stake.md` |
| `36` | calculate_nexus_defense_count | `MIG\_verify15\D\spec_36_calculate_nexus_defense_count.md` |
| `37` | i_am_chosen_defender | `MIG\_verify15\D\spec_37_i_am_chosen_defender.md` |
| `38` | can_tower_focused_when_battle | `MIG\_verify15\D\spec_38_can_tower_focused_when_battle.md` |
| `39` | is_wave_priority_start_line | `MIG\_verify15\D\spec_39_is_wave_priority_start_line.md` |
...
**이 배치 몫 = 27건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G10 class 오분류] 6건**
  [35] resolve_fight_stake                             class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  0.ll:39915) 내부에서 baseline 이 net_value/line 에 어떻게 반영되는지는 안 읽었다 — 여기선 '기준선 인자로 abandon.net_value 를 넘긴다' 는 사실만 확정
  [35] resolve_fight_stake                             class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  #1(597:52) 로 tcx 에 있으나 IR 에는 인라인돼 별도 define 이 없다(fnparts: 서브프로그램 73 vs define 5). 극성·순서는 본문 phi(%113/%114)로 확정
  [37] i_am_chosen_defender                            class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  ected_damage_target·is_area·Blackboard::is_recent_visible 내부는 안 읽었다(별도 함수·game_core) — 여기선 호출 계약(인자·부호 판정)만 확정
  [37] i_am_chosen_defender                            class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  → i64@0 → u64@8 → u64@24)는 IR 비교 순서로 복원한 것 — 메모리 오프셋은 rustc 재배치 순이라 소스 튜플 선언 순과 다를 수 있으나 비교 순서 = 선언 순이라 논리는 확정

**[G12 src_line 대조] 10건**
  [36] calculate_nexus_defense_count                   consts[10] src_line=2362 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2329, 2337, 2341, 2368]
  [36] calculate_nexus_defense_count                   consts[11] src_line=2364 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2338]
  [36] calculate_nexus_defense_count                   consts[12] src_line=2364 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2329, 2330, 2343, 2369]
  [37] i_am_chosen_defender                            consts[4] src_line=317 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [315, 316, 323, 329, 353, 364]
  [37] i_am_chosen_defender                            consts[5] src_line=318 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [298, 308, 315, 360]
  [37] i_am_chosen_defender                            consts[16] src_line=464 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [301, 318]
  [37] i_am_chosen_defender                            consts[17] src_line=465 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [353]
  [37] i_am_chosen_defender                            consts[18] src_line=474 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [298, 308, 315, 360]

**[G13 knobs.where 대조] 20건**
  [35] resolve_fight_stake                             knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m10.ll:42385  인용=icmp ult i64 %1, 2
  [36] calculate_nexus_defense_count                   knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m13.ll:56492  인용=icmp ult i64 %121, 22500000001
  [36] calculate_nexus_defense_count                   knobs[1] 인용한 명령이 그 줄(±2)에 없다
  m13.ll:56592  인용=icmp ult i64 %156, 90000000001
  [36] calculate_nexus_defense_count                   knobs[3] 인용한 명령이 그 줄(±2)에 없다
  m13.ll:56942  인용=icmp ugt i64 %282, 5
  [36] calculate_nexus_defense_count                   knobs[4] 인용한 명령이 그 줄(±2)에 없다
  m12.ll:42952  인용=add nsw i8 %30, -3
  [37] i_am_chosen_defender                            knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:59375  인용=icmp ult i64 %272, 90000000001
  [37] i_am_chosen_defender                            knobs[1] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:59431  인용=add nsw i8 %307, -3
  [37] i_am_chosen_defender                            knobs[2] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:58991  인용=llvm.umax.i64(%99, 1)
  [37] i_am_chosen_defender                            knobs[3] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:15733  인용=icmp ugt %50, 2
  [38] can_tower_focused_when_battle                   knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m07.ll:51201  인용=add i64 %5, 15000
  [39] is_wave_priority_start_line                     knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m15.ll:52887  인용=icmp ugt i64 %21, 8

**[G16 params.role 대조] 10건**
  [35] resolve_fight_stake                             sig.params[0] P2 sret 규약 위반 — sret out-ptr 이 `params[]` 에 없다
  IR m10.ll:42357 첫 인자 `ptr dead_on_unwind noalias noundef writable sret([64 x i8]) align 8 ca` 가 반환 out-ptr 인데 params[0] 은 `version`. 8차 확정 규약 = sret 를
  [35] resolve_fight_stake                             sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 14개(sret보정 -1) > params 11개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[], 덮은 레지스터 []). 자리 매핑 보류
  [37] i_am_chosen_defender                            sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 5개(sret보정 -0) > params 4개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[], 덮은 레지스터 []). 자리 매핑 보류

**[G7 과열림] 4건**
  [37] i_am_chosen_defender                            ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  expected_trade_net_hp(battle.rs:2493)·Effect::expected_damage_target·is_area·Blackboard::is_recent_visible 내부는 안 읽었다(별도
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 785 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 351 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 228 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 530 | ⚠**86행만 IR 앵커**(`ev3`). 나머지 444행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 15, "batch": "D",
  "errors": [
    {"path": "/specs[35]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[35]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 15 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify15/D 만 써라**

```
MIG/_verify15/D/patch.json      ← 기계 적용분(필수)
MIG/_verify15/D/REPORT.md       ← 산문 보고서
MIG/_verify15/D/oracle/         ← 오라클 .rs · 출력 로그
```
