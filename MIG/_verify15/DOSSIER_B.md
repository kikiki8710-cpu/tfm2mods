...
# 15차 배치 B 도시에 — 담당 `25`~`29` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 15 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `25` | v22_visible_enemy_is_runaway_threat | in:game_ai | 11 | 5 | 3 | 4 | 18 |
| `26` | v23_objective_setup_pressure_line | pub | 10 | 10 | 5 | 5 | 25 |
| `27` | nexus_under_direct_attack | pub | 18 | 4 | 2 | 4 | 23 |
| `28` | v21_should_defer_support_target | in:game_ai | 18 | 12 | 7 | 5 | 36 |
| `29` | v23_recent_visible_enemies_near_point | pub | 10 | 3 | 3 | 4 | 16 |
...
| # | 이름 | 파일 |
|---|---|---|
| `25` | v22_visible_enemy_is_runaway_threat | `MIG\_verify15\B\spec_25_v22_visible_enemy_is_runaway_threat.md` |
| `26` | v23_objective_setup_pressure_line | `MIG\_verify15\B\spec_26_v23_objective_setup_pressure_line.md` |
| `27` | nexus_under_direct_attack | `MIG\_verify15\B\spec_27_nexus_under_direct_attack.md` |
| `28` | v21_should_defer_support_target | `MIG\_verify15\B\spec_28_v21_should_defer_support_target.md` |
| `29` | v23_recent_visible_enemies_near_point | `MIG\_verify15\B\spec_29_v23_recent_visible_enemies_near_point.md` |
...
**[G10 class 오분류] 6건**
  [25] v22_visible_enemy_is_runaway_threat             class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  었다 — '적 enemy 가 target=champ 에 대해 갖는 최대 사거리'라는 것은 인자명(data/champ/target, battle.rs:2332 DILocalVariable)으로만 확정

**[G13 knobs.where 대조] 20건**
  [25] v22_visible_enemy_is_runaway_threat             knobs[2] 인용한 명령이 그 줄(±2)에 없다
  g07.ll:157005  인용=add i64 %24, 120
  [27] nexus_under_direct_attack                       knobs[1] 인용한 명령이 그 줄(±2)에 없다
  g07.ll:157005  인용=add i64 %24, 120
  [28] v21_should_defer_support_target                 knobs[6] 인용한 명령이 그 줄(±2)에 없다
  g07.ll:157005  인용=add i64 %24, 120
  [29] v23_recent_visible_enemies_near_point           knobs[2] 인용한 명령이 그 줄(±2)에 없다
  g07.ll:157005  인용=add i64 %24, 120

**[G16 params.role 대조] 10건**
  [26] v23_objective_setup_pressure_line               sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 4개(sret보정 -0) > params 3개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[], 덮은 레지스터 []). 자리 매핑 보류

**[G20 명세간 공유사실] 18건**
  [26] v23_objective_setup_pressure_line               [R1] AbstractGameWithCache 0x21c0 — 같은 (base,offset) 을 4가지 이름으로 부른다: bottom_lead / mid_lead / top_lead / top_lead/mid_lead/bottom_lead [][]
  [26]`top_lead/mid_lead/bottom_lead` · [36]`top_lead/mid_lead/bottom_lead [][]`
  [27] nexus_under_direct_attack                       [R1] Entity 0x90 — 같은 (base,offset) 을 2가지 이름으로 부른다: ty.Minion.info.nearest_enemy.Some.0 / ty@Minion.info.nearest_enemy@Some.0
  [27]`ty@Minion.info.nearest_enemy@Some.0` · [34]`ty.Minion.info.nearest_enemy.Some.0` · [36]`ty@Minion.info.nearest_enemy@Some.0` · [37]`ty@Minion.inf

**[G7 과열림] 4건**
  [25] v22_visible_enemy_is_runaway_threat             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible 의 vtable 슬롯 +0xf8 = is_visible / +0x150 = get_player_by_champion_id / +0x28 = tick 은 divtable(Abstract
  [25] v22_visible_enemy_is_runaway_threat             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  L1169 의 `A && B` 표기(is_recent_visible 를 부정 조건으로 조기 반환하는지, 하나의 && 식인지)는 column 정보 부재로 표기 불가 — 동작(가시 아니면 false)은 확정
```
...
```json
{
  "round": 15, "batch": "B",
  "errors": [
    {"path": "/specs[25]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[25]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 15 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify15/B 만 써라**

```
MIG/_verify15/B/patch.json      ← 기계 적용분(필수)
MIG/_verify15/B/REPORT.md       ← 산문 보고서
MIG/_verify15/B/oracle/         ← 오라클 .rs · 출력 로그
```
