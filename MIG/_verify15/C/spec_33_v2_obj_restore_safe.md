...
### `33` v2_obj_restore_safe — 오브젝트 후 회복(restore)이 안전한지: 150000 안 '최근 가시' 적 챔프가 없거나, 있어도 2초 안에 안 죽으면 true

| 항목 | 값 |
|---|---|
| id | `handler__v2_obj_restore_safe` |
| 심볼 | `_RNvMNtNtCshdEBA0ozCnw_7game_ai11plan_legacy7handlerNtB2_17LegacyPlanHandler19v2_obj_restore_safe` |
| 소스 | `game-ai\src\plan_legacy\handler.rs:501` |
| IR | `m13.ll` 11386~11527행 |
| 경로·가시성 | `game_ai::plan_legacy::handler::LegacyPlanHandler::v2_obj_restore_safe` · **in:game_ai::plan_legacy::handler** |
| 계층 | 플랜 핸들러 |
| exe | `14986944` (None) · None바이트 · None명령 |
| 라운드 | 기준 `r7` · 통과 1회 |

**시그니처(tcx 정본, ev3)**
```rust
fn(&game_ai::plan_legacy::handler::LegacyPlanHandler, usize, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &game_core::OperationData, &mut game_core::DebugFrameData) -> bool
```
...
| # | i | 이름 | 타입 | 역할 | ev |
|---|---|---|---|---|---|
| 0 | 0 | self | &LegacyPlanHandler | DISubprogram arg1 은 self 이나 본문에서 전혀 안 읽혀 internal fastcc 승격 시 제거됨(IR 인자는 version/rnd/player/data/debug 5개) | 4 |
| 1 | 1 | version | usize | 본문 분기엔 안 쓰임. 스택(%10)에 저장해 check_kill_die_tick 와 필터 클로저(is_ignored_well_enemy) 에 전달 | 4 |
| 2 | 2 | rnd | &mut StdRng(320B, align16) | check_kill_die_tick 에만 전달 | 4 |
| 3 | 3 | player | &PlayerState(2528B) | info.team(+0x930)·info.position(+0x9c0) 읽고, judger 로 check_kill_die_tick 에 전달 | 4 |
| 4 | 4 | data | &OperationData(24B) | +0x0 cache / +0x8 context / +0x10 blackboard | 4 |
| 5 | 5 | debug | &mut DebugFrameData | check_kill_die_tick 에만 전달 | 4 |
</details>
...
[L513] my_die = fight_check::check_kill_die_tick(
            ...
            enemy  = near_enemies,                                  // memcpy 로 소유권 이동
[L514]      towers = Vec::new_in(pool),                             // 항상 빈 벡터 (ptr=dangling 8, cap 0, len 0)
            debug)
[L515] return my_die > tps * 2                                     // tps = context.setting.tick_per_second(+0x12f8), shl 1
[L516] }

※ 즉 '2초(tps*2) 안에 죽지 않는다' 고 예측되면 회복(restore) 을 허용. 정확히 tps*2 틱이면 false(ugt).
※ 부작용: rnd/debug 는 check_kill_die_tick 안에서 변할 수 있음. near_enemies 는 bump pool 에 할당·이동.
```

**`mem` 메모리 접근 14건** — ★`ev` 상한은 **3**이다(오프셋 주장의 최강 근거가 tcx 라서).
| # | 베이스 | 오프셋 | 이름 | 방향 | 근거 | ev | chk |
|---|---|---|---|---|---|---|---|
| 0 | PlayerState | 0x930 | info.team | r | gep 2352. >=2 → panic_bounds_check(len 2). 클로저 안에서도 1-team 으로 다시 읽음 (m13.ll:58236) | 4 | OK |
| 1 | PlayerState | 0x9c0 | info.position (Position i32 태그) | r | gep 2496, as_index(entity.rs:581) → player_champion[team][position] | 4 | OK |
| 2 | OperationData | 0x0 | cache | r | &AbstractGameWithCache | 4 | OK |
| 3 | OperationData | 0x8 | context | r | &GameContext. pool(+0x0)·setting(+0x8) | 4 | OK |
| 4 | OperationData | 0x10 | blackboard | r | &[Blackboard;2]. 클로저 캡처 → [1-team].is_recent_visible (aux m13.ll:58255~58262, Blackboard stride = 그 gep 구조체 크기) | 4 | OK |
| 5 | AbstractGameWithCache | 0x1e0 | player_champion [[Option<&Entity>;5];2] | r | gep 480. [team][position] = champ (null → return true). [1-team] 5칸 = iter_champions 의 슬라이스 (m13.ll:11407~11419) | 4 | OK |
| 6 | AbstractGameWithCache | 0x0 | game (dyn AbstractGame data ptr) | r | 클로저 캡처 → is_recent_visible 인자 | 4 | OK |
| 7 | AbstractGameWithCache | 0x8 | game (dyn AbstractGame vtable ptr) | r | 클로저 캡처 | 4 | OK |
| 8 | GameContext | 0x0 | pool: &Bump | r | near_enemies 와 빈 towers Vec 의 할당자 (m13.ll:11449~11450) | 4 | OK |
| 9 | GameContext | 0x8 | setting: &GameSetting | r |  | 4 | OK |
| 10 | GameSetting | 0x12f8 | tick_per_second | r | gep 4856 = tps. 임계 tps*2 (m13.ll:11512~11514) | 4 | OK |
| 11 | Vec<&Entity>(bumpalo, 32B) | 0x18 | len | r | near_enemies.is_empty() → true 반환 (m13.ll:11454) | 4 | OK |
| 12 | Entity | 0x660 | x | r | aux 클로저: gep 1632. 적 e 와 내 champ 의 x (distance_sq 인라인, entity.rs:2158) | 4 | OK |
| 13 | Entity | 0x668 | y | r | aux 클로저: gep 1640 | 4 | OK |

**`consts` 상수 4건** — `src_line` 은 **G12 가 IR 사슬과 대조**한다.
| # | 값 | src_line | 종류 | 뜻 | ev | folded_from |
|---|---|---|---|---|---|---|
| 0 | 2 | 504 | 임계 | player_champion / blackboard 의 팀 배열 길이 2 — panic_bounds_check(len 2) (본체 m13.ll:11400, aux 58270) | 4 |  |
| 1 | 1 | 506 | 계수 | enemy team = 1 - team (본체 %26, aux %35). 또 tps*2 가 `shl i64 %tps, 1` 로 접혀 리터럴 2 없음 (handler.rs:515) | 4 |  |
| 2 | 1 | 515 | 계수 | 안전 임계 tps*2 (=2초). `shl i64 %59, 1` (m13.ll:11514). my_die > tps*2 이면 안전 | 4 | 2 |
| 3 | 22500000001 | 507 | 미상 | aux 클로저: 150000^2 + 1. distance_sq(e, champ) < 이 값 ⇔ 거리 <= 150000 (약 4.7셀) 인 적만 near_enemies 후보 (m13.ll:58254) | 4 |  |

**`knobs` 조정점 2건** — `where` 는 **G13 이 그 줄(±2)에 인용 명령이 있는지** 본다.
| # | 무엇 | 어디 | 값 | 효과 | ev | src |
|---|---|---|---|---|---|---|
| 0 | 회복 안전 시간창 | handler.rs:515 (IR m13.ll:11514 `shl i64 %59, 1` → 11515 `icmp ugt %55, %60`) | 2 | tps*2 틱 = 2초. 올리면 더 오래 살아남아야 안전으로 보아 회복을 덜 하고(보수적), 내리면 위험해도 회복을 시도한다. 리터럴 2 는 shl 1 로 접혀 IR 에 없다 | 4 | 기존 |
| 1 | 근처 적 탐색 반경 | handler.rs:507 — aux m13.ll:58254 (call_mut 심) | 22500000001 | 150000^2+1. 내 챔프에서 150000 이내(제곱비교 <) 의 적만 사망 예측에 포함. 올리면 더 먼 적까지 위협으로 세어 회복이 더 자주 '불안전' 판정 | 4 | 기존 |
...
| 9 | position | game_core::PlayerAiContext::<'a, 'b, 'r>::position | pub | fn(&game_core::PlayerAiContext<'a/#0, 'b/#1, 'r/#2>) -> game_core::Position | game-core\src\mod_ai.rs:587 | True | True | 4 | ⚠미확정 — tcx 3크레이트에 같은 leaf 가 2개. IR 범위의 직접 호출 심볼과는 안 맞는다(`logic` 산문에서 긁혔거나 인라인·접힘). 상위 2개 |
| 10 | position | game_core::transfer::TeamRosterPlanningContext::position | pub | fn(&game_core::transfer::TeamRosterPlanningContext, game_core::Position) -> std::option::Option<&game_core::transfer::PositionPlanningContext> | game-core\src\transfer\roster_context.rs:234 | False | False | 4 | ⚠미확정 — tcx 3크레이트에 같은 leaf 가 2개. IR 범위의 직접 호출 심볼과는 안 맞는다(`logic` 산문에서 긁혔거나 인라인·접힘). 상위 2개 |
...
| 15 | v2_obj_restore_safe | game_ai::plan_legacy::handler::LegacyPlanHandler::v2_obj_restore_safe | in:game_ai::plan_legacy::handler | fn(&game_ai::plan_legacy::handler::LegacyPlanHandler, usize, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &game_core::OperationData, &mut game_core::DebugFrameData) -> bool | game-ai\src\plan_legacy\handler.rs:501 | False | False | 4 | ⚠미확정 — tcx 3크레이트에 같은 leaf 가 1개. IR 범위의 직접 호출 심볼과는 안 맞는다(`logic` 산문에서 긁혔거나 인라인·접힘). ★후보가 1개라는 것은 **정답이라는 뜻이 아니다** — tcx 에 동명이 하나뿐일 뿐이다. 상위 1개 |
</details>

⚠**미매칭 3개**: `drop  ⟵ IR 에 이 leaf 호출이 있으나 tcx 3크레이트에 구현이 없다(std·bumpalo·core 등). 후보 1개는 **전부 다른 함수**라 싣지 않는다`, `player_champion`, `pool`
> tcx 3크레이트에 같은 leaf 이름의 Fn/AssocFn 이 없는 것들. 대부분 std·클로저·매크로이거나 `logic` 산문에서 긁힌 잡음이다. ⚠단 **여기 이름이 실제 게임 술어인데 이름이 달라서 못 잡힌 경우**가 섞일 수 있으니, 3차에서 판정에 쓰이는 술어가 이 목록에 있으면 손으로 확인할 것.

**호출처 5곳** (m13.ll:7673, m13.ll:7730, m13.ll:7805, m13.ll:7858, m13.ll:11643) · **형제 41개** (LegacyPlanHandler)
...
| # | 분류 | 물음 | ev | 시도 |
|---|---|---|---|---|
| 0 | 미탐색 | self(&LegacyPlanHandler) 는 DISubprogram 상 arg1 로 존재하나 본문이 안 읽어 fastcc 에서 제거됨 — 핸들러 상태에 의존하지 않는 판정임은 확정 | 4 |  |
| 1 | 미탐색 | check_kill_die_tick(m?.ll) 내부(사망 틱 예측 방식)는 담당 범위 밖 — TLS 메모 캐시가 있다는 METHOD_MAP 주의사항만 인지 | 4 |  |
| 2 | 미탐색 | is_recent_visible / is_ignored_well_enemy 의 내부 조건은 안 읽음(각각 _gcbc / fight_model 소속) | 4 |  |
| 3 | 미탐색 | Blackboard 구조체 크기(stride) 는 aux 의 gep 구조체 리터럴로만 관측, tcxdict 로 별도 확인 안 함 | 3 |  |
| 4 | 미탐색 | towers=Vec::new_in 의 dangling ptr 리터럴 8 (`inttoptr (i64 8 to ptr)`) 은 판정 상수가 아니라 정렬값이라 constants 에서 제외 | 4 |  |
