...
# 15차 배치 C 도시에 — 담당 `30`~`34` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 15 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `30` | objective_is_damaged | in:game_ai | 11 | 3 | 1 | 3 | 13 |
| `31` | v3_epic_formation_role | pub | 9 | 9 | 2 | 3 | 19 |
| `32` | serpen_giveup_chat_reason | pub | 10 | 7 | 1 | 3 | 17 |
| `33` | v2_obj_restore_safe | in:game_ai::plan_legacy::handler | 14 | 4 | 2 | 5 | 20 |
| `34` | has_line_defense_threat | pub | 17 | 5 | 2 | 5 | 23 |
...
| # | 이름 | 파일 |
|---|---|---|
| `30` | objective_is_damaged | `MIG\_verify15\C\spec_30_objective_is_damaged.md` |
| `31` | v3_epic_formation_role | `MIG\_verify15\C\spec_31_v3_epic_formation_role.md` |
| `32` | serpen_giveup_chat_reason | `MIG\_verify15\C\spec_32_serpen_giveup_chat_reason.md` |
| `33` | v2_obj_restore_safe | `MIG\_verify15\C\spec_33_v2_obj_restore_safe.md` |
| `34` | has_line_defense_threat | `MIG\_verify15\C\spec_34_has_line_defense_threat.md` |
...
**[G1 자기모순] 2건**
  [33] v2_obj_restore_safe                             constants[1] src_line=506 인데 meaning 은 같은 파일 handler.rs:515 를 가리킨다
  enemy team = 1 - team (본체 %26, aux %35). 또 tps*2 가 `shl i64 %tps, 1` 로 접혀 리터럴 2 없음 (handler.rs:515)

**[G10 class 오분류] 6건**
  [33] v2_obj_restore_safe                             class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  self(&LegacyPlanHandler) 는 DISubprogram 상 arg1 로 존재하나 본문이 안 읽어 fastcc 에서 제거됨 — 핸들러 상태에 의존하지 않는 판정임은 확정

**[G13 knobs.where 대조] 20건**
  [30] objective_is_damaged                            knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m15.ll:51739  인용=icmp ult %40, %42
  [31] v3_epic_formation_role                          knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m09.ll:64821  인용=icmp ugt %34, %36
  [32] serpen_giveup_chat_reason                       knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m05.ll:53081  인용=icmp ule %39, %50
  [34] has_line_defense_threat                         knobs[0] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:60443  인용=icmp slt i64 %22, -3000
  [34] has_line_defense_threat                         knobs[1] 인용한 명령이 그 줄(±2)에 없다
  m04.ll:60448  인용=icmp slt i32 %26, -2

**[G16 params.role 대조] 10건**
  [33] v2_obj_restore_safe                             sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 5개(sret보정 -0) vs params 6개 ⟹ 소거 1개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류
  [33] v2_obj_restore_safe                             sig.params[0] P3 `i` 번호 규약 위반
  `sig.tcx` 소스 인자 6개 ⟹ i 는 [1, 2, 3, 4, 5, 6] 여야 한다(sret=0, 소스 1..n). 현재 [0, 1, 2, 3, 4, 5]. 규약 정본 = 07·15

**[G7 과열림] 4건**
  [33] v2_obj_restore_safe                             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible / is_ignored_well_enemy 의 내부 조건은 안 읽음(각각 _gcbc / fight_model 소속)
```
...
```json
{
  "round": 15, "batch": "C",
  "errors": [
    {"path": "/specs[30]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[30]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 15 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify15/C 만 써라**

```
MIG/_verify15/C/patch.json      ← 기계 적용분(필수)
MIG/_verify15/C/REPORT.md       ← 산문 보고서
MIG/_verify15/C/oracle/         ← 오라클 .rs · 출력 로그
```
