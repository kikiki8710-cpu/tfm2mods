...
$work="$env:TEMP\tfm2_rdoc"; New-Item -ItemType Directory -Force -Path $work | Out-Null
$cmd = "`"$RUSTUP`" run nightly-2026-05-24 rustdoc --edition 2021 -L dependency=$DEPS -L native=$NAT --extern mod_api=$MODAPI --extern engine_ui=$EUI --extern engine_core=$ECORE --extern game_core=$GCORE --extern game_view=$GVIEW --extern common=$COMMON --extern game_ai=$GAI -Z unstable-options --output-format json --document-private-items $Extra `"$Src`" --out-dir `"$work`" 2>&1"
cmd /c $cmd
