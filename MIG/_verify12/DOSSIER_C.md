...
# 12차 배치 C 도시에 — 담당 `10`~`14` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 12 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `10` | should_end_object_finish_kill_priority_battle | in:game_ai | 22 | 6 | 8 | 0 | 1 |
| `11` | v3_fall_back_to_passive | pub | 23 | 19 | 22 | 0 | 10 |
| `12` | handle_chat | pub | 26 | 12 | 22 | 0 | 10 |
| `13` | target_bush_v30 | in:game_ai::plan_legacy::old::line_gank::cover | 18 | 16 | 11 | 0 | 5 |
| `14` | update | pub | 28 | 21 | 7 | 0 | 1 |
...
| # | 이름 | 파일 |
|---|---|---|
| `10` | should_end_object_finish_kill_priority_battle | `MIG\_verify12\C\spec_10_should_end_object_finish_kill_priority_battle.md` |
| `11` | v3_fall_back_to_passive | `MIG\_verify12\C\spec_11_v3_fall_back_to_passive.md` |
| `12` | handle_chat | `MIG\_verify12\C\spec_12_handle_chat.md` |
| `13` | target_bush_v30 | `MIG\_verify12\C\spec_13_target_bush_v30.md` |
| `14` | update | `MIG\_verify12\C\spec_14_update.md` |
...
**[G20 명세간 공유사실] 8건**
  [13] target_bush_v30                                 [R1] Entity 0x128 — 같은 (base,offset) 을 3가지 이름으로 부른다: Tower.ty / ty.Tower.info.ty / ty.Tower.ty
  [13]`Tower.ty` · [14]`ty.Tower.ty` · [15]`ty.Tower.info.ty`
  [13] target_bush_v30                                 [R1] Entity 0x70 — 같은 (base,offset) 을 2가지 이름으로 부른다: ty = Tower / ty payload = &Tower
  [13]`ty payload = &Tower` · [14]`ty = Tower`
  [13] target_bush_v30                                 [R3] PlayerState 0x930 — dir 미기재 — 다른 명세 14개는 `r` 로 적었다
  [13]`-`
  [13] target_bush_v30                                 [R3] PlayerState 0x9c0 — dir 미기재 — 다른 명세 10개는 `r` 로 적었다
  [13]`-`
```
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 12 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify12/C 만 써라**

```
MIG/_verify12/C/patch.json      ← 기계 적용분(필수)
MIG/_verify12/C/REPORT.md       ← 산문 보고서
MIG/_verify12/C/oracle/         ← 오라클 .rs · 출력 로그
```
