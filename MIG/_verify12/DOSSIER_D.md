...
# 12차 배치 D 도시에 — 담당 `15`~`19` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 12 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `15` | single_try_engage | in:game_ai | 19 | 8 | 9 | 1 | 9 |
| `16` | max_range_nearly_can_use | pub | 31 | 7 | 9 | 0 | 4 |
| `17` | new | pub | 48 | 4 | 5 | 0 | 6 |
| `18` | v3_epicops_buff_window | in:game_ai | 22 | 8 | 27 | 1 | 29 |
| `19` | best_jungle_goal | pub | 17 | 5 | 8 | 0 | 1 |
...
| # | 이름 | 파일 |
|---|---|---|
| `15` | single_try_engage | `MIG\_verify12\D\spec_15_single_try_engage.md` |
| `16` | max_range_nearly_can_use | `MIG\_verify12\D\spec_16_max_range_nearly_can_use.md` |
| `17` | new | `MIG\_verify12\D\spec_17_new.md` |
| `18` | v3_epicops_buff_window | `MIG\_verify12\D\spec_18_v3_epicops_buff_window.md` |
| `19` | best_jungle_goal | `MIG\_verify12\D\spec_19_best_jungle_goal.md` |
...
**이 배치 몫 = 1건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G16 params.role 대조] 1건**
  [18] v3_epicops_buff_window                          sig.params[2] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[2] rnd `readnone` — IR %2 = ptr noalias noundef nonnull align 16 dereferenceable(320) %2
```
...
```json
{
  "round": 12, "batch": "D",
  "errors": [
    {"path": "/specs[15]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[15]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 12 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify12/D 만 써라**

```
MIG/_verify12/D/patch.json      ← 기계 적용분(필수)
MIG/_verify12/D/REPORT.md       ← 산문 보고서
MIG/_verify12/D/oracle/         ← 오라클 .rs · 출력 로그
```
