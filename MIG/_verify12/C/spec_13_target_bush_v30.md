...
|---|---|---|---|---|---|---|---|
| 0 | LineGankCoverPlan | 0x20 | line: LineType(0=Top,1=Mid,2=Bottom) | - | ★`match line` 의 switch 키이자 이 함수의 1차 분기축인데 **표에 통째로 빠져 있었다**(11차 배치C). fastcc 인자승격이라 본문(m10.ll 11483~11731)에는 gep 가 없고 `%0:i8` 로 들어온다 — 참조용 행이므로 `dir` 은 `"-"` 가 맞다. 로드는 **호출부**에 있다: `_gaibc/m10.ll:11787` `%23 = getelementptr inbounds nuw i8, ptr %1, i64 32` → `11788` `%24 = load i8, ptr %23` → `11791` 에서 그대로 1번 인자로 전달. 32 = 0x20. ★같은 소스의 문자단위 복제본인 `14 LineGankerPlan::update`(그 명세 notes[0])는 같은 필드를 `mem[3] LineGankerPlan 0x28 line` 으로 싣고 있다 — 두 명세가 같은 코드를 다른 규격으로 적고 있었다. 이 명세의 `closed[2]` 는 4차에 「간접증거→직접증거, 호출부 m10.ll:11778~11791 에 gep 그대로」로 닫혔는데 그 결론이 표에 전파되지 않았다(G17 형) | 4 | OK |
| 1 | PlayerState | 0x930 | info.team: usize | - | ★인자승격(`%1:i64`)이라 본문에 gep 가 없다 — 참조용 행이므로 `dir` 은 `"-"`. 로드는 호출부 `_gaibc/m10.ll:11762` `%8 = getelementptr inbounds nuw i8, ptr %4, i64 2352` → `11763` `%9 = load i64, ptr %8` 이고, 같은 자리에서 `11764` `%10 = icmp ult i64 %9, 2` / `11768` `panic_bounds_check(%9, 2)` 로 **경계검사까지 호출부가 한다**(logic L36 의 「team>=2 면 panic_bounds_check」는 이 함수가 아니라 호출부 책임이라는 뜻). 2352 = 0x930. 이 함수 안에서는 `*_tower[team]` · `player_champion[team][pos]` 두 인덱스로 쓰인다. 복제본 명세 `14` 는 같은 필드를 `mem[4]`(dir=r — 그쪽은 본문에 gep 가 있다)로 싣는다 | 4 | OK |
| 2 | PlayerState | 0x9c0 | info.position: Position(i32, range 0..5) | - | ★인자승격(`%2:i32`)이라 본문에 gep 가 없다 — 참조용 행이므로 `dir` 은 `"-"`. 로드는 호출부 `_gaibc/m10.ll:11773` `%13 = getelementptr inbounds nuw i8, ptr %4, i64 2496` → `11774` `%14 = load i32, ptr %13, !range` → `11775` `zext nneg i32 %14 to i64` 후 `player_champion[team][pos]` 의 안쪽 인덱스로 쓰인다(`11779` `%19 = getelementptr inbounds nuw ptr, ptr %18, i64 %15`). 2496 = 0x9c0. `mem[6]`(player_champion)이 이 값을 인덱스로 쓴다고만 적고 출처 오프셋은 표에 없었다. 복제본 명세 `14` 는 `mem[5]` 로 싣는다 | 4 | OK |
| 3 | AbstractGameWithCache | 0x180 | top_tower[team] | r | [Option<&Entity>;2] — line=Top 일 때 1순위 타워 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=0)) | 3 | OK |
| 4 | AbstractGameWithCache | 0x190 | top_tower2[team] | r | top_tower 가 None 이면 폴백(Option::or, option.rs:1622) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=1)) | 3 | OK |
| 5 | AbstractGameWithCache | 0x1a0 | mid_tower[team] | r | line=Mid 1순위 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=2)) | 3 | OK |
| 6 | AbstractGameWithCache | 0x1b0 | mid_tower2[team] | r | line=Mid 폴백 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=3)) | 3 | OK |
| 7 | AbstractGameWithCache | 0x1c0 | bottom_tower[team] | r | line=Bottom 1순위 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=4)) | 3 | OK |
| 8 | AbstractGameWithCache | 0x1d0 | bottom_tower2[team] | r | line=Bottom 폴백 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=5)) | 3 | OK |
| 9 | AbstractGameWithCache | 0x1e0 | player_champion[team][position] | r | [[Option<&Entity>;5];2] 80B. unwrap — None 이면 unwrap_failed 패닉(cover.rs:145:95) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=6)) | 3 | OK |
| 10 | Entity | 0x68 | ty (EntityType 판별자) | r | 타워 엔티티. ==2 → EntityType::Tower. is_tower2()의 1단계(entity.rs:1308) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=7)) | 3 | OK |
| 11 | Entity | 0x70 | ty payload = &Tower (변수명 info) | - | EntityType::Tower 페이로드의 **시작 표식**이다 — 참조용 행이므로 `dir` 은 `"-"` 가 맞다. m10.ll 11483~11731 에 `getelementptr .. i64 112` 는 **0건**이고, 페이로드 필드는 Entity **절대 오프셋**으로 직접 접근된다(`+0x88` nearest_enemy = gep 136, `+0x128` TowerType = gep 296). Top(L149)/Bottom(L179) 분기에서 쓰이는 것은 그 절대 오프셋들이다. 태그!=2 면 unreachable!()(L160/L190) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=8)) | 3 | OK |
| 12 | Entity | 0x88 | Tower.nearest_enemy 판별자 | r | Tower+0x18, Option<(usize,usize)> 의 태그. !=0 이면 is_some(option.rs:632) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=9)) | 3 | OK |
| 13 | Entity | 0x128 | Tower.ty (TowerType) | r | Tower+0xb8, i8 range[0,8). >4 → Top2/Mid2/Bottom2 = is_second_tower(tower.rs:99~100) · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=10)) | 3 | OK |
| 14 | Entity | 0x660 | x | r | player_champion 으로 얻은 아군 챔피언 좌표 x. Mid 분기에서만 사용 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=11)) | 3 | OK |
| 15 | Entity | 0x668 | y | r | 같은 챔피언 좌표 y. Mid 분기에서만 사용 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=12)) | 3 | OK |
| 16 | GameContext | 0x8 | setting (&GameSetting) | r | is_top_side(map_regions.rs:21)가 받는 context 에서 꺼냄 · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=13)) | 3 | OK |
| 17 | GameSetting | 0x12c0 | height | r | u64. map_regions.rs:22 의 ry = height - y · tcx 정본 대조( `offset_of!` 실행 대조 MISMATCH 0 (`_verify6/C/o1_off.out`·`o2_off_ai.out`·`o3_off2.out`, 오프셋 51 + 구조체 크기 16 전건 OK) · `tcxdict` 행별 재조회 MATCH (`_verify6/C/memchk.tsv` i=13 idx=14)) | 3 | OK |
...
|---|---|---|---|---|---|---|---|---|---|
| 0 | is_top_side | game_core::is_top_side | pub | fn(&game_core::GameContext, u64, u64) -> bool | game-core\src\simulation\map_regions.rs:21 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 1 | is_tower2 | game_core::EntityType::is_tower2 | pub | fn(&game_core::EntityType) -> bool | game-core\src\simulation\entity.rs:1308 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 2 | target_bush_v30 | game_ai::plan_legacy::old::LineGankerPlan::target_bush_v30 | in:game_ai::plan_legacy::old::line_gank::ganker | fn(&game_ai::plan_legacy::old::LineGankerPlan, &game_core::PlayerState, &game_core::OperationData) -> usize | game-ai\src\plan_legacy\old\line_gank\ganker.rs:248 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 3 | target_bush_v30 | game_ai::plan_legacy::old::LineGankCoverPlan::target_bush_v30 | in:game_ai::plan_legacy::old::line_gank::cover | fn(&game_ai::plan_legacy::old::LineGankCoverPlan, &game_core::PlayerState, &game_core::OperationData) -> usize | game-ai\src\plan_legacy\old\line_gank\cover.rs:134 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 4 | tower | game_core::AbstractGameWithCache::<'a, 'b>::tower | pub | fn(&game_core::AbstractGameWithCache<'a/#0, 'b/#1>, game_core::LineType, usize) -> std::option::Option<&game_core::Entity> | game-core\src\simulation.rs:1822 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
</details>
