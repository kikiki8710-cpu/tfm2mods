...
# 12차 배치 A 도시에 — 담당 `00`~`04` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `4cc9a34ba642ab87` | 2026-09-11 23:26:33 |
| `_spec/specs20.json` (손이 닿는 정본) | `2494f8a75e3d6ad3` | 2026-09-11 22:55:03 |

| `mkdossier.py`(이 파일을 만든 도구) | `d5f7b43886a758b6` | 2026-09-11 23:46:05 |

이 파일 생성 = `2026-09-11 23:46:22`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 12 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `00` | ult | pub | 29 | 10 | 9 | 0 | 5 |
| `01` | calculate_jungle_action_score | pub | 8 | 12 | 7 | 0 | 0 |
| `02` | sub_plan | pub | 21 | 6 | 5 | 1 | 2 |
| `03` | defensive_crisis | pub | 22 | 8 | 21 | 1 | 14 |
| `04` | handle_line_defense | pub | 28 | 5 | 7 | 1 | 2 |
...
| `MIG\SPEC_RUNBOOK.md` | 968 | `ce408ddb14a54497` | ★라운드 절차. **§S5-b 게이트표 · §S5-c 「검사받지 않는 축」 · §S5-d 파이프라인 4계약**. |
| `MIG\SPEC_GUIDE.md` | 364 | `0b45fe38ad5ebf34` | 명세 필드의 뜻과 채우는 법. |
| `MIG\TOOLS.md` | 221 | `0fc2b169795bd202` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `00` | ult | `MIG\_verify12\A\spec_00_ult.md` |
| `01` | calculate_jungle_action_score | `MIG\_verify12\A\spec_01_calculate_jungle_action_score.md` |
| `02` | sub_plan | `MIG\_verify12\A\spec_02_sub_plan.md` |
| `03` | defensive_crisis | `MIG\_verify12\A\spec_03_defensive_crisis.md` |
| `04` | handle_line_defense | `MIG\_verify12\A\spec_04_handle_line_defense.md` |
...
**[G11 ev 지시 미반영] 1건**
  [03] defensive_crisis                                history[9] 가 **ev 4→2** 를 지시했는데 표가 안 따라갔다 (1행)
  consts[2] ev=4  ⟹ 해당 행의 근거 문면에 실행 근거를 적어야 ev 가 내려온다

**[G17 history 전파] 1건**
  [03] defensive_crisis                                consts[2] [B] history[9] 이 `ev→2` 로 선언했는데 표는 ev=4
  EntityType 태그 13 = Champion ★**src_line 정정 : 43**(`icmp eq i

**[G20 명세간 공유사실] 8건**
  [03] defensive_crisis                                [R1] Entity 0xb8 — 같은 (base,offset) 을 3가지 이름으로 부른다: ty.Champion.0.skill_cooldown / ty.Champion.skill_cooldown / ty.Minion.info.attack_cooldown
  [03]`ty.Champion.skill_cooldown` · [16]`ty.Champion.0.skill_cooldown/ty.Minion.info.attack_cooldown`
  [03] defensive_crisis                                [R1] Entity 0xc0 — 같은 (base,offset) 을 2가지 이름으로 부른다: ty.Champion.0.skill2_cooldown / ty.Champion.skill2_cooldown
  [03]`ty.Champion.skill2_cooldown` · [16]`ty.Champion.0.skill2_cooldown`
  [03] defensive_crisis                                [R1] Entity 0xc8 — 같은 (base,offset) 을 3가지 이름으로 부른다: ty.Bear.info.attack_cooldown / ty.Champion.0.ult_cooldown / ty.Champion.ult_cooldown
  [03]`ty.Champion.ult_cooldown` · [16]`ty.Champion.0.ult_cooldown/ty.Bear.info.attack_cooldown`
  [03] defensive_crisis                                [R2] AbstractGameWithCache game — 같은 이름이 오프셋 0x0 / 0x8 에 동시에 있다 — 표기로 설명되지 않는다
  [03]`0x0` · [04]`0x0` · [06]`0x0` · [07]`0x0` · [12]`0x8` · [18]`0x0` · [18]`0x8`
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 470 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 193 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 126 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 262 | ⚠**56행만 IR 앵커**(`ev3`). 나머지 206행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 12 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify12/A 만 써라**

```
MIG/_verify12/A/patch.json      ← 기계 적용분(필수)
MIG/_verify12/A/REPORT.md       ← 산문 보고서
MIG/_verify12/A/oracle/         ← 오라클 .rs · 출력 로그
```
