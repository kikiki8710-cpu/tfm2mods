...
| 11 | Entity(champ/target) | 0x470 | stat_buff_cached.radius_mult | r | i32. stat_buff_cached(0x370)+0x100. Entity::radius() 에서 %보정 ★`as usize` 로 **부호확장**돼 쓰인다(MIR entity.rs:1511 `_2 = move _3 as usize (IntToInt)`, IR `sext i32 %9 to i64` @m04.ll:43306 · `%33` @43339). 이후 `100 + mult` 는 wrapping usize 산술. ★in-함수 인용 보강(11차 배치A): 인용된 43306·43339 는 `ult` 본문(43967~44365)이 아니라 `linear_cast_range_with_margin`(m04.ll:43272~43375)의 인라인 사본이다 — `ult` 안의 같은 접근은 champ 쪽 44251 `%162 = load i32, ptr %161`(=%21+1136) → 44262 `%168 = sext i32 %162 to i64`, target 쪽 44283 `%185 = load i32, ptr %184`(=%6+1136) → 44294 `%191 = sext i32 %185 to i64` 다 | 3 | OK |  |
...
| 9 | 150000 | 333 | 오프셋가감 | ★이동 목표 산정 시 총사거리에서 빼는 여유분(saturating_sub). 셀=32000 기준 약 4.7셀. 사거리 끝보다 이만큼 안쪽에 서려고 접근한다 · 오라클 실행 확증( A6_o2_main.tsv / A6_o2_radius.tsv / A6_o2_samepos.tsv — margin 149999/150000/150001 중 150000 만 MATCH_ULT=YES) | 2 |
...
| 4 | 궁 봉인 CC 집합 | can_ult _gcbc/g06.ll:81878~81879 (`%100 = add nsw i32 %99, -6` / `%101 = icmp ult i32 %100, -4` — cc 태그 로드는 81874 `%99 = load i32, ptr %95, !range !31790`) | ∉{2,3,4,5} | Bind·BlockAttack 만 궁 허용. 여기를 바꾸면 예컨대 Taunt 중 궁 허용 가능 | 4 | 신규 |
| 5 | cooltime_use_count(충전 수) | Action vtable +0xa8, 디폴트 1 (_gcbc/g02.ll:306881 `ret i64 1`) | 1 | 2 이상으로 만들면 그 스킬이 충전형이 됨(쿨 절반만 차도 사용 가능) | 4 | 신규 |
...
|---|---|---|---|---|---|---|---|---|---|
| 0 | adjust_position | game_core::Game::adjust_position | pub | fn(&game_core::MapDef, &game_core::GameSetting, i64, i64) -> (u64, u64) | game-core\src\simulation\game.rs:818 | False | False | 3 | IR 호출 심볼 일치 |
| 1 | as_index | game_core::BigGoal::as_index | pub | fn(&game_core::BigGoal) -> usize | game-core\src\simulation\ai_interface.rs:338 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 2 | as_index | game_core::Position::as_index | pub | fn(&game_core::Position) -> usize | game-core\src\simulation\entity.rs:580 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 3 | can_ult | game_core::Entity::can_ult | pub | fn(&game_core::Entity) -> bool | game-core\src\simulation\entity.rs:1734 | False | False | 3 | IR 호출 심볼 일치 |
| 4 | check | game_core::CastingTarget::check | pub | fn(&game_core::CastingTarget, &game_core::Entity, &game_core::Entity) -> bool | game-core\src\simulation\effect\type.rs:227 | False | False | 3 | IR 호출 심볼 일치 |
| 5 | cooldown_reduce | game_core::Entity::cooldown_reduce | pub | fn(&game_core::Entity, bool) -> usize | game-core\src\simulation\entity.rs:2437 | False | False | 3 | IR 호출 심볼 일치 |
| 6 | distance_sq | game_core::utils::distance_sq | pub | fn(u64, u64, u64, u64) -> u64 | game-core\src\utils.rs:6 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 7 | distance_sq | game_core::Entity::distance_sq | pub | fn(&game_core::Entity, &game_core::Entity) -> u64 | game-core\src\simulation\entity.rs:2157 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 2개 중 상위 2개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 8 | effect_range_with_radii | game_ai::abstract_input::effect_range_with_radii | in:game_ai::abstract_input | fn(&game_core::Effect, &game_core::Entity, &game_core::Entity) -> u64 | game-ai\src\abstract_input.rs:190 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 9 | get_input_target | game_ai::abstract_input::get_input_target | in:game_ai::abstract_input | fn(usize, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &game_core::OperationData, &game_core::PositioningScoreData, &game_core::Entity, &game_core::Effect, usize) -> std::option::Option<game_core::InputTarget> | game-ai\src\abstract_input.rs:345 | False | False | 3 | IR 호출 심볼 일치 |
| 10 | is_visible | game_core::World::is_visible | pub | fn(&game_core::World, usize, usize) -> bool | game-core\src\simulation\game\data.rs:384 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 7개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 11 | is_visible | game_core::VisibleState::is_visible | pub | fn(&game_core::VisibleState) -> bool | game-core\src\simulation\game\data.rs:121 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 7개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 12 | is_visible | game_core::AbstractGame::is_visible | pub | fn(&Self/#0, usize, usize) -> bool | game-core\src\simulation.rs:125 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 7개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 13 | is_visible_from | game_core::Entity::is_visible_from | pub | fn(&game_core::Entity, &game_core::Entity) -> bool | game-core\src\simulation\entity.rs:1481 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 14 | isqrt | game_core::utils::isqrt | pub | fn(i64) -> i64 | game-core\src\utils.rs:85 | False | False | 3 | IR 호출 심볼 일치 |
| 15 | level | <game_ai::WindowStatView<'_> as game_core::AbstractEntity>::level | pub | fn(&game_ai::WindowStatView</#0>) -> usize | game-ai\src\fight_check.rs:96 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 16 | level | game_core::AbstractEntity::level | pub | fn(&Self/#0) -> usize | game-core\src\simulation\entity.rs:718 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 17 | level | <game_core::Entity as game_core::AbstractEntity>::level | pub | fn(&game_core::Entity) -> usize | game-core\src\simulation\entity.rs:772 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 18 | linear_cast_range_with_margin | game_ai::abstract_input::linear_cast_range_with_margin | in:game_ai::abstract_input | fn(&game_core::Effect, &game_core::Entity, &game_core::Entity) -> u64 | game-ai\src\abstract_input.rs:195 | False | False | 3 | IR 호출 심볼 일치 |
| 19 | radius | game_core::Entity::radius | pub | fn(&game_core::Entity) -> usize | game-core\src\simulation\entity.rs:1509 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 20 | range | game_core::Effect::range | pub | fn(&game_core::Effect, &game_core::Entity) -> u64 | game-core\src\simulation\effect.rs:25 | True | True | 3 | IR 호출 심볼 일치 |
| 21 | range_adjust | game_core::Effect::range_adjust | pub | fn(&game_core::Effect, &game_core::Entity, &game_core::Entity) -> u64 | game-core\src\simulation\effect.rs:29 | False | False | 3 | IR 호출 심볼 일치 |
| 22 | safe_move_avoiding_enemy_well | game_ai::safe_move_avoiding_enemy_well | pub | fn(usize, &game_core::PlayerState, &game_core::OperationData, &game_core::Entity, u64, u64) -> std::option::Option<game_core::Input> | game-ai\src\abstract_input.rs:122 | False | False | 3 | IR 호출 심볼 일치 |
| 23 | team | game_view::ClientData::team | pub | fn(&game_view::ClientData, usize) -> std::option::Option<std::cell::Ref< game_core::Team>> | game-view\src\logic\client\data.rs:1651 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 24 | team | game_view::ClientDatabase::team | pub | fn(&game_view::ClientDatabase, usize) -> std::option::Option<&game_core::Team> | game-view\src\logic\client\data.rs:968 | False | False | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 25 | team | game_core::PlayerAiContext::<'a, 'b, 'r>::team | pub | fn(&game_core::PlayerAiContext<'a/#0, 'b/#1, 'r/#2>) -> usize | game-core\src\mod_ai.rs:583 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 3개 중 상위 3개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
| 26 | ult_effect | game_core::Entity::ult_effect | pub | fn(&game_core::Entity) -> &std::option::Option<game_core::Effect> | game-core\src\simulation\entity.rs:1700 | True | True | 4 | ⚠미확정 — 같은 leaf 후보 1개 중 상위 1개만 실음(IR 범위에 이 호출이 없다 = `logic` 산문에서 긁혔거나 접힘) |
</details>
...
| # | 물음 | 닫은 근거 |
|---|---|---|
| 0 | dyn EffectType vtable 슬롯 +0x118(fn(&self)->bool, 296행)·+0xf8(fn(&self)->(i64,i64), 310행)의 메서드 이름 — ★해소(11차 배치A) — **+0x118 = EffectType::on_caster · +0xf8 = EffectType::linear_move_speed** 로 확정돼 있고 이미 mem[24]·mem[25]·history[3] 이 그 이름을 싣고 있다. ~~도구가 '없음: EffectType' 을 돌려주고 vtable 전역이 `_gaibc` 에 없어 불가~~ 는 **닫을 당시의 도구 사정이 화석으로 남은 것**이다 — 지금은 `_gcbc` 자동 폴백이 있어 `python -X utf8 divtable.py EffectType 0x118` 이 `@anon.75300de3978c94cc946f88c448be65c0.1131`(g02.ll·슬롯 35개·296B·일치율 94%) 과 `@anon....1293` 두 vtable 에서 `0x118 → …EffectType9on_caster` 를 직접 돌려준다(11차 실행). 관측 사실만: Position 캐스팅에선 bool 이 true 여야, Direction 캐스팅에선 (i64,i64).0 이 1이어야 사거리 검사 단계로 진행한다. vtable 레이아웃은 0x0 drop/0x8 size/0x10 align/0x18~ 메서드 이므로 각각 32번째·28번째 메서드 슬롯 | 본문에 해소 표기가 있다 |
...
| 7 | param1 version 도 본문 분기에 쓰이지 않는다 — get_input_target / safe_move_avoiding_enemy_well 로 전달만. ★해소(11차 배치A) — **버전 게이트는 `safe_move_avoiding_enemy_well` 안에 있다**: m04.ll:43914 `%288 = icmp ugt i64 %1, 1`(= version > 1)이 그 함수(m04.ll:43378~43964)에서 `%1`(version)을 쓰는 **유일한 비교**이고, v1 이면 무조건 None · v2+ 면 m04.ll:43950 `%311 = icmp ugt i64 %310, 4000000` 으로 갈린다. 같은 파일 history[4](c) 와 knobs[7] 이 이미 그렇게 적고 있었는데 이 항목에 전파되지 않았다 | 본문에 해소 표기가 있다 |
