...
# 26차 배치 A 도시에 — 담당 `204`~`204` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `204` | calculate_score_parameter | pub | 212 | 88 | 28 | 34 | 319 |
...
| # | 이름 | 파일 |
|---|---|---|
| `204` | calculate_score_parameter | `MIG\_verify26\A\spec_204_calculate_score_parameter.md` |
...
**[G12 src_line 대조] 44건**
  [204] calculate_score_parameter                       consts[20] src_line=1782 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [2069, 2082, 2097]

**[G7 과열림] 22건**
  [204] calculate_score_parameter                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 A) L1507/L1515 의 `is_some_and` 안 두 조건(id!=champ.id / is_recent_visible 과 distance_sq) 의 소스 표기 순서 — IR 은 각각 id 비교→거리,
  [204] calculate_score_parameter                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 A) Blackboard::is_recent_visible 의 내부(`game.is_visible(team,id)` vtable+0xf8 · get_player_by_champion_id vtable+0x15
```
...
```json
{
  "round": 26, "batch": "A",
  "errors": [
    {"path": "/specs[204]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[204]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify26/A 만 써라**

```
MIG/_verify26/A/patch.json      ← 기계 적용분(필수)
MIG/_verify26/A/REPORT.md       ← 산문 보고서
MIG/_verify26/A/oracle/         ← 오라클 .rs · 출력 로그
```
