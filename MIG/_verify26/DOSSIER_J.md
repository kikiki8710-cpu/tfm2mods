...
# 26차 배치 J 도시에 — 담당 `228`~`233` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 J      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `228` | TeamPlan::handle_nexus_attack | pub | 11 | 11 | 2 | 5 | 24 |
| `229` | AgentVerHamster::count_nearby_enemies | in:game_ai | 7 | 6 | 1 | 3 | 14 |
| `230` | LineSafeSubPlan::score | pub | 28 | 15 | 5 | 4 | 47 |
| `231` | LineWaitSubPlan::score | pub | 28 | 15 | 5 | 5 | 47 |
| `232` | v30_line_champion_action_tower_aggro_risk | pub | 34 | 18 | 3 | 2 | 55 |
| `233` | nexus_final_stand_uncached | in:game_ai::plan_legacy::old::defense_nexus | 11 | 5 | 2 | 5 | 18 |
...
| # | 이름 | 파일 |
|---|---|---|
| `228` | TeamPlan::handle_nexus_attack | `MIG\_verify26\J\spec_228_TeamPlan__handle_nexus_attack.md` |
| `229` | AgentVerHamster::count_nearby_enemies | `MIG\_verify26\J\spec_229_AgentVerHamster__count_nearby_enemies.md` |
| `230` | LineSafeSubPlan::score | `MIG\_verify26\J\spec_230_LineSafeSubPlan__score.md` |
| `231` | LineWaitSubPlan::score | `MIG\_verify26\J\spec_231_LineWaitSubPlan__score.md` |
| `232` | v30_line_champion_action_tower_aggro_risk | `MIG\_verify26\J\spec_232_v30_line_champion_action_tower_aggro_risk.md` |
| `233` | nexus_final_stand_uncached | `MIG\_verify26\J\spec_233_nexus_final_stand_uncached.md` |
...
**이 배치 몫 = 8건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G16 params.role 대조] 22건**
  [229] AgentVerHamster::count_nearby_enemies           sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 3개(sret 행 없음) ⟹ 3행이어야 하는데 5행. 현재 i = [1, 1, 2, 3, 3]
  [229] AgentVerHamster::count_nearby_enemies           sig.params[3] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[3] data.cache `nonnull` — IR %3 = ptr readonly captures(address_is_null) %3

**[G19 knobs.value 값] 14건**
  [230] LineSafeSubPlan::score                          knobs[1] 주장한 값을 IR 관측에서 못 찾는다
  value=50 · 관측(창)=[1, 2, 3, 104]
  [230] LineSafeSubPlan::score                          knobs[2] 주장한 값을 IR 관측에서 못 찾는다
  value=100 · 관측(창)=[3, 1168]
  [231] LineWaitSubPlan::score                          knobs[1] 주장한 값을 IR 관측에서 못 찾는다
  value=50 · 관측(창)=[1, 2, 3, 104]
  [231] LineWaitSubPlan::score                          knobs[2] 주장한 값을 IR 관측에서 못 찾는다
  value=100 · 관측(창)=[3, 1168]

**[G5 sig 정본 대조] 4건**
  [229] AgentVerHamster::count_nearby_enemies           sig.tcx 인자 3개(팻포인터 확장 3) vs params 5개 — 개수가 다르다
  &game_core::Entity | &game_core::PlayerState | &game_core::OperationData

**[G7 과열림] 22건**
  [229] AgentVerHamster::count_nearby_enemies           ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  is_recent_visible 내부(g07.ll:157005 · game_core 경계) — 시그니처만: (self:&Blackboard, game:&dyn AbstractGame, viewer:&PlayerSta
```
...
```json
{
  "round": 26, "batch": "J",
  "errors": [
    {"path": "/specs[228]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[228]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only J --dry
```
...
## §6 출력 디렉터리 — **_verify26/J 만 써라**

```
MIG/_verify26/J/patch.json      ← 기계 적용분(필수)
MIG/_verify26/J/REPORT.md       ← 산문 보고서
MIG/_verify26/J/oracle/         ← 오라클 .rs · 출력 로그
```
