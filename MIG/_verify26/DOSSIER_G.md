...
# 26차 배치 G 도시에 — 담당 `215`~`218` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 G      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `215` | v16_gambler_ult_cc_bonus | in:game_ai | 31 | 23 | 8 | 5 | 60 |
| `216` | unsafe_v19_non_champion_walkup | in:game_ai::plan_legacy::sub_plan::line_defense | 59 | 21 | 7 | 6 | 86 |
| `217` | v17_runaway_counterattack_bonus | in:game_ai | 22 | 23 | 10 | 5 | 55 |
| `218` | AgentVerHamster::update_state | pub | 32 | 16 | 2 | 8 | 49 |
...
| # | 이름 | 파일 |
|---|---|---|
| `215` | v16_gambler_ult_cc_bonus | `MIG\_verify26\G\spec_215_v16_gambler_ult_cc_bonus.md` |
| `216` | unsafe_v19_non_champion_walkup | `MIG\_verify26\G\spec_216_unsafe_v19_non_champion_walkup.md` |
| `217` | v17_runaway_counterattack_bonus | `MIG\_verify26\G\spec_217_v17_runaway_counterattack_bonus.md` |
| `218` | AgentVerHamster::update_state | `MIG\_verify26\G\spec_218_AgentVerHamster__update_state.md` |
...
**[G12 src_line 대조] 44건**
  [218] AgentVerHamster::update_state                   consts[3] src_line=463 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [591, 811]
  [218] AgentVerHamster::update_state                   consts[4] src_line=463 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [591, 811]
  [218] AgentVerHamster::update_state                   consts[5] src_line=445 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [592]
  [218] AgentVerHamster::update_state                   consts[6] src_line=445 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [592]
  [218] AgentVerHamster::update_state                   consts[7] src_line=547 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [570, 572, 574, 684, 690]
  [218] AgentVerHamster::update_state                   consts[13] src_line=825 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [560, 576, 598]
  [218] AgentVerHamster::update_state                   consts[14] src_line=825 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [560, 576, 581, 596, 598]
  [218] AgentVerHamster::update_state                   consts[15] src_line=825 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [560, 576, 579, 581, 594, 596, 598]

**[G7 과열림] 22건**
  [216] unsafe_v19_non_champion_walkup                  ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  positioning_score_at · check_kill_die_tick · enemy_minion_line_action_danger_damage_at · possible_risk · max_range_nearl
```
...
```json
{
  "round": 26, "batch": "G",
  "errors": [
    {"path": "/specs[215]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[215]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only G --dry
```
...
## §6 출력 디렉터리 — **_verify26/G 만 써라**

```
MIG/_verify26/G/patch.json      ← 기계 적용분(필수)
MIG/_verify26/G/REPORT.md       ← 산문 보고서
MIG/_verify26/G/oracle/         ← 오라클 .rs · 출력 로그
```
