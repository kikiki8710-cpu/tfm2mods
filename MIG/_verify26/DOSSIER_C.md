...
# 26차 배치 C 도시에 — 담당 `206`~`206` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `206` | PassiveLinePlan::update | pub | 160 | 62 | 23 | 23 | 231 |
...
| # | 이름 | 파일 |
|---|---|---|
| `206` | PassiveLinePlan::update | `MIG\_verify26\C\spec_206_PassiveLinePlan__update.md` |
...
**[G12 src_line 대조] 44건**
  [206] PassiveLinePlan::update                         consts[7] src_line=704 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 243, 248, 345, 498, 1171, 1400]
  [206] PassiveLinePlan::update                         consts[10] src_line=1655 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 519, 521, 1138, 1140, 1248]
  [206] PassiveLinePlan::update                         consts[11] src_line=1701 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 248, 456, 1179, 1193, 1214, 1227, 1396]
  [206] PassiveLinePlan::update                         consts[20] src_line=1422 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 1411]
  [206] PassiveLinePlan::update                         consts[21] src_line=1422 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 1411]
  [206] PassiveLinePlan::update                         consts[25] src_line=742 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [222, 243, 248, 354, 360, 361, 400, 401]

**[G19 knobs.value 값] 14건**
  [206] PassiveLinePlan::update                         knobs[0] 주장한 값을 IR 관측에서 못 찾는다
  value=46 · 관측(창)=[-1, 1224, 1272]
  [206] PassiveLinePlan::update                         knobs[15] 주장한 값을 IR 관측에서 못 찾는다
  value=x∈[rx-68000,rx] · y∈[ry-64000,ry] · 관측(창)=[-1949, 0, 1, 3, 5, 6, 8, 11, 16, 18]

**[G7 과열림] 22건**
  [206] PassiveLinePlan::update                         ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 J) data.blackboard[1 - team](적 팀 인덱스)를 is_recent_visible 의 &self 로 쓰는 것이 소스 의도인지(콜리 본문은 self.last_visible[target.pos
  [206] PassiveLinePlan::update                         ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 J) is_recent_visible 의 120 틱 상수는 콜리 소유라 constants 미등록(callee_contracts 에만)
```
...
```json
{
  "round": 26, "batch": "C",
  "errors": [
    {"path": "/specs[206]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[206]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify26/C 만 써라**

```
MIG/_verify26/C/patch.json      ← 기계 적용분(필수)
MIG/_verify26/C/REPORT.md       ← 산문 보고서
MIG/_verify26/C/oracle/         ← 오라클 .rs · 출력 로그
```
