...
# 26차 배치 B 도시에 — 담당 `205`~`205` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `205` | LegacyPlanHandler::get_small_action | pub | 145 | 56 | 16 | 30 | 211 |
...
| # | 이름 | 파일 |
|---|---|---|
| `205` | LegacyPlanHandler::get_small_action | `MIG\_verify26\B\spec_205_LegacyPlanHandler__get_small_action.md` |
...
**[G10 class 오분류] 6건**
  [205] LegacyPlanHandler::get_small_action             class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  %1204(디버그 루프 종료) 양쪽에서 들어오므로 **게이트 없음 확정** — 단 with_score 가 비면 L291 unwrap_failed 로 패닉하므로 실전에서는 후속이 없다(패닉 경로).

**[G12 src_line 대조] 44건**
  [205] LegacyPlanHandler::get_small_action             consts[52] src_line=394 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [103, 395]
  [205] LegacyPlanHandler::get_small_action             consts[53] src_line=394 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [52, 140, 141, 147, 179, 190, 256, 271]
  [205] LegacyPlanHandler::get_small_action             consts[55] src_line=394 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [9, 13, 16, 20, 29, 73, 88, 92]

**[G7 과열림] 22건**
  [205] LegacyPlanHandler::get_small_action             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 E) L70/L194 blackboard 인덱스가 **1-team(적팀 인덱스)** 인 이유 — IR(46426 `gep …, i64 %350`, %350=1-team) 은 확정, 의미(관측 주체 vs 관측
  [205] LegacyPlanHandler::get_small_action             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 F) L228 closure#10 의 소스 문면 순서(`is_recent_visible && dist<=…` 인지 그 반대인지): IR 은 is_recent_visible 을 무조건 호출(48362)한 뒤 s
  [205] LegacyPlanHandler::get_small_action             ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 F) L235(80자)·L236(113자)의 정확한 문면: IR 루트는 L237(count) 하나에 모여 있고 closure#11 이 236:25 에 있으므로 `enemies.iter().flatten().f
```
...
```json
{
  "round": 26, "batch": "B",
  "errors": [
    {"path": "/specs[205]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[205]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify26/B 만 써라**

```
MIG/_verify26/B/patch.json      ← 기계 적용분(필수)
MIG/_verify26/B/REPORT.md       ← 산문 보고서
MIG/_verify26/B/oracle/         ← 오라클 .rs · 출력 로그
```
