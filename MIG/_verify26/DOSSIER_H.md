...
# 26차 배치 H 도시에 — 담당 `219`~`222` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 H      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `219` | BattleSubPlan::calculate_score_parameter_value | pub | 37 | 26 | 9 | 7 | 72 |
| `220` | PassiveLinePlan::v46_stage2 | in:game_ai::plan_legacy::old::passive_line | 44 | 12 | 6 | 5 | 60 |
| `221` | precompute_champion_powers | pub | 33 | 9 | 4 | 7 | 45 |
| `222` | nexus_last_stand_uncached | in:game_ai::plan_legacy::old::defense_nexus | 9 | 4 | 2 | 7 | 15 |
...
| # | 이름 | 파일 |
|---|---|---|
| `219` | BattleSubPlan::calculate_score_parameter_value | `MIG\_verify26\H\spec_219_BattleSubPlan__calculate_score_parameter_value.md` |
| `220` | PassiveLinePlan::v46_stage2 | `MIG\_verify26\H\spec_220_PassiveLinePlan__v46_stage2.md` |
| `221` | precompute_champion_powers | `MIG\_verify26\H\spec_221_precompute_champion_powers.md` |
| `222` | nexus_last_stand_uncached | `MIG\_verify26\H\spec_222_nexus_last_stand_uncached.md` |
...
**[G16 params.role 대조] 22건**
  [219] BattleSubPlan::calculate_score_parameter_value  sig.params[5] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[5] parameter `readonly` — IR %5 = ptr noalias noundef align 8 captures(address, read_provenance) dereferenceable(5384) %5
  [220] PassiveLinePlan::v46_stage2                     sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 6개(sret 행 없음) ⟹ 6행이어야 하는데 7행. 현재 i = [1, 2, 3, 4, 5, 6, 6]

**[G5 sig 정본 대조] 4건**
  [220] PassiveLinePlan::v46_stage2                     sig.tcx 인자 6개(팻포인터 확장 6) vs params 7개 — 개수가 다르다
  &game_ai::plan_legacy::old::Passiv | &game_core::PlayerState | &game_core::OperationData | &game_core::Entity | &game_core::Entity | &bumpalo::collect

**[G7 과열림] 22건**
  [219] BattleSubPlan::calculate_score_parameter_value  ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard::is_recent_visible(&self, cache, context, player, entity) -> bool 내부(game_core g07.ll:157005) — 계약만
```
...
```json
{
  "round": 26, "batch": "H",
  "errors": [
    {"path": "/specs[219]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[219]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only H --dry
```
...
## §6 출력 디렉터리 — **_verify26/H 만 써라**

```
MIG/_verify26/H/patch.json      ← 기계 적용분(필수)
MIG/_verify26/H/REPORT.md       ← 산문 보고서
MIG/_verify26/H/oracle/         ← 오라클 .rs · 출력 로그
```
