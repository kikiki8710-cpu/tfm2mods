...
# 26차 배치 I 도시에 — 담당 `223`~`227` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 I      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `223` | DefenseNexusSubPlan::score | pub | 34 | 20 | 5 | 5 | 57 |
| `224` | v21_defensive_cc_score | in:game_ai | 32 | 31 | 11 | 7 | 73 |
| `225` | AgentVerHamster::item_v26 | in:game_ai | 11 | 3 | 2 | 5 | 15 |
| `226` | build_game_finish_check_state | in:game_ai | 37 | 17 | 9 | 4 | 62 |
| `227` | v47_tower_focus_position_dangerous | pub | 26 | 7 | 4 | 4 | 37 |
...
| # | 이름 | 파일 |
|---|---|---|
| `223` | DefenseNexusSubPlan::score | `MIG\_verify26\I\spec_223_DefenseNexusSubPlan__score.md` |
| `224` | v21_defensive_cc_score | `MIG\_verify26\I\spec_224_v21_defensive_cc_score.md` |
| `225` | AgentVerHamster::item_v26 | `MIG\_verify26\I\spec_225_AgentVerHamster__item_v26.md` |
| `226` | build_game_finish_check_state | `MIG\_verify26\I\spec_226_build_game_finish_check_state.md` |
| `227` | v47_tower_focus_position_dangerous | `MIG\_verify26\I\spec_227_v47_tower_focus_position_dangerous.md` |
...
**[G12 src_line 대조] 44건**
  [224] v21_defensive_cc_score                          consts[21] src_line=609 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [602]

**[G16 params.role 대조] 22건**
  [225] AgentVerHamster::item_v26                       sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 6개(sret보정 -0) vs params 7개 ⟹ 소거 1개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류
  [225] AgentVerHamster::item_v26                       sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 6개(sret 행 없음) ⟹ 6행이어야 하는데 7행. 현재 i = [1, 2, 3, 4, 5, 6, 6]

**[G18 logic↔표 상호참조] 3건**
  [225] AgentVerHamster::item_v26                       logic/P1 logic 이 `+0x30` 를 인용하는데 mem 표에 그 오프셋이 없다
  item_list = &context.item_list                                   // +0x30 (IR 는 승격된 %2)

**[G5 sig 정본 대조] 4건**
  [225] AgentVerHamster::item_v26                       sig.tcx 인자 6개(팻포인터 확장 6) vs params 7개 — 개수가 다르다
  &mut game_ai::AgentVerHamster | &mut rand::rngs::std::StdRng | &game_core::PlayerState | &game_core::GameContext | usize | std::option::Option<usize>
```
...
```json
{
  "round": 26, "batch": "I",
  "errors": [
    {"path": "/specs[223]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[223]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only I --dry
```
...
## §6 출력 디렉터리 — **_verify26/I 만 써라**

```
MIG/_verify26/I/patch.json      ← 기계 적용분(필수)
MIG/_verify26/I/REPORT.md       ← 산문 보고서
MIG/_verify26/I/oracle/         ← 오라클 .rs · 출력 로그
```
