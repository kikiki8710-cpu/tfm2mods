...
# 26차 배치 E 도시에 — 담당 `209`~`211` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `209` | build_minion_wave_snapshot | pub | 37 | 19 | 6 | 8 | 60 |
| `210` | StealSubPlan::action_candidates | pub | 34 | 32 | 6 | 6 | 72 |
| `211` | fight_check::check_kill_die_tick_uncached | in:game_ai::fight_check | 31 | 21 | 7 | 7 | 57 |
...
| # | 이름 | 파일 |
|---|---|---|
| `209` | build_minion_wave_snapshot | `MIG\_verify26\E\spec_209_build_minion_wave_snapshot.md` |
| `210` | StealSubPlan::action_candidates | `MIG\_verify26\E\spec_210_StealSubPlan__action_candidates.md` |
| `211` | fight_check::check_kill_die_tick_uncached | `MIG\_verify26\E\spec_211_fight_check__check_kill_die_tick_uncached.md` |
...
**[G1 자기모순] 1건**
  [211] fight_check::check_kill_die_tick_uncached       자기모순(센티널 폭): 같은 명세에 'usize::MAX' 와 'i64::MAX' 가 동시에 있다

**[G12 src_line 대조] 44건**
  [209] build_minion_wave_snapshot                      consts[4] src_line=654 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [655, 677, 691, 711, 724, 725]
  [209] build_minion_wave_snapshot                      consts[6] src_line=728 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [695, 724, 733, 738, 753]
  [211] fight_check::check_kill_die_tick_uncached       consts[5] src_line=989 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1007, 1012, 1017, 1023, 1028, 1033, 1038, 1052]
  [211] fight_check::check_kill_die_tick_uncached       consts[6] src_line=989 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1007, 1012, 1017, 1023, 1028, 1033, 1038, 1052]
  [211] fight_check::check_kill_die_tick_uncached       consts[7] src_line=989 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1007, 1012, 1017, 1023, 1028, 1033, 1038, 1052]
  [211] fight_check::check_kill_die_tick_uncached       consts[8] src_line=989 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1007, 1012, 1017, 1023, 1028, 1033, 1038, 1052]
  [211] fight_check::check_kill_die_tick_uncached       consts[9] src_line=989 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [1007, 1012, 1017, 1023, 1028, 1033, 1038, 1052]

**[G16 params.role 대조] 22건**
  [211] fight_check::check_kill_die_tick_uncached       sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
  IR 인자 6개(sret보정 -0) vs params 8개 ⟹ 소거 2개가 필요한데 role 이 소거를 주장한 것은 0개(p[]). 자리 매핑 보류
```
...
```json
{
  "round": 26, "batch": "E",
  "errors": [
    {"path": "/specs[209]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[209]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify26/E 만 써라**

```
MIG/_verify26/E/patch.json      ← 기계 적용분(필수)
MIG/_verify26/E/REPORT.md       ← 산문 보고서
MIG/_verify26/E/oracle/         ← 오라클 .rs · 출력 로그
```
