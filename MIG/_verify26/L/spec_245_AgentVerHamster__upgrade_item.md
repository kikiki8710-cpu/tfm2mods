...
### `245` AgentVerHamster::upgrade_item — 아이템 승급 판단 진입점: 아이템빌드가 있으면 v26 경로(보유 슬롯→item_v26→tier≠0 만 채택해 (인벤 인덱스, 아이템)), 없으면 레거시 game_ai::upgrade_item 위임

| 항목 | 값 |
|---|---|
| id | `AgentVerHamster__upgrade_item` |
| 심볼 | `_RNvMs1_CshdEBA0ozCnw_7game_aiNtB5_15AgentVerHamster12upgrade_item` |
| 소스 | `game-ai\src\lib.rs:1193` |
| IR | `m14.ll` 35109~35234행 |
| 경로·가시성 | `game_ai::AgentVerHamster::upgrade_item` · **pub** |
| 계층 | 기타 |
| exe | `e8e160` (None) · None바이트 · None명령 |
| 라운드 | 기준 `r17` · 통과 1회 |
...
| # | i | 이름 | 타입 | 역할 | ev |
|---|---|---|---|---|---|
| 0 | 0 | (sret) | *mut Option<(usize,usize)> 24B (%0) |  | 4 |
| 1 | 1 | self | &mut AgentVerHamster(10704B) (%1) | DI self = %1 | 4 |
| 2 | 2 | rnd | &mut StdRng(320B) (%2) | DI rnd = %2 | 4 |
| 3 | 3 | player | &PlayerState(2528B) (%3) | DI player = %3 | 4 |
| 4 | 4 | game.data_ptr | *const dyn AbstractGame (%4) | tcx sig arg4 = &dyn AbstractGame | 3 |
| 5 | 4 | game.vtable_ptr | AbstractGame vtable ptr (%5) |  | 4 |
| 6 | 5 | context | &GameContext(64B) (%6) | DI context = %6 | 4 |
</details>

**의사코드 `logic`**
> ★재구현용 의사코드다. **오프셋·상수·시그니처의 정본은 `mem`/`consts`/`callees`/`sig.tcx` 이고, 여기와 어긋나면 그쪽이 맞다.** (v2 에서 정정이 표에만 반영되고 이 블록이 옛 값으로 남는 사고가 반복됐다)
```
fn upgrade_item(&mut self, rnd, player, game:&dyn AbstractGame, context) -> Option<(usize,usize)>   // lib.rs:1193~1207
  // L1194
...
|---|---|---|---|---|---|---|---|---|
| 0 | PlayerState | 0x4f0 | info.item_builds.len | r | L1194 (gep 1264) ==0 → 레거시. assume(<2^60) | 4 | OK |  |
| 1 | AgentVerHamster | 0x2910 | version | r | L1205 (gep 10512) 죽은 load — 레거시 upgrade_item 첫 인자는 poison | 4 | OK |  |
| 2 | GameContext | 0x30 | item_list | r | L1195 (gep 48) | 4 | OK |  |
| 3 | Vec<Box<dyn ItemInfo>> | 0x8 | item_list.ptr | r | L1195 | 4 | 확인불가(tcx 사전에 타입 없음) |  |
| 4 | Vec<Box<dyn ItemInfo>> | 0x10 | item_list.len | r | L1195 · L1199 색인 bounds(panic_bounds_check) | 4 | 확인불가(tcx 사전에 타입 없음) |  |
| 5 | ItemInfo vtable | 0x70 | tier | r | L1199 fn(&self)->usize · ==0 이면 승급 아님 → None | 4 | 확인불가(tcx 사전에 타입 없음) |  |
| 6 | sret Option<(usize,usize)> | 0x0 | tag | w | None 4 사이트 = m14.ll:35176/35190/35202/35221 · Some = 35229 | 4 | 확인불가(tcx 사전에 타입 없음) | 0 (L1195 외부 None · L1196 inventory None · L1198 item_v26 None · L1200 tier==0) / 1 (L1203) |
| 7 | sret Option<(usize,usize)> | 0x8 | .0 inventory_index | w | L1203 · m14.ll:35226 · Some 경로만 | 4 | 확인불가(tcx 사전에 타입 없음) | %29 = item_v26_slot sret+16 (Some 페이로드) |
| 8 | sret Option<(usize,usize)> | 0x10 | .1 item | w | L1203 · m14.ll:35228 · Some 경로만 | 4 | 확인불가(tcx 사전에 타입 없음) | %35 = item_v26 반환 payload |

**`consts` 상수 3건** — `src_line` 은 **G12 가 IR 사슬과 대조**한다.
| # | 값 | src_line | 종류 | 뜻 | ev |
|---|---|---|---|---|---|
| 0 | 0 | 1194 | 태그 | item_builds.len()==0 → 레거시 / None 태그 store / L1199 tier()==0 거부 | 4 |
| 1 | -1 | 1195 | 센티널 | item_v26_slot 외부 None 니치(sret+8==usize::MAX) | 4 |
| 2 | 1 | 1198 | 태그 | item_v26 호출 시 inventory_index 태그 Some(상수 1) · L1203 반환 태그 Some | 4 |

**`knobs` 조정점 2건** — `where` 는 **G13 이 그 줄(±2)에 인용 명령이 있는지** 본다.
| # | 무엇 | 어디 | 값 | 효과 | ev | src |
|---|---|---|---|---|---|---|
| 0 | 레거시/v26 경로 스위치 | lib.rs:1194 · m14.ll:35127 | item_builds.len()==0 | 빌드 목록이 비면 레거시 upgrade_item(lib.rs:1603) 만 돈다 — buy_item 과 같은 데이터 스위치 | 4 | 기존 |
| 1 | 승급 채택 tier 조건 | lib.rs:1199 · m14.ll:35213 | tier()!=0 | tier 0 도 허용하면 '기본템으로의 승급' 이 반환돼 buy_item 과 채택이 겹침 | 4 | 기존 |

<details><summary>`callees` 피호출자 9건 (tcx 자동 생성)</summary>

| # | 이름 | 경로 | vis | 시그니처 | 정의처 | mir | xinl | ev | pick |
|---|---|---|---|---|---|---|---|---|---|
| 0 | is_empty | game_core::PatchSetting::is_empty | pub | fn(&game_core::PatchSetting) -> bool | game-core\src\patch_engine.rs:853 | True | True | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 3개 중 상위 3개 |
| 1 | is_empty | game_core::ModAiRegistry::is_empty | pub | fn(&game_core::ModAiRegistry) -> bool | game-core\src\mod_ai.rs:15 | True | True | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 3개 중 상위 3개 |
| 2 | is_empty | game_core::PublicChampionEvidence::is_empty | pub | fn(&game_core::PublicChampionEvidence) -> bool | game-core\src\banpick\public_evidence.rs:96 | True | True | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 3개 중 상위 3개 |
| 3 | item_v26 | game_ai::AgentVerHamster::item_v26 | in:game_ai | fn(&mut game_ai::AgentVerHamster, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &game_core::GameContext, usize, std::option::Option<usize>) -> std::option::Option<usize> | game-ai\src\lib.rs:1149 | False | False | 3 | IR 호출 심볼 일치(토큰 경계) |
| 4 | item_v26_slot | game_ai::AgentVerHamster::item_v26_slot | in:game_ai | fn(&game_core::PlayerState, &[std::boxed::Box<dyn [Binder { value: Trait(game_core::ItemInfo), bound_vars: [] }] + 'static, std::alloc::Global>]) -> std::option::Option<(usize, std::option::Option<usize>)> | game-ai\src\lib.rs:1125 | False | False | 3 | IR 호출 심볼 일치(토큰 경계) |
| 5 | tier | game_core::ItemInfo::tier | pub | fn(&Self/#0) -> usize | game-core\src\setting\item.rs:437 | False | False | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 54개 중 상위 3개 |
| 6 | tier | <game_core::ModItemEntry as game_core::ItemInfo>::tier | pub | fn(&game_core::ModItemEntry) -> usize | game-core\src\setting\item.rs:499 | True | True | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 54개 중 상위 3개 |
| 7 | tier | <game_core::BaseItemInfo as game_core::ItemInfo>::tier | pub | fn(&game_core::BaseItemInfo) -> usize | game-core\src\setting\item.rs:617 | True | True | 4 | ⚠간접호출(vtable) — 망글링 심볼이 **원래 없다**. 확정하려면 `divtable` 로 슬롯을 풀어야 한다. 정답은 대개 트레이트 메서드이고 impl 은 런타임 분기라 하나로 못 고른다. 후보 54개 중 상위 3개 |
| 8 | upgrade_item | game_ai::upgrade_item | in:game_ai | fn(usize, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &dyn [Binder { value: Trait(game_core::AbstractGame), bound_vars: [] }] + , &game_core::GameContext) -> std::option::Option<(usize, usize)> | game-ai\src\lib.rs:1603 | False | False | 3 | IR 호출 심볼 일치(토큰 경계) |
</details>

⚠**미매칭 1개**: `as_slice`
> tcx 3크레이트에 같은 leaf 이름의 Fn/AssocFn 이 없는 것들. 대부분 std·클로저·매크로이거나 `logic` 산문에서 긁힌 잡음이다. ⚠단 **여기 이름이 실제 게임 술어인데 이름이 달라서 못 잡힌 경우**가 섞일 수 있으니, 3차에서 판정에 쓰이는 술어가 이 목록에 있으면 손으로 확인할 것.

**호출처 1곳** (m14.ll:56215) · **형제 90개** (AgentVerHamster)
...
| # | 분류 | 물음 | ev | 시도 |
|---|---|---|---|---|
| 0 | 미탐색 | item_v26_slot(lib.rs:1125 · m14.ll:35237) 내부 — 계약만: fn(&PlayerState, &[Box<dyn ItemInfo>]) -> Option<(usize, Option<usize>)> · sret 24B(+0 build_slot · +8 내부 태그/외부 니치 usize::MAX · +16 inventory_index) | 4 |  |
| 1 | 미탐색 | game_ai::upgrade_item(lib.rs:1603 · m14.ll:6620 · 레거시) 내부 — 계약만: fn(version, rnd, player, game, context) -> Option<(usize,usize)> sret 24B · version/game 미사용(poison 전달) | 4 |  |

**`notes` 1건 — 확정된 사실 서술이다. 파지 말고, 틀렸다고 보면 반증하라.**
| # | 내용 | ev | class |
|---|---|---|---|
| 0 | 반환 튜플의 소비처(game_core 쪽 AiAgent::upgrade_item 호출부)가 .0 을 인벤 슬롯, .1 을 item_list 인덱스로 쓰는지는 game_core 경계 — 이 함수의 store 위치·DI 이름에서만 확정 | 4 | 사실 서술 |
