...
# 26차 배치 K 도시에 — 담당 `234`~`240` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 K      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `234` | JungleSubPlan::score | pub | 22 | 8 | 3 | 3 | 33 |
| `235` | AttackNexusSubPlan::score | pub | 20 | 10 | 3 | 3 | 33 |
| `236` | v15_can_keep_support_pressure | in:game_ai | 15 | 7 | 4 | 4 | 25 |
| `237` | RecallSubPlan::score | pub | 12 | 13 | 5 | 4 | 30 |
| `238` | v22_current_line_non_champion_action_tower_risk | pub | 24 | 10 | 2 | 4 | 35 |
| `239` | around::SmallActionAroundBush::update_state | in:game_ai | 20 | 8 | 3 | 2 | 31 |
| `240` | LineDefenseSubPlan::calculate_score_parameter_value | pub | 17 | 9 | 6 | 5 | 32 |
...
| # | 이름 | 파일 |
|---|---|---|
| `234` | JungleSubPlan::score | `MIG\_verify26\K\spec_234_JungleSubPlan__score.md` |
| `235` | AttackNexusSubPlan::score | `MIG\_verify26\K\spec_235_AttackNexusSubPlan__score.md` |
| `236` | v15_can_keep_support_pressure | `MIG\_verify26\K\spec_236_v15_can_keep_support_pressure.md` |
| `237` | RecallSubPlan::score | `MIG\_verify26\K\spec_237_RecallSubPlan__score.md` |
| `238` | v22_current_line_non_champion_action_tower_risk | `MIG\_verify26\K\spec_238_v22_current_line_non_champion_action_tower_risk.md` |
| `239` | around::SmallActionAroundBush::update_state | `MIG\_verify26\K\spec_239_around__SmallActionAroundBush__update_state.md` |
| `240` | LineDefenseSubPlan::calculate_score_parameter_value | `MIG\_verify26\K\spec_240_LineDefenseSubPlan__calculate_score_parameter_value.md` |
...
**이 배치 몫 = 5건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G10 class 오분류] 6건**
  [238] v22_current_line_non_champion_action_tower_ris  class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  필드 의미(비활성 시작 시각인지 종료 시각인지)는 game_core setting.rs 미독으로 미확정(미탐색 = `_gcbc` 에서 tower_attack_disable_tick 소비처 grep)

**[G12 src_line 대조] 44건**
  [235] AttackNexusSubPlan::score                       consts[9] src_line=201 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [178, 193]

**[G16 params.role 대조] 22건**
  [240] LineDefenseSubPlan::calculate_score_parameter_  sig.params[4] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[4] parameter `readnone` — IR %4 = ptr noalias noundef align 8 captures(none) dereferenceable(5384) %4
  [240] LineDefenseSubPlan::calculate_score_parameter_  sig.params[4] P4 role 이 주장한 IR 속성이 `define` 에 없다
  p[4] parameter `readonly` — IR %4 = ptr noalias noundef align 8 captures(none) dereferenceable(5384) %4

**[G7 과열림] 22건**
  [236] v15_can_keep_support_pressure                   ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  blackboard 인덱스가 `1 - player.info.team`(적팀 번호)인 의미 — Blackboard 배열이 '팀별 관측 보드'인지 '상대팀에 대한 보드'인지는 game_core 소관(is_recent_v
```
...
```json
{
  "round": 26, "batch": "K",
  "errors": [
    {"path": "/specs[234]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[234]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only K --dry
```
...
## §6 출력 디렉터리 — **_verify26/K 만 써라**

```
MIG/_verify26/K/patch.json      ← 기계 적용분(필수)
MIG/_verify26/K/REPORT.md       ← 산문 보고서
MIG/_verify26/K/oracle/         ← 오라클 .rs · 출력 로그
```
