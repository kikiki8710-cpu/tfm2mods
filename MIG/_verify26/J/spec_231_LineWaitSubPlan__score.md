...
### `231` LineWaitSubPlan::score — LineWait 서브플랜의 후보 액션 점수: evaluate_action(Lane 우선순위·Lane 앵커) Some 이면 그 값, None 이면 interaction_score+경제보정+액션별 가산(Attack/Skill/Skill2=calculate_action_score, Around 아군구조물 원거리=50/100, 대상 소실=-99999)

| 항목 | 값 |
|---|---|
| id | `line_wait__LineWait__score` |
| 심볼 | `_RNvMNtNtNtCshdEBA0ozCnw_7game_ai11plan_legacy8sub_plan9line_waitNtB2_15LineWaitSubPlan5score` |
| 소스 | `game-ai\src\plan_legacy\sub_plan\line_wait.rs:159` |
| IR | `m15.ll` 22517~22838행 |
| 경로·가시성 | `game_ai::plan_legacy::sub_plan::LineWaitSubPlan::score` · **pub** |
| 계층 | 기타 |
| exe | `eb5840` (None) · None바이트 · None명령 |
| 라운드 | 기준 `r17` · 통과 1회 |

**시그니처(tcx 정본, ev3)**
```rust
fn(&game_ai::plan_legacy::sub_plan::LineWaitSubPlan, usize, &game_ai::ScoreParameter, &mut rand::rngs::std::StdRng, &game_core::PlayerState, &game_core::OperationData, &game_ai::SmallActionPlay, &mut game_core::DebugFrameData) -> i64
```

**TLS 메모 절(`sig.tls` · 정정 경로 `/specs[231]/sig/tls/<키>`)**
...
| # | i | 이름 | 타입 | 역할 | ev |
|---|---|---|---|---|---|
| 0 | 1 | self | &LineWaitSubPlan(1B) — {line: LineType@+0x0} |  | 4 |
| 1 | 2 | version | usize |  | 4 |
...
let add = match action.get_action() {
    ...
    // idx 14 Skill2{target_id}  (L189)
    Skill2{target_id} => if let Some(t) = get_entity_by_id(target_id) {
            let effect = (if champ.level /*+0x5c8*/ > 2 { champ.skill2_effect.as_ref() } else { None }).unwrap();   // L190 entity.rs:1693 · +0x530 / +0x500 · level≤2 또는 None 이면 unwrap_failed
            calculate_action_score(version, rnd, player, data, parameter, &champ.skill2 /*+0x590*/, effect, champ.cooldown_reduce(false), t, Pull, debug)   // L191
        } else { -99999 },
    // SmallAction::Around{target_id} ← SmallActionPlay idx 2 Around / 3 AroundHide / 10 LaneMinionPosition (모두 payload+0x8 = target)  (L197)
    Around{target_id} => if let Some(t) = get_entity_by_id(target_id) {
            if t.team == champ.team /*L198 · TeamType derived eq: 태그(+0x0) 같고, 둘 다 Player(0) 면 +0x8 도 같아야*/ {
                let dist = (|t.x-champ.x|)^2 + (|t.y-champ.y|)^2;   // L199 · +0x660/+0x668 · u64 abs-diff 후 제곱합
                if dist > 39999999999 /*L202 · ≥200000² = 6.25셀*/ {
                    match t.ty /*+0x68*/ { Minion(1) | Tower(2) => 50, Nexus(3) => 100, _ => 0 }   // L203~L205
                } else { 0 }
            } else { 0 }
...
return base + economy_adjustment + add;   // L169 add 순서: (economy+base)+add · L222

★복붙 대조: LineSafeSubPlan::score(line_safe.rs:104, m02.ll 48427~48748) 의 **완전 복제본** — 정규화 diff(레지스터명·!dbg·@anon·attr 번호 제거) 0줄 · rmeta_srcmap 줄길이 line_wait.rs:159~223 ↔ line_safe.rs:104~168 전 줄 일치(유일한 차이 = 주석줄 170(204B/한글40) vs 115(170B/한글37) — IR 에 안 나옴). PriorityProfile::Lane(tag 2)·Anchor::Lane(self.line)·200000²·50/100·Pull(0)·-99999 전부 동일 — LineWait 고유값 0. ★복붙 의심: 「대기(Wait)」플랜인데 priority 가 LineSafe 와 같은 Lane 이고 원거리 아군 구조물 가산도 같다 — 플랜 구분은 이 함수가 아니라 action_candidates/next_plan 쪽에서만 생긴다.
...
|---|---|---|---|---|---|---|---|---|
| 0 | LineWaitSubPlan | 0x0 | line | r | L160 · ActionContext.anchor = Anchor::Lane{line} 의 페이로드(+0x11)로 복사 | 4 | OK |  |
| 1 | PlayerState | 0x930 | info.team | r | L163 · player_champion[team] 1차 인덱스 · <2 bounds check(panic_bounds_check) | 4 | OK |  |
| 2 | PlayerState | 0x9c0 | info.position@tag(i32) | r | L163(player.rs:581 인라인) · zext 후 player_champion[team][pos] 2차 인덱스 | 4 | OK |  |
| 3 | OperationData | 0x0 | cache | r | L163 · &AbstractGameWithCache | 4 | OK |  |
| 4 | AbstractGameWithCache | 0x0 | game.data_ptr | r | L173/126/134/142 · dyn AbstractGame 팻포인터 데이터 | 4 | OK |  |
| 5 | AbstractGameWithCache | 0x8 | game.vtable_ptr | r | L173/126/134/142 · vtable+0x1f0(=496) = AbstractGame::get_entity_by_id 슬롯(divtable 확인) | 3 | OK |  |
| 6 | AbstractGameWithCache | 0x1e0 | player_champion[2][5] | r | L163 · Option<&Entity> — null 이면 unwrap_failed(entity.rs:1011/option.rs:1013) | 4 | OK |  |
| 7 | SmallActionPlay | 0xb1 | tag(u8, niche) | r | L169(small_action.rs:309 get_action 인라인) · idx = tag>2 ? tag-3 : 7(AroundPosition 암묵) · llvm.assume(tag≠10) | 4 | OK |  |
...
| 9 | Entity(champ) | 0x4c0 | attack_effect@tag(i32, -1=None) | r | L174(option.rs:742 as_ref) · None 이면 unwrap_failed | 4 | OK |  |
| 10 | Entity(champ) | 0x490 | attack_effect@Some.0 (&Effect) | r | L175 · calculate_action_score 의 effect 인자 | 4 | OK |  |
| 11 | Entity(champ) | 0x570 | attack (Box<dyn Action>) | r | L175(entity.rs:1494 인라인) · calculate_action_score 의 action 인자 | 4 | OK |  |
| 12 | Entity(champ) | 0x4f8 | skill_effect@tag | r | L182 | 4 | OK |  |
| 13 | Entity(champ) | 0x4c8 | skill_effect@Some.0 | r | L183 | 4 | OK |  |
| 14 | Entity(champ) | 0x580 | skill (Box<dyn Action>) | r | L183(entity.rs:1665 인라인) | 4 | OK |  |
| 15 | Entity(champ) | 0x5c8 | level | r | L190(entity.rs:1693 인라인) · level>2 아니면 skill2_effect 가 None 취급 → unwrap_failed | 4 | OK |  |
| 16 | Entity(champ) | 0x530 | skill2_effect@tag | r | L190 | 4 | OK |  |
| 17 | Entity(champ) | 0x500 | skill2_effect@Some.0 | r | L190(entity.rs:1694) | 4 | OK |  |
| 18 | Entity(champ) | 0x590 | skill2 (Box<dyn Action>) | r | L191(entity.rs:1670 인라인) | 4 | OK |  |
| 19 | Entity(champ, t) | 0x0 | team@tag (TeamType: 0=Player,1=Neutral) | r | L198(entity.rs:1127 derived PartialEq) · t.team == champ.team | 4 | OK |  |
| 20 | Entity(champ, t) | 0x8 | team@Player.0 (usize) | r | L198 · 태그가 둘 다 0(Player) 일 때만 비교 | 4 | OK |  |
| 21 | Entity(champ, t) | 0x660 | x (u64) | r | L199(entity.rs:2158 거리제곱 인라인) | 4 | OK |  |
| 22 | Entity(champ, t) | 0x668 | y (u64) | r | L199 | 4 | OK |  |
| 23 | Entity(t) | 0x68 | ty@tag (EntityType) | r | L203(entity.rs:1261 is_any_type_minion 인라인 · dloc 확인) · switch: 1=Minion,2=Tower→50 · 3=Nexus→100 · 그외→0. 1261 스코프는 switch 명령 하나에 붙어 있어 Tower 판정이 is_any_type_minion 안인지 별도 is_tower(entity.rs:1386) 인지 IR 로 구분 불가 | 4 | OK |  |
| 24 | AbstractGame vtable | 0x1f0 | get_entity_by_id(&self, id:usize)->Option<&Entity> | r | 간접 호출 4곳(L173/126/134/142) — 심볼 호출이 아니라 calls 에 없음 | 4 | 확인불가(vtable 슬롯) |  |
| 25 | ActionContext(스택 alloca %9, 40B) | 0x0 | priority@tag | w | L160 · evaluate_action 의 ctx 인자. &mut 인자(rnd/debug)에는 본문 직접 쓰기 0 · initializes 속성 없음 | 4 | OK | 2 (= PriorityProfile::Lane 메모리태그 · tcxdict --enum: niche_start=2, Battle 암묵) |
| 26 | ActionContext(스택 alloca %9, 40B) | 0x10 | anchor@tag | w | L160 | 4 | OK | 1 (= Anchor::Lane, Direct 인코딩) |
| 27 | ActionContext(스택 alloca %9, 40B) | 0x11 | anchor@Lane.line | w | L160 · +0x8(priority 페이로드)·+0x18~ 은 미기록(undef) — 콜리가 Lane 태그에서는 안 읽는 슬롯 | 4 | OK | self.line (LineType u8) |

**`consts` 상수 15건** — `src_line` 은 **G12 가 IR 사슬과 대조**한다.
| # | 값 | src_line | 종류 | 뜻 | ev |
|---|---|---|---|---|---|
| 0 | 2 | 160 | 태그 | PriorityProfile::Lane 의 메모리태그(store i64 2 → ActionContext+0x0). 별도로 L163 의 `icmp ult team, 2` 는 [2] 배열 bounds check(임계 아님) | 4 |
| 1 | 1 | 160 | 태그 | Anchor::Lane 의 태그(store i8 1 → ActionContext+0x10) | 4 |
| 2 | 10 | 169 | 센티널 | SmallActionPlay 니치 구멍(AroundPosition 암묵 variant 자리) — llvm.assume(tag≠10). 판정 아님 | 4 |
| 3 | 3 | 169 | 태그 | SmallActionPlay 태그→논리 idx 환산(tag-3, niche_start=3). 판정 아님 | 4 |
| 4 | 7 | 169 | 태그 | tag≤2 이면 idx 7 = AroundPosition(untagged). 판정 아님 | 4 |
| 5 | -1 | 174 | 센티널 | Option<Effect> None 니치(casting@tag i32 = -1) — attack/skill/skill2_effect 의 as_ref().unwrap() 검사 | 4 |
| 6 | 0 | 168 | 태그 | MinionActionType::Pull(=0) — line_action_economy_adjustment 6번째 인자(i8 0) 와 calculate_action_score 10번째 인자(ty) 둘 다 Pull. DI 지역상수 action_type=i8 0 | 4 |
| 7 | -99999 | 173 | 산출값 | Attack/Skill/Skill2 의 target_id 가 get_entity_by_id 로 안 풀리면(대상 소실) 가산값 = -99999 (L173·L181·L189 세 arm 공통) | 4 |
| 8 | 39999999999 | 202 | 임계 | 거리제곱 임계: dist² > 39999999999 ⟺ dist ≥ 200000 (= 6.25셀). Around 계열 액션의 대상이 같은 팀이고 이만큼 멀 때만 구조물 가산 | 4 |
| 9 | 1 | 203 | 태그 | EntityType::Minion 태그 → 가산 50 | 4 |
| 10 | 2 | 203 | 태그 | EntityType::Tower 태그 → 가산 50 | 4 |
| 11 | 3 | 203 | 태그 | EntityType::Nexus 태그 → 가산 100 | 4 |
| 12 | 50 | 203 | 산출값 | Around 대상이 아군 Minion/Tower 이고 dist≥200000 일 때 가산 | 4 |
| 13 | 100 | 205 | 산출값 | Around 대상이 아군 Nexus 이고 dist≥200000 일 때 가산 (그 외 EntityType → 0) | 4 |
| 14 | 2 | 190 | 임계 | champ.level > 2 여야 skill2_effect 를 Some 으로 봄(entity.rs:1693 인라인) — 아니면 unwrap_failed(패닉). Skill2 후보가 level≤2 에서 생성되면 크래시 경로 | 4 |

**`knobs` 조정점 5건** — `where` 는 **G13 이 그 줄(±2)에 인용 명령이 있는지** 본다.
| # | 무엇 | 어디 | 값 | 효과 | ev | src |
|---|---|---|---|---|---|---|
| 0 | Around 구조물 가산의 거리 임계 | line_wait.rs:202 | 39999999999 | 내리면 더 가까운 아군 구조물 주변 대기도 50/100 가산을 받아 구조물 근처 배회 선호가 강해짐. 올리면 멀리 떨어진 구조물로 갈 때만 가산 | 4 | 기존 |
| 1 | 아군 Minion/Tower 주변 대기 가산 | line_wait.rs:203 | 50 | 올리면 LineSafe 중 원거리 아군 미니언/타워 쪽 Around 후보가 더 자주 선택됨 (헬퍼: is_any_type_minion — Tower 도 같은 50) | 4 | 기존 |
| 2 | 아군 Nexus 주변 대기 가산 | line_wait.rs:205 | 100 | 올리면 넥서스로 후퇴 대기 선호↑ (Minion/Tower 50 의 2배) | 4 | 기존 |
| 3 | 대상 소실 페널티 | line_wait.rs:173/181/189 | -99999 | Attack/Skill/Skill2 의 target 이 사라진 후보를 사실상 배제. 값을 완화하면 죽은 대상 공격 후보가 살아남을 수 있음 | 4 | 기존 |
| 4 | 경제 보정 미니언 액션 타입 | line_wait.rs:168 | 0 | MinionActionType::Pull 고정. Normal(1)/Push(2)로 바꾸면 line_action_economy_adjustment 의 라인 경제 판정 기준이 바뀜(AttackNexus 는 Push=2 사용) | 4 | 기존 |
...
**호출처 1곳** (m12.ll:37243) · **형제 10개** (LineWaitSubPlan)
...
| # | 분류 | 물음 | ev | 시도 |
|---|---|---|---|---|
| 0 | 표기 불가 | L160 `if let Some(v)` 의 소스 표기(if let / map_or / ? 등) — IR 은 {i64,i64} 의 bit0 검사 뒤 즉시 반환. 동작 확정, 표기 불가(column 부재). | 4 |  |
| 1 | 미탐색 | L169 match arm 의 소스 순서(Attack→Skill→Skill2→Around 인지) — switch 로 접혀 순서 근거는 arm 루트 줄번호(173<181<189<197)뿐. 동작엔 무관. | 4 |  |
| 2 | 표기 불가 | L203 헬퍼 = entity.rs:1261 is_any_type_minion(dloc 확인, define 없음=전부 인라인) — 태그 1(Minion)·2(Tower) 가 같은 50 으로 접혀 있어 `is_any_type_minion()` 이 Tower 까지 포함하는지, `\|\| t.is_tower()` 가 붙은 것인지 표기 불가(줄길이 64B 는 16칸 들여쓰기 + `if t.is_any_type_minion() \|\| t.is_tower() { 50 }`(48자) 와 일치 — 추정). 100 은 Nexus(3) · 0 은 그 외 — 동작 확정. | 3 |  |
...
| 4 | 미탐색 | 이 함수가 LineSafe::score 와 동일 코드인 것이 의도(공용 헬퍼 대신 복붙)인지 — 소스 부재로 판정 불가. IR·줄길이 증거상 문자 단위 복제. | 3 |  |

**`notes` 1건 — 확정된 사실 서술이다. 파지 말고, 틀렸다고 보면 반증하라.**
| # | 내용 | ev | class |
|---|---|---|---|
| 0 | L198 `t.team == champ.team` 이 소스에서 `==` 인지 헬퍼(예: is_same_team) 인지 — entity.rs:1127 PartialEq 인라인만 보임. 동작 확정. | 4 | 사실 서술 |
