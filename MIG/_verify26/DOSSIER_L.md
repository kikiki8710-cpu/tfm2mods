...
# 26차 배치 L 도시에 — 담당 `241`~`248` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 L      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `241` | RecallSubPlan::action_candidates | pub | 15 | 6 | 2 | 3 | 22 |
| `242` | StealSubPlan::score | pub | 12 | 13 | 4 | 4 | 28 |
| `243` | trace::SmallActionTrace::is_end | in:game_ai | 17 | 3 | 2 | 3 | 22 |
| `244` | move_actions::SmallActionRecall::is_end | in:game_ai | 15 | 3 | 2 | 3 | 20 |
| `245` | AgentVerHamster::upgrade_item | pub | 9 | 3 | 2 | 2 | 14 |
| `246` | SerpenCheckSubPlan::score | pub | 13 | 5 | 1 | 2 | 18 |
| `247` | AgentVerHamster::buy_item | pub | 6 | 3 | 2 | 3 | 11 |
| `248` | cast::SmallActionUlt::is_end | in:game_ai | 12 | 3 | 1 | 3 | 15 |
...
| # | 이름 | 파일 |
|---|---|---|
| `241` | RecallSubPlan::action_candidates | `MIG\_verify26\L\spec_241_RecallSubPlan__action_candidates.md` |
| `242` | StealSubPlan::score | `MIG\_verify26\L\spec_242_StealSubPlan__score.md` |
| `243` | trace::SmallActionTrace::is_end | `MIG\_verify26\L\spec_243_trace__SmallActionTrace__is_end.md` |
| `244` | move_actions::SmallActionRecall::is_end | `MIG\_verify26\L\spec_244_move_actions__SmallActionRecall__is_end.md` |
| `245` | AgentVerHamster::upgrade_item | `MIG\_verify26\L\spec_245_AgentVerHamster__upgrade_item.md` |
| `246` | SerpenCheckSubPlan::score | `MIG\_verify26\L\spec_246_SerpenCheckSubPlan__score.md` |
| `247` | AgentVerHamster::buy_item | `MIG\_verify26\L\spec_247_AgentVerHamster__buy_item.md` |
| `248` | cast::SmallActionUlt::is_end | `MIG\_verify26\L\spec_248_cast__SmallActionUlt__is_end.md` |
...
**이 배치 몫 = 3건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G10 class 오분류] 6건**
  [248] cast::SmallActionUlt::is_end                    class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  — dloc 사슬(entity.rs:1482 is_visible_from → data.rs:122 is_visible)로 is_visible_from 경유 확정, 부정(!)은 phi 극성에서 확정

**[G16 params.role 대조] 22건**
  [245] AgentVerHamster::upgrade_item                   sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 5개(sret 행 있음) ⟹ 6행이어야 하는데 7행. 현재 i = [0, 1, 2, 3, 4, 4, 5]
  [247] AgentVerHamster::buy_item                       sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 5개(sret 행 없음) ⟹ 5행이어야 하는데 6행. 현재 i = [1, 2, 3, 4, 4, 5]
```
...
```json
{
  "round": 26, "batch": "L",
  "errors": [
    {"path": "/specs[241]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[241]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only L --dry
```
...
## §6 출력 디렉터리 — **_verify26/L 만 써라**

```
MIG/_verify26/L/patch.json      ← 기계 적용분(필수)
MIG/_verify26/L/REPORT.md       ← 산문 보고서
MIG/_verify26/L/oracle/         ← 오라클 .rs · 출력 로그
```
