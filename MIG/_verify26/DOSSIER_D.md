...
# 26차 배치 D 도시에 — 담당 `207`~`208` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `9451345d9b9ff946` | 2026-09-16 05:45:55 |
| `_spec/specs20.json` (손이 닿는 정본) | `4dc3d0d1844bae4b` | 2026-09-16 05:42:53 |

| `mkdossier.py`(이 파일을 만든 도구) | `232c166a31001480` | 2026-09-16 00:21:13 |

이 파일 생성 = `2026-09-16 05:53:35`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 26 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `207` | resolve_fight_uncached | in:game_ai::plan_legacy::old::fight_model | 30 | 22 | 11 | 18 | 59 |
| `208` | get_input | pub | 116 | 43 | 11 | 19 | 170 |
...
| `MIG\TOOLS.md` | 284 | `42a081575fa47aa2` | 도구 인벤토리(자동 생성). **무엇이 있는가**만 센다 — 무엇을 집는가는 `METHOD_MAP §0`. |
...
| # | 이름 | 파일 |
|---|---|---|
| `207` | resolve_fight_uncached | `MIG\_verify26\D\spec_207_resolve_fight_uncached.md` |
| `208` | get_input | `MIG\_verify26\D\spec_208_get_input.md` |
...
**[G12 src_line 대조] 44건**
  [208] get_input                                       consts[2] src_line=855 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [906]

**[G15 consts.kind 관측] 1건**
  [207] resolve_fight_uncached                          consts[21] kind=임계 NEG
  `meaning` 이 「icmp ult %idx, 5` · panic_bounds_check(…,5)). 임계 아님 · stride 아님 ·」라 적었는데 kind=임계

**[G16 params.role 대조] 22건**
  [207] resolve_fight_uncached                          sig.params[0] P1 정렬 불변식 깨짐(SROA 인자 승격 미기재)
  IR 인자 15개(sret보정 -0) > params 11개인데 role 이 `%k` 로 승격 대응을 전부 적지 않았다(적은 것 p[2], 덮은 레지스터 ['%2', '%3']). 자리 매핑 보류

**[G20 명세간 공유사실] 168건**
  [208] get_input                                       [R1] AgentVerHamster 0x2860 — 같은 (base,offset) 을 3가지 이름으로 부른다: goal_x) / small_action payload +0x8 / small_action.<variant>+0x8
  [208]`small_action.<variant>+0x8` · [212]`small_action payload +0x8/goal_x)`
  [208] get_input                                       [R1] AgentVerHamster 0x2868 — 같은 (base,offset) 을 6가지 이름으로 부른다: AroundPositionBush.target_y) / AroundRegion.goal_x / RunAway·Positioning.goal_y / small_action.<Attack|Skill|Skill2|Ult>.is_act / small_action.<cast>.is_act / small_action.<variant>+0x10
  [208]`small_action.<variant>+0x10/AroundRegion.goal_x/RunAway·Positioning.goal_y/AroundPositionBush.target_y)` · [212]`small_action.<cast>.is_act` · [
  [208] get_input                                       [R1] AgentVerHamster 0x2870 — 같은 (base,offset) 을 5가지 이름으로 부른다: AroundBush target_x) / AroundBush.target_x) / AroundRegion.goal_y / small_action payload +0x18 / small_action.<variant>+0x18
  [208]`small_action.<variant>+0x18/AroundRegion.goal_y/AroundBush.target_x)` · [212]`small_action payload +0x18/AroundBush target_x)`
  [208] get_input                                       [R1] AgentVerHamster 0x2878 — 같은 (base,offset) 을 2가지 이름으로 부른다: small_action payload +0x20 / small_action@AroundBush.target_y
  [208]`small_action@AroundBush.target_y` · [212]`small_action payload +0x20`
  [208] get_input                                       [R1] AgentVerHamster 0x2888 — 같은 (base,offset) 을 3가지 이름으로 부른다: AroundPosition.around_input.target_x / small_action.RunAway.prog_best_tick / small_action@AroundPosition.around_input.target_x
  [208]`small_action@AroundPosition.around_input.target_x` · [212]`small_action.RunAway.prog_best_tick/AroundPosition.around_input.target_x`
  [208] get_input                                       [R1] AgentVerHamster 0x2890 — 같은 (base,offset) 을 2가지 이름으로 부른다: small_action payload +0x38 / small_action@AroundPosition.around_input.target_y
  [208]`small_action@AroundPosition.around_input.target_y` · [212]`small_action payload +0x38`
  [208] get_input                                       [R1] AgentVerHamster 0x28b0 — 같은 (base,offset) 을 2가지 이름으로 부른다: small_action.Trace.start_tick / small_action@Recall.goal_y
  [208]`small_action@Recall.goal_y` · [218]`small_action.Trace.start_tick`
  [208] get_input                                       [R1] AgentVerHamster 0x28b8 — 같은 (base,offset) 을 2가지 이름으로 부른다: small_action.Trace.target / small_action@Trace.target
  [208]`small_action@Trace.target` · [212]`small_action.Trace.target`
  [208] get_input                                       [R1] AgentVerHamster 0x28ea — 같은 (base,offset) 을 2가지 이름으로 부른다: small_action.Trace.abandoned / small_action@Trace.abandoned
  [208]`small_action@Trace.abandoned` · [212]`small_action.Trace.abandoned`
  [208] get_input                                       [R1] AgentVerHamster 0xb18 — 같은 (base,offset) 을 2가지 이름으로 부른다: plan_system.plan / plan_system.plan@tag
  [208]`plan_system.plan` · [208]`plan_system.plan@tag` · [212]`plan_system.plan@tag`
  [208] get_input                                       [R2] AgentVerHamster small_action@tag — 같은 이름이 오프셋 0x28b1 / 0x2909 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [208]`0x28b1` · [208]`0x2909` · [212]`0x2909` · [218]`0x2909`

**[G7 과열림] 22건**
  [208] get_input                                       ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  (배치 N) L1018~1019 `data.blackboard[1 - team]`(적팀 인덱스) 로 is_recent_visible 을 부르는 의미론 — IR 사실(m14.ll:40826~40828 · getelem
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 8361 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 3999 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 1526 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 5538 | ⚠**1932행만 IR 앵커**(`ev3`). 나머지 3606행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 26, "batch": "D",
  "errors": [
    {"path": "/specs[207]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[207]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 26 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify26/D 만 써라**

```
MIG/_verify26/D/patch.json      ← 기계 적용분(필수)
MIG/_verify26/D/REPORT.md       ← 산문 보고서
MIG/_verify26/D/oracle/         ← 오라클 .rs · 출력 로그
```
