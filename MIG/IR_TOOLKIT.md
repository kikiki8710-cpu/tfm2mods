...
⚠**`~~취소선~~` 오탐 — 2026-09-11 수정.** `--prose` 가 `~~이미 정정된 옛 값~~` 까지 스캔해
「오귀속」으로 되잡았다(실측: `~~PlayerState+0x2496~~` 이 배치 A·D 감사에서 나란히 잡힘).
§7 정정형 기록은 옛 값을 **그 자리에 남기라**고 요구하므로, 이건 정정을 쓸 때마다
오탐이 하나씩 영구히 쌓이는 구조였다 — 즉 **감사 도구가 문서 규약을 벌하고 있었다**.
지금은 취소선 구간을 길이 보존 공백으로 덮고(문맥 창 정렬 유지) **제외 건수를 리포트 머리에 찍는다**
(조용한 억제 금지). 수정 후 재실측: **오귀속 0 · 제외 5건**(4갈래 리포트 + specs20 + resolved 동시 스캔).
