...
- ★**소유 타입 열을 봐라.** 같은 이름의 메서드가 다른 타입에 있으면 어느 쪽을 읽는지 알아야 한다.
  소유자는 망글 심볼의 **이름 바로 앞 성분**으로 잡는다.
- ★★**소유 타입이 다르다고 조각을 버리지 마라 — 먼저 복제본인지 확인하라.**
  `LineGankCoverPlan::target_bush_v30` 과 `LineGankerPlan::target_bush_v30` 은
  **문자 단위 동일 복제본**이다: rmeta SourceMap 줄 길이가 `cover.rs:134~186` ↔ `ganker.rs:248~300`
  **53줄 전부 일치**(오프셋 차 +114)이고 다른 것은 `self` 타입뿐
  (`LineGankCoverPlan+0x20` ↔ `LineGankerPlan+0x28`).
  ⟹ 담당 함수에 `define` 이 없어도 **복제본이 유효한 대리 관측점**이 된다. 실제로 그 조각 하나가
  `13`·`14` 의 실행 검증 90행을 만들었고, 그게 **유일한 통로**였다.
  **확인 방법**: `rmeta_srcmap` 으로 양쪽 줄 길이 배열을 뽑아 대조한다(전 줄 일치 = 복제본).
