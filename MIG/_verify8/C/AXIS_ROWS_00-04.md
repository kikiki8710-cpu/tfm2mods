# 8차 배치 C — 축 ``sig.params[].role` — 인자의 역할` · 함수 `00`~`04` (33행)

> `mkdossier.py --axis` 생성물. **칸을 하나도 줄이지 않았다** — 7차까지 지시 오류는 전부 「줄여 적은 자리」에서 났다.

| # | 함수 | idx | i | name | ty | role | ev |
|---|---|---|---|---|---|---|---|
| `00` | ult | 0 | 1 | (sret) |  | 반환값 출력 슬롯. +0x0=태그(-1=None, 5=Input::Ult), +0x8=InputTarget(24B) | 4 |
| `00` | ult | 1 | 2 | version |  | AI 버전 게이트. 이 함수 본문에선 분기 없음 — get_input_target / safe_move_avoiding_enemy_well 로 그대로 전달만 한다 | 4 |
| `00` | ult | 2 | 3 | rnd |  | 본문에서 직접 안 씀. get_input_target 으로 전달만 | 4 |
| `00` | ult | 3 | 4 | player |  | info.team(0x930) / info.position(0x9c0) 을 읽어 자기 챔피언을 찾는 데 씀 | 4 |
| `00` | ult | 4 | 5 | data |  | cache(0x0)=AbstractGameWithCache, context(0x8)=GameContext | 4 |
| `00` | ult | 5 | 6 | positioning_score |  | 본문에서 직접 안 씀. get_input_target 으로 전달만 | 4 |
| `00` | ult | 6 | 7 | target |  | 궁극기를 쓸 상대(적) 엔티티. 거리·가시성·반경 계산의 기준 | 4 |
...
| `02` | sub_plan | 0 | 1 | self |  | 필드 team:usize@0x0 · line:LineType@0x8. 본문은 line 만 읽는다(team 은 안 씀 — team 은 player.info.team 에서 가져옴) | 4 |
| `02` | sub_plan | 1 | 2 | version |  | AI 버전 게이트. ★이 함수에선 한 번도 참조되지 않음(분기 없음) | 4 |
| `02` | sub_plan | 2 | 3 | rnd |  | readnone — 난수 미사용 | 4 |
| `02` | sub_plan | 3 | 4 | player |  | info.team(0x930) · info.position(0x9c0) 만 읽음 | 4 |
| `02` | sub_plan | 4 | 5 | data |  | cache(0x0) · context(0x8) 사용. blackboard(0x10) 미사용 | 4 |
| `02` | sub_plan | 5 | 6 | _team_plan |  | readnone — 미사용(이름의 _ 접두와 일치) | 4 |
| `02` | sub_plan | 6 | 7 | _debug |  | readnone — 미사용 | 4 |
...
| `03` | defensive_crisis | 4 | 5 | target |  | 위기 판정 대상 챔피언(보통 아군 자신). id 와 좌표만 직접 쓰인다 | 4 |
