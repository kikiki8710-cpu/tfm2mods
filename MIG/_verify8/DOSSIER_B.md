...
# 8차 배치 B 도시에 — **축 ``consts[].kind` — 상수의 종류`** (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG && python -X utf8 dossierfresh.py 8 B
```
...
**담당 축 = `consts[].kind` — 상수의 종류** — 186행. ★**무검사가 아니라 「무측정」 축**이다 — `mkspec3` 이 `meaning` 낱말로 유도하고 **잔여를 전부 `임계`로 떨군다**(그래서 「임계 111행」은 분류가 아니라 나머지다). 게다가 **v2 에 없는 파생 필드**라 `errors[]` 로 직접 못 쓴다.

이 축의 전 20함수 행 = **186개**. (§3 에 전량 있다.)

### 할 일 (순서대로)

1. `MIG\_gates\consts_kind\` 에 **다른 배치들이 만든 검사기 후보가 모여 있다.** 전부 돌려 보고 **왜 결과가 갈리는지** 규명하라.
2. 하나로 **통합**해서 `MIG\_verify8\B\gate.py` 로 낸다. 통합판은 다른 후보들의 장점을 흡수해야 한다.
...
그리고 **`MIG\_gates\consts_kind\` 의 후보 전부**(각 파일 첫 docstring 에 설계 의도와 그 배치가 본 실측이 적혀 있다).
...
| 파일 | 행 | 크기 |
|---|---|---|
| `MIG\_verify8\B\AXIS_ROWS_00-04.md` | 40 | 10KB |
| `MIG\_verify8\B\AXIS_ROWS_05-09.md` | 40 | 8KB |
| `MIG\_verify8\B\AXIS_ROWS_10-14.md` | 74 | 20KB |
| `MIG\_verify8\B\AXIS_ROWS_15-19.md` | 32 | 7KB |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 8 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify8/B 만 써라**

```
MIG/_verify8/B/patch.json      ← 기계 적용분(필수)
MIG/_verify8/B/REPORT.md       ← 산문 보고서
MIG/_verify8/B/oracle/         ← 오라클 .rs · 출력 로그
```
