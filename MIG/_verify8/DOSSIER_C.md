...
# 8차 배치 C 도시에 — **축 ``sig.params[].role` — 인자의 역할`** (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG && python -X utf8 dossierfresh.py 8 C
```
...
**담당 축 = `sig.params[].role` — 인자의 역할** — 124행. 후보 4벌. ★7차에 배치C 는 「초판 6건이 전부 오탐」이라 했고 배치B 는 「담당 0건」이었다 — **절 단위 분해와 SROA 인자 제외**가 설계의 갈림길이다.

이 축의 전 20함수 행 = **124개**. (§3 에 전량 있다.)

### 할 일 (순서대로)

1. `MIG\_gates\params_role\` 에 **다른 배치들이 만든 검사기 후보가 모여 있다.** 전부 돌려 보고 **왜 결과가 갈리는지** 규명하라.
2. 하나로 **통합**해서 `MIG\_verify8\C\gate.py` 로 낸다. 통합판은 다른 후보들의 장점을 흡수해야 한다.
...
| 파일 | 행 | 크기 |
|---|---|---|
| `MIG\_verify8\C\AXIS_ROWS_00-04.md` | 33 | 4KB |
| `MIG\_verify8\C\AXIS_ROWS_05-09.md` | 31 | 4KB |
| `MIG\_verify8\C\AXIS_ROWS_10-14.md` | 31 | 4KB |
| `MIG\_verify8\C\AXIS_ROWS_15-19.md` | 29 | 4KB |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 8 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify8/C 만 써라**

```
MIG/_verify8/C/patch.json      ← 기계 적용분(필수)
MIG/_verify8/C/REPORT.md       ← 산문 보고서
MIG/_verify8/C/oracle/         ← 오라클 .rs · 출력 로그
```
