...
# 8차 배치 A 도시에 — **축 ``mem[].dir` — 읽기/쓰기 방향`** (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG && python -X utf8 dossierfresh.py 8 A
```
...
**담당 축 = `mem[].dir` — 읽기/쓰기 방향** — 451행. 후보 **4벌**(A·B·C·D)이 전역에서 각각 **불일치 3 / 46 / 7** 로 갈린다.

이 축의 전 20함수 행 = **451개**. (§3 에 전량 있다.)

### 할 일 (순서대로)

1. `MIG\_gates\mem_dir\` 에 **다른 배치들이 만든 검사기 후보가 모여 있다.** 전부 돌려 보고 **왜 결과가 갈리는지** 규명하라.
2. 하나로 **통합**해서 `MIG\_verify8\A\gate.py` 로 낸다. 통합판은 다른 후보들의 장점을 흡수해야 한다.
...
그리고 **`MIG\_gates\mem_dir\` 의 후보 전부**(각 파일 첫 docstring 에 설계 의도와 그 배치가 본 실측이 적혀 있다).
...
| 파일 | 행 | 크기 |
|---|---|---|
| `MIG\_verify8\A\AXIS_ROWS_00-04.md` | 107 | 23KB |
| `MIG\_verify8\A\AXIS_ROWS_05-09.md` | 106 | 23KB |
| `MIG\_verify8\A\AXIS_ROWS_10-14.md` | 112 | 33KB |
| `MIG\_verify8\A\AXIS_ROWS_15-19.md` | 126 | 28KB |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 8 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify8/A 만 써라**

```
MIG/_verify8/A/patch.json      ← 기계 적용분(필수)
MIG/_verify8/A/REPORT.md       ← 산문 보고서
MIG/_verify8/A/oracle/         ← 오라클 .rs · 출력 로그
```
