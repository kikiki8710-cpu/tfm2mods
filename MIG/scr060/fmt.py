...
base=0x140000000
# fmt::Arguments pieces: struct {pieces_ptr, pieces_len, args_ptr, args_len}? Actually static Arguments piece array: [(ptr,len)...]
for a in sys.argv[1:]:
    o=rva2off(int(a,16)-base)
    # try as array of &str pieces
    print('==',a)
    for i in range(8):
        p_,l=struct.unpack_from('<QQ',d,o+i*16)
        if base<=p_<base+0x5000000 and 0<l<300:
            s=d[rva2off(p_-base):rva2off(p_-base)+l].decode('latin1')
            print(' piece',i,repr(s))
        else: break
