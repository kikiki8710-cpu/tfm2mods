...
# 17차 배치 D 도시에 — 담당 `55`~`56` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 17 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `55` | EntityPositioningCache::new | pub | 70 | 14 | 6 | 7 | 89 |
| `56` | PassiveLinePlan::sub_plan | pub | 52 | 29 | 12 | 12 | 89 |
...
| # | 이름 | 파일 |
|---|---|---|
| `55` | EntityPositioningCache::new | `MIG\_verify17\D\spec_55_EntityPositioningCache__new.md` |
| `56` | PassiveLinePlan::sub_plan | `MIG\_verify17\D\spec_56_PassiveLinePlan__sub_plan.md` |
...
**[G10 class 오분류] 17건**
  [56] PassiveLinePlan::sub_plan                       class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  L1003 nearest_tower 가 Tower 가 아닐 때(nexus 로 대체된 경우 ty==3 Nexus): Recall 검사 없이 LineDefense — IR 확정
  [56] PassiveLinePlan::sub_plan                       class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  orial 게이트: 태그 1,2,3,4,6(First/TopSolo/Bottom/MidSolo/JungleOnly)은 시간 게이트 없이 2v1 검사로 진입 — switch default 경로로 확정

**[G12 src_line 대조] 7건**
  [55] EntityPositioningCache::new                     consts[2] src_line=142 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [131, 184, 185, 186, 188, 189, 190, 193]
  [56] PassiveLinePlan::sub_plan                       consts[28] src_line=969 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [868, 891, 893, 922, 947, 948, 971, 972]

**[G18 logic↔표 상호참조] 1건**
  [56] PassiveLinePlan::sub_plan                       logic/P1 logic 이 `+0x60` 를 인용하는데 mem 표에 그 오프셋이 없다
  //  [L1039] wave_pushed = bb[team].bottom_minion_state.from_mid(+0x60) < 1000

**[G4 술어 시그니처] 6건**
  [56] PassiveLinePlan::sub_plan                       callees_unmatched 12개 — 판정에 쓰이는 술어가 섞였는지 손으로 확인
  bushes, drop  ⟵ IR 에 이 leaf 호출이 있으나 tcx 3크레이트에 구현이 없다(std·bumpalo·core 등). 후보 1개는 **전부 다른 함수**라 싣지 않는다, first_spawn_tick, from_mid, front_minion, map_
```
...
```json
{
  "round": 17, "batch": "D",
  "errors": [
    {"path": "/specs[55]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[55]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 17 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify17/D 만 써라**

```
MIG/_verify17/D/patch.json      ← 기계 적용분(필수)
MIG/_verify17/D/REPORT.md       ← 산문 보고서
MIG/_verify17/D/oracle/         ← 오라클 .rs · 출력 로그
```
