...
# 17차 배치 C 도시에 — 담당 `50`~`54` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 17 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `50` | check_epic_giveup | pub | 22 | 13 | 5 | 7 | 40 |
| `51` | check_press_tower_opportunity | pub | 22 | 25 | 7 | 8 | 54 |
| `52` | v25_objective_far_split_pressure | pub | 14 | 11 | 4 | 6 | 29 |
| `53` | check_epic_setup | pub | 30 | 19 | 10 | 10 | 57 |
| `54` | TeamPlan::update | pub | 75 | 30 | 11 | 11 | 111 |
...
| # | 이름 | 파일 |
|---|---|---|
| `50` | check_epic_giveup | `MIG\_verify17\C\spec_50_check_epic_giveup.md` |
| `51` | check_press_tower_opportunity | `MIG\_verify17\C\spec_51_check_press_tower_opportunity.md` |
| `52` | v25_objective_far_split_pressure | `MIG\_verify17\C\spec_52_v25_objective_far_split_pressure.md` |
| `53` | check_epic_setup | `MIG\_verify17\C\spec_53_check_epic_setup.md` |
| `54` | TeamPlan::update | `MIG\_verify17\C\spec_54_TeamPlan__update.md` |
...
**[G10 class 오분류] 17건**
  [50] check_epic_giveup                               class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  visible_objective_overload(player, data, x, y) 내부는 안 봄 — 이름상 '목표 지점 시야 과밀'이고 true 면 포기. 인자 (x,y)=camp_pos 만 확정
  [50] check_epic_giveup                               class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  Blackboard::is_recent_visible 의 정확한 의미(최근 N틱 관측)는 game_core 미열람. 인덱스가 적팀 판(1-team)인 것만 확정
  [52] v25_objective_far_split_pressure                class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  top_lead/bottom_lead 의 정의(파괴한 타워 수 차이?)는 game_core 미열람 — '> 1' 이 리드 2 이상이라는 것만 확정
  [53] check_epic_setup                                class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  judgement_ready 도 DW_OP_not 1회(= !(setup_score < penalty)). 분기: penalty != 0 && score < penalty → false 로 확정
  [53] check_epic_setup                                class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  봄. 인자 = (&blackboard[1-team], game data ptr, game vtable, player, entity). self 가 적팀 판이라는 것은 인덱스(1-team)로만 확정
  [54] TeamPlan::update                                class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  L311 블록에 있어 L359·L368 에서 재사용된다 — L305 루프가 항상 먼저 실행되므로 정의는 도달 보장. 시그니처 (game, team, xi, yi) → bool 은 호출 형태로만 확정

**[G4 술어 시그니처] 6건**
  [53] check_epic_setup                                callees_unmatched 16개 — 판정에 쓰이는 술어가 섞였는지 손으로 확인
  drop  ⟵ IR 에 이 leaf 호출이 있으나 tcx 3크레이트에 구현이 없다(std·bumpalo·core 등). 후보 1개는 **전부 다른 함수**라 싣지 않는다, entry, enumerate, epic_giveup_tick, format_inner, grow
  [54] TeamPlan::update                                callees_unmatched 19개 — 판정에 쓰이는 술어가 섞였는지 손으로 확인
  drop  ⟵ IR 에 이 leaf 호출이 있으나 tcx 3크레이트에 구현이 없다(std·bumpalo·core 등). 후보 1개는 **전부 다른 함수**라 싣지 않는다, epic_camp_last_visible_tick, epic_giveup_tick, epic_sp

**[G7 과열림] 3건**
  [53] check_epic_setup                                ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  Blackboard::is_recent_visible 내부는 안 봄. 인자 = (&blackboard[1-team], game data ptr, game vtable, player, entity). self 가 적팀
```
...
```json
{
  "round": 17, "batch": "C",
  "errors": [
    {"path": "/specs[50]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[50]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 17 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify17/C 만 써라**

```
MIG/_verify17/C/patch.json      ← 기계 적용분(필수)
MIG/_verify17/C/REPORT.md       ← 산문 보고서
MIG/_verify17/C/oracle/         ← 오라클 .rs · 출력 로그
```
