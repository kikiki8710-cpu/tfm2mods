...
# 17차 배치 A 도시에 — 담당 `40`~`44` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 17 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `40` | v3_assign_anchor | in:game_ai::plan_legacy::handler | 10 | 10 | 5 | 5 | 25 |
| `41` | fight_participants | in:game_ai | 19 | 11 | 4 | 6 | 34 |
| `42` | SinglePlanBattle::with_runaway | pub | 7 | 6 | 3 | 4 | 15 |
| `43` | BattlePlan::with_runaway | pub | 7 | 6 | 3 | 5 | 15 |
| `44` | LegacyPlanHandler::take_misunderstood_received_chat | in:game_ai::plan_legacy::handler | 13 | 5 | 0 | 3 | 18 |
...
| # | 이름 | 파일 |
|---|---|---|
| `40` | v3_assign_anchor | `MIG\_verify17\A\spec_40_v3_assign_anchor.md` |
| `41` | fight_participants | `MIG\_verify17\A\spec_41_fight_participants.md` |
| `42` | SinglePlanBattle::with_runaway | `MIG\_verify17\A\spec_42_SinglePlanBattle__with_runaway.md` |
| `43` | BattlePlan::with_runaway | `MIG\_verify17\A\spec_43_BattlePlan__with_runaway.md` |
| `44` | LegacyPlanHandler::take_misunderstood_received_chat | `MIG\_verify17\A\spec_44_LegacyPlanHandler__take_misunderstood_received_chat.md` |
...
**[G10 class 오분류] 17건**
  [41] fight_participants                              class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  엔티티 필드(level·stat_buff_cached.range)까지 읽는다 — 시그니처가 range(&self, &Entity) 인지 클로저가 더 더하는지는 dbg 로 구분 못 함. 합계식만 확정
  [41] fight_participants                              class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  도 define(`…fight_participants…s0_0…min_by…fold`, calls 에 fold 로 등록)에 있고 본체는 안 읽음 — 이름으로 min_by(Ord::cmp) 임만 확정
  [42] SinglePlanBattle::with_runaway                  class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  game.tick() 은 vtable+0x28 (divtable 정적 vtable 기준). 런타임 구현체는 미확정
  [43] BattlePlan::with_runaway                        class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  game.tick() 은 vtable+0x28 (divtable 정적 vtable 기준). 런타임 구현체는 미확정

**[G12 src_line 대조] 7건**
  [41] fight_participants                              consts[8] src_line=26 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [631]
  [41] fight_participants                              consts[10] src_line=623 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [617, 619, 624, 630, 635]
```
...
```json
{
  "round": 17, "batch": "A",
  "errors": [
    {"path": "/specs[40]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[40]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 17 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify17/A 만 써라**

```
MIG/_verify17/A/patch.json      ← 기계 적용분(필수)
MIG/_verify17/A/REPORT.md       ← 산문 보고서
MIG/_verify17/A/oracle/         ← 오라클 .rs · 출력 로그
```
