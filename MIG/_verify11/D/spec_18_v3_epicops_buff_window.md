...
| # | i | 이름 | 타입 | 역할 | ev |
|---|---|---|---|---|---|
| 0 | 1 | self | &mut TeamPlan(1064B) | objective/chats/eo_serpen_punish_issues/v3_press_chat_line 을 갱신하는 쓰기 대상 | 4 |
| 1 | 2 | version | usize(range 2..) | AI 버전 게이트. 이 함수 본문엔 version 분기가 없고 v3_serpen_contest_clear_win 으로 그대로 전달만 한다 | 4 |
| 2 | 3 | rnd | &mut rand::rngs::std::StdRng(320B) | PlayerState::strategy 호출에만 넘긴다(전략 샘플링용). 본문에서 직접 안 쓴다 | 4 |
| 3 | 4 | player | &PlayerState(2528B) | info.team(+0x930)·info.position(+0x9c0) 두 필드만 직접 읽는다 | 4 |
| 4 | 5 | data | &OperationData(24B) | cache(+0x0) 만 직접 역참조. cache.game(&dyn AbstractGame) 과 cache.player_champion(+0x1e0) 을 쓴다 | 4 |
| 5 | 6 | goal_data | &GoalData(248B) | is_object_being_taken_by_enemy 로 그대로 전달만. 이 함수는 필드를 안 읽는다 | 4 |
| 6 | 7 | plan | &BigPlan(384B) | v3_epicops_repair_need 로 그대로 전달만. 이 함수는 필드를 안 읽는다 | 4 |
</details>
...
|---|---|---|---|---|---|---|---|---|
| 0 | PlayerState | 0x930 | info.team | r | usize. v3_epicops_repair_need 의 1번 인자이자 cache.player_champion[team] 의 팀 인덱스. 사용 직전 team<2 경계검사(panic_bounds_check)를 받는다 · tcx 정본 대조( offset_of!(PlayerState, info.team)=+0x930 MISMATCH 0 (o1.txt)) | 3 | OK |  |
| 1 | PlayerState | 0x9c0 | info.position | r | Position(i32, range 0..5). Position::as_index() 로 usize 화해서 announcer 와 비교 — '내가 발표자인가' 판정 · tcx 정본 대조( offset_of!(PlayerState, info.position)=+0x9c0 MISMATCH 0 (o1.txt)) | 3 | OK |  |
| 2 | OperationData | 0x0 | cache | r | &AbstractGameWithCache. gep 없이 바로 load 된다 · tcx 정본 대조( offset_of!(OperationData, cache)=+0x0 (o1.txt)) | 3 | OK |  |
| 3 | AbstractGameWithCache | 0x0 | game(데이터 포인터) | r | ref$<dyn AbstractGame> 의 첫 워드. PlayerState::strategy 의 3번 인자로 전달 · tcx 정본 대조( offset_of!(AbstractGameWithCache, game)=+0x0 (o1.txt)) | 3 | OK |  |
| 4 | AbstractGameWithCache | 0x8 | game(vtable 포인터) | r | dyn 의 둘째 워드. IR 이 dereferenceable(816) 로 표시하는데 divtable.py 가 보고한 AbstractGame vtable 총 크기 816B(슬롯 100개)와 정확히 일치해 vtable 로 확정 | 3 | OK |  |
| 5 | AbstractGameWithCache | 0x1e0 | player_champion | r | array 총 80B = [5 x ptr] x 2팀. player_champion[team][x] 를 x=0..4 로 훑으며 null(=None) 여부를 본다 · tcx 정본 대조( offset_of!(AbstractGameWithCache, player_champion)=+0x1e0 (o1.txt) + 실제 [[Option<&Entity>;5];2] 인덱싱 동작(o3/o4/o5 HP 행)) | 3 | OK |  |
| 6 | Strategy(스택 로컬 24B) | 0x4 | morgard_use | r | MorgardUseStrategy(8B enum). i64 로 통째 읽어 v3_epic_group_line·v3_epic_formation_role 의 1번 인자로 넘긴다 · tcx 정본 대조( offset_of!(Strategy, morgard_use)=+0x4 (o1.txt) + 태그 직독 MUTAG 5/6/암묵 (o2.txt)) | 3 | OK |  |
| 7 | TeamPlan | 0xc0 | chats.cap | r | Vec<Chat> 의 용량. len==cap 이면 RawVec::grow_one · tcx 정본 대조( offset_of!(TeamPlan, chats)=+0xc0 + Vec 3워드 순서 실측(w0=cap=4, w2=len=1) (o1.txt VEC 행)) | 3 | OK |  |
| 8 | TeamPlan | 0xc8 | chats.ptr | r | Vec<Chat> 데이터 포인터. 원소 stride 24B({i8,[23 x i8]}) · tcx 정본 대조( chats+0x8=ptr — Vec 워드 순서 실측(o1.txt VEC) + tcxaudit chats.buf.inner.ptr) | 3 | OK |  |
| 9 | TeamPlan | 0xd0 | chats.len | r | Vec<Chat> 길이. 읽고 +1 해서 되쓴다 · tcx 정본 대조( chats+0x10=len — Vec 워드 순서 실측(o1.txt VEC, len_api 일치)) | 3 | OK |  |
| 10 | TeamPlan | 0x410 | eo_serpen_punish_issues | r | usize. 세르펜 징벌 경로에서 읽어 +1 · tcx 정본 대조( offset_of!(TeamPlan, eo_serpen_punish_issues)=+0x410 (o1.txt)) | 3 | OK |  |
| 11 | TeamPlan | 0x41e | v3_press_chat_line | r | Option<LineType>(1B, None=0xFF). ① group_line 과 같으면 즉시 종료 ② None 이면 Chat::Press, Some 이면 Chat::PressChange 선택 · tcx 정본 대조( tcx adt TeamPlan v3_press_chat_line=+0x41e(Option<LineType>) — private 필드라 offset_of! 불가, 대신 +0x41e 에 raw 기록/판독이 일관(o4/o5) · Option<LineType>::None 니치=0xff 실측(o1.txt)) | 3 | OK |  |
| 12 | TeamPlan | 0x41f | objective | w | repair_need 가 1 또는 2 일 때. dienum MainObjective 7 = Repair(페이로드 없음)이라 태그 1바이트만 쓴다 | 3 | OK | Some(MainObjective::Repair) — 태그 바이트 7 |
| 13 | TeamPlan | 0x41f | objective | w | dienum MainObjective 1 = Serpen, 페이로드 enum+0x1 phase(ObjectPhase), enum+0x2 with_battle(bool). 저장값 1/1 = phase=Setup, with_battle=true | 3 | OK | Some(MainObjective::Serpen{phase, with_battle}) — +0x41f=1, +0x420=1, +0x421=1 |
| 14 | TeamPlan | 0x410 | eo_serpen_punish_issues | w | 세르펜 징벌 경로 카운터 · tcx 정본 대조( offset_of!(TeamPlan, eo_serpen_punish_issues)=+0x410 (쓰기 대상 동일 오프셋, o1.txt)) | 3 | OK | 기존값 + 1 |
| 15 | Chat(chats 버퍼 원소, 24B) | 0x0 | Chat 태그 (Repair 23 / SerpenSetup 25 / Press 21 / PressChange 22) | w | 원소 24B. 태그 @+0, Press/PressChange 는 LineType @+1, 공통 usize 필드 @+8 에 항상 0 을 넣는다. ★**베이스는 TeamPlan 이 아니라 힙 버퍼다** — TeamPlan+0xc8(chats.ptr)은 이 함수에서 **읽기 전용**이고(쓰기 0건) 그 읽은 포인터가 가리키는 원소에 쓴다. 필드 쪽 읽기는 mem[8] 이 이미 담고 있다 ★**행 분리 **: 한 행에 오프셋을 묶으면 `tcxaudit` 기계 검사가 무력화된다. 14 는 3차에 같은 이유로 분리됐는데 **18 만 남아 있었다.** | 3 | OK | Chat::Repair(0) \| Chat::SerpenSetup(0) \| Chat::Press(group_line, 0) \| Chat::PressChange(group_line, 0) |
| 16 | TeamPlan | 0xd0 | chats.len(push 로 +1) | w | 원소 24B. 태그 @+0, Press/PressChange 는 LineType @+1, 공통 usize 필드 @+8 에 항상 0 을 넣는다 ★**행 분리 **: 한 행에 오프셋을 묶으면 `tcxaudit` 기계 검사가 무력화된다. 14 는 3차에 같은 이유로 분리됐는데 **18 만 남아 있었다.** | 3 | OK | 기존 len + 1 |
| 17 | TeamPlan | 0x41e | v3_press_chat_line | w | 압박 경로 마지막에 무조건 갱신 — 채팅을 실제로 뿌렸든(내가 발표자) 안 뿌렸든 갱신한다 · tcx 정본 대조( tcx adt TeamPlan v3_press_chat_line=+0x41e (쓰기 대상 동일 오프셋)) | 3 | OK | group_line (Some(LineType) 또는 -1=None 은 오지 않음) |
| 18 | TeamPlan | 0x420 | objective.Serpen.phase | w | 세르펜 징벌 경로(epic.rs:654)에서 `store i8 1`. `MainObjective`(3B)의 Serpen 페이로드 첫 바이트 — 태그(+0x41f)만 쓰고 여기를 안 쓰면 objective 가 미초기화된다. IR 원문 m09.ll:6978 `%48 = getelementptr inbounds nuw i8, ptr %0, i64 1056` → 6979 `store i8 1, ptr %48` | 4 | OK | ObjectPhase::Setup = 1 |
| 19 | TeamPlan | 0x421 | objective.Serpen.with_battle | w | 세르펜 징벌 경로(epic.rs:654)에서 `store i8 1`. `MainObjective` Serpen 페이로드 둘째 바이트(bool). IR 원문 m09.ll:6980 `%49 = getelementptr inbounds nuw i8, ptr %0, i64 1057` → 6981 `store i8 1, ptr %49` | 4 | OK | true = 1 |
| 20 | Chat(chats 버퍼 원소, 24B) | 0x1 | Chat.__0 (Press/PressChange 의 LineType) | w | epic.rs:674/676 경로에서만 쓰인다(Repair/SerpenSetup 은 이 바이트를 안 건드린다). IR 원문 m09.ll:7214 `%143 = getelementptr inbounds nuw i8, ptr %142, i64 1` → 7215 `store i8 %41, ptr %143` (%41 = `v3_epic_group_line` 반환값) | 4 | 오귀속(사전은 다른 필드를 준다) | group_line (LineType 1B) |
| 21 | Chat(chats 버퍼 원소, 24B) | 0x8 | Chat.__1 (공통 usize 슬롯) | w | 네 경로(Repair 23 / SerpenSetup 25 / Press 21 / PressChange 22) 전부 `store i64 0`. IR 원문 m09.ll:6941 `store i64 0, ptr %27` · 7015 `store i64 0, ptr %60` · 7217 `store i64 0, ptr %144` (각각 chat+0x8) | 4 | OK | 항상 0 |
...
| # | 무엇 | 어디 | 값 | 효과 | ev | src |
|---|---|---|---|---|---|---|
| 0 | 수리 선점의 우선순위 | epic.rs:635 (v3_epicops_repair_need 반환값 해석) | 1 / 2 | 반환 1은 '목표=Repair + 팀채팅', 2는 '목표=Repair, 조용히'. 둘 다 true 를 반환해 이후 세르펜·압박 판정을 통째로 건너뛴다. 2를 1로 바꾸면 수리 상황에서 채팅이 늘고, 이 두 case 를 지우면 수리보다 세르펜·압박이 우선하게 된다 | 4 | 기존 |
| 1 | 세르펜 징벌 진입 조건(2중 AND) | epic.rs:651~652 | is_object_being_taken_by_enemy(…, WavePriorityObject::Serpen /*=1*/) && v3_serpen_contest_clear_win(version, …) ※5번째 인자는 `WavePriorityObject`(1B C-enum, 0=Morgard/1=Serpen)이고 IR 의 i1 은 그 ABI 표현이다 | 둘 다 참이어야 목표를 세르펜(Setup, 전투 포함)으로 확정한다. clear_win 쪽을 완화하면 불리해도 오브젝트 뺏김에 달려들고, 강화하면 상대에게 그냥 내준다 | 4 | 기존 |
| 2 | 세르펜 진입 시의 phase/with_battle | epic.rs:654 | phase=ObjectPhase::Setup(1), with_battle=true(1) | phase 를 Assemble(2)/Hunt(3) 로 올리면 준비 단계를 건너뛰고 더 공격적으로 붙는다. with_battle=false 로 두면 교전 의사 없이 오브젝트만 노린다 | 4 | 기존 |
| 3 | 발표자 선정 규칙 | epic.rs:667~670 | 슬롯 0..4 중 '챔피언이 존재하고 formation_role 이 Some 이며 is_split=false' 인 최초 인덱스 | 내 포지션 인덱스가 그 인덱스와 같을 때만 압박 채팅을 발화한다. 조건을 !is_split 대신 is_split 으로 뒤집으면 스플릿 담당자가 발표자가 되고, 최초 인덱스 대신 특정 슬롯을 고정하면 항상 같은 포지션이 콜을 한다 · 오라클 실행 확증( v3_epic_formation_role(pub) 진리표 450칸 + 발표자 산출 90행 — Gather→슬롯0, Split14(Top)/Split131(Top,*)→슬롯1 (o2.txt GL/ANN)) | 2 | 기존 |
| 4 | 압박 채팅 중복 억제 | epic.rs:666 · 679 | self.v3_press_chat_line != group_line 일 때만 진입, 진입하면 무조건 group_line 으로 갱신 | 같은 라인을 연속 압박하는 동안은 채팅이 안 나간다. 이 비교를 지우면 매 판단마다 Press/PressChange 채팅이 도배된다. 반대로 679 갱신을 지우면 매번 PressChange 가 반복된다 | 4 | 기존 |
| 5 | 첫 압박 vs 라인 변경 문구 | epic.rs:673 (`Option::is_none` 인라인) -> 674/676 에서 Chat 구성. ⚠인라인 체인에 보이는 682 는 core/src/option.rs 줄번호다 | None → Chat::Press(21), Some → Chat::PressChange(22) | 문구 선택만 바꾼다. 판정 자체에는 영향이 없다 | 4 | 기존 |
| 6 | 수리 우선순위1 부상 HP% 임계 | m09.ll:65185 (icmp ult .., 41 — 슬롯 5개 언롤 사본 65185·65219·65254·65289·65324 중 첫 번째) | 41 | 올리면 더 멀쩡한 팀에도 '수리' 선점이 걸린다 | 4 | 신규 |
| 7 | 수리 우선순위1 부상자 수 임계 | m09.ll:65338 (icmp samesign ugt i64 %141, 1) | > 1 (2명 이상) | 0 으로 내리면 1명만 다쳐도 수리 | 4 | 신규 |
| 8 | 수리 우선순위2 '건강' HP% 임계 | m09.ll:65390 (ugt .., 29 — 슬롯 5개 언롤 사본 65390·65424·65459·65494·65529 중 첫 번째) | 29 | 올리면 우선순위2 가 훨씬 자주 발동 | 4 | 신규 |
| 9 | 우선순위1 봉쇄 게이트 | m09.ll:65128 (`%59 = icmp eq i8 %58, 5`, epic.rs:837 — `BigPlan::goal()` sret 태그 직독) | BigGoal::Battle | 이 비교를 죽이면 전투 중에도 수리 선점이 가능해진다 | 4 | 신규 |
| 10 | 세르펜 아군 전력 산입 HP% | m05.ll:62483 (ugt .., 39) | 39 | 내리면 빈사 아군까지 전력에 포함 → 징벌 진입 증가 | 4 | 신규 |
| 11 | 오브젝트 피탈 판정 시간창 | objective_helpers.rs / m15.ll (mul .., 20) | tps*20 | 올리면 '적이 가져가는 중'으로 더 오래 본다 | 4 | 신규 |
| 12 | 압박라인 타워 점수표 | m09.ll:64600 (`%70 = icmp samesign ugt i8 %69, 4` — `Entity+0x128` TowerType, epic.rs:875 `is_tower2`) | TowerType>4 → score 1 | 뒤집으면 '안쪽 타워 남은 라인' 을 우선하게 된다 | 4 | 신규 |
| 13 | 압박라인 정렬 키 | m09.ll:64620~64650 | (score, L==Mid, L==obj_line) | `L==Mid` 항을 빼면 미드 편향이 사라진다 | 4 | 신규 |
| 14 | 오브젝트 쪽 라인 판정식 | m09.ll:64391(epic.rs:857 `v3_split14_far_line`) / 64471(epic.rs:867 `v3_group_press_line`) — 둘 다 `icmp ugt i64 <+0x1b0>, <+0x1e0>` | epic.next_respawn > serpen.next_respawn | 극성을 뒤집으면 스플릿 담당자가 반대 라인으로 간다 | 4 | 신규 |
| 15 | 후보 라인 집합(튜토리얼별) | @anon….291/.290/.35/.36/.31 + line_exists(m13.ll:53138) | 정적표 | 튜토리얼 모드에서 압박 대상 라인을 강제 변경 · 오라클 실행 확증( v3_epic_group_line(pub) 을 TutorialType 9종 × morgard_use 5종 × 팀2 로 전수 실행 — {None,Line,Total}→Mid / TopSolo→Top / {First,Bottom}→Bottom(Split14 는 None) / MidSolo·MidBottom→Mid / JungleOnly→전부 None (o2.txt GL)) | 2 | 신규 |
| 16 | 발표자 선정식 | m09.ll:7189~7202 (`%133 = phi i64 [0,%76],[1,%88],[2,%100],[3,%112],[4,%124]` @7189 → `%134 = gep %3, i64 2496` @7194 → `%137 = icmp eq i64 %133, %136` @7202) | announcer == player.info.position | 이 비교를 지우면 전원이 채팅(스팸). 고정 인덱스로 바꾸면 특정 포지션 전담 | 4 | 신규 |
| 17 | 대표 자격 술어 | m09.ll:7070~7073 (슬롯0; 언롤 사본 5개 = 7070·7098·7126·7154·7182 의 `icmp ne i8 %role0, 2` → `trunc i8 %role0 to i1` → `xor` → `freeze`) | Some(f) && !f.is_split | `!f.is_split` 을 빼면 스플릿 담당자도 대표가 될 수 있다 · 오라클 실행 확증( role 바이트0 과 `is_some_and(\|f\| !f.is_split)` 이 450/450 에서 b0==0 ⟺ ok==true 로 일치 (o2.txt GL b0/ok 열)) | 2 | 신규 |
| 18 | Strategy.morgard_use 직접 강제 | World+0xb3b0 의 [Strategy;2], 팀별 24B stride, +0x4 | 5=Gather / 6=Split14 / 0~4=Split131(값=첫 Position) | 모르가드 교전 대형을 직접 고정. v3_epic_group_line·v3_epic_formation_role 의 3분기가 결정된다 · 오라클 실행 확증( MorgardUseStrategy 5종 실값의 워드 직독 — Gather low32=5 / Split14 low32=6·high32=Position / Split131 low32=position1·high32=position2 (o2.txt MUTAG)) | 2 | 신규 |
| 19 | Game::set_strategy(team, &Strategy) | _gcbc/g15.ll:176782 | 24B memcpy | 한 방으로 팀 전략 13개 필드 전부 교체 — 개별 필드 패치보다 이 진입점이 싸다 | 4 | 신규 |
| 20 | 팀 성향 난수 분포 | _gcbc/g15.ll:5094 (gen_range(0,3) / gen_range(0,5)) | 균등 | 팀 생성 시 대형 분포를 편향시킨다. Split131 은 p1≠p2 재추첨 루프가 있다 | 4 | 신규 |
| 21 | 팀 성향을 난수 대신 지정 | _gcbc/g01.ll:106699·106716 (TeamSetting::create) | — | **유일한 단일 개입점** | 4 | 신규 |
| 22 | 세르펜 "도달 가능" 거리 여유 | serpen.rs:26 | 150000 | 캠프 기준 도달 판정 완화/강화 | 4 | 신규 |
| 23 | 적 건강 하한 | serpen.rs:30·37 | hp*100/max > 49 | 전력 산입 기준(50% 초과) | 4 | 신규 |
| 24 | 미드라인 근접 밴드 | map_regions.rs is_near_mid_line | 192001 / 192000 / 64000 | 미드 근접 판정 폭 | 4 | 신규 |
| 25 | resolve_fight 히스테리시스 | fight_model.rs:510 | hyst = mean(ally_values) | Commit/Disengage 사이 Hold 구간의 폭 | 4 | 신규 |
| 26 | 팀 색깔전략 활성 개수 | team.rs:2450 | gen_range(5..=9) | 필드당 Some 확률 7/12 를 직접 좌우 | 4 | 신규 |
...
| 2 | from_index | game_core::Position::from_index | pub | fn(usize) -> game_core::Position | game-core\src\simulation\entity.rs:622 | False | False | 3 |
| 3 | from_index | game_core::ChampionTier::from_index | pub | fn(usize) -> game_core::ChampionTier | game-core\src\data\team.rs:59 | True | True | 3 |
| 4 | from_index | game_core::GamingHouseLevel::from_index | pub | fn(usize) -> game_core::GamingHouseLevel | game-core\src\data\team.rs:108 | False | False | 3 |
| 5 | is_object_being_taken_by_enemy | game_ai::plan_legacy::team_plan::objective_helpers::is_object_being_taken_by_enemy | in:game_ai | fn(&game_core::PlayerState, &game_core::OperationData, &game_ai::GoalData, &game_ai::plan_legacy::team_plan::TeamPlan, game_ai::plan_legacy::team_plan::objective_helpers::WavePriorityObject) -> bool | game-ai\src\plan_legacy\team_plan\objective_helpers.rs:313 | False | False | 3 |
| 6 | position | game_core::transfer::TeamRosterPlanningContext::position | pub | fn(&game_core::transfer::TeamRosterPlanningContext, game_core::Position) -> std::option::Option<&game_core::transfer::PositionPlanningContext> | game-core\src\transfer\roster_context.rs:234 | False | False | 3 |
| 7 | position | game_core::PlayerAiContext::<'a, 'b, 'r>::position | pub | fn(&game_core::PlayerAiContext<'a/#0, 'b/#1, 'r/#2>) -> game_core::Position | game-core\src\mod_ai.rs:587 | True | True | 3 |
| 8 | push | <game_core::AthleteStat as engine_core::spitz::SpitzDatable>::serialize::push | in:game_core::setting::athlete | fn(&mut std::vec::Vec<u8, std::alloc::Global>, &T/#0) | game-core\src\setting\athlete.rs:167 | False | False | 3 |
| 9 | push | <game_core::Athlete as engine_core::spitz::SpitzDatable>::serialize::push | in:game_core::data::athlete | fn(&mut std::vec::Vec<u8, std::alloc::Global>, &T/#0) | game-core\src\data\athlete.rs:434 | False | False | 3 |
| 10 | push | <game_core::TrainingExp as engine_core::spitz::SpitzDatable>::serialize::push | in:game_core::data::athlete | fn(&mut std::vec::Vec<u8, std::alloc::Global>, &T/#0) | game-core\src\data\athlete.rs:1137 | False | False | 3 |
| 11 | strategy | <game_core::Game as game_core::AbstractGame>::strategy | pub | fn(&game_core::Game, usize) -> game_core::Strategy | game-core\src\simulation\game.rs:1830 | True | True | 3 |
| 12 | strategy | game_core::PlayerState::strategy | pub | fn(&game_core::PlayerState, &mut rand::rngs::std::StdRng, &dyn [Binder { value: Trait(game_core::AbstractGame), bound_vars: [] }] + ) -> game_core::Strategy | game-core\src\simulation\state\player.rs:1576 | False | False | 3 |
| 13 | strategy | <game_core::ExpectedGame<'a> as game_core::AbstractGame>::strategy | pub | fn(&game_core::ExpectedGame<'a/#0>, usize) -> game_core::Strategy | game-core\src\simulation\expected_game.rs:57 | False | False | 3 |
| 14 | v3_epic_formation_role | game_ai::plan_legacy::old::v3_epic_formation_role | pub | fn(game_core::MorgardUseStrategy, game_core::Position, &game_core::PlayerState, &game_core::OperationData) -> std::option::Option<game_ai::plan_legacy::old::V3EpicFormation> | game-ai\src\plan_legacy\old\epic.rs:781 | False | False | 3 |
| 15 | v3_epic_group_line | game_ai::plan_legacy::old::v3_epic_group_line | pub | fn(game_core::MorgardUseStrategy, &game_core::PlayerState, &game_core::OperationData) -> std::option::Option<game_core::LineType> | game-ai\src\plan_legacy\old\epic.rs:814 | False | False | 3 |
...
| 17 | v3_epicops_repair_need | game_ai::plan_legacy::old::epic::v3_epicops_repair_need | in:game_ai | fn(&game_core::PlayerState, &game_core::OperationData, &game_ai::plan_legacy::types::BigPlan) -> u8 | game-ai\src\plan_legacy\old\epic.rs:833 | False | False | 3 |
| 18 | v3_serpen_contest_clear_win | game_ai::plan_legacy::old::v3_serpen_contest_clear_win | pub | fn(usize, &game_core::PlayerState, &game_core::OperationData, &game_ai::plan_legacy::team_plan::TeamPlan) -> bool | game-ai\src\plan_legacy\old\serpen.rs:52 | False | False | 3 |
</details>
...
| # | 물음 | 닫은 근거 |
|---|---|---|
| 0 | goal_data(6번 인자, GoalData 248B) 와 plan(7번 인자, BigPlan 384B) 은 이 함수가 필드를 하나도 안 읽는다 — 각각 is_object_being_taken_by_enemy / v3_epicops_repair_need 로 그대로 넘길 뿐이라 ~~어떤 필드가 판정에 쓰이는지는 이 범위에서 확정 불가~~ → ★**해소: `history[4]` 참조** — `goal_data` 는 `serpen.epic_enemy_tick`(+0xc0) **오직 1개**(m15.ll:53994), `plan` 은 `BigPlan::goal()` 판별자 하나(m09.ll:65128 `icmp eq i8 %58, 5`)뿐이다 | G7 자동적발 → history 에 답 있음: goal_data 는 `serpen.epic_enemy_tick`(+0xc0) 오직 1개(m15.ll:53994, Serpen 분기 / `epic.epic_enemy_tick`+0x88 은 Morgard 분기라 이 호출부에선 죽은 경로), plan 은 `BigPlan::goal()` 판별자 태그 하나(== BigGoal::Battle(5), m09.ll:65038~65044) |
| 1 | is_object_being_taken_by_enemy 의 ~~5번째 인자 리터럴 true 의 의미 — 피호출 함수 밖이라 확정 못 함~~ → ★**해소: `history[3]` 참조** — bool 이 아니라 **`target: WavePriorityObject`(1B C-enum)의 ABI 표현**이고 `Morgard=0 / Serpen=1` (DWARF `m15.ll !58647`), 즉 리터럴 `true` = **Serpen** | 2차 배치D: WavePriorityObject(0=Morgard/1=Serpen) 확정 |
| 2 | Chat::Repair / SerpenSetup / Press / PressChange 가 공통으로 갖는 usize 필드(enum+0x8)에 항상 0 을 넣는데, ~~dienum 이 준 이름이 __0/__1 뿐이라 그 0 의 의미(틱? 대상 id?) 확정 불가~~ → ★**부분 해소: 명세 `17`(DeathMatchBattle::new)의 `resolved[0]` 참조** — **`Chat` enum+0x8 = 대상 엔티티 id 슬롯(틱 아님)**이 확정됐다(`m05.ll:17745~17749` 에서 `Chat::Battle` 이 그 자리에 `goal.__0`=대상 id 를 넣는다). ⚠적용 범위: 「+0x8 이 무엇인가」는 확정, 「Repair/SerpenSetup/Press 가 거기 0 을 넣는 뜻(=대상 없음)」은 소비측 미탐색 | 4차 배치D: /specs[17]/closed 와 동상(Chat 튜플 variant = 이름 부재) |
| 3 | epic.rs 655~663 줄에 해당하는 IR 이 없다. 세르펜 경로의 chat push 는 !DILexicalBlockFile(file:!13457, line 1) 로 찍혀 소스 줄이 소실됐다 — ~~채팅 push 가 매크로/헬퍼를 거쳐 인라인된 것으로 보이나 원 소스 형태는 미확정~~ → ★**해소: `history[6]`·`closed[8]` 참조** — `!dbg` 는 소실되지 않았다(`inlinedAt` 루트까지 타면 나온다). **658 = `self.chats.push(Chat::SerpenSetup(0))`** 로 복원됐고 654~657 도 줄 길이 산술로 확정됐다 | resolved 가 '소실되지 않았다'로 정면 반박 |
| 4 | epic.rs 671~678 의 원 소스 구조(if 문인지 match 인지)는 ~~!dbg 가 671~678 을 하나도 안 남겨 복원 불가~~ → ★**해소: `history[6]` 참조** — `inlinedAt` 루트를 타면 666·667·668·669~670·673·674/676·679 가 전부 나온다(같은 명세 `logic` 이 이미 그 복원판이다). IR 은 '비교 후 push' 한 덩어리만 남았다 | resolved 가 '소실되지 않았다'로 정면 반박 |
| 5 | ~~player.strategy(...) 는 이 IR 파일에 define 이 없다 … rnd 를 어떻게 쓰는지 미확인~~ → ★**해소: `history[5]` 참조** — 본체는 `_gcbc/g15.ll:130480~130505` 에 있고 **`rnd` 인자에 `readnone` 이 붙어 있다** ⟹ **RNG 상태를 전혀 안 건드린다**(받고 버리는 잔재) — 여기선 strategy.morgard_use(+0x4) 만 소비한다 | 1차 배치D: _gcbc/g15.ll:130480 define 확정(+rnd readnone) |
| 6 | AbstractGameWithCache.player_champion 의 원소 타입을 distruct 가 array$<?> 로만 알려준다. IR 상 [5 x ptr] 이고 null 비교로 Option 니치를 쓰므로 ~~Option<&엔티티류> 인 것은 확실하나 정확한 T 는 미확정~~ → ★**해소(10차 배치D)**: `tcxdict AbstractGameWithCache` = `0x1e0 player_champion : [[std::option::Option<&game_core::Entity>; 5]; 2] (80B)` ⟹ **T = `&game_core::Entity`**. 같은 명세 `mem[5]` 는 이미 그렇게 적고 있었다 | 1차 배치D: tcxdict 로 [[Option<&Entity>;5];2] 확정 |
| 7 | ~~v3_epic_group_line / v3_epic_formation_role 내부는 안 봄~~ → ★**해소: `history[0]` 참조(전량 복원)** + 오라클 실행 확증(`knobs[15]`·`knobs[17]` = TutorialType 9 × morgard_use 5 × 팀2 전수, role 진리표 450칸) — group_line 이 어떤 근거로 Top/Mid/Bottom 을 고르는지, formation_role 의 is_split 이 무엇으로 결정되는지는 이 명세 범위 밖 | resolved 가 두 함수 모두 확정 |
| 8 | `epic.rs` 655~678 — ★**대부분 확정(2026-09-11). 직전 「재료 부재로 종결, 남은 건 exe 디스어셈뿐」 판정은 과했다.** **막힌 것은 MIR 경로뿐**이었고, 「**줄 길이 산술**(rmeta_srcmap) + tcx ADT 필드명·태그 + IR 분기 유무」 조합으로 뚫렸다. ★전제도 틀렸었다: DILocation 전수 집계 결과 **`659`·`674`·`676` 은 사실 IR 이 있다**. 진짜 미해명은 `655·656·670·675·681` 이었고 **그중 4개 확정**:   L654~657 = `self.objective = Some(MainObjective::Serpen {` / `  phase: ObjectPhase::Setup,` / `  with_battle: true,` / `});`     (근거: 세 스토어가 전부 `!dbg epic.rs:654` 단일 + 사이에 분기 없음 ⟹ 655 는 `if` 가 될 수 없는 **계속행**. `MainObjective::Serpen{phase, with_battle}` 가 tcx 상 네임드 필드라 표기가 강제되고 `ObjectPhase` 태그 1 = `Setup` 이 길이를 ±0 으로 맞춘다)   L675 = `} else {` / L681 = `false`   L674/676 = `self.chats.push(Chat::Press(epic_line, 0));` / `…PressChange…` (IR `select(v3_press_chat_line.is_none(), 21, 22)`. 두 줄 길이차 6 = `len("PressChange")−len("Press")` 로 자기정합) **남은 것**: ①674/676 의 **1자 잔차**(양쪽 동일) ②`667~670` 이터레이터/클로저 체인 본문(**미탐색** — IR 은 `player_champion[team][i].is_some()` 필터 + `v3_epic_formation_role(...)` 5회 언롤까지 확인) ③`648~650`·`662~663`·`685~686` 의 한글 `//` 주석 원문(**재료 부재** — 라인주석은 rmeta 미포함). 전문 = RE\2026-09-11_IR정밀독해-exit_src16_23-region_point-battle잔여-epic줄복원.md | 본문에 해소 표기가 있다 |
| 9 | `FightLine::CommitAfterJoin`(1) 의 생산 지점 — ★**"죽은 variant" 로 닫힘(2026-09-11)**. `FightPrediction.line`(+0x38)/`line_absolute`(+0x39) 로의 store 를 `_gaibc` 전량 gep 체인 누적 스캔한 결과 **값 1 을 쓰는 코드 0건**. 유일한 진짜 생산자 `fight_model.rs:520`(m10.ll:47092/47105)의 phi 값역이 **{0,2,3}**. 후보였던 `resolve_join_stake`·`battle::update_v32` 는 **둘 다 복사만** 한다(탈락). 소비 측 switch 에는 케이스가 살아 있으나 **Hold 와 같은 블록으로 접혀** 있다. ⚠적용 범위: `_gaibc`·`_gcbc`·`_gvbc` IR 한정. 미탐색 = 미추출 rlib(`libengine_*`) · rmeta MIR · exe. 전문 = RE6-09-11_fight_model-심층부4건-join_stake-tower_dive-net_value.md | 죽은 variant 로 닫힘 — `_gaibc` 전량 gep 체인 누적 스캔 결과 값 1 을 쓰는 곳이 없다. 어휘는 **재료 부재(범위 = _gaibc·_gcbc·_gvbc)** 가 정확하다(3차 배치D P-14 지적) |
</details>

<details><summary>`history` 정정 이력 16건 (참조용 — 본문 아님)</summary>

| # | 옛 값 | 현재 | ★정정_첫인자 | MorgardUseStrategy | group_line | ★알맹이_v3_group_press_line | is_split | 호출부 | 본문 |
|---|---|---|---|---|---|---|---|---|---|
| 0 | v3_epic_group_line / v3_epic_formation_role 내부 — group_line 이 어떤 근거로 라인을 고르는지, is_split 이 무엇으로 결정되는지 | ★전부 확정. | 이 둘의 **1번 인자는 `version` 이 아니라 `strategy.morgard_use` 다**(m09.ll:7112 — `%40 = load i64, %8+4` = Strategy sret 의 +0x4). `distruct Strategy` → `0x4 morgard_use : MorgardUseStrategy(8B)`. 8바이트 통째가 넘어간다. | 니치 밀림. 하위 u32 가 태그: **5=Gather(페이로드 없음) / 6=Split14(상위 u32 = position) / 그 외=Split131(0x0 position1, 0x4 position2)**. IR 디스패치(m09.ll:64338~64346, 64778~64786) = `sel = (low32>4) ? low32-5 : 2` → 0=Gather/1=Split14/2=Split131. | match morgard_use { Gather => v3_group_press_line(team, cache, ctx, skip=None); Split14 => { side = obj_side_line(game); v3_group_press_line(..., skip=side) }; Split131 => if line_exists(ctx, Mid) { Mid } else { v3_group_press_line(..., skip=None) } }. 반환 `Option<LineType>`, -1=None/0=Top/1=Mid/2=Bottom. | m09.ll:64422~64774 (internal).  1) get_game_mode 가 Moba 아니면 즉시 None  2) obj_line = epic.next_respawn_tick(+0x1b0) > serpen.next_respawn_tick(+0x1e0) ? Bottom(2) : Top(0)  3) 후보 라인 = ctx.tutorial(GameContext+0x38) 별 **정적 슬라이스**: {0,7,8}→[Top,Mid,Bottom] / {1,3}→[Bottom] / 2→[Top] / 4→[Mid] / 5→[Mid,Bottom] / 6→[] (→None)  4) 각 후보 L(단 L==skip 이면 건너뜀)에 대해 **적팀 타워** t = cache.{L}_tower[1-team].or({L}_tower2[1-team]):       score = None→0 / ty!=Tower→2 / Tower.info.ty(+0x128)>4(안쪽 타워)→1 / 그 외(바깥 타워)→2  5) ★**정렬 키 = (score, L==Mid, L==obj_line) 사전식 최대(첫 최대 유지)** — 바깥 타워 남은 라인 > 안쪽만 남은 라인 > 타워 없는 라인. 동점이면 Mid 우선, 그 다음 오브젝트 쪽 라인. | `v3_epic_formation_role`(m09.ll:64777~64972) 반환 = `Option<V3EpicFormation>{is_split: bool, line: LineType}`(2B). 바이트0 니치 = **0=Some(is_split=false) / 1=Some(is_split=true) / 2=None**. 호출부의 `(b0!=2) XOR (b0&1)` 는 대수적으로 `b0==0`, 즉 정확히 `Some(f) && !f.is_split`.   Gather: line = v3_group_press_line(...); Some{is_split:false, line}   Split14: side = obj_line; if pos == morgard_use.position && line_exists(ctx, side) → Some{is_split:TRUE, line:side}(분열 담당 1명) else → skip=side 로 Some{is_split:false,…}   Split131: a=position1, b=position2; Top 우선 → a(또는 champ[a] 없으면 b) 가 Some{TRUE, Top}; Bottom → b(또는 champ[b] 없으면 a) 가 Some{TRUE, Bottom}; 아니면 Mid 로 Some{false, Mid}  ⟹ ★**`is_split=true` 는 오직 '이 챔피언이 분열 담당으로 지목된 경우'**다. 그래서 발표자는 항상 **그룹(mid) 쪽 최초 슬롯**이 된다. |  |  |
| 1 | v3_epicops_repair_need 내부 — 수리 선점의 우선순위 1/2 가 어떤 조건에서 갈리는지 | ★확정(m09.ll:64975~65551). `fn(team, cache, plan: &BigPlan) -> u8 /*0..3*/`:   n_mine/n_enemy = 생존 챔프 수   **if plan.goal() 이 BigGoal::Battle(태그 5)이 아니면**: hurt = hp*100/max < 41 인 아군 수; `if n_mine <= n_enemy && hurt > 1 { return 1 }`   healthy = hp*100/max > 29 인 아군 수; `if healthy < n_enemy && healthy < n_mine { return 2 }`   return 0  ⟹ **1/2 를 가르는 것 3가지**: ①plan.goal()==Battle 이면 **1 자체가 불가**(그 블록을 통째로 건너뜀, m09.ll:65043) ②1 = 인원 열세(≤) + HP41% 미만 2명 초과 ③2 = HP29% 초과 인원이 적팀 생존수보다도 우리 생존수보다도 적다. |  |  |  |  |  | 1 → objective = Some(MainObjective::Repair)(태그 7) + `chats.push(Chat::Repair(0))`(epic.rs:638) → true. 2 → objective 만 세팅, 채팅 없음 → true. 0 → 다음 단계. |  |
...
| 3 | is_object_being_taken_by_enemy 의 5번째 인자 리터럴 true 의 의미 | ★확정 — **bool 이 아니라 `target: WavePriorityObject` 2값 C-enum 이 i1 로 전달된 것**. DWARF `m15.ll !58647`(arg:5, type !9496 = DW_TAG_enumeration_type "WavePriorityObject", DIFlagEnumClass): **Morgard=0, Serpen=1** ⟹ 리터럴 `true` = **Serpen**.  극성 교차검증: `%4==true` 분기(m15.ll:53841)의 !dbg 가 `rule_scope::serpen_exists`(objective_helpers.rs:326), false 분기가 `morgard_exists`(:317). 읽는 필드도 serpen_camp_* / jungle_runner.serpen.* 계열. |  |  |  |  |  |  | tps = ctx.setting.tick_per_second. match target { Morgard => morgard_exists(tutorial∈{0,7,8}) 아니면 false; vis = obj_spawn.epic_camp_last_visible_tick(+0x80)+tps >= tick; kill = goal_data.epic.epic_enemy_tick(+0x88); cid = jungle_runner.epic.live_list[0] \| Serpen => serpen_exists(tutorial∈{0,5,7,8}); vis = serpen_camp_last_visible_tick(+0x88)+tps >= tick; kill = goal_data.serpen.epic_enemy_tick(+0xc0); cid = jungle_runner.serpen.live_list[0] }; e = get_entity_by_id(cid); is_visible(team, e.id) 아니면 false; damaged = e.hp < e.stat_cached.hp; **return vis && damaged && kill <= tps*20** |
