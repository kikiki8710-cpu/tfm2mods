...
# 11차 배치 D 도시에 — 담당 `15`~`19` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 11 D      # ← 이것만 돌리면 된다
```
...
| # | 이름 | 파일 |
|---|---|---|
| `15` | single_try_engage | `MIG\_verify11\D\spec_15_single_try_engage.md` |
| `16` | max_range_nearly_can_use | `MIG\_verify11\D\spec_16_max_range_nearly_can_use.md` |
| `17` | new | `MIG\_verify11\D\spec_17_new.md` |
| `18` | v3_epicops_buff_window | `MIG\_verify11\D\spec_18_v3_epicops_buff_window.md` |
| `19` | best_jungle_goal | `MIG\_verify11\D\spec_19_best_jungle_goal.md` |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 11 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify11/D 만 써라**

```
MIG/_verify11/D/patch.json      ← 기계 적용분(필수)
MIG/_verify11/D/REPORT.md       ← 산문 보고서
MIG/_verify11/D/oracle/         ← 오라클 .rs · 출력 로그
```
