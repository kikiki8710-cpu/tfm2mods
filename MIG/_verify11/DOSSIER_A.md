...
# 11차 배치 A 도시에 — 담당 `00`~`04` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 11 A      # ← 이것만 돌리면 된다
```
...
| # | 이름 | 파일 |
|---|---|---|
| `00` | ult | `MIG\_verify11\A\spec_00_ult.md` |
| `01` | calculate_jungle_action_score | `MIG\_verify11\A\spec_01_calculate_jungle_action_score.md` |
| `02` | sub_plan | `MIG\_verify11\A\spec_02_sub_plan.md` |
| `03` | defensive_crisis | `MIG\_verify11\A\spec_03_defensive_crisis.md` |
| `04` | handle_line_defense | `MIG\_verify11\A\spec_04_handle_line_defense.md` |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 11 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify11/A 만 써라**

```
MIG/_verify11/A/patch.json      ← 기계 적용분(필수)
MIG/_verify11/A/REPORT.md       ← 산문 보고서
MIG/_verify11/A/oracle/         ← 오라클 .rs · 출력 로그
```
