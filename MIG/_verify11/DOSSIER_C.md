...
# 11차 배치 C 도시에 — 담당 `10`~`14` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 11 C      # ← 이것만 돌리면 된다
```
...
| # | 이름 | 파일 |
|---|---|---|
| `10` | should_end_object_finish_kill_priority_battle | `MIG\_verify11\C\spec_10_should_end_object_finish_kill_priority_battle.md` |
| `11` | v3_fall_back_to_passive | `MIG\_verify11\C\spec_11_v3_fall_back_to_passive.md` |
| `12` | handle_chat | `MIG\_verify11\C\spec_12_handle_chat.md` |
| `13` | target_bush_v30 | `MIG\_verify11\C\spec_13_target_bush_v30.md` |
| `14` | update | `MIG\_verify11\C\spec_14_update.md` |
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 11 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify11/C 만 써라**

```
MIG/_verify11/C/patch.json      ← 기계 적용분(필수)
MIG/_verify11/C/REPORT.md       ← 산문 보고서
MIG/_verify11/C/oracle/         ← 오라클 .rs · 출력 로그
```
