...
# 24차 배치 F 도시에 — 담당 `186`~`187` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 24 F      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `186` | calculate_action_score | pub | 53 | 31 | 8 | 6 | 92 |
| `187` | position_eval_at | pub | 22 | 26 | 2 | 7 | 49 |
...
| # | 이름 | 파일 |
|---|---|---|
| `186` | calculate_action_score | `MIG\_verify24\F\spec_186_calculate_action_score.md` |
| `187` | position_eval_at | `MIG\_verify24\F\spec_187_position_eval_at.md` |
...
**[G1 자기모순] 2건**
  [187] position_eval_at                                자기모순(Option 겹수): 같은 명세에 'Option<Option<' 와 'Option<PositioningScore>' 가 동시에 있다

**[G10 class 오분류] 3건**
  [187] position_eval_at                                class=미탐색 인데 문면이 **사실 서술**로 끝난다 — 다음 라운드가 이걸 또 판다
  efensive)=4608 이 소스 91·92 어느 줄인지 — 24604 select 가 `;L0` 로 줄 소실. 값 대응(태그0→4096, 태그1→4608 · trunc nuw i8→i1)은 확정

**[G12 src_line 대조] 21건**
  [186] calculate_action_score                          consts[17] src_line=161 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [63, 169, 173, 174, 184, 333, 346, 354]
  [187] position_eval_at                                consts[10] src_line=91 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [101, 296]
  [187] position_eval_at                                consts[11] src_line=91 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [101, 296]
  [187] position_eval_at                                consts[15] src_line=84 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [299, 307, 309, 312, 317]
  [187] position_eval_at                                consts[25] src_line=426 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [83, 101, 296, 299, 305, 307, 309, 312]

**[G19 knobs.value 값] 5건**
  [186] calculate_action_score                          knobs[6] 주장한 값을 IR 관측에서 못 찾는다
  value=240 · 관측(창)=[-3, 0, 1, 2, 3, 8, 10, 80, 100, 104]
  [186] calculate_action_score                          knobs[7] 주장한 값을 IR 관측에서 못 찾는다
  value=20 · 관측(창)=[0, 1, 3, 64, 104, 296, 576]

**[G3 형제 함수] 6건**
  [186] calculate_action_score                          siblings.plan == null 인데 자유 함수 화이트리스트에 없다 — 망글링 파싱 실패를 의심하라
  _RNvNtCshdEBA0ozCnw_7game_ai12action_score22calculate_action_score
  [187] position_eval_at                                siblings.plan == null 인데 자유 함수 화이트리스트에 없다 — 망글링 파싱 실패를 의심하라
  _RNvNtCshdEBA0ozCnw_7game_ai13position_eval16position_eval_at
```
...
```json
{
  "round": 24, "batch": "F",
  "errors": [
    {"path": "/specs[186]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[186]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 24 --only F --dry
```
...
## §6 출력 디렉터리 — **_verify24/F 만 써라**

```
MIG/_verify24/F/patch.json      ← 기계 적용분(필수)
MIG/_verify24/F/REPORT.md       ← 산문 보고서
MIG/_verify24/F/oracle/         ← 오라클 .rs · 출력 로그
```
