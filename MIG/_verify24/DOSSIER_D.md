...
# 24차 배치 D 도시에 — 담당 `183`~`183` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 24 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `183` | interaction_score | pub | 67 | 35 | 10 | 9 | 111 |
...
| # | 이름 | 파일 |
|---|---|---|
| `183` | interaction_score | `MIG\_verify24\D\spec_183_interaction_score.md` |
...
**[G12 src_line 대조] 21건**
  [183] interaction_score                               consts[1] src_line=309 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [618, 954]
  [183] interaction_score                               consts[2] src_line=309 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [618, 679, 784, 952]
  [183] interaction_score                               consts[3] src_line=309 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [618, 779]
  [183] interaction_score                               consts[20] src_line=907 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [593]
  [183] interaction_score                               consts[21] src_line=907 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [743, 953]

**[G18 logic↔표 상호참조] 3건**
  [183] interaction_score                               logic/P2 logic 이 필드 `s.tower_risk` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `tower_risk` 는 distruct 의 실재 필드명

**[G3 형제 함수] 6건**
  [183] interaction_score                               siblings.plan == null 인데 자유 함수 화이트리스트에 없다 — 망글링 파싱 실패를 의심하라
  _RNvNtCshdEBA0ozCnw_7game_ai12action_score17interaction_score
```
...
```json
{
  "round": 24, "batch": "D",
  "errors": [
    {"path": "/specs[183]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[183]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 24 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify24/D 만 써라**

```
MIG/_verify24/D/patch.json      ← 기계 적용분(필수)
MIG/_verify24/D/REPORT.md       ← 산문 보고서
MIG/_verify24/D/oracle/         ← 오라클 .rs · 출력 로그
```
