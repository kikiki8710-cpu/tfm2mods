...
# 24차 배치 C 도시에 — 담당 `182`~`182` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 24 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `182` | SmallActionRunAway::get_input | in:game_ai | 58 | 36 | 15 | 9 | 107 |
...
| # | 이름 | 파일 |
|---|---|---|
| `182` | SmallActionRunAway::get_input | `MIG\_verify24\C\spec_182_SmallActionRunAway__get_input.md` |
...
**[G15 consts.kind 관측] 1건**
  [182] SmallActionRunAway::get_input                   consts[29] kind=임계 NEG
  `meaning` 이 「towers 결과의 고정 배열 길이 6 (타워 Option 6칸 순회 인라인) — 임계 아님」라 적었는데 kind=임계

**[G20 명세간 공유사실] 118건**
  [182] SmallActionRunAway::get_input                   [R4] r i s k _ w e i g h t   단 계 — 같은 노브가 값 "1/3/5 (nearby 0~1/2/3+, 반경 200000)" / "nearby>2→5, ==2→3, else 1" 를 동시에 갖는다
  [182]`"1/3/5 (nearby 0~1/2/3+, 반경 200000)"` · [184]`"nearby>2→5, ==2→3, else 1"`
```
...
```json
{
  "round": 24, "batch": "C",
  "errors": [
    {"path": "/specs[182]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[182]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 24 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify24/C 만 써라**

```
MIG/_verify24/C/patch.json      ← 기계 적용분(필수)
MIG/_verify24/C/REPORT.md       ← 산문 보고서
MIG/_verify24/C/oracle/         ← 오라클 .rs · 출력 로그
```
