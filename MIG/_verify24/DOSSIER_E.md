...
# 24차 배치 E 도시에 — 담당 `184`~`185` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 24 E      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `184` | SmallActionRecall::get_input | in:game_ai | 49 | 28 | 13 | 9 | 90 |
| `185` | get_input_target | in:game_ai::abstract_input | 45 | 17 | 5 | 5 | 66 |
...
| # | 이름 | 파일 |
|---|---|---|
| `184` | SmallActionRecall::get_input | `MIG\_verify24\E\spec_184_SmallActionRecall__get_input.md` |
| `185` | get_input_target | `MIG\_verify24\E\spec_185_get_input_target.md` |
...
**[G20 명세간 공유사실] 118건**
  [185] get_input_target                                [R1] stack - — 같은 (base,offset) 을 4가지 이름으로 부른다: EntityIter 64B / ProjectileIter 40B / jrng Option<NoiseRng> 16B / target_x/target_y
  [185]`target_x/target_y` · [186]`EntityIter 64B/ProjectileIter 40B` · [186]`jrng Option<NoiseRng> 16B`

**[G3 형제 함수] 6건**
  [185] get_input_target                                siblings.plan == null 인데 자유 함수 화이트리스트에 없다 — 망글링 파싱 실패를 의심하라
  _RNvNtCshdEBA0ozCnw_7game_ai14abstract_input16get_input_target

**[G7 과열림] 8건**
  [184] SmallActionRecall::get_input                    ★open 이 `shared.is_recent_visible` 를 부르는데 그 항목은 **확정**이다 — 과열림
  L792 `data.blackboard[1-team]` — 적팀 Blackboard 를 self 로 is_recent_visible 을 부른다(IR gep 인덱스 %454 = 1-team 확정). 소스 의도(적이 나
```
...
```json
{
  "round": 24, "batch": "E",
  "errors": [
    {"path": "/specs[184]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[184]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 24 --only E --dry
```
...
## §6 출력 디렉터리 — **_verify24/E 만 써라**

```
MIG/_verify24/E/patch.json      ← 기계 적용분(필수)
MIG/_verify24/E/REPORT.md       ← 산문 보고서
MIG/_verify24/E/oracle/         ← 오라클 .rs · 출력 로그
```
