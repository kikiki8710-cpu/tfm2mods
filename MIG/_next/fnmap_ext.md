...
|---|---|---|---|---|---|---|
| `d70620` | can_tower_focused 의 Chain<Chain<…>> fold 인스턴스(count · 판단은 클로저 e3b840 등 · ghidra 09-16) | 1 | 확정 | 1,997,255,248 | v3_pve_monster_aggro_damage · engage_requires_dive · v3_survival_incoming | can_tower_focused |
...
| `e25450` | SmallActionLaneMinionPosition::choose_goal 후보 클로저(push_candidate 경로 · can_tower_focused 캐시 래퍼 아님 · ghidra 09-16) | 1 | 추정/정적 | 292,520,750 | can_tower_focused · position_eval_at | SmallActionLaneMinionPosition::choose_goal(ghidra 09-16) |
| `e0cf10` | battle::max_range_cached 외곽(로스터 idx→TLS 키 · 폴백 e0e890 max_range · ghidra 09-16) | 1 | 확정 | 291,998,412 | LocalKey::with (max_range_cached TLS 메모 본체) | FUN_c9d5b0 · FUN_ca0a10 · FUN_ca4790 · FUN_d66a10 |
...
| `e49a50` | LegacyPlanHandler::update_on_dead(update 아웃라인 조각 아님 · ghidra 09-16) | 1 | 확정 | 91,740,819 | update · GoalData::update · passive_plan · handle_chat · LegacyPlanHandler::take_misunderstood_received_chat … | <AgentVerHamster as AiAgent>::update_on_dead vtable thunk(slot 17 · ghidra 09-16) |
...
| `eb5dd0` | fight_check::expected_dps(오프셋 6개 일치 · ghidra 09-16) | 1 | 확정 | 70,752,171 | estimate_damage_to | FUN_e34a30 · FUN_ebe220 · decide_deathmatch · fight_model::resolve_fight_uncached |
...
| `dd9f30` | TeamPlan::can_near_enemies(can_near_enemies_range 위임 · ghidra 09-16) | 1 | 추정/정적 | 54,703,182 | fmt | sub_plan · update · update · SinglePlanLine::sub_plan |
| `e9bf10` | <AgentVerHamster as AiAgent>::upgrade_item vtable thunk(vtable 0x33ebba0 slot 16 · ghidra 09-16) | 1 | 확정 | 49,350,013 | AgentVerHamster::upgrade_item |  |
| `e9c610` | <AgentVerHamster as AiAgent>::buy_item vtable thunk(slot 15 · ghidra 09-16) | 1 | 확정 | 49,273,990 | AgentVerHamster::buy_item |  |
...
| `ca6700` | LegacyPlanHandler::get_small_action 점수 합성 클로저(SubPlan::score 디스패처 e388c0 호출 · W[CATTBL]/1000 · ghidra 09-16) | 1 | 확정 | 40,155,041 | serpen_check::SerpenCheckSubPlan::score · steal::StealSubPlan::score | auction::LegacyPlanHandler::get_small_action |
...
| `caf2e0` | BigPlan::update 디스패처(JT 0x33d75b8 · 실호출 9 · ghidra 09-16) | 1 | 확정 | 31,779,042 | passive_line::PassiveLinePlan::update · update · update · FUN_d2e3f0 | update |
| `d758e0` | (미명명) | 1 | 미확정 | 30,495,471 | estimate_damage_to | v3_deadly_edge_cells |
| `e35bd0` | SubPlan::action_candidates 디스패처(JT 0x33e8910 · 17 arm 전부 아웃라인 · ghidra 09-16) | 1 | 확정 | 26,384,930 | LineDefenseSubPlan::action_candidates · BattleSubPlan::action_candidates · JungleSubPlan::action_candidates · recall::RecallSubPlan::action_candidates · DefenseNexusSubPlan::action_candidates … | auction::LegacyPlanHandler::get_small_action |
...
| `c88300` | LocalKey::with<interaction_ctx 클로저>(InterActionCtx 메모 캐시 · action_score.rs:577 · ghidra 09-16) | 1 | 확정 | 18,779,483 | FUN_e2fee0 | interaction_score |
| `e360e0` | SubPlan::calculate_score_parameter_value 디스패처(JT 0x33e8954 · 17 arm · 인라인 14 · 단순 필드 산출 · ghidra 09-16) | 1 | 확정 | 18,028,111 | line_defense::LineDefenseSubPlan::calculate_score_parameter_value · battle::BattleSubPlan::calculate_score_parameter_value | auction::LegacyPlanHandler::get_small_action |
...
| `e03360` | buff_value::noncombat_steroid_window(ghidra 09-16 · ⚠지도 d75cb0 라벨과 충돌 · 확인 필요) | 1 | 확정 | 10,508,057 | noncombat_steroid_window_cl · noncombat_steroid_window · aoe_heal_covers_low_ally · FUN_d717e0 | calculate_interaction_action_score |
...
| `e25030` | [추정 강] SmallActionLaneMinionPosition::target_score(ghidra 09-16) | 1 | 확정 | 8,168,055 | estimate_damage_to | FUN_d6fc40 · FUN_d71230 |
...
| `cafe10` | BigPlan::next_plan 디스패처(JT 0x33d76ac · 실호출 3 · ghidra 09-16) | 1 | 확정 | 4,315,170 | PassiveJunglePlan::next_plan · LineGankerPlan::next_plan · LineGankCoverPlan::next_plan(BigPlan::next_plan JT arm 9 · ghidra 09-16) | update |
| `d84c60` | abstract_input::attack 래퍼(position_eval 근방) | 1 | 추정/정적 | 3,973,681 | abstract_input::attack | get_input |
| `d2e3f0` | (미명명) | 1 | 미확정 | 3,949,695 | is_cleared · best_jungle_goal | BigPlan::update 디스패처(JT 0x33d75b8 · 실호출 9 · ghidra 09-16) |
...
| `dfb5d0` | (미명명) | 1 | 미확정 | 1,105,009 | base_sub_goal | LineGankCoverPlan::next_plan(BigPlan::next_plan JT arm 9 · ghidra 09-16) · PassiveJunglePlan::next_plan · make_gank_battle · LineGankerPlan::next_plan |
...
| `cd5ee0` | battle::kite_reposition_point(1,193B · ghidra 09-16 · ⚠지도 cadcd0 라벨과 충돌) | 1 | 확정 | 843,744 | v48_on_cast_line | BattleSubPlan::action_candidates |
...
| `e2f4f0` | buff_value_v54 내부 Filter/Map fold<min_by_key>(champion_hp_value 최소 선택 · ghidra 09-16) | 1 | 확정 | 254,388 | can1v1win | buff_value_v54 |
...
| `e02bc0` | buff_value::v54_aoe_ally_heal_value(ghidra 09-16) | 1 | 확정 | 80,297 | can1v1win | calculate_interaction_action_score |
| `ca82f0` | (미명명) | 1 | 미확정 | 78,500 | FUN_dc8c40 | LineGankerPlan::next_plan |
| `ca8460` | (미명명) | 1 | 미확정 | 78,500 | FUN_dc8d50 | LineGankerPlan::next_plan |
| `df1f50` | LineGankCoverPlan::next_plan(BigPlan::next_plan JT arm 9 · ghidra 09-16) | 1 | 확정 | 72,262 | get_die_tick_player · next_plan · FUN_dfb5d0 | BigPlan::next_plan 디스패처(JT 0x33d76ac · 실호출 3 · ghidra 09-16) |
...
| `eba320` | [추정 강] fight_check::v48_projectile_profile(ExpectedGame 캐스트 시뮬 · JT 9 arm · ghidra 09-16 · ⚠지도 c875a0 라벨과 충돌) | 1 | 확정 | 33,264 | FUN_c7e5a0 · FUN_c7e920 | v48_cast_beams (LocalKey::with 클로저) |
...
| `ca4790` | (미명명) | 2 | 미확정 | 0 | battle::max_range_cached 외곽(로스터 idx→TLS 키 · 폴백 e0e890 max_range · ghidra 09-16) | FUN_e04c60 |
| `d66a10` | (미명명) | 2 | 미확정 | 0 | battle::max_range_cached 외곽(로스터 idx→TLS 키 · 폴백 e0e890 max_range · ghidra 09-16) | FUN_d6a860 · battle_common::v17_runaway_counterattack_bonus |
| `d69690` | (미명명) | 2 | 미확정 | 0 | battle::max_range_cached 외곽(로스터 idx→TLS 키 · 폴백 e0e890 max_range · ghidra 09-16) | BattleSubPlan::action_candidates · battle_common::v15_can_keep_support_pressure |
| `e04c60` | (미명명) | 2 | 미확정 | 0 | FUN_ca4790 · battle::max_range_cached 외곽(로스터 idx→TLS 키 · 폴백 e0e890 max_range · ghidra 09-16) · check_kill_die_tick LocalKey::with(DieTickCache 래퍼) | FUN_ca4900 · fight_participants · resolve_join_stake |
