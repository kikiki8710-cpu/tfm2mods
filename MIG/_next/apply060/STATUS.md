...
| `cb7540` | `f356b0` | HideSubPlan::action_candidates | B — 게이트·신규 블록·태그는 asm 확정. 재현 완전성은 미독 격자 헬퍼 3종(ee0fe0/ee0dd0/ | 8288 |
| `cc4260` | `eafb50` | JungleSubPlan::action_candidates | A(본체) / B(v3 경로 재계산 꼬리·격자 콜리 3종 내부 미독) — 재현 시 ee0fe0/ee2310  | 9197 |
...
| `d2f180` | `fa47a0` | PassiveJunglePlan::next_plan | B — 골격·호출 순서·오프셋은 asm 으로 확인했으나 v3 블록 A/절1 전처리 세부와 fa3350 산식은 | 12104 |
...
| `dcee40` | `f024d0` | TeamPlan::handle_none_or_gank_objective | B — 선두(①~③·최근교전 armed 분기)·꼬리(귀환 v3·V6 commit 골격·:1005~1094 삭 | 17351 |
...
| `dda220` | `f10f70` | TeamPlan::update_objective_after_steal | B — 골격(A~K)·콜 시퀀스·오프셋은 asm+RE 로 확인, 세부 산식은 RE 의존 · 핸들러 5종(§S | 34484 |
...
| `df36e0` | `dcce40` | BattlePlan::update_v32 | B — 구역 지도·exit/br 값·필드표는 확정(디컴 재확인)이나 신규 헬퍼 6종의 정확식과 P15/P16 | 38404 |
| `dfb840` | `dd8e70` | BattlePlan::update | A — 본체 동치 판정 + 신규 에필로그가 디컴에서 그대로 확인됨. 잔여 불확실 = update_v32 가  | 10223 |
