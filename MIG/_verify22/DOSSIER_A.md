...
# 22차 배치 A 도시에 — 담당 `112`~`117` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 22 A      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `112` | nontarget_windup_perceived | pub | 11 | 11 | 3 | 3 | 19 |
| `113` | v3_deadly_edge_cells | pub | 10 | 10 | 5 | 5 | 25 |
| `114` | attack_summon_action | pub | 41 | 10 | 2 | 5 | 53 |
| `115` | is_cleared | pub | 14 | 3 | 2 | 3 | 14 |
| `116` | can_tower_focused_when_attack | pub | 14 | 5 | 2 | 5 | 19 |
| `117` | camp_idx | pub | 0 | 5 | 0 | 1 | 5 |
...
| # | 이름 | 파일 |
|---|---|---|
| `112` | nontarget_windup_perceived | `MIG\_verify22\A\spec_112_nontarget_windup_perceived.md` |
| `113` | v3_deadly_edge_cells | `MIG\_verify22\A\spec_113_v3_deadly_edge_cells.md` |
| `114` | attack_summon_action | `MIG\_verify22\A\spec_114_attack_summon_action.md` |
| `115` | is_cleared | `MIG\_verify22\A\spec_115_is_cleared.md` |
| `116` | can_tower_focused_when_attack | `MIG\_verify22\A\spec_116_can_tower_focused_when_attack.md` |
| `117` | camp_idx | `MIG\_verify22\A\spec_117_camp_idx.md` |
...
**[G18 logic↔표 상호참조] 2건**
  [112] nontarget_windup_perceived                      logic/P2 logic 이 필드 `p.position` 를 읽는데 mem 표에 그 필드 행이 없다
  잎 `position` 는 distruct 의 실재 필드명

**[G20 명세간 공유사실] 95건**
  [113] v3_deadly_edge_cells                            [R4] 버 전   게 이 트 — 같은 노브가 값 1 / 2 를 동시에 갖는다
  [113]`2` · [127]`1`

**[G9 callees 오염] 1건**
  [112] nontarget_windup_perceived                      ★`caster.action_state.time` — 중간 `action_state` 는 enum(game_core::simulation::entity::champion:) 이라 필드 경로를 이을 수 없다. `time` 는 **메서드**이고 callees 자동수집에서 빠진다
  logic 표기를 `time()` 로 고칠 것
```
...
```json
{
  "round": 22, "batch": "A",
  "errors": [
    {"path": "/specs[112]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[112]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 22 --only A --dry
```
...
## §6 출력 디렉터리 — **_verify22/A 만 써라**

```
MIG/_verify22/A/patch.json      ← 기계 적용분(필수)
MIG/_verify22/A/REPORT.md       ← 산문 보고서
MIG/_verify22/A/oracle/         ← 오라클 .rs · 출력 로그
```
