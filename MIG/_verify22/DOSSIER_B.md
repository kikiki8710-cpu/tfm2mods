...
# 22차 배치 B 도시에 — 담당 `118`~`123` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 22 B      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `118` | base_attacking_minion_uncached | in:game_ai::plan_legacy::old::defense_nexus | 13 | 3 | 2 | 3 | 15 |
| `119` | check_nontarget | in:game_ai::small_action::cast | 23 | 11 | 4 | 4 | 36 |
| `120` | line_recall_pressure_penalty | in:game_ai | 3 | 7 | 6 | 3 | 16 |
| `121` | safe_move_avoiding_enemy_well | pub | 29 | 19 | 10 | 6 | 56 |
| `122` | convert_to_move_action_target | pub | 12 | 3 | 2 | 3 | 15 |
| `123` | attack_structure_skill_action | pub | 32 | 8 | 1 | 6 | 41 |
...
| # | 이름 | 파일 |
|---|---|---|
| `118` | base_attacking_minion_uncached | `MIG\_verify22\B\spec_118_base_attacking_minion_uncached.md` |
| `119` | check_nontarget | `MIG\_verify22\B\spec_119_check_nontarget.md` |
| `120` | line_recall_pressure_penalty | `MIG\_verify22\B\spec_120_line_recall_pressure_penalty.md` |
| `121` | safe_move_avoiding_enemy_well | `MIG\_verify22\B\spec_121_safe_move_avoiding_enemy_well.md` |
| `122` | convert_to_move_action_target | `MIG\_verify22\B\spec_122_convert_to_move_action_target.md` |
| `123` | attack_structure_skill_action | `MIG\_verify22\B\spec_123_attack_structure_skill_action.md` |
...
**이 배치 몫 = 2건** (게이트 머리줄의 건수는 **20함수 전역 합계**다 — 혼동 말 것)
...
**[G12 src_line 대조] 5건**
  [121] safe_move_avoiding_enemy_well                   consts[8] src_line=102 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [111, 143]

**[G16 params.role 대조] 4건**
  [119] check_nontarget                                 sig.params[0] P1 정렬 불변식 깨짐(소거 주장 수 불일치)
...
```json
{
  "round": 22, "batch": "B",
  "errors": [
    {"path": "/specs[118]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[118]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 22 --only B --dry
```
...
## §6 출력 디렉터리 — **_verify22/B 만 써라**

```
MIG/_verify22/B/patch.json      ← 기계 적용분(필수)
MIG/_verify22/B/REPORT.md       ← 산문 보고서
MIG/_verify22/B/oracle/         ← 오라클 .rs · 출력 로그
```
