...
# 22차 배치 C 도시에 — 담당 `124`~`128` (게임 0.5.8)
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 22 C      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `124` | v55_seal_value | in:game_ai | 18 | 7 | 4 | 4 | 27 |
| `125` | SmallActionLaneMinionPosition::should_suppress_direct_minion_attack | pub | 21 | 4 | 0 | 6 | 24 |
| `126` | v55_banish_penalty | in:game_ai | 21 | 7 | 4 | 4 | 31 |
| `127` | is_safe_recall | in:game_ai | 61 | 16 | 7 | 6 | 83 |
| `128` | SmallActionTrace::expected_goal_position | in:game_ai | 41 | 7 | 1 | 5 | 48 |
...
| # | 이름 | 파일 |
|---|---|---|
| `124` | v55_seal_value | `MIG\_verify22\C\spec_124_v55_seal_value.md` |
| `125` | SmallActionLaneMinionPosition::should_suppress_direct_minion_attack | `MIG\_verify22\C\spec_125_SmallActionLaneMinionPosition__should_suppress_direct_minion_attack.md` |
| `126` | v55_banish_penalty | `MIG\_verify22\C\spec_126_v55_banish_penalty.md` |
| `127` | is_safe_recall | `MIG\_verify22\C\spec_127_is_safe_recall.md` |
| `128` | SmallActionTrace::expected_goal_position | `MIG\_verify22\C\spec_128_SmallActionTrace__expected_goal_position.md` |
...
**[G12 src_line 대조] 5건**
  [126] v55_banish_penalty                              consts[5] src_line=598 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [601, 620]

**[G20 명세간 공유사실] 95건**
  [124] v55_seal_value                                  [R4] 결 과   상 한 — 같은 노브가 값 160 / 80 를 동시에 갖는다
  [124]`80` · [126]`80` · [130]`160`
  [128] SmallActionTrace::expected_goal_position        [R2] AbstractGameWithCache game — 같은 이름이 오프셋 0x0 / 0x8 에 동시에 있다 — **값이 틀렸거나, 이름이 두 자리를 구별하지 못한다**(후자면 이름을 갈라라)
  [128]`0x0` · [128]`0x8` · [130]`0x0` · [133]`0x0`
```
...
```json
{
  "round": 22, "batch": "C",
  "errors": [
    {"path": "/specs[124]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[124]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 22 --only C --dry
```
...
## §6 출력 디렉터리 — **_verify22/C 만 써라**

```
MIG/_verify22/C/patch.json      ← 기계 적용분(필수)
MIG/_verify22/C/REPORT.md       ← 산문 보고서
MIG/_verify22/C/oracle/         ← 오라클 .rs · 출력 로그
```
