...
# 22차 배치 D 도시에 — 담당 `129`~`133` (게임 0.5.8)

## §0 정본 스탬프 — **읽기 전에 스스로 확인하라**

| 정본 | sha256[:16] | mtime |
|---|---|---|
| `_spec/specs20_v3.json` | `6a6d3bd7721deec9` | 2026-09-14 09:43:25 |
| `_spec/specs20.json` (손이 닿는 정본) | `cfd7cceb848cf7b6` | 2026-09-14 09:41:10 |

| `mkdossier.py`(이 파일을 만든 도구) | `802add3b146ab88e` | 2026-09-14 02:13:55 |

이 파일 생성 = `2026-09-14 09:52:08`
...
```bash
cd /c/tfm2mods/MIG
python -X utf8 dossierfresh.py 22 D      # ← 이것만 돌리면 된다
```
...
|---|---|---|---|---|---|---|---|
| `129` | should_add_self_etc_buff_action | in:game_ai | 20 | 8 | 3 | 3 | 31 |
| `130` | v55_mark_value | in:game_ai | 26 | 10 | 5 | 5 | 39 |
| `131` | noncombat_steroid_value | in:game_ai | 30 | 17 | 9 | 6 | 56 |
| `132` | champion_hp_value_uncached | in:game_ai::utils | 16 | 13 | 5 | 5 | 34 |
| `133` | buff_value_v54 | in:game_ai | 80 | 17 | 12 | 8 | 105 |
...
| 파일 | 줄 | sha256[:16] | 왜 |
|---|---|---|---|
| `MIG\METHOD_MAP.md` | 221 | `4e6a73ead74b191e` | ★**「불가」를 쓰기 전에 §0 라우팅표를 보라.** 09-11 에 「원리적 불가」가 7건 뒤집혔고 원인은 매번 「가진 재료의 한계를 문제의 한계로 착각」이었다. |
...
| # | 이름 | 파일 |
|---|---|---|
| `129` | should_add_self_etc_buff_action | `MIG\_verify22\D\spec_129_should_add_self_etc_buff_action.md` |
| `130` | v55_mark_value | `MIG\_verify22\D\spec_130_v55_mark_value.md` |
| `131` | noncombat_steroid_value | `MIG\_verify22\D\spec_131_noncombat_steroid_value.md` |
| `132` | champion_hp_value_uncached | `MIG\_verify22\D\spec_132_champion_hp_value_uncached.md` |
| `133` | buff_value_v54 | `MIG\_verify22\D\spec_133_buff_value_v54.md` |
...
**[G12 src_line 대조] 5건**
  [132] champion_hp_value_uncached                      consts[2] src_line=934 인데 IR 사슬에 그 줄이 없다
  실제 후보 = [885, 977, 985]

**[G15 consts.kind 관측] 2건**
  [129] should_add_self_etc_buff_action                 consts[7] kind=임계 NEG
  `meaning` 이 「팀 배열 bounds(34737) — 임계 아님」라 적었는데 kind=임계
  [131] noncombat_steroid_value                         consts[15] kind=임계 NEG
  `meaning` 이 「2 (sdiv 35830). 34986·35481 의 2 는 팀 배열 bounds(임계 아님). L502 `count」라 적었는데 kind=임계

**[G16 params.role 대조] 4건**
  [129] should_add_self_etc_buff_action                 sig.params[0] P3 `params[]` 행 수가 `sig.tcx` 소스 인자 수와 다르다
  `sig.tcx` 소스 인자 4개(sret 행 없음) ⟹ 4행이어야 하는데 5행. 현재 i = [1, 2, 3, 4, 4]

**[G18 logic↔표 상호참조] 2건**
  [129] should_add_self_etc_buff_action                 logic/P1 logic 이 `+0x90` 를 인용하는데 mem 표에 그 오프셋이 없다
  // L609 vtable+0x90 (34649~34652 → %314 true)

**[G19 knobs.value 값] 4건**
  [132] champion_hp_value_uncached                      knobs[4] 주장한 값을 IR 관측에서 못 찾는다
  value=10 · 관측(창)=[0, 1, 8, 72]
```
...
| 축 | 전체 건수 | 상태 |
|---|---|---|
| `mem.dir`(읽기/쓰기 방향) | 3890 | ✅**G14** — IR load/store 대조 |
| `consts.kind`(상수 종류) | 1832 | ✅**G15** — IR 소비 오프코드 반증식 |
| `sig.params.role`(인자 역할) | 800 | ✅**G16** — `define` 속성 대조 |
| `callees[]` 의 **경로·시그니처** | 2404 | ⚠**730행만 IR 앵커**(`ev3`). 나머지 1674행은 `ev4` + 「미확정」 — 같은 leaf 이름 후보가 여럿이라 **하나를 고른 것이 아니라 못 고른 것**이다 |
...
```json
{
  "round": 22, "batch": "D",
  "errors": [
    {"path": "/specs[129]/consts[3]/src_line",
     "kind": "실오류",
     "old": 293, "new": 310,
  ...
  "ev_up": [
    {"path": "/specs[129]/mem[0]", "from": 4, "to": 3,
...
```bash
cd /c/tfm2mods/MIG && PYTHONIOENCODING=utf-8 python -X utf8 applypatch.py 22 --only D --dry
```
...
## §6 출력 디렉터리 — **_verify22/D 만 써라**

```
MIG/_verify22/D/patch.json      ← 기계 적용분(필수)
MIG/_verify22/D/REPORT.md       ← 산문 보고서
MIG/_verify22/D/oracle/         ← 오라클 .rs · 출력 로그
```
