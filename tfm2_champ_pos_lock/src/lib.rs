...
fn recompute_blocklist(root: &Node) {
    ...
    if scene < 0x10000 {
        // ★[2026-09-04] 문구 정정 — 이 코드는 **밴 페이즈·화면 전환에서도 정상적으로 나온다.**
        //   `is_ban` 판정이 이 체크보다 **뒤에** 있어서, 밴 화면 전환 프레임에 씬 포인터가
        //   잠깐 비면 `[6] 밴 차례` 가 아니라 여기 `[1]` 로 찍힌다(동작은 같다 = 아무것도 안 막음).
        //   실측(2026-09-04 수동 판): 밴 페이즈 내내 `[6]`↔`[1]` 이 번갈아 찍히고, 픽 페이즈에
        //   들어가면 곧바로 `내픽 N` 으로 복귀했다. **이걸 결함으로 오판한 이력이 있다.**
        exit_note(
            1,
            "밴픽 씬 미포착(scene_cap=0) — 밴 페이즈·화면 전환 중이면 정상. \n픽 차례인데 계속 이 상태면 scene_step 훅 미설치/무효화 의심",
        );
        return;
    }
    if stamp == last {
...
impl ModExtension for PosLockExt {
    fn post_update(&self, scene: &mut Scene, ui: &mut GameUI, _assets: &mut Assets, _dt: f32) {
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            // ★★[2026-09-04] FZ 재활성 — **12인자 규약 확정 후**(RE6-09-04_코치위임픽-개입점).
            //   구 크래시(`fault=0x18`)의 원인은 주소가 아니라 **인자 개수 오인(10→12)** 이었다.
            //   이 함수가 코치/AI 픽의 진짜 결정 지점이다.
